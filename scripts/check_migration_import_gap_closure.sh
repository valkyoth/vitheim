...
for phrase in \
    ...
    'startup never scans' 'The following writer set is closed' 'PreparingOpen alone admits new Begin/Dispatch and Stage/Verify' 'Every later covered mutation while the attempt is already' 'cumulative advance commitment' 'domain realization of VIT-INV-006' 'never bearer authority' 'Exactly one application-level provider invocation may' 'Attempt mutation, covered auxiliary mutation'
do
