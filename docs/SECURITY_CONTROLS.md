...
- One canonical roadmap manifest accounts for all 402 exact stops and their
...
independently authorizes exact retained-namespace safety; `0.51.21–0.51.36`
...
creation and final activation so neither the restriction branch nor eligibility
can be consumed incorrectly, atomically consume both candidates only after
complete admission and bridge-ownership transfer, reconcile both reinstall
release families, permit already-operational lowering only through a complete
prepared root and one guard-slot CAS, and release the old top only through
per-enforcement-point authenticated conservation with funded retention.
