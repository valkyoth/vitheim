...
| IDOR, authority-revocation TOCTOU, grant-lineage fork, double/final-attempt redemption, two-stream dispatch, offline-human impersonation, or confused deputy | Typed commands, immutable tenant/initiator/delegation/capability/target/request binding, typed live/grant/service authority, one grant-lineage stream, co-located redemption guard, stable attempt claim/receipt, bounded canonical dispatch-authority fence set with monotonic local epochs, no privileged use of bounded-stale external facts, no worker-derived authority |
...
| Partial quota reservation, cross-partition transaction, capacity or encumbrance duplication/loss, deadlock/retry duplication, token/member substitution, refund/write-off fraud, cross-kind settlement, liability loss, or recovery starvation | Local non-aggregate quota authority, canonical all-or-none acquisition in one partition, immutable set token/membership, per-kind fenced capacity leases, expiry-safe encumbrance and exactly-once transfer, parent/free-remainder conservation, canonical composite lock order, bounded identity-preserving retry, whole-set restore/quarantine, active/active write rejection, per-kind evidence, separate compensation, fair recovery partition |
