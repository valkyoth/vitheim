...
| A composite law retroactively acquires a future root, activates a future planning tuple, accepts a self-consistent attacker-authored manifest, merges global/rollout/topology/local owners, lets rollout or discovery invent topology, clones a disk-held workload key or local admission, treats a receipt digest/channel transcript as authentication, runs competing catalog rollouts without one winner, accepts late superseded authorization, mutates rollout membership, admits missing/contradictory/cross-generation receipts, activates an unfenced quorum, abandons after activation, loses revocation across an unreachable placement, creates a catalog coverage gap/overlap, accepts field-shaped text, trusts mutable storage or an untrusted clock, accepts wrong predecessor/scope/profile, rolls back catalog/time/placement fences or tombstones, extends expiry through restore, claims planned semantic code, omits ancestry, changes semantics behind a label, or starts unresolved | Strict canonical catalog validation; planning-superset separation; explicit-successor floors with exact milestone coverage; separate global `VIT-INV-057`, exact local `VIT-INV-058`, rollout-root `VIT-INV-059`, and topology-generation `VIT-INV-060` ownership; `VIT-LAW-007` admission and generation-qualified `VIT-LAW-008` rollout composition; compiled static bootstrap then expected-version topology successors/fences/tombstones; hardware-attested or orchestrator-attested single-active-lease workload profile; authenticated canonical receipts; monotonic `ActiveRolloutGeneration` and terminal `Superseded` losers; immutable topology manifest; closed process-manager states; transactional outbox/inbox; `AllRequired` or fully fenced quorum; current-authority revocation failure; runtime/CLI verifier for decode/digests/ancestry/predecessor/trust/scope; trusted-time uncertainty/rollback/suspend/expiry ratchets; checkpoint/restore binding; exhaustive Rust P/N/M/F realization; future, drift, malformed, rollback, and unresolved rejection |
