...
| A composite law retroactively acquires a future root, changes semantics behind an unchanged label, presents altered/noncanonical recovery state, or a milestone starts with unresolved authority | Canonical `LawGenerationManifestV1` bytes and digest with exact effective dependency/coordinator/semantic/activation/migration/rollback/recovery binding; future-root, future-generation, semantic/latest-view, and digest drift rejection; meaningful removal/semantic-only evolution; status-aware authority disposition for every post-bootstrap milestone |
