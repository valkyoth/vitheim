...
| IDOR, authority/target-state TOCTOU, stale/deleted/superseded target, grant-lineage fork, double/final-attempt redemption, two-stream dispatch, offline-human impersonation, or confused deputy | Typed commands, immutable tenant/initiator/delegation/capability/target/request binding, typed live/grant/service authority, one grant-lineage stream, co-located redemption guard, stable attempt claim/receipt, bounded canonical dispatch-authority fence set with monotonic local epochs, typed co-located target fence with lifecycle/deletion/supersession epoch, no privileged use of bounded-stale authority or target projections, no worker-derived authority |
...
| Partial quota reservation, cross-partition transaction, capacity or encumbrance duplication/loss, free-at-both-ends transfer, deadlock/retry duplication, token/member substitution, refund/write-off fraud, cross-kind settlement, liability loss, or recovery starvation | Local non-aggregate quota authority, canonical all-or-none acquisition in one partition, immutable set token/membership, per-kind fenced capacity leases, expiry-safe encumbrance, stable outbox/inbox transfer state and receipts, authenticated acknowledgement plus old-epoch fence proof, conservative double-entry conservation over at-least-once delivery, parent/free-remainder conservation, canonical composite lock order, bounded identity-preserving retry, whole-set restore/quarantine, active/active write rejection, per-kind evidence, separate compensation, fair recovery partition |
