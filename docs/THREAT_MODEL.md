...
| A composite law retroactively acquires a future root, activates a future planning tuple, accepts a self-consistent attacker-authored manifest, merges global/rollout/topology/local owners, circularly or simultaneously authorizes static/dynamic topology, lets rollout/discovery invent topology, clones a disk-held workload key or uses an offline lease, treats a receipt digest/channel transcript/ordinary row as authentication, abandons or supersedes after activation authorization, accepts delayed authorization after revocation, runs competing catalog rollouts without one pinned winner, mutates rollout membership, admits missing/contradictory/cross-generation receipts, activates an unfenced quorum, loses revocation across an unreachable placement, creates a catalog coverage gap/overlap, accepts field-shaped text, trusts mutable storage or an untrusted clock, accepts wrong predecessor/scope/profile, rolls back catalog/time/placement fences or tombstones, extends expiry through restore, claims planned semantic code, omits ancestry, changes semantics behind a label, or starts unresolved | Strict canonical catalog validation; separate global/local/rollout/topology owners; irreversible `ActivationAuthorized` with atomic receipt/outbox and pinned generation; globally serialized activate/revoke; staged dormant exact-singleton handoff after generation-1 epoch-12 convergence/local generation-2 admission; exclusive topology source; hardware-attested identity or orchestrator online single-use action claims with fixed maximum lifetime and zero offline authority; closed signed/authority-MAC/attested-channel-admission receipt variants with replay tombstones and durable integrity anchors; terminal pre-authorization `Superseded`; immutable manifest; transactional outbox/inbox; `AllRequired` or fully fenced quorum; current-authority revocation failure; runtime/CLI canonical verification; trusted-time ratchets; checkpoint/restore binding; exhaustive Rust P/N/M/F realization; future, drift, malformed, rollback, and unresolved rejection |
