...
| IDOR, delayed-effect revocation, grant-lineage fork, double/final-attempt redemption, two-stream dispatch, offline-human impersonation, or confused deputy | Typed commands, immutable tenant/initiator/delegation/capability/target/request binding, typed live/grant/service authority, one authoritative grant-lineage stream, immutable approval receipt plus outbox for dedicated ownership, pre-issuance revocation, atomic successor supersession, co-located fenced redemption guard, stable attempt claim/receipt, no worker-derived authority |
...
| Partial quota reservation, cross-partition distributed transaction, capacity-lease over-allocation, deadlock, token/member substitution, refund/write-off fraud, cross-kind settlement, liability loss, or recovery starvation | Local non-aggregate quota authority, canonical all-or-none overlap acquisition in one quota partition, immutable set ID/digest/token/membership, exact-set consumption without reacquisition, fenced hierarchical capacity leases with parent/child conservation, idempotent set/claim settlement, whole-set restore/quarantine, active/active authoritative-write rejection, per-kind evidence rules, separate compensation, fair recovery partition |
