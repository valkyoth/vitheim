...
| IDOR, delayed-effect revocation, grant-lineage fork, offline-human impersonation, or confused deputy | Typed commands, immutable tenant/initiator/delegation/capability/target/request binding, typed live/grant/service authority, one authoritative grant-lineage stream, immutable approval receipt plus outbox for dedicated ownership, pre-issuance revocation, atomic successor supersession, fenced redemption, no worker-derived authority |
...
| Partial quota reservation, deadlock, token/member substitution, refund/write-off fraud, cross-kind settlement, liability loss, or recovery starvation | Local non-aggregate quota authority, canonical all-or-none overlap acquisition, immutable set ID/digest/token/membership, exact-set consumption without reacquisition, idempotent set/claim settlement, whole-set restore/quarantine, per-kind evidence rules, separate compensation, fair recovery partition |
