...
| A composite law retroactively acquires a future root, activates a future planning tuple, accepts a self-consistent attacker-authored manifest, leaves catalog lifecycle ownerless, trusts mutable storage as its root, omits signed-envelope scope/validity/signer/root/revocation/successor fields from the digest, uses an ambiguous combined profile, rolls back a local catalog ratchet, has malformed structure, claims planned-only semantic files/tests, interprets prose as authority, omits an ancestor, changes semantics behind an unchanged label, or starts unresolved | Strict canonical manifest/payload/envelope validation; planning-superset separation; immutable maximum-version active catalogs; `VIT-INV-057` one-owner CAS lineage and local high-watermarks; `VIT-LAW-007` safety-floor/dispatch/start composition; exact `CompiledCatalog`/`SignedCatalog`; fully content-bound envelope and separately rooted trust; artifact/checkpoint/backup/restore binding; stage-aware exhaustive Rust semantic realization with typed outcomes/recovery/P/N/M/F tests; complete ancestry; future, drift, neutral-delta, malformed-field, rollback, and unresolved-authority rejection |
