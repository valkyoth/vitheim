...
| A composite law retroactively acquires a future root, accepts a self-consistent attacker-authored manifest, trusts mutable storage as its root, has malformed/ambiguous structure, interprets prose as executable authority, omits an ancestor, changes semantics behind an unchanged label, presents altered/noncanonical recovery state, or starts with unresolved authority | Strict canonical `LawGenerationManifestV1` and structural validation; independently anchored compiled/platform-law-signed exact-tuple admission catalog; catalog binding in artifacts/checkpoints/backups/restore; closed exhaustive Rust semantic realizations with typed outcomes/recovery/P-M-F tests and unknown-ID rejection; complete `g01..gNN` ancestry enumeration; future-root/generation, semantic/latest-view, digest/catalog drift, neutral-delta, malformed-field, and unresolved-authority rejection |
