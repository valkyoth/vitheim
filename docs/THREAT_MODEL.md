...
| Authorization issuance abuse or years of consumed/expired receipts exhaust storage; compaction races replay, deletes before checkpoint, forks or rolls back a checkpoint, loses an archive/key, or makes an old request look unused | Pre-allocation per-deployment and issuer/class rate/outstanding limits; monotonic issuance sequence; frozen exact replay horizon; authenticated predecessor-linked set/archive checkpoint committed before deletion; covered-through high-watermark and key ratchets; typed unavailable-history denial; fail-closed issuance on ambiguous proof or backlog saturation; migration/HA/DR/key-rotation/concurrent-replay tests; bounded storage accounting and alerts |
