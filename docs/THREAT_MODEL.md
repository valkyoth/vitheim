...
| Fleet-wide evaluator invalidation misses credentials between epoch change and job creation, trusts an incomplete projection, loses a cross-shard move or late mutation, completes without every shard, causes a re-evaluation storm, provider-rate collapse, hostile-tenant starvation, stale-evidence reuse, lost jobs, or old-output use while queued | Epoch and invalidation-campaign root commit atomically; authoritative append-only membership journals; sealed per-shard generations/high-watermarks; capability mutation intents; fenced source-first moves; one fenced scan receipt per canonical manifest shard; final membership barrier; capability-owner mismatch reconciliation; search/projection non-authority; generation-bound idempotent materialization; explicit lifecycle dispositions; successor campaign tombstones; durable queues; lease fencing; bounded concurrency/rates/fairness/cleanup lane; `ReevaluationRequired` remains an operation fence |
