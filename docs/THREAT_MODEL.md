...
| A composite law retroactively acquires a future root, activates a future planning tuple, accepts a self-consistent attacker-authored manifest, merges global lineage with independently updated local ratchets, leaves a catalog lifecycle ownerless, creates a catalog coverage gap/overlap, accepts field-shaped text instead of verified bytes, trusts mutable storage or an untrusted clock, omits scope/validity/uncertainty/signer/root/revocation/successor fields from the digest, accepts the wrong predecessor/build scope/profile, rolls back a catalog/time ratchet, extends an expired window through restore, loses revocation during partial rollout, claims planned-only semantic files/tests, interprets prose as authority, omits an ancestor, changes semantics behind an unchanged label, or starts unresolved | Strict canonical manifest/payload/envelope validation; planning-superset separation; explicit-successor activation-floor catalogs with every milestone covered exactly once; global `VIT-INV-057` plus independent local `VIT-INV-058` ownership; `VIT-LAW-007` composition; one runtime/CLI verifier for canonical decode, digests, ancestry, actual predecessor, compiled/signature trust, and exact scope; admitted time intervals, uncertainty ceiling, continuity/rollback/suspend/expiry ratchets; partial-rollout/revocation recovery; exact `CompiledCatalog`/`SignedCatalog`; checkpoint/backup/restore binding; stage-aware exhaustive Rust P/N/M/F realization; future, drift, malformed, rollback, and unresolved-authority rejection |
