...
| Fleet-wide evaluator invalidation misses snapshots between epoch change and job creation, produces an incomplete campaign, causes a re-evaluation storm, provider-rate collapse, hostile-tenant starvation, stale-evidence reuse, lost jobs, or old-output use while queued | Epoch and invalidation-campaign root commit atomically; authoritative evaluator-epoch/snapshot-generation index; frozen cutoff and shard manifest; stable page cursors; generation-bound idempotent materialization; explicit concurrent create/move/delete/quarantine/rotation disposition; completeness reconciliation and terminal manifest; successor campaign tombstones; durable queues; lease fencing; bounded concurrency/rates/fairness/cleanup lane; `ReevaluationRequired` remains an operation fence |
