...
| Fleet-wide evaluator invalidation causes a re-evaluation storm, provider-rate collapse, hostile-tenant starvation, stale-evidence reuse, lost jobs, or old-output use while queued | Stable generation-bound tenant/provider/account jobs; durable queues/cursors; lease fencing; bounded concurrency/retry/elapsed time; provider-rate claims; global fair share, tenant ceilings, and starvation bounds; protected non-borrowable cleanup lane; privileged/near-term priority; freshness-bound evidence refetch; successor tombstones; visible escalation; `ReevaluationRequired` remains an operation fence |
...
| An invariant has no owner, competing owners, an eventually updated guard, an untested enforcement point, unsupported storage semantics, or restore/migration omits monotonic state | Stable machine-checked invariant registry; exactly one owner and owner-updated guard; transaction-domain placement; enforcement inventory and negative tests; semantic storage capability negotiation; P/N/M/F test classes; complete restore/migration epoch, cursor, tombstone, quota, and recovery manifest |
