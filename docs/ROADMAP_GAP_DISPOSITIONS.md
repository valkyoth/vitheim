...
| Interchange support was vague | Assign implementation owners at `0.53.3` (SAML), `0.54.1` (SCIM), `0.71.1` (syslog/security webhooks), `0.71.2` (STIX/TAXII), `0.78.5` (CVE/CVSS/VEX), and `0.89.1` (SPDX/CycloneDX); `0.140.9` can only revalidate implemented evidence or defer a profile. |
...
| Mandatory audit was not crash-atomic with commands | Insert `0.15.1` before the mutation transaction and strengthen `0.16.0`: successful protected commands commit events/head/receipt/audit/outbox/integrity together; denied commands commit only idempotent outcome plus audit authority. |
...
| Semantic search lacked a production adapter | Add conditional `0.98.1` for an admitted hosted semantic-index adapter with tenant/purpose/policy partitioning, deletion/erasure, model migration, rebuild, outage, and leakage evidence; otherwise `0.125.0` and the production AI profile keep similarity disabled. |
| Core service-operations capabilities lacked typed owners | Add `0.38.1` for SLI/SLO/error budgets, `0.39.1–0.39.3` for on-call/paging/preferences, `0.48.1` for stakeholder/status communications, and `0.82.1` for provenance-aware service health; repeat workflow/alert integration at `0.70.0` and `0.80.0`. |
