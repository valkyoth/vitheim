...
| --- | --- | --- |
| `0.29.0` | Add the single-use six-state retirement authorization, genesis-created Operational/RetirementPending/Retired fence and transaction-atomic terminalization reserve. Make that fence the first universal lock rank. Replace multi-transaction child re-cost prose with one parent/selector active-slot epoch-cut campaign, a total state/recovery-intent table, bounded post-cut fold/tail, three-bucket conservation, dual-profile charges, explicit reversible abort or conservative quarantine, physical migration-workspace reservation and authority-consuming atomic activation. Use ValidNonWeakening/ValidWeakening/InvalidOrUnverifiable classification and reserve complete parent-checkpoint verification capacity before a checkpoint becomes restorable | Race authorization admission/revocation/expiry/consumption; prove no partial reserve/fence/Pending commit; recheck the retirement fence at every boundary; reject overlapping campaigns; race live allocation/release against apply/finalize/recover/abort/activate; prove constant-sized activation, exact refunds only from complete evidence, current authority consumption and completed workspace; reject invalid profiles without destructive override; show an admitted maximum checkpoint can finish |
| `0.30.0` | Export/import preserves retirement authorization tombstones, first-rank fence and completion reserve; campaign epoch/fence, snapshot/commitment, post-cut log/high-watermark, active versus pending charge, forward/inverse transfers, cursor/lease/budget/abort result; classifier outcome/proof/budget; and verification reservation exactly | Missing/defaulted state, copied source charges, partial campaign import, invalid-as-weakening mapping, retained aborted staging charge, a restorable unreserved checkpoint or changed retry material leaves the destination fenced and unready |
| `0.140.1` | Freeze retirement grant authentication/time/quorum/nonce preimages, symbolic-classifier proof format/arithmetic/validity and the final re-cost activation authority preimage | Independent review proves the retirement issuer cannot self-admit or operate the deployment, no authority can override invalid accounting, and selector/profile activation cannot outlive or split from the consumed weakening grant or ordinary-owner result |
| `0.140.2` | Freeze all rows, indexes, universal lock ranks, active campaign slot, state/intent/fence generations, three aggregates, post-cut fold/tail bounds, command results/conflicts, workspace reservation/checkpoint and atomic parent-bucket/child-forward/inverse/final-activation transactions | Every selected storage adapter proves first-rank retirement, one campaign per parent/selector, live dual-profile mutation, whole-delta reservation, equal-bucket transfers, bounded finalization, evidence-correct abort/quarantine, durable migration peak capacity and complete-verification reservation or declares the profile unsupported |
| `0.140.3` | Freeze issuer/admitter, quorum and separation-of-duty roles; separate retirement, campaign owner, fenced-recovery authorizer, re-cost worker, cost classifier/authorizer, parent allocator and parent verifier capabilities | No role can both authorize and consume destructive retirement, classify and approve weakening, approve and execute fenced recovery, or repair/credit capacity |
| `0.140.5` | Freeze retention and custody rules for grant tombstones, active-slot/fence history, fold roots, campaign transfers/results/intents, workspace evidence, classifier proofs and verification reservation/spend evidence | Compaction and erasure cannot reopen authority, hide a partial campaign, invent a refund, reset work, discard cleanup reserve or make old custody usable |
| `0.140.6` | Freeze HA takeover, lease/fence generation, response-loss reconciliation and restore behavior for all refinements | Failover resumes the same authorization winner, active slot, campaign epoch/log/fold/cursor/buckets/pending charges/intent/results/workspace and verification reservation without overlapping campaign, duplicate transfer, free rescan or inferred terminal state |
| `0.142.0–0.148.0` | Carry slot/state/intent/authority/accounting/fold/workspace status through RPC; model-check races; drill restore; fuzz maxima and crash boundaries; audit roles/capacity; freeze incompatible legacy encodings | Each milestone retains the exact `0.29.0` security state and produces its named evidence before advancing |
| `0.149.0` | External pentest targets authority theft/replay, overlapping campaigns, lock inversion or partial reserve commit, live churn/log-tail exhaustion, three-bucket confusion, unbounded activation, abort/refund fraud, Fenced recovery escalation, workspace under-reservation, invalid-profile authorization, classifier exhaustion and under-reserved maximum snapshots | Zero unresolved critical/high findings and no waived destructive-authority, semantic-validity, bounded-work, conservation, locking, physical-capacity or recovery issue |
| `0.150.0` and `1.0.0` | Demonstrate install/upgrade/failover/restore continuity for the frozen contract and retain the final exact-commit evidence | No production claim until every selected adapter passes and the final exact commit passes the independent pentest |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze independently rooted retirement authority/states and empty-bootstrap versus old-custody quarantine; exact one-attempt slot; six-operation governance CAS with per-transition terminal key/receipt/status and immutable mutual exclusion; request/finality/compaction semantics. Independent signatures, lost-root self-retirement, dual terminal disposition or inferred custody readiness deny |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist retirement, terminal CAS, completion reserve/two heads; governed cost-profile trust/lineage/head/transition/activation/checkpoint/high-watermark/classifier; unique active campaign slot; total campaign state/recovery intent; three parent aggregates; post-cut fold cursor/checkpoint and bounded tail; typed command results/conflicts; physical workspace reservation/completion; authority-consuming final activation; Recovery parent aggregate/exact membership/commitment/predecessor checkpoints and VerificationPending/Ready/Fenced cursor-budget state beside prior ledgers/transfers/rank/settlement. Missing bounded snapshot/fold, transactionality, analytical comparison, conservative logical/physical bound, protected capacity or continuity refuses |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate retirement authority, publication root/profile owner, terminal-fence service, cost-profile trust/ordinary owner/destructive weakening classifier-authorizer, campaign owner, fenced-recovery authorizer, backend selector/re-cost worker, parent verifier/allocator, child transfer and custody roles. Retirement cannot activate/import; adapters cannot classify/authorize cost weakening; campaign owners cannot approve their own Fenced recovery; verifiers cannot repair ledger state |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain retirement/incident/custody result, transition terminal receipts/status, cost lineage/classifier, campaign slot/state/intent/fold/transfer/result, workspace reservation/completion and parent membership/checkpoints/verification charges plus prior evidence until governed compaction. Erasure cannot turn EvidenceUnavailable into usable custody, admit a second terminal meaning, hide weakening, invent a refund, discard cleanup reserve or reset verification |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze failover reconciliation for retirement, six-way governance CAS/terminal map, cost lineage/activation, campaign slot/fence generation/fold/recovery/workspace and parent aggregate/membership/checkpoint/cursor/budget/state plus prior controls. Failover cannot deadlock empty bootstrap, split a terminal winner, overlap or partially activate re-cost, invent a refund, reset/rescan verification or mutate a non-Ready partition |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries retirement state/authority result, transition terminal CAS receipt/status, cost-profile lineage/classifier, active campaign slot/state/intent/fence/fold/command result/workspace checkpoint and bounded parent-verification checkpoint/cursor/budget/state beside prior proofs without remote retirement, terminal, recovery, cost, repair, debit, credit, release or lock authority |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check retirement collection versus empty bootstrap/import; Activate/Abort/Seal/Rotate/Distrust/Retire with one terminal meaning; campaign start cardinality, three-bucket transfers, bounded finalization, Fenced intent/recovery/quarantine and authority/workspace-bound activation; parent verification crash/cursor/chunk/churn with bounded work. Require quarantined unavailable custody, one terminal result, no invented refund or partial cost activation and no unbounded/non-Ready capacity mutation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore retirement terminal, unique transition terminal map, cost lineage/high-watermark, active campaign slot/state/intent/fence generation/fold/three aggregates/results/workspace and parent VerificationPending with fixed checkpoint/cursor/cumulative budget. Empty replacement remains possible; old custody, cost activation and affected capacity stay unavailable until their distinct proofs complete |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz retirement EvidenceUnavailable, all six governance CAS operations, terminal response loss, bounded symbolic cost classification, overlapping start CAS, every campaign transition, log-tail maxima, equal aggregate transfer, abort/refund/quarantine, activation authority race, workspace crash cleanup, parent aggregate overflow, checkpoint gaps, cursor recreation, free rescans, child churn and protected-capacity exhaustion |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit retirement independence and non-authority, terminal mutual exclusion, complete-domain cost comparison/destructive weakening, campaign/recovery SoD, three-bucket conservation, bounded fold, physical peak capacity, atomic authority consumption, bounded parent verification/protected capacity and every prior control |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze retirement states/receipt, six-operation governance fence/terminal map, cost lineage/classifier, campaign slot/state/intent/fold/aggregate/result/workspace encodings and parent aggregate/membership/checkpoint/verification state. Legacy missing-retirement, independently writable journal, signed-only cost replacement, two-bucket parent equation, overlapping campaign or scan-on-start schemas fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack retirement deadlock/self-signing/custody escape; Activated+Aborted/Unresolved dual meaning; abort-before-local-CAS races; cost signer/lineage rollback and sampled-comparison bypass; overlapping campaign, ambiguous fence, RecostPending omission, unbounded tail, invented refund, recovery escalation, authority-expiry activation and physical-workspace exhaustion; parent aggregate forgery, unbounded startup, cursor reset/free replay, churn and non-Ready mutation |
