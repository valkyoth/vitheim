...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze non-borrowable per-lane capacity; immutable typed mandatory-drain profile lineage; governed emergency/aggregate change; provisioning proof; derived lane/aggregate fence; fixed locking; dual-profile admission; typed denial; atomic fence events; activation-selected recovery; exact action-bound `CapacityDrainAuthorizationV1` for all begin-drain/activate/reject/abandon actions; and canonical non-wrapping predecessor-linked `CapacityProfileActivationRecordV1` plus checkpoint-before-delete in the atomic active-head/supersession/fence/audit/result/outbox bundle |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Stall/fill lane resources; race shrink, both stages, compaction, fence install/admit/activate/reject, classification, aggregate locking, failover/restore and Recovery isolation; additionally attack unauthorized/expired/replayed/self-approved/substituted drain actions, policy/approval/predecessor/expiry races, rejection cycling, activation record deletion/gap/fork/reorder/duplicate/wrap, checkpoint/high-watermark rollback, response loss and active-row contradiction |
| `0.23.0` | SQLite single-node adapter | Persist profile lineage/diff/activation/provisioning/obligations, derived fence/dual admission, drain-authorization replay/audit and canonical activation record/sequence/checkpoint; prove mandatory drain, physical/aggregate bounds, continuous convergence, fail-atomic authorization/bundle, exhaustion refusal, idempotent response-loss replay and checkpointed recovery or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage/profile CAS/safe shrink/provisioning/aggregate bounds, fixed lane/aggregate locking, final-admission/fence serialization and failover recovery plus drain-authorization policy/predecessor locking, single-use replay defense, atomic record/head/supersession/fence/audit/result/outbox commit, non-wrapping sequence and checkpoint recovery |
| `0.25.0` | Experimental MySQL adapter | Prove typed mandatory-drain profile/fence/dual-admission/activation-record recovery plus drain-authorization and canonical activation-record/checkpoint transaction parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove typed profile/fence/lane/aggregate bounds, activation-selected restore, drain-authorization and activation-record/checkpoint parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific typed profile/fence/lane/aggregate bounds, activation-selected restore, drain-authorization and activation-record/checkpoint parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve profile lineage/generation/digest/state/diff/provisioning/obligations, pending/derived fence and high-watermarks plus exact drain authorization/audit/replay identity and full activation-record chain, sequence, transaction/journal binding, checkpoints and external high-watermarks; reject numeric merge, old-writer mutation, obligation movement, field loss, authorization reuse, record fork/gap/reorder, checkpoint rollback or atomic-bundle split |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger mandatory-drain/provisioning/derived-fence/dual-admission/typed-denial/Recovery-isolation semantics plus drain-authorization binding/SoD/replay, canonical activation-record atomicity/chain/checkpoint and recovery validation; otherwise refuse import/profile |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze mapping ownership/identity/generation/fence/profile and emergency-promotion SoD plus drain-authorization authentication, deployment/tenant/action/diff/coverage/policy/change/incident bindings, requestor/approver/activator quorum, expiry/nonce/idempotency and exact activation/provisioning authorization digest profile |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze profile lineage/diff/activation/provisioning/obligation and derived-fence rows, fixed locking/uniqueness/dual admission/typed denial/atomic events plus drain-authorization replay/audit state, canonical activation-record/head/checkpoint schema, non-wrapping sequence/exhaustion, predecessor chain, transaction/journal binding and atomic head/supersession/activation/fence/event/audit/result/outbox transaction |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Minimize charge checkpoints while preserving debit/terminal/result meaning; minimize drain authorization/audit and activation history only through authenticated predecessor-linked activation checkpoints preserving the complete activation head and active/pending/fence tuple |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze activation-selected `CapacityRecoveryStateV1`, pending/exact fence, obligations and lineage/activation high-watermarks plus activation-record chain/head/checkpoint and external-high-watermark recovery, rejecting raw-generation/numeric merge, half-state, gaps/forks/reorder/duplicates/active-row disagreement/missing checkpoint and authorization replay/rollback |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement immutable typed mandatory-drain profile lineage/provisioning/obligations, derived fence/fixed locking/dual admission/typed denial/atomic events/recovery plus action-bound drain authorization/replay/audit and canonical activation record/head/checkpoint atomicity, sequencing, idempotency and recovery |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve profile/diff/activation/provisioning/obligations/derived fence/dual admission plus exact drain authorization and activation record/checkpoint envelopes across RPC; timeout/response loss/stale workers cannot reuse authority, split the bundle, append twice or invent head/checkpoint state |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over activation-selected profile/pending/fence/obligations/high-watermarks plus drain-authorization replay/audit and activation record/head/checkpoint chain without numeric/raw-generation merge, half-state, reuse, atomic split, duplicate sequence, fork/gap, rollback or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore activation-selected profile/pending/fence/obligations/high-watermarks plus authenticated drain-authority replay/audit and activation record/head/checkpoint chain; reject older/numeric/raw-generation merge, half-state, authorization rollback, gap/fork/reorder/duplicate, active-row disagreement, missing checkpoint and external-high-watermark rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain shrink/provisioning/classification/aggregate-lock/fence traffic stress; race drain authorization policy/approval/predecessor/expiry and record atomicity/response loss/checkpoint deletion across crash/failover/restore; prove one exact authorized result/chain and Recovery availability |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit profile governance/SoD/lineage/classification/activation/provisioning/recovery/fence controls plus drain authorization scope/SoD/expiry/replay/action separation and activation-record encoding, sequence exhaustion, predecessor integrity, atomic bundle, checkpoint and rollback fencing |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze prior profile/diff/activation/recovery/fence formats plus drain-authorization fields/digests/replay outcomes and activation record/sequence/predecessor/versions/diff/fence/provisioning/authorization/continuity/transaction/journal/integrity/checkpoint fields with canonical-none direct activation |
| `0.149.0` | Release candidate and external pentest remediation | Retain every profile/fence/classification/recovery attack; add unauthorized Normal/emergency/aggregate drain DoS, approval reuse/substitution, unauthorized rejection cycling, record deletion/reorder/fork/duplicate/wrap, checkpoint rollback, response loss and active-row contradiction; retest fixes |
