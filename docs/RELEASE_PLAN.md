...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Activate both generation-2 laws; declare VIT-INV-060/061 separately; issue class-bounded grants with authenticated trusted-time/profile/continuity evidence; consume only under a monotonic conservative interval proving pre-expiry CAS; ratchet challenge/sequence/expiry topology receipts; reject stale placement despite rollout/fence loss |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Typed topology successors; issuer/topology credential separation; idempotent authorization issuance/unknown recovery; cross-service time-profile preservation and skew-denial; profile discrimination; fresh topology challenges/sequences; claims/control receipts; replacement/clone fencing |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Race authorization issue versus epoch/revoke/expiry/CAS, lose issuance response, fail over issuer/topology owners separately, replay signed-old topology receipts, roll back or step clocks, widen uncertainty, suspend/resume, and preserve bounded grants, time ratchets, pinned rollout winner, MAC roles, and owner separation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover three-state handoff, topology receipt/observation ratchets, independent authorization lineage/issuance/unknown/break-glass state, issuer time evidence, consumer lower-bound/profile/continuity ratchets, expired-grant/control/claim tombstones, and exact owners; reject inferred issuance, validity extension, ratchet rollback, reissue, owner merge, clone, or resurrection |
