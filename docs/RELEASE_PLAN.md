...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain prior controls; freeze transactional attempt-capacity ledger/reservation set, global head→key/attempt→capacity→domain lock order, reservation-free joins, success/no-write original-bucket transfer and per-leg exact-once checkpoint/deletion settlement |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain prior attacks; crash/fault every lock/check/reserve/insert/transition/bundle/transfer/checkpoint/delete/settlement/response edge, reorder/duplicate/lose responses, race expired stale execution and change profiles across settlement |
| `0.23.0` | SQLite single-node adapter | Prove atomic reservation-plus-attempt, fixed order, reservation-free join, pre-success recheck, original-bucket transfer and per-leg exact-once settlement across busy/crash/reopen or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove ledger row locking, transfer/settlement uniqueness and unknown-outcome reconciliation under concurrency/pool failover with no duplicate/recomputed release |
| `0.25.0` | Experimental MySQL adapter | Prove all prior attempt-ledger transaction/order/original-bucket/exact-once settlement plus lifecycle and publication parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior attempt-ledger transaction/order/original-bucket/exact-once settlement plus lifecycle parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific attempt-ledger transaction/order/original-bucket/exact-once settlement plus lifecycle parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve reservation-set IDs/original buckets, active-to-terminal transfers, settlement IDs/legs/results/balances and lock-order contract plus all prior attempt state; reject recomputation, split state or double release |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require conserved original reservation sets and every exact-once settlement leg with same-or-stronger transaction/order semantics plus prior attempt parity; otherwise fence execution and refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze ledger/reservation-set/settlement schema, global lock order, atomic creation/transfer, success rechecks and unique per-set/leg settlement alongside every prior attempt/replay field |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Freeze checkpoint-versus-envelope release legs, retained checkpoint occupancy, terminal-attempt minimization and cleanup prerequisites plus prior archive/GC policy |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze reservation-set/settlement recovery, original-bucket balances and unknown-outcome reconciliation plus prior attempt owner/lifecycle/replay state; no double release or profile recomputation |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus atomic attempt-capacity ledger, original reservation sets, fixed transition order, pre-success revalidation and exact-once checkpoint/deletion settlement |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve reservation-set/settlement IDs, original buckets, legs/results and attempt/head/fence recheck evidence across RPC; peers cannot invent joins, transfers or releases |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over original reservation sets/balances, transfers, settlement records and unknown outcomes with prior attempt state; no split creation, stale execution, recomputation or double decrement |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore reservation sets/original buckets, balances, transfers and settlement legs/results with prior attempt state; reject reserved-without-attempt, missing completion capacity or release drift |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; fault every ledger boundary, duplicate/reorder/lose settlements, race stale/expired success, change profiles and prove exact conservation under failover |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit atomic creation, lock order, success recheck, original-bucket transfer, checkpoint/delete split and exact-once settlement plus all prior lifecycle controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze reservation-set/bucket/quantity, settlement-ID/leg/result and lock-order formats plus prior attempt schemas; old nodes cannot recompute, collapse legs or double release |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add split reservation/create, join overcharge, stale/expired success, missing transfer, premature/wrong-leg/double release, unknown settlement and profile-recompute attacks; retest fixes |
