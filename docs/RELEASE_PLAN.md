...
| `0.51.46` | Parent-local one-winner Release or Retention effect intent committed before remote dispatch | Release/retain race, two destinations, branch/owner/effect substitution, outbox split, intent mutating member version |
| `0.51.47` | Complete expected-version authorization-issuance ledger plus closed one-shot activation-, intent-, route- and local-version-bound remote authority consumed with the physical effect | Issue/seal race, dispatch without issuance, head/count rollback or overflow, identity/digest fork, inactive/disposed root, stale route/object ABA, replay, receipt substitution |
...
| `0.51.50` | Complete authorization/delivery cut and closure root consumed with the conflict head by the terminal member CAS | Issuance racing cut, invented no-issue/expiry, unresolved outbox/inbox, late valid receipt, stale closure terminalization |
| `0.51.51` | Create-or-join physical-effect observation ledger, independent current-generation incident and funded residual ownership for truly unauthorized post-closure effects | Duplicate/reordered observation, double charge/owner, same-ID fork, route ABA, terminal history reopened, old capacity reused, incident overflow |
| `0.51.52` | No-effect intent disposition plus immutable cumulative lifetime budget, explicit exhaustion, abandonment or predecessor-linked funded replanning | Dead destination, uncertain delivery mislabeled absent, identity/ledger reuse, budget refund/reset/overflow, concurrent last replan, unauthorized widening |
