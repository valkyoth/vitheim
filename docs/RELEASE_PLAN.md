...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze publication-profile root/bootstrap/rotation/high-watermark, fail-closed new-identity reprovisioning after root loss/compromise, tenant/deployment scope and one attempt slot, compatibility/ratchet and the linearizable activation-fence port/receipt/status/finality contract. Independent signatures are not CAS; old profiles may terminalize but activate only under MayActivateExisting |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist activation-fence/attempt-slot state, completion reserve, two heads, split work/physical ledgers, signed backend storage-cost profile, Recovery parent ledger/double-entry transfers/equation, universal lock rank/traces, ReleasePending and settlement/archive/deletion/parent-credit evidence. Missing transactionality, cost/fence continuity, participant or capacity refuses |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check concurrent Activate/Rotate/Distrust port CAS, root loss, attempt-slot cardinality, publication reserve admission, two heads, both child equations, parent debit/credit equation, cost-profile transitions, universal lock traces and reclaim. Require one fence winner, one attempt, atomic parent/child transfer and no stranded terminalization |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore external fence receipt/status/finality and one-attempt slot, operational Active/candidate, root-loss terminal fence, completion reserve, destination cost profile, parent/child ledgers/transfers/equations, universal rank and exact reclaim/credit membership. Older snapshots cannot activate, debit, credit or release |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz CAS concurrency, independent-signature substitution, root loss, slot collisions, publication saturation, backend/schema/index cost drift, destination re-cost, parent/child response loss, transfer overflow, lock inversion and archive/delete/credit crashes |
...
| `0.149.0` | Release candidate and external pentest remediation | Attack non-linearizable fence providers, quorum nonintersection, fork/rollback/status loss, inferred root recovery, cross-tenant/shared-profile slot exhaustion, cost undercharge, source-counter copy, parent leak/double credit/split state, terminal-capacity starvation, head aliasing, premature reclaim and lock inversion |
