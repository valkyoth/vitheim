...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain every prior sparse-replay control; freeze one cumulative per-scope non-wrapping predecessor/root/CAS head, greatest-committed-head-plus-hot proof, Staged→Verified→CommittedHead→HotRowsDeleted→OrphanGcEligible publication, immutable upload-before-local-CAS/delete, reader authority and fenced GC without distributed transactions |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain all prior attacks; race consumption versus snapshot/upload/verify/head-CAS/delete, competing publishers, stale readers, head exhaustion/fork/substitution, coalescing, delayed visibility, unknown outcomes, staged/orphan reads, premature GC and committed-head rollback |
| `0.23.0` | SQLite single-node adapter | Persist all prior sparse state plus cumulative head/publication lifecycle/covered-row evidence; prove immutable filesystem upload, verified head-CAS/exact-delete, unknown-outcome reconciliation and GC fencing or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove concurrent cumulative-head CAS, current-hot-row proof, publication visibility, indivisible local head/delete, coalescing and orphan cleanup without cross-system transactions |
| `0.25.0` | Experimental MySQL adapter | Prove all prior semantics plus cumulative-head/publication/local-CAS-delete/GC parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior semantics plus cumulative-head/publication/local-CAS-delete/GC parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific prior semantics plus cumulative-head/publication/local-CAS-delete/GC parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve all prior state plus cumulative head sequence/predecessor/root/scope/version, publication states/identity, covered-row evidence and object references; reject head synthesis/fork/rollback, staged promotion, lost hot rows or unsafe GC |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger prior semantics plus cumulative-head/current-hot proof and publication/unknown-outcome/GC meaning; otherwise refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain prior sparse schema; freeze cumulative-head row/CAS/index, publication states, immutable object references, verified visibility, exact covered-row local transaction, coalescing/orphan-GC metadata and unknown-outcome reconciliation |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Retain prior recovery; freeze cumulative head/publication/current-hot/object-visibility/GC recovery and high-watermarks, rejecting stale-head proof, fork, partial head/delete, staged authority or rollback |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus cumulative replay head and staged immutable publication/local head-CAS-delete/unknown-outcome/GC protocol without distributed transactions |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve all prior fields plus cumulative-head/publication/current-hot/covered-row evidence across RPC; peers cannot promote staged data, use stale heads or invent deletion/visibility/GC |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over all prior state plus cumulative head/publication/current-hot/object visibility/GC without stale proof, fork, partial head/delete, staged authority, rollback or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore every prior field plus cumulative head/publication/current-hot/object/GC state; reject stale proof, fork, staged promotion, partial delete, premature GC and head rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; exercise all publication races, stale readers, competing publishers, coalescing, delayed visibility, unknown outcomes, orphan cleanup and migration under traffic/failover |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit every prior control plus cumulative-head uniqueness/CAS, greatest-head-plus-hot proof, publication authority, indivisible local delete, reconciliation and GC fences |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze all prior formats plus head sequence/predecessor/root/scope/version, publication states/identity, covered-row evidence, reader rules and GC eligibility; forbid cross-system atomicity assumptions |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add consumption/compaction races, stale heads, competing publishers, head fork/substitution, coalescing, visibility/unknown outcomes, staged/orphan authority, premature GC and rollback; retest fixes |
