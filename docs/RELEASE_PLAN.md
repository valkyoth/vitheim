...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add predecessor-linked capacity checkpoint/replay head; owned manifest/budget/cursor/publication lifecycle; typed Stage/Verify/Commit/MarkOrphan/FinalizeGc; stable genesis high-watermark guard; Prepare and exclusive Reconcile; witness disposition; and governed witness conformance profile/result. Every head writer follows routing→residual→capacity→attempt-set→high-watermark; ordinary writers must read/deny PreparePending or Witnessed, while archive writers continue publication→replay. Every immediate/delayed/query/replay witness outcome enters only Reconcile; one stable disposition ID owns one terminal disposition charge/state/result and distinct later Commit charge. Model canonical archive counters as one equal sequence pair, advanced only by successful capacity-archive Commit (`ArchiveFinalize`, never FinalizeGc), with one atomic `(MAX, MAX)` sentinel | Same-material retry returns its result, changed material conflicts, no unnamed callback or duplicate disposition charge exists, no writer passes the fence or reverses locks, local rollback cannot refund capacity, and sequence equality/sentinel atomicity cannot split |
| `0.30.0` | Export/import checkpoint/replay/high-watermark ancestry, witness profile/authority/distrust epochs, Prepare/request/disposition IDs and charges/results, publication lifecycle/budgets/cursors, universal lock-rank identity, archive sequence pair/sentinel/fence, exact-set proofs and hot suffix | Import reads the independent watermark first, preserves immediate/recovered disposition equivalence, exact-completes witnessed state or stays unready, and rejects lock/profile/material mismatch, missing charge, older head, fork, partial deletion or asymmetric pair |
| `0.140.1` | Freeze checkpoint/replay/publication/high-watermark/witness-disposition/sequence-pair domains and widths; witness conformance fields; closed tags/commands/conflicts; canonical ordering; proof budgets/cursors; exact proof preimages; and definition of ArchiveFinalize | Review proves adapter-defined verification/witness meaning, callback mutation, disposition alias/rebind, FinalizeGc confusion, lifecycle confusion, sequence split and lock-rank ambiguity impossible |
| `0.140.2` | Freeze one routing→residual→capacity→attempt-set→high-watermark→publication→replay order. Begin creates the guard; ordinary writers lock/read it before omitting rows. Freeze Prepare-before-traffic, exclusive Reconcile disposition import and archive final CAS. Finalization consumes its distinct charge, advances the sequence pair, CASes Verified→ConsumedByCommit, installs replay head and deletes exact captured rows; all Prepare/disposition/Commit charges remain hot | Backend acquisition traces for every ordinary writer plus Prepare/Reconcile/Commit/MarkOrphan/GC match exactly; immediate and recovered witness paths persist the same state/result; no lock inversion, fence bypass, split transition or self-deletion is observable |
| `0.140.3` | Separate charge/attempt-head/guard owner, checkpoint builder, publisher, bounded verifier, tenant/lineage witness authority/profile governor, disposition importer, publication/replay owner, orphan collector, result reader, Replan planner and restore validator | No adapter/callback writes Witnessed, writer chooses lock subset/class, witness self-governs profile/distrust, replay owner forges negative evidence, GC collects referenced state or planner mints capacity |
| `0.140.5` | Retain lock-rank/profile/conformance evidence, checkpoint/replay/high-watermark ancestry, Prepare requests, all disposition IDs/charges/results, manifests/chunks/lifecycle/cursors, exact proofs, epochs, pair sentinel/fence and hot suffix; GC only unreferenced OrphanGcEligible data | Retention cannot erase a fence/disposition, make immediate/query outcomes differ, collect winning state, manufacture unseen history, split the pair sentinel or detach any result from its charge |
| `0.140.6` | Freeze failover for every lock acquisition/CAS/cursor, Prepare request, immediate/delayed/query/replay result, witness authority/key rotation/distrust/non-equivocation/durability, deterministic successor, negative abort, deletion response, concurrent writers/Replan and pair exhaustion | Takeover rejoins one disposition/result, unknown stays fenced, stale/failed witness profiles deny, witnessed state exact-commits or remains unready, and no duplicate charge, bypass, equivocation, older fallback, sequence fork or one-sided sentinel appears |
| `0.142.0` | RPC exposes read-only guard/publication/progress/budget, witness profile/result/authority/disposition, checkpoint/replay/watermark identity, archive sequence-pair/sentinel status and typed replay/conflict/unavailable outcomes | RPC cannot import witness outside Reconcile, select charge/class/lock order, declare negative evidence, widen headroom, clear pair sentinel, mark viable state orphan or authorize from stale guard/profile |
| `0.143.0` | Model every ordinary writer versus Prepare/Reconcile/Commit/MarkOrphan/GC; immediate versus lost witness response; changed disposition material; witness equivocation/rotation/distrust/failover; proof-budget exhaustion; compaction retry; coordinated rollback; Replan; Commit-versus-exhaustion and asymmetric pair states | One ordered writer/disposition/charge/result meaning is reachable; paired sequences advance only with capacity-archive Commit, never GC, and invalid witness or asymmetric exhaustion never authorizes progress |
| `0.145.0` | Restore/migrate by reading independent watermark/profile/distrust first, verifying exact Prepare/disposition/charges/results/chunks/cursors, completing witnessed successor, replaying hot suffix and reproving class/head/pair-sentinel equations and Replan obligations | Missing profile/witness/key/chunk/result/charge, direct callback state, rollback/fork, unfinished completion, deletion mismatch or one-sided sentinel is unavailable/corrupt—not repaired, inferred or older-head fallback |
| `0.146.0` | Fuzz lock acquisition traces; lifecycle/disposition tags; immediate/query/replay encodings; witness CAS/non-equivocation/signatures/key epochs/negative proofs; manifests/budgets/cursors; exact proofs; pair values/sentinel/fence; ArchiveFinalize versus FinalizeGc; and last/rollover states | Inversion, skipped guard, callback injection, duplicate/missing charge, material substitution, forged negative, equivocation, ABA, false membership, invalid GC, rollback, pair overflow/gap/asymmetry fail closed |
| `0.147.0` | Audit universal lock order/guard reads, exclusive witness import and exact-once disposition charge, witness conformance/governance, bounded publication, anti-rollback, atomic finalization, pair proof/sentinel, Replan, HA and restore | Zero high/critical lock-order, fence-bypass, callback-authority, witness-equivocation, charge, lifecycle, unbounded-work, rollback, split-sentinel, sequence or result-integrity finding |
| `0.148.0` | Reject legacy imports lacking expanded lock-rank evidence, stable guard, exclusive disposition path/charge, governed witness conformance, independent watermark, closed publication, exact history or atomic sequence-pair sentinel | No shim trusts callback/adapter state, guesses disposition, maps separate sentinels, treats GC as ArchiveFinalize, picks older history, fabricates profile/cursor/sequence or reconstructs results |
| `0.149.0` | External pentest targets skipped guard/lock inversion, direct immediate callback, duplicate/missing disposition charge, changed witness material, predecessor equivocation, stale signer/rotation/distrust, forged negative evidence, failover durability, publication races/budget bypass, rollback, partial deletion, Replan reset, Commit/exhaustion race and asymmetric pair restore | Zero unresolved critical/high findings and no waived locking, witness authority/import, exact-charge, publication, anti-rollback, atomic pair exhaustion, sequence or capacity-conservation issue |
| `0.150.0` and `1.0.0` | Demonstrate one lock order, deterministic witness import/conformance, closed bounded publication, independently anchored rollback safety, exact charge replay and atomic archive-sequence-pair conservation on every backend/RPC/HA/Replan/import/restore path | Production remains blocked until the unchanged candidate passes acquisition, witness, lifecycle, rollback, pair-boundary, fault/model campaigns and independent pentest |
