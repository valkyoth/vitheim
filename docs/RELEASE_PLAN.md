...
| `0.18.3` | Declared ownership roots including global lineage, exact local placement owners, serialized durable rollout root/law, activation-floor catalogs, shared verifier, and closed semantic registry | `VIT-INV-057/058/059` and `VIT-LAW-007/008@g01`; compiled immutable singleton topology; typed attested/fenced workload identity and closed authenticated-receipt contract; irreversible `ActivationAuthorized` with atomic receipt/outbox and pinned generation; terminal `Superseded`; `AllRequired`; no distributed transaction; cryptographic artifact/time/ancestry gates; P/N/M/F |
| `0.18.4` | Source-delivery-complete evaluator re-evaluation plus first real catalog successor rollout | Actual predecessor verification; crash before/after authorization state/receipt/outbox commit; authorization versus abandon/supersede/revoke/delayed delivery/failover; two candidates with lost responses and exactly one authorized winner; static-topology mutation rejection; cloned/stale identity; forged receipt digest; unreachable placement; revocation reconciliation |
...
| `0.19.0` | Integrity chains and signed-checkpoint interface | Event tamper/domain separation plus verified catalog envelope, immutable rollout/placement manifest, authorization state/receipt/outbox/pinned generation atomicity, prepare/global/convergence/revocation receipts, exact local owner identity/fence, global activation receipt, and local catalog/distrust/time ratchets |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Distinct `VIT-CAP-057/058/059`; exact local owner key/fence; irreversible authorization state/receipt/outbox/pin support; future dormant/committed handoff support; planning/active separation; exact envelope/verifier; closed realization |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Every tuple including `VIT-LAW-007/008`; authorization/abandon/supersede/revoke races; closed receipt-profile substitution; cloned identity, mutable topology, missing receipt, unfenced quorum, topology race, time rollback, artifact/ancestry/predecessor/scope/signature mutation, and untrusted rejection |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve separate global/rollout/local owners, immutable rollout/placement generations, authorization state/receipt/outbox/pin, messages/deadlines, exact local identity/fences, payload/envelope/revocation state, and anti-resurrection |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Invoke verifier; reject rollout/receipt or owner-identity loss, cloned placement, topology rewrite, planning confusion, ancestry/scope/predecessor omission, untrusted/future tuple, upgrade, truncation, and integrity loss |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/receipt authentication, and credential-operation decision | Freeze verifier/trust roots plus non-exportable hardware identity or orchestrator online single-use action claims with fixed maximum lifetime and zero offline authority; freeze exactly signed, authority-MAC, or attested-channel-admission receipt variants and durable integrity anchors |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology state, active catalogs/ratchets, and provider state | Separate future `VIT-INV-060` dormant/committed handoff row plus `VIT-INV-057/058/059`; rollout authorization state/receipt/outbox/pin; expected-version manifest/fence/tombstone rows; local identity and catalog/time ratchets without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology, and recovery decision | Select `AllRequired` or prove fully fenced quorum; authorization versus abandon/supersede/revoke/failover; online action-claim expiry; closed receipt variants/anchors; independent topology owner; four-owner failover; revocation convergence; time loss; typed RPO/RTO |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus governed topology-authority handoff | Initialize an exact dormant singleton; activate/converge epoch 12 under `VIT-LAW-008@g01`; require local generation-2 admission; commit the bound handoff CAS; prove exclusive static-or-dynamic authority and every crash boundary |
| `0.142.0` | Split deployments with governed credential-operation TCB and exact topology-issued catalog placement owners | Typed topology successors; online single-use orchestrator action claims with zero offline authority; exact closed receipt variants/anchors; authenticated routing; replacement/clone fencing; incompatible executor rejection |
...
| `0.143.0` | HA atomic work, catalog rollout/topology, governed execution, and cancellation recovery | Authorization commit versus abandon/supersede/revoke and lost/delayed responses; pinned authorized winner and pre-authorization loser; independent topology-command races; cloned keys/action claims; four-owner failover |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover dormant/committed handoff, exclusive topology authority, authorization state/receipt/outbox/pin, action-claim tombstones, receipt integrity anchors, global lineage, and exact local ratchets; reject inferred commit, reopened authorization, clones, resurrection, and untrusted catalogs |
