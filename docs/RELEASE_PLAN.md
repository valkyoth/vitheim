...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Independently authorized manifest initialization creates one generation zero; every successor is Prepared, externally checkpointed and verified, then Active before its CAS head may move. Restore compares local Active history with the independent high-watermark and returns typed rollback/unavailable when continuity is not provable. Activation binds that witnessed head, creates registry generation zero, protected corruption-control reserve and an obligation-wide control lineage under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→clearance-anchor-source-manifest-head→corruption-control-reserve→history-obligation/corruption-control-lineage/corruption-fence/clearance-anchor-registry/lineage-disposition→retention/legal-hold→audit/result/outbox. Every fence episode precharges non-resetting cumulative limits; exhaustion permanently quarantines and release is checkpointed/exact-once. Registry advancement atomically rebinds Open scope state. Manifest weakening and permanent rebuild rejection use complete local lifecycles plus canonical full-field destructive-revocation preimages. Typed restoration and parent/proposal/one-successor rebuild retain every prior control |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve manifest transition state/checkpoint chain/publication receipts/external high-watermark/current head/Active records/destructive-authority tombstones, registry ratchet/rebinding, protected reserve, obligation-wide control lineage/episode ordinals/cumulative limits/disposition/checkpoint/settlements, scope lifetime state, typed joins and rebuild parent/proposal/rejection/successor isomorphically. Local-below-witness rollback, missing receipt/lineage, recreated/reset counters, duplicate reserve settlement, raw-generation selection, remote-only revocation, stranded scope, invalid join or consumed parent denies |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Select the independent manifest-checkpoint authority profile—witness, secure monotonic store or external quorum—and freeze receipt, high-watermark, trusted-time/key-continuity/rotation/revocation behavior. Freeze complete weakening/rebuild-rejection grant and revocation preimages, including target-covering lifetime and distinct revocation idempotency. No local database or remote message alone establishes latest state or destructive effect |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist Prepared/published/Active manifest transitions, checkpoint chain/receipt/high-watermark and current-head CAS; registry rebinding; protected reserve; obligation-wide control lineage, episode/cumulative maxima, dispositions, checkpoint and exact-once settlements; scope state; typed algebra and rebuild lifecycle. Missing/reset/recreated/duplicate state or local rollback refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate manifest transition preparation, external checkpoint publication/verification, final activation, weakening/revocation, control-lineage release/settlement, scope/attempt and rebuild roles from clearance/registry/collection authority. No actor self-witnesses a head, self-admits a destructive grant, resets capacity or infers rejection/release |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain manifest checkpoint/receipt/high-watermark and destructive-authority history, control-lineage episodes/counters/checkpoint/settlements, registry/scope tombstones, typed-join evidence and rebuild lineage. Erasure cannot erase rollback evidence, reset capacity, duplicate settlement or reopen head/scope/reserve/parent authority |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve external checkpoint verification and manifest activation, protected reserve/control lineage, atomic registry/scope rebind, scope work, typed join and one parent/successor across failover. Witness outage fails closed; restore below external high-watermark is unready; failover cannot reset episodes/counters, duplicate settlement, abbreviate revocation or infer rejection/release |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries manifest transition/checkpoint/publication receipts, external-high-watermark evidence, full destructive revocation preimages, registry/scope rebinding and control-lineage evidence, but cannot self-witness or activate a head, reset episodes/counters, settle reserve, apply remote revocation, weaken policy, reject, release or select a successor remotely |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check Prepared/published/Active manifest and head-CAS races, witness receipt/high-watermark races, every destructive-grant/revocation CAS, repeated fence episodes, control-lineage precharge/quarantine/rebuild/release/settlement, registry/scope rebinding, typed joins and successor activation. Require one witnessed head, one obligation lineage and no reset or duplicate release |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore manifest transitions/head only after external checkpoint high-watermark verification; restore complete destructive tombstones, reserve/control lineage/episodes/counters/disposition/checkpoint/settlements, registry/scope and rebuild parent before work. Older self-consistent snapshots, reset lineage or duplicated reserve release remain unready |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz witness outage/substitution, checkpoint/head fork/gap/reorder/rollback, key rotation/revocation, destructive-revocation field omission/substitution, repeated clear/re-fence cycles, episode exhaustion, settlement crashes, scope rebinding, algebra and invalid proposals |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit external manifest checkpoint provenance/high-watermarks, destructive authorization/revocation preimages, obligation-wide lineage episode/accounting conservation, exact-once reserve settlements, scope rebinding, typed joins and successor lineage |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze manifest transition/checkpoint/receipt/high-watermark, full destructive-revocation and control-lineage/episode/checkpoint/settlement encodings. Old local-only head, reset lineage, absent reserve and abbreviated grants/revocations fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack local-database rollback below external witness, witness/key outage and substitution, head activation before publication, destructive revocation omission/substitution, repeated re-fence budget reset, episode exhaustion, release/settlement duplication, saturated fencing and every prior clearance/rebuild case; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for externally witnessed manifest state, complete destructive authority/revocation, protected obligation-wide control lineage and exact-once settlement, registry/scope, typed algebra, rebuild authority and every prior recovery artifact |
