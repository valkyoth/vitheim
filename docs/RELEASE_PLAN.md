...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Implement pinned genesis trust and the existing total registry machine—including absorbing AuthorityFenced—and structural `c_max` capacity controls; add claim-acquisition Admit/Publish/Reconcile/status with closed outcomes; store PreparedNonAuthoritative only as a no-journal-identity proposal; finalize LocalFinalizeCommitted by fresh immutable event/result/audit/outbox append plus unsigned commit record; add named post-commit signer port/profile and signing response-loss lane; extend `R_claim`; checkpoint terminal proposals; close claim-history publication as Staged/Verified/Committed or OrphanEligible/Collected under Commit reconciliation, current non-reference and protected GC capacity; preserve provider redemption, observation lanes, evidence lattice, `C/H` versus `E/J`, `B_EM`/`R_first` | Crash/race acquisition, proposal, confirm, journal append, signer, receipt settlement and archive cleanup. Prove no proposal is journal-visible/successful, event bodies match confirmed bytes, lost issuance/signature cannot recreate permits, pre-signing fails, unknown claims remain live, and Committed/frontier-referenced history never orphans |
| `0.30.0` | Export/import claim-acquisition attempts/outcomes/settlements; proposal bytes/digests/checkpoints without journal identity; immutable journal-finalization and unsigned commit records; signer identity/key/distrust/history/signing status/signed receipts; claim publication state, Commit status, non-reference proof, orphan-GC budget/results; plus all prior authority/capacity/observation/restore state | Round trip every crash cut. Missing/forked/defaulted proposal/journal separation, acquisition status, commit record/signature, signer history, orphan proof/budget or archive state is RestoreUnready; import never promotes proposals, invents permits/signatures/non-reference or collects committed history |
| `0.140.1` | Freeze encodings/domains/result-conflict surfaces for acquisition, proposal body versus journal envelope, unsigned commit record, signer profile/signing lane, proposal checkpoint/collection and publication orphan states/proofs/budgets alongside all prior authority schemas | Review rejects provisional-event flags, preallocated journal identities, generic acquisition/signing status, unsigned registry receipts, ambiguous signer/key history, orphaning before Commit reconciliation, incomplete reference authorities and unknown fields |
| `0.140.2` | Freeze acquisition capacity/reservation CAS; physically separate proposal transactions; confirm then final immutable event/result/audit/outbox/commit-record transaction; zero signer I/O inside DB transaction; post-commit signing and registry receipt settlement; ClaimHistory Commit-status/MarkOrphan/Collect lock/CAS order; all prior authority/effect/restore transactions | Fault every boundary. No proposal enters journal/projection or command success, no unsigned/pre-signed record settles, no lost issuance/signature recreates a permit, no stale non-reference collects, and no backend requires a distributed transaction |
| `0.140.3` | Separate registry claim issuer, acquisition publisher/reconciler, proposal writer, immutable journal finalizer, signer/key custodian, signed-receipt publisher/settler, claim-history publisher, non-reference proof issuer, retention/legal-hold owner, collector, provider executor and existing registry/fence/restore roles | No actor self-issues and uses a lost claim, promotes its proposal, signs before commit, controls signer distrust and settlement, self-proves orphan non-reference, bypasses holds or erases committed/frontier-referenced history |
| `0.140.5` | Retain acquisition attempts/receipts, proposal evidence/checkpoints, immutable journal/commit records, signer key and historical verification, signed settlement, publication Commit-status/non-reference/orphan decisions and prior claim/frontier history | Compaction cannot convert proposal to event, lose retry/status/signature provenance, invalidate historical receipts, infer Commit absence, collect held/referenced/Committed material, starve cleanup capacity or weaken prior authority/custody controls |
| `0.140.6` | Freeze failover during claim acquisition, proposal/confirm/finalize, post-commit signing/rotation/distrust/status, final receipt settlement, publication Commit reconciliation/non-reference/orphan collection and all prior registry/archive/restore boundaries | Takeover uses status/Reconcile only, recreates no acquisition/signing permit, preserves proposal/journal separation and exact cleanup state/capacity, and refuses authority when registry/signer/archive history is unavailable |
| `0.140.11` | Decide proposal-store versus immutable journal backend separation; registry acquisition outcomes/expiry proof; external HSM/KMS or explicit in-process signer profile, signer identity/SoD/key rotation/distrust/history; proposal retention; claim archive Commit-status/non-reference authorities; protected orphan-GC row/byte/work ceilings; and all previously required registry/capacity/shard/region/SLO profiles | Architecture/security/legal/forensic/operations approve one exact end-to-end profile. Any backend, registry, signer or archive unable to prove physical separation, loss-safe acquisition/signing, historical verification and non-starvable safe cleanup refuses recovery activation |
| `0.142.0` | Expose read-only acquisition outcome, proposal evidence metadata, journal-finalization/unsigned-record/signature/settlement status, signer profile/key epoch/history availability and claim-publication/orphan-GC state alongside prior recovery observability | RPC cannot return proposal bodies as events, expose signing material, acquire claims, reconstruct permits, invoke signer, publish receipts, mark/collect or translate Unknown/Unavailable/Unsigned into authority |
| `0.143.0` | Model acquisition response loss, proposal/journal type separation, exact body-byte finalization, post-commit signer/receipt settlement, signer rotation/distrust, publication Commit reconciliation/non-reference/orphan capacity and the prior authority/capacity/history state machines | No reachable state promotes a proposal, allocates journal identity early, signs uncommitted material, forgets an issued/unknown claim, settles unsigned state, or collects committed/referenced history |
| `0.145.0` | Restore acquisition attempts/status, proposal checkpoint and immutable journal relation, commit records, signer/key history/signatures/settlement, publication Commit/orphan state, non-reference proof and cleanup budget with greatest claim replay head/hot suffix and prior recovery state | Missing acquisition, proposal/journal linkage, committed-record signature history, Commit status, reference authority or protected cleanup state remains RestoreUnready; restore never re-signs blindly, promotes a proposal or infers orphan eligibility |
| `0.146.0` | Load/soak acquisition and signing registry round trips, proposal store and immutable journal finalization, signer HSM/KMS or selected in-process profile, proposal/archive cleanup under normal-claim saturation, sharding and regional failure; fuzz all new IDs/states/proofs/keys plus prior equations | Throughput/tail-latency/backpressure/fairness SLOs pass. Lost responses, byte/envelope substitution, pre-signing, key distrust, stale non-reference, cleanup starvation, ABA/wrap/overflow and one-short reserves fail closed |
| `0.147.0` | Audit immutable-event separation, claim-acquisition loss safety, post-commit signer/key lifecycle and claim-history/proposal orphan controls together with registry absorption, capacity, provider fence, observation, custody and DR | Zero high/critical provisional-event, permit-recreation, pre-signing, unverifiable-receipt, unsafe-orphan/cleanup-starvation, state-reopening, drain, stale-send, evidence, accounting, trust or role finding |
| `0.148.0` | Reject imports lacking explicit proposal namespace and no-allocation proof, acquisition status, commit record/signer provenance, signed settlement, proposal checkpoint, publication Commit/non-reference/orphan state and protected cleanup budget, plus all prior recovery state | No shim maps proposal to event, Issued/Unknown to absent, unsigned to signed, signer-key absence to valid, staged publication to orphan, missing proof to non-reference or inferred cleanup capacity |
| `0.149.0` | Pentest proposal/journal aliasing and projection leakage, acquisition receipt loss/permit replay, pre-commit/in-transaction signing, signer/key substitution/rotation/distrust, signed-receipt loss, Commit-response ambiguity, stale/forged orphan proof, hold bypass and cleanup starvation plus prior registry/fence/archive attacks | Zero unresolved critical/high and no waived immutable-event, acquisition, signer, settlement, orphan-GC, authority-absorption, complete-drain, provider-fence, evidence, anti-rollback, trust, archive or custody issue |
| `0.150.0` and `1.0.0` | Demonstrate one unchanged candidate across selected proposal store, immutable event journal, registry, signer, claim-history archive/GC, provider fence, database, sharding/region, HA and restore profiles, including every response-loss, projection-rebuild, key-history, orphan-race and hard-exhaustion case | Production remains blocked until Phase C, cross-adapter/signer conformance, performance/soak, DR, independent audit and pentest pass with no high/critical issue; unsupported profiles remain refused |
