...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add predecessor-linked capacity checkpoint/replay head; owned manifest/budget/cursor/publication lifecycle; typed Stage/Verify/Commit/MarkOrphan/FinalizeGc; stable high-watermark guard; Prepare and exclusive Reconcile; witness disposition/conformance; acyclic CapacityArchiveWitnessProposal; and bounded Recovery-funded reconciliation-query budget/attempt admission. The authority signs only a proposal digest excluding future receipt/results; Reconcile binds proposal+receipt and Commit binds proposal+receipt+Reconcile. The core path owns three distinct hot charges/head advances—Prepare, terminal Reconcile and ArchiveFinalize Commit—plus `q` separately classed query admissions. Ordinary writers acquire the common prefix through the guard; archive writers continue publication→replay. Model archive counters as one equal pair advanced only by ArchiveFinalize, never FinalizeGc, with one atomic `(MAX, MAX)` sentinel | Mechanical dependency-DAG verification is acyclic; no callback, self-reference, encoding ambiguity, unbounded query traffic, hidden/duplicate charge, omitted-row inversion, rollback refund or split sentinel exists |
| `0.30.0` | Export/import checkpoint/replay/high-watermark ancestry, canonical proposal/receipt/result DAG, witness profile/authority/distrust epochs, all three core charges/results, query budget/attempt IDs/counters/backoff/admission charges, publication lifecycle, lock-rank identity, archive sequence pair/sentinel/fence, exact-set proofs and hot suffix | Import reads the independent watermark first, proves `3 + q` charged advances and DAG identity, preserves query exhaustion and immediate/recovered equivalence, exact-completes witnessed state or stays unready, and rejects missing/forked/cyclic/ambiguous/asymmetric state |
| `0.140.1` | Freeze proposal/receipt/Reconcile/Commit dependency directions, canonical encoding/domain separation, checkpoint/replay/publication/high-watermark/disposition/query-budget/sequence-pair schemas and widths, conformance fields, closed tags/conflicts, proof preimages and ArchiveFinalize definition | Generated schema-reference/hash-dependency graph rejects cycles, signature-field inclusion, alternate encoding and cross-epoch substitution; review proves callback mutation, disposition rebind, budget widening, FinalizeGc confusion and sequence split impossible |
| `0.140.2` | Freeze routing→residual→capacity→attempt-set→high-watermark common prefix, guard omission rule and archive-only publication→replay suffix. Freeze query admission before its one process-local permit, exclusive Reconcile import and final CAS. Preserve three core hot charges plus query admissions; finalization advances the pair, installs replay head and deletes exact captured rows | Traces require the common prefix for ordinary writers, full suffix for archive writers and canonical relative order for every acquired subset; no omitted earlier row is acquired later. Faults prove `3 + q`, one permit per query ID and no terminal charge on timeout/absence |
| `0.140.3` | Separate charge/attempt-head/guard owner, checkpoint builder, publisher, verifier, witness authority/profile governor, proposal builder, signature verifier, disposition importer, query-budget owner/executor, publication/replay owner, orphan collector, reader, Replan planner and restore validator | No authority signs self-referential material, adapter/callback writes Witnessed, query worker widens budget/reconstructs a permit, writer chooses rank/class, replay owner forges negative evidence, GC collects referenced state or planner mints capacity |
| `0.140.5` | Retain dependency-graph evidence, proposals/receipts/results, lock traces, profile/conformance, checkpoint/replay/high-watermark ancestry, all three core and query-admission charges/results, query counters/backoff/exhaustion, lifecycle/cursors/proofs/epochs, pair sentinel/fence and hot suffix | Retention cannot create a cycle, erase a fence/attempt/charge, turn exhaustion into authority, make immediate/query outcomes differ, collect winning state, manufacture unseen history or detach a result from its charge |
| `0.140.6` | Freeze failover at every lock/CAS/cursor, proposal/signature/receipt, each core charge, query admission/permit boundary/backoff/exhaustion, immediate/delayed/query/replay result, witness key rotation/distrust/durability, negative abort, deletion, Replan and pair exhaustion | Takeover rejoins one proposal/disposition/result, never reconstructs a query permit, unknown/exhausted stays fenced, witnessed state exact-commits or remains unready, and no duplicate charge, cyclic rebind, bypass, older fallback or one-sided sentinel appears |
| `0.142.0` | RPC exposes read-only guard/publication/progress, proposal/receipt/DAG identity, witness profile/result/authority/disposition, three-charge equation, query budget/attempt/backoff/exhaustion, checkpoint/replay/watermark and pair-sentinel status | RPC cannot sign/rewrite a proposal, import outside Reconcile, issue a query permit, select charge/class/order, widen/reset budget, infer negative evidence, clear exhaustion/sentinel or authorize from stale state |
| `0.143.0` | Model schema-dependency nodes, every ordinary/archive writer, no-query and `q`-query paths, admission/permit crash boundaries, immediate/lost response, budget exhaustion, changed witness material, equivocation/rotation/failover, compaction, rollback, Replan and pair exhaustion | Only an acyclic proposal→receipt→Reconcile→Commit history and exactly `3 + q` charged advances are reachable; unknown/exhausted never becomes unwitnessed and archive sequences advance only with Commit |
| `0.145.0` | Restore/migrate by reading watermark/profile/distrust first and verifying proposal DAG, three core plus query-admission charges/results, query budget/counter/backoff/exhaustion, chunks/cursors, hot suffix and class/head/pair equations | Missing/cyclic proposal edge, signature/key, query admission, charge/result, rollback/fork, unfinished completion, deletion mismatch or asymmetric pair is unavailable/corrupt—not inferred, repaired or replaced by an older head |
| `0.146.0` | Fuzz generated dependency graphs and canonical encodings; lock subsets; lifecycle/disposition/query tags; query counters/bounds/backoff/permits; witness CAS/signatures/key epochs/negative proofs; three-charge equations; manifests/proofs; pair sentinel; ArchiveFinalize versus FinalizeGc | Cycle, signature inclusion, encoding/epoch substitution, inversion, skipped guard, permit reconstruction, query-budget bypass, duplicate/missing charge, forged negative, invalid GC, rollback and pair overflow/asymmetry fail closed |
| `0.147.0` | Audit acyclic commitment construction, common-prefix/archive-suffix lock traces, exclusive witness import, exact three-plus-query charges, bounded Recovery query lane, conformance/governance, publication, rollback protection, pair sentinel, Replan, HA and restore | Zero high/critical dependency-cycle, lock-order, callback-authority, permit, unbounded-I/O, witness-equivocation, charge, lifecycle, rollback, split-sentinel, sequence or result-integrity finding |
| `0.148.0` | Reject legacy imports lacking proposal DAG/canonical encoding, exact three-core-charge evidence, bounded query state, expanded rank evidence, stable guard, exclusive disposition path, governed conformance, independent watermark, exact history or atomic pair sentinel | No shim reconstructs a proposal/permit/result, trusts callback state, guesses a charge/query outcome, maps split sentinels, treats GC as ArchiveFinalize, picks older history or fabricates budget/profile/cursor/sequence |
| `0.149.0` | External pentest targets commitment cycles/signature inclusion/encoding substitution, skipped guard/lock inversion, callback import, missing/duplicate core or query charge, query amplification/permit replay/backoff bypass, changed witness material, equivocation/stale signer/forged negative/failover, rollback, deletion and asymmetric restore | Zero unresolved critical/high findings and no waived commitment-DAG, bounded-query, locking, witness authority/import, exact-charge, publication, anti-rollback, pair-exhaustion or capacity-conservation issue |
| `0.150.0` and `1.0.0` | Demonstrate acyclic canonical witness commitments, common-prefix/archive-suffix ordering, deterministic bounded witness recovery, exactly `3 + q` charge conservation, closed publication, rollback safety and atomic pair conservation on every backend/RPC/HA/Replan/import/restore path | Production remains blocked until the unchanged candidate passes dependency-graph, acquisition, query-budget/permit, charge, witness, lifecycle, rollback, boundary, fault/model and independent-pentest campaigns |
