...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add lineage-wide RevalidationCounterCapacityState with proof ID/version, plan/lineage, closed writer-class enum including Replan, per-class admitted/consumed/remaining `u128` counts, total attempt-set-head writes, revalidation advance/rollover subsets, per-domain sentinel reservations, current counter tuple and predecessor-linked state version/digest. Add immutable RevalidationCounterWriterCharge keyed by lineage+class+command/idempotency and carrying prior/successor state/counts/counters, mutation/head and result/audit/outbox. Every head writer locks capacity state before the head and commits one charge+state CAS+mutation+head+result atomically | Per class admitted=consumed+remaining, total consumed equals head delta, state-version delta equals charges plus terminal bit, typed subsets equal advances/rollovers, exact retry rejoins and CAS losers consume nothing |
...
| `0.142.0` | RPC exposes class-specific admitted/consumed/remaining, total/head equation, charge/result lookup and proof/sentinel status as read-only evidence | RPC cannot choose a writer class, decrement, reset, synthesize a charge or authorize from stale totals |
...
| `0.146.0` | Fuzz class tags, admitted/consumed/remaining arithmetic, state versions/digests, charge IDs, request/result bindings, counter tuples and coverage boundaries | Overflow, underflow, substitution, duplicate, omission, ABA and head/count mismatch fail closed |
...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add generational high-watermark guard with immutable Committed/Aborted tombstones; Prepare-only claim whose sole process-local permit may submit only after the winning Prepare commit; closed query attempts; and immutable per-attempt terminalization reservations. Commit atomically installs replay/deletes captured rows/opens `g+1`. Abort instead enters AbortDrainPending, denies new queries and invokes `SealCapacityArchiveWitnessProposalDefinitelyUnwitnessed`; authority-side expected-predecessor/proposal CAS permanently tombstones rejection and blocks every late submit. Only its independently discoverable receipt, fully terminal queries with consumed reservations/settlements and no positive receipt permit rollover; positive-after-seal fences authority equivocation | Race submission against Prepare commit, wrong/stale states, seal, delayed submit and live queries. Prove no submit before commit, no status-based abort, no late submit after seal, no rollover before drain, exactly `q` reservations created/consumed and no unfenced contradictory evidence |
| `0.30.0` | Export/import guard generation/current binding/terminal chain, submission and abort-seal claims, proposal DAG, authority rejection tombstone/seal receipt/equivocation fence, three core charges, query charges/states/outcomes/terminalization reservations/concurrency settlements, trusted-time bindings, replay/watermark ancestry, publication lifecycle, rank evidence and pair sentinel | Import reads authority state first, independently rediscovers positive and permanent-seal evidence, proves generation continuity, `3 + 2q - e` and `q` reservation conservation, and rejects older open guards, AbortDrainPending half-state, missing seal/tombstone/reservation/settlement, contradictory receipts, identity reuse, cycles or asymmetric pair |
| `0.140.1` | Freeze guard/state/disposition, submission-claim, abort-seal-claim, permanent-seal receipt/tombstone/equivocation-fence and query-terminalization-reservation schemas; proposal directions, trusted-time preimages, query-ID derivation, widths and ArchiveFinalize definition | Review proves only committed Prepare returns submission authority, ordinary negative status cannot roll, seal permanently rejects its proposal, positive-after-seal fences, reservations cannot be reinterpreted, query counter is absent and dependency encodings are unambiguous/acyclic |
| `0.140.2` | Freeze atomic Prepare→permit ordering, Commit rollover, PreparePending→AbortDrainPending claim, authority seal CAS, final fully-drained abort rollover, common lock order, and query admission+reservation→permit/Reconcile+reservation-consumption+settlement transactions | Backend faults expose each bundle all-or-none. Admission is no-write without complete escrow; `3 + 2q - e`, exactly `q` created/consumed reservations, one permit/outcome/settlement, permanent late-submit rejection and no stuck successful or prematurely reopened guard hold |
| `0.140.3` | Separate guard owner, proposal builder/verifier, submission executor, abort-seal executor, authority implementation, disposition importer, query admission/reservation owner, terminalizer/settler, charge/head owner, publication/replay/GC owners, Replan and restore validators | No role can submit before Prepare commit, both submit and seal, self-declare permanent negative, recreate a permit, release/reassign another attempt's reservation, settle twice, suppress equivocation, omit rollover or mutate authority state through an adapter |
| `0.140.5` | Retain every generation/tombstone/result, both claims, permanent authority rejection tombstone/seal receipt/equivocation fence, proposal DAG, core/query charges, attempts/outcomes/terminalization reservations/settlements, trusted-time evidence, traces, ancestry, lifecycle, pair fence and hot suffix | Retention cannot reopen an old guard, forget a sealed rejection or equivocation, enable late submission, erase/move/release a reservation, leak concurrency, turn timeout/exhaustion into negative evidence or detach replay state from its generation |
| `0.140.6` | Freeze failover at Prepare commit/permit, submission/seal race, AbortDrainPending, query admission/reservation/permit/invocation/response/terminalization/settlement, trusted-time continuity, both rollovers, contradictory authority receipts, deletion, Replan and counter boundaries | Takeover returns status only, rediscovers seal/positive evidence independently, preserves every reservation, terminalizes each query once, fences equivocation, opens exactly one next guard only after complete success/permanent abort drain and never exposes split replay or late submission |
| `0.142.0` | RPC exposes read-only guards including AbortDrainPending, replay/watermark binding, both claim statuses, permanent seal/tombstone/equivocation state, query outcome/reservation/settlement/time/backoff/exhaustion, charge/reservation equations, proposal DAG and pair sentinel | RPC cannot issue/reconstruct submit/seal/query permits, roll a guard, release/move a reservation, import/settle outside Reconcile, suppress contradictory evidence or authorize from terminal/stale state |
| `0.143.0` | Model repeated Commit/permanent-seal rollovers; stale writers; Prepare/submit/seal ordering; delayed original submit versus live queries; every query/reservation/outcome/settlement race; trusted-time drift; no-query and `q`-query paths; graph, compaction, Replan and exhaustion | Only continuous histories and exact `3 + 2q - e` plus `q` reservation conservation are reachable; seal or submit wins authority CAS, never both silently, abort drains fully, every query terminalizes/settles once and contradiction fences |
| `0.145.0` | Restore/migrate by independently querying authority for both positive and permanent-seal evidence, then proving guard/tombstone/replay continuity, both claims, rejection tombstone/equivocation fence, proposal DAG, query reservations/outcomes/settlements/time, charges/hot suffix and pair equations | Older guard, AbortDrainPending half-state, missing permanent-seal evidence, positive-after-seal, split rollover, live concurrency without reservation, duplicated consumption, time rollback, identity reuse or missing/cyclic evidence is fenced/unavailable—not repaired |
| `0.146.0` | Fuzz guard tags/bindings; submit/seal/query claim material; authority CAS/tombstone/receipt conflicts; query reservations/outcomes/settlements and sequence-derived IDs; time/backoff; charge equations; DAG encodings; lock subsets and pair sentinel | Pre-commit/wrong-state submission, late submit after seal, premature abort rollover, equivocation suppression, reservation release/reassignment/double use, concurrency leak, invented counter, time rollback, charge mismatch, cycle and pair asymmetry fail closed |
| `0.147.0` | Audit generational liveness/anti-rollback, Prepare-before-submit, permanent abort sealing, authority equivocation fencing, closed query lifecycle and reservation/concurrency conservation, IDs/time/charges, DAG, lock order, publication, pair sentinel, Replan, HA and restore | Zero high/critical submission-order, seal-race, late-submit, abort-drain, equivocation, reservation, query/concurrency, replay/rollover, time/counter/dependency/charge/lock/rollback/sequence/result-integrity finding |
| `0.148.0` | Reject legacy imports lacking guards/tombstones/next binding, both claims, authority rejection tombstone/seal receipt/equivocation fence, closed query reservation/state/outcome/settlement, time backoff, writer-derived ID, charges, DAG, rank, independent watermark or pair sentinel | No shim infers permanent negative, opens AbortDrainPending/Witnessed, invents or releases reservations, reconstructs permits/results, hides contradictory evidence, guesses outcomes, treats GC as ArchiveFinalize or selects older history |
| `0.149.0` | External pentest targets submission before Prepare commit, submit/seal CAS races, delayed submit and live queries, forged/missing seal receipt, positive-after-seal, premature abort rollover, stale generation, replay rollback, permit replay/resubmission, reservation theft/release, outcome substitution, concurrency/time/counter/charge/DAG/pair failures | Zero unresolved critical/high findings and no waived submission-order, permanent-seal, authority-equivocation, abort-drain, query-reservation/settlement, generational-lifecycle, exact-charge, locking, anti-rollback or pair-conservation issue |
| `0.150.0` and `1.0.0` | Demonstrate continuous guards, repeatable archives, post-commit one-shot submission, permanently sealed fully drained abort, equivocation fencing, deterministic bounded query recovery, exactly `q` reservation creation/consumption, `3 + 2q - e`, trusted time, acyclic commitments, closed publication, rollback safety and pair conservation across every backend/RPC/HA/Replan/import/restore path | Production remains blocked until the unchanged candidate passes submission/seal/drain/equivocation, guard-liveness, query reservation/settlement/concurrency/time, dependency, acquisition, charge, authority, rollback, boundary, fault/model and independent-pentest campaigns |
