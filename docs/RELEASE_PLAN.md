...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze non-borrowable per-lane durable charge capacity; immutable VIT-INV-061 profile identity/generation/digest/predecessor lineage; expected-version activation; mandatory PendingDrain for every typed canonical reduction; governed emergency/aggregate changes; physical provisioning evidence; activation-record recovery; downgrade refusal; canonically derived lane/aggregate `TopologyAuthorizationPresentationChargeLedgerCapacityDrainFenceV1`; one successor/fence; dual-profile admission; typed pre-debit denial; and atomic installed/consumed events with no callable fence helper |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain every resource, shrink, provisioning, generation and fence race; additionally test initially safe and aggregate-only reductions, overflow/unknown/incomparable/mixed-schema classification, derived lane coverage/fixed lock order, higher unactivated generations, multiple-active/contradictory/unreachable/half-state recovery, direct helper denial, and Recovery preservation |
| `0.23.0` | SQLite single-node adapter | Persist profile lineage, typed diff, committed activation records/sequences, obligations and exact derived fence; prove mandatory drain, aggregate coverage, atomic events, activation-selected recovery and continuous-traffic convergence or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage work, profile CAS, mandatory shrink/drain, non-borrowable maintenance, provisioning/aggregate bounds, fixed lane/aggregate locking, atomic fence events, final-admission serialization, exact rejection and activation-record failover recovery |
| `0.25.0` | Experimental MySQL adapter | Prove profile lineage, typed mandatory drain, activation-record restore, lane/aggregate lifecycle, derived fence and dual-profile parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove profile lineage, typed mandatory drain, activation-record restore, lane/aggregate lifecycle and derived-fence bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific profile lineage, typed mandatory drain, activation-record restore, lane/aggregate lifecycle and derived-fence bounds or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve profile lineage/generation/digest/state/typed classification/evidence, committed activation records/sequences/high-watermarks, reservations, pending drain and exact fence predecessor/successor/digests/derived lanes/reduced aggregates/sequence/continuity; reject numeric merge, activation inference, old-writer mutation, obligation movement, fence loss/duplication/helper clear/bypass or rollback |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger mandatory-drain classification, activation-record recovery, provisioning, derived lane/aggregate fence, dual-profile admission, typed denial, atomic event and Recovery-isolation semantics; otherwise refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze profile lineage, typed diff/mandatory-drain transition, committed activation record/sequence, provisioning and obligations plus derived lane/aggregate fence rows, fixed row locking, one-successor uniqueness, dual-profile admission, typed denial, atomic events and capability refusal |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze activation-selected `CapacityRecoveryStateV1`, lineage/activation high-watermarks, optional pending/exact derived fence, obligation reconstruction, authenticated failover/restore, contradictory/half-state refusal, stale-worker fencing and Recovery availability |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement immutable profile lineage, typed mandatory-drain activation, provisioning, lane/aggregate obligations, derived fence, dual-profile admission, typed denial, atomic fence events and activation-selected recovery alongside the two-stage lifecycle |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve profile classification, activation records/sequences, provisioning, obligations, derived fence coverage and dual-profile admission across RPC; timeout/stale worker cannot invoke helpers, bypass, activate, transfer, cancel or invent state |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over committed activation state, lineage/activation high-watermarks, obligations, pending successor and derived fence without raw-generation inference, numeric merge, half-state, duplicate/bypass, loss or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore `CapacityRecoveryStateV1` from committed activation records plus pending/exact derived fence and high-watermarks; reject raw-generation inference, older-larger/numeric merge, multiple-active/contradictory/unreachable/half-state, unauthenticated coverage and downgrade writers |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain all shrink/provisioning/generation/fence stress; race initially safe and aggregate-only typed reductions, fixed row locking and atomic fence events across crash/failover/restore; prove activation-selected convergence, typed denial and Recovery |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit profile governance/SoD, immutable lineage, typed mandatory drain, activation CAS/records/high-watermarks, provisioning, exact recovery, derived aggregate coverage, fence authenticity/uniqueness, atomic events, dual-profile predicate and stale-worker/downgrade resistance |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze profile identity/generation/digest/predecessor/state/typed diff, activation record/sequence/high-watermarks, obligations, recovery tuple, fence digests/derived lanes/reduced aggregates/sequence/continuity, atomic events and typed denial; mixed versions cannot infer activation, supply coverage, invoke helpers or weaken meaning |
| `0.149.0` | Release candidate and external pentest remediation | Retain every profile/fence attack and target direct safe/aggregate-only activation, comparison overflow/schema ambiguity, incomplete derived coverage, row-lock races, raw-generation recovery, contradictory/multiple-active/unreachable/half-state restore and direct fence-helper invocation; retest every fix |
