...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Preserve all seal, deficit, authorization and budget controls. Add a total remediation action/result matrix: provision→ResumableCommit, delete→VerifiedDeletionAndReplan, migrate→MigratedPlanReadyForReplan, explicit RetainPermanentlyFenced→PermanentlyFencedRetained. Failed/Unknown work retains predecessor/charge; permanent retention creates a named member containing base+deficit. Split Completed into WithoutResidualCustody/WithResidualCustody and atomically transfer every surviving unknown member plus protected budget/authority/deficit/fence provenance into a dedicated residual obligation. Make losing pre-Begin cleanup exact-candidate scoped under a durable winner mapping and non-aliasing settlement identity | No action selects another policy outcome; every retained charge has a member; completed lineages cannot strand or reopen custody work; losing cleanup cannot block or release the winner |
| `0.30.0` | Export/import the remediation attempt/action/continuation and permanently fenced member, completed-disposition/residual obligation/budget/transfer result, and candidate winner mapping plus exact cleanup settlement alongside every previously required artifact | Missing/defaulted/forked mapping, member, residual transfer or candidate proof preserves predecessor charge, completed fence and losing candidate encumbrance |
...
| `0.140.2` | Freeze action-specific success CAS, base+deficit→permanently-fenced-member transaction, final-Commit residual transfer/disposition CAS, residual command matrix and candidate-scoped cleanup/winner recheck | Every backend proves one exact policy/member/residual/candidate winner atomically within limits or refuses |
| `0.140.3` | Freeze distinct provision/delete/migrate/permanent-retention authority and SoD, residual-obligation owner/resolver, completed-lineage proof owner, candidate cleanup owner and winner activator separation | No failed operator self-selects retention, completed lineage borrows work, or cleanup actor releases winning capacity |
| `0.140.5` | Retain action/attempt/result/member history, residual obligation budget/authority/deficit/fence provenance and candidate winner/cleanup settlement ancestry | Compaction cannot remap an action, orphan a retained charge, erase residual authority or make a loser resemble the winner |
| `0.140.6` | Freeze response-loss/failover/restore for failed/Unknown remediation, permanent-member settlement, With/WithoutResidual selection, residual operations and many-candidate cleanup | Unknown retains predecessor; completed residuals restore independently; every loser remains distinguishable from the winner |
| `0.142.0` | RPC exposes typed mapped continuation/attempt, permanent member, completed disposition/residual obligation and candidate winner/cleanup evidence only | Remote status cannot choose retention, mutate a completed lineage, spend residual budget or release candidate capacity |
...
| `0.145.0` | Restore named permanent members, exact residual transfer/budget/authorization/deficit/fence provenance and candidate winner plus all cleanup settlements independently of compacted plan rows | Missing relationship retains the larger charge, residual fence and candidate encumbrance; no inferred winner or cleanup |
| `0.146.0` | Fuzz action/continuation discriminants, remediation attempt states, member/residual arithmetic, completed command matrix, winner mapping and activation/cleanup identity domains | Fall-through, enum confusion, omission/double count, budget borrowing, alias, ABA and cross-candidate cleanup fail closed |
| `0.147.0` | Audit action-specific authority, permanent custody representation, residual lifetime budgeting/restore, completed-lineage closure and candidate-scoped cleanup alongside all prior controls | Zero high/critical policy, conservation, custody, resource, authority, lifecycle, restore or parity finding |
| `0.148.0` | Reject legacy encodings without total remediation mapping/attempt/member, explicit completed disposition/residual transfer and candidate winner mapping | No compatibility shim invents a continuation, retained member, residual budget or candidate ownership |
| `0.149.0` | External pentest targets action/result substitution, failure→retention fall-through, permanently fenced capacity omission, post-completion budget bypass/exhaustion, residual compaction loss, many-candidate cleanup deadlock and winner release | Zero unresolved critical/high findings and no waived policy totality, conservation, boundedness, custody or authority issue |
| `0.150.0` and `1.0.0` | Demonstrate continuity for the total remediation matrix, named permanent member, exact With/WithoutResidual completion and independent residual resolution, plus arbitrary losing-candidate cleanup without winner impact, while retaining all earlier seal/deficit/authorization guarantees | Production remains blocked until selected backends pass, residual obligations survive full lifecycle/restore and the unchanged commit passes independent pentest |
