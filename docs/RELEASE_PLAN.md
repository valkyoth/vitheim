...
| `0.29.0` | Preserve pinned genesis, AuthorityFenced, `R_first`, `B_EM`, `c_max`, PreparedNonAuthoritative, typed LocalFinalize, post-commit signing, DomainAggregateTarget/ControlOwnerTarget/CanonicalNoAggregateTarget, ProposedEventDescriptor, ClaimAcquisitionPending, no-classified-plaintext payload lifecycle, predecessor chain, independently unique identities with one-sided conflict, CommitReceiptSignerRecoveryRequired and every prior guarantee. Add tenant PayloadLifecycleMaintenanceCapacity with `L_max`, landing reservation and atomic `P_claim` handoff/conservation; extend the post-bootstrap lock order through structurally keyed payload-use/membership/lifecycle rows, CommitCutRecordedTimePort/RecordedTimeAuthorityRatchet, aggregate/journal and commit/result/audit/outbox. Preserve PayloadReferenceUseClaim, CanonicalNoDomainEventChain, `e_claim_max/p_claim_max/n_signer_max/k_signer` and cardinality-safe `P_claim/S_claim` | Race handoff/settlement/hold/cleanup, opposite reference order, collection/retention/erasure/archive/restore, ratchet/stream append and every backend commit-cut mechanism. No prior test is removed; no dual/missing owner, `c_max` retention pin, ABBA order, capability lie or time fallback is accepted |
| `0.30.0` | Export/import lifecycle-capacity profile/landing/handoff/conservation/checkpoint roots, universal structural keys/order, commit-cut port/profile/evidence beside prior payload-use/cardinality/ratchet/chain/identity/signer state | Round trip every cut. Missing/forked handoff owner, lifecycle charge, key/order version or backend commit-cut proof is RestoreUnready; import never releases both ledgers, pins command capacity, reorders locks or upgrades an incapable adapter |
| `0.140.1` | Freeze PayloadLifecycleMaintenanceCapacity/Handoff schemas and equations, payload/time structural key encodings, CommitCutRecordedTimePort mechanisms/evidence plus prior PayloadReferenceUseClaim/cardinality/ratchet/chain/identity/signer schemas | Review rejects ambiguous capacity lifetime, duplicate/no owner, opaque-only/process-local keys, transaction-start-as-commit, mutable conformance claim and every prior unsafe alias |
| `0.140.2` | Freeze landing reservation with claim admission, atomic post-signing/pre-finalization-publication `P_claim→L_maintenance` handoff, terminal-no-commit release, lifecycle ledger/checkpoint CAS, universal subset lock order/deadlock retry and per-backend commit-cut/head/ratchet transaction beside every prior transaction | Fault every boundary. Registry settlement releases `c_max` only after the durable handoff permits publication. No handoff gap/duplication, cleanup starvation, partial membership, ABBA inversion, false commit interval, distributed transaction, duplicate identity, second append or result rewrite can commit |
| `0.140.3` | Separate operational/lifecycle capacity owners, membership/handoff/release/hold/cleanup, structural-key/order validator, commit-cut port/conformance verifier, ratchet/finalizer and prior signer/archive roles | No actor self-handoffs/releases, borrows protected cleanup, chooses an opaque/process-local key, certifies its own backend shortcut, fabricates commit containment or bypasses prior SoD |
| `0.140.4` | Freeze redacted lifecycle landing/occupancy/handoff/conservation, commit-cut capability/evidence and lock-order conflict status beside prior payload/ratchet/chain/cardinality/recovery APIs | API reveals no reference/store/key/signer evidence and never maps handoff pending, ledger saturation, missing commit-cut proof, transfer unknown or RecoveryRequired to absent/released/success |
| `0.140.5` | Retain lifecycle profile/landing/handoff/conservation/checkpoints, payload/time structural keys, backend conformance evidence and prior use/ratchet/chain/signer history under retention/hold/erasure policy | Compaction cannot lose the sole capacity owner, release held lifecycle work, pin deleted operational claims, change lock keys, promote an unsupported backend or weaken prior custody/cleanup |
| `0.140.6` | Freeze failover during settlement handoff/lifecycle cleanup, shared-subset lock acquisition, commit-cut source/topology changes and every prior transfer/ratchet/domain/signer/archive/restore boundary | Takeover joins exact owner/occupancy/order/profile state, preserves protected cleanup and `P_claim/S_claim/L_max`, recreates no permit/plaintext, and unreadies on missing evidence |
| `0.140.11` | Decide per backend/tenant: `L_max` class/physical ceilings, landing/handoff/no-commit policy, protected reconcile/release/erasure/compaction workers, payload/time structural key mapping and deadlock retry; exact SQLite/PostgreSQL/MySQL/MongoDB/SurrealDB CommitCutRecordedTime mechanism/version/topology; plus prior `e/p/n/k`, identity/attester/archive/SLO choices | Architecture/security/privacy/legal/forensic/operations approve one exact profile. A backend without conserved lifecycle handoff, protected cleanup, universal ordering, passing commit-cut conformance, erasable storage or durable identities refuses governed domain claims |
| `0.142.0` | Expose read-only redacted lifecycle landing/occupancy/handoff/conservation, lock-key/order version and commit-cut capability/provenance beside prior payload-use/ratchet/chain/cardinality/RecoveryRequired status | RPC cannot reveal opaque references/evidence, mutate capacity/order/profile, authorize release, weaken hold, free command capacity early or imply domain support without proof |
| `0.143.0` | Model operational-to-lifecycle capacity ownership, `L_max` saturation/cleanup priority, universal subset lock order, all five backend commit-cut ports and all prior payload/cardinality/ratchet/chain/signer state machines | No reachable state has dual/no owner, retained `c_max`, classified-admission spillover, cleanup starvation, ABBA deadlock, stale membership/time mutation, capability lie or weakened prior guarantee |
| `0.145.0` | Restore lifecycle capacity/landing/handoff/conservation/checkpoint state, structural key/order version, backend commit-cut profile/evidence and all prior payload/ratchet/chain/claim replay evidence | Missing/forked evidence remains RestoreUnready; restore never guesses an owner, refunds both ledgers, resets occupancy/time, reorders subsets, promotes a backend, frees IDs or resets capacity |
| `0.146.0` | Load/soak lifecycle handoff and long holds to `L_max`, protected cleanup under saturation, opposite-order shared-reference/finalization/collection/archive races and each backend delay/failover corpus; fuzz owner/key/order/profile/evidence beside prior fields | Unrelated commands and existing cleanup meet SLO while new classified payloads backpressure. Dual release, capacity pin/theft, ABBA, commit-cut substitution, overflow and one-short reserves fail closed |
| `0.147.0` | Audit Phase A lifecycle inheritance, operational/lifecycle capacity conservation, protected cleanup, universal lock ordering, backend commit-cut trust and prior payload/cardinality/ratchet/identity/signer/archive/DR controls | Zero high/critical plaintext, capacity-lifetime, cleanup-starvation, lock-order, backend-time, shared-release, replay-identity, signer-recovery, immutable-event, evidence or role finding |
| `0.148.0` | Reject imports lacking lifecycle capacity/landing/handoff/conservation, structural key/order version, exact backend commit-cut profile/evidence or any prior recovery field | No shim maps operational release to lifecycle ownership, held occupancy to free, opaque key to structural key, start/insert/client time to commit or unsupported adapter to DomainAggregateTarget |
| `0.149.0` | Pentest handoff crash/duplication/loss, `L_max`/physical saturation and cleanup starvation, cross-tenant borrowing, opposite-order shared-reference deadlocks, hold/erasure/archive/restore races, all five backend commit-cut lies and every prior payload/identity/signer/archive/fence attack | Zero unresolved critical/high and no waived lifecycle-capacity, lock-order, commit-cut, payload-use, cardinality, ratchet, chain-kind, signer-recovery, identity, attestation, settlement, evidence or trust issue |
| `0.150.0` and `1.0.0` | Demonstrate one unchanged candidate across selected lifecycle-capacity/handoff, universal lock-order, backend commit-cut, payload-use/ratchet/chain, dual-identity, cardinality-funded signer-recovery, journal/registry/attester/archive/HA/restore profiles | Production remains blocked until Phase A/B/C and all five promised-adapter conformance gates, long-retention/hold capacity tests, maximum-cardinality progress, performance/soak, DR, independent audit and pentest pass with no high/critical issue; unsupported profiles remain refused |
