...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Persist one destination-local operation/job/budget/reservation/lifecycle/fence/candidate/tombstone/authorization/revocation-inbox/barrier/result/cleanup lineage. Derive `MigrationImportOwnerManifestV1` only from source/destination schemas, migration plan, the currently admitted destination VIT-LAW-009 tuple/manifest and its authenticated dependency set; no separate invariant catalog or importer-selected universe exists. Owners issue dormant receipts through owner-held capability. Activation authorization has closed Issued/Consumed/ExpiredUnused/RevokedUnused state; revocation takes effect only when an authenticated intent advances the issuer sequence and locally CAS-transitions the same row activation consumes. The co-located transaction activates every selected owner plus barrier/result/audit/outbox, or none |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze migration activation and revocation-intent signer/key/authentication profiles, issuer continuity and non-wrapping revocation sequence, trusted time, authenticated outbox/inbox receipts, exact retry/conflict and tombstones; only destination-local CAS is revocation |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist the admitted VIT-LAW-009 tuple/manifest/dependencies with manifest derivation; immutable operation/budget/lifecycle/candidate state; authorization row; revocation intent/inbox/sequence/tombstone/result; and barrier/cleanup. Activation, expiry and revocation share job→candidate/barrier→authorization locking and one terminal CAS; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact requestor/approver/activator/issuer separation and post-issuance rule: existing authority remains candidate-bound until consumed, expired or destination-locally revoked; remote emission alone has no effect |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve admitted-law owner derivation and exact migration/import job/candidate/receipt/barrier state plus revocation intent/inbox/issuer sequence/tombstone/result/outbox across HA/DR; failover cannot reset a sequence, reinterpret remote emission as revocation, turn prepared/terminal state active or reverse Consumed |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries authenticated VIT-LAW-009 tuple/manifest/dependencies and revocation intent provenance; a peer cannot supply an invariant catalog, select owners or make revocation effective before the destination-local inbox/CAS |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over admitted-law derivation, revocation inbox/sequence and one durable job/candidate/authorization/barrier lineage. Lose responses around revocation and activation commits; recover exactly one terminal result with no partial owner visibility, sequence reset or remote-revocation inference |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore the admitted law tuple/manifest/dependency closure, revocation intent/inbox/sequence/tombstone/result and authorization row before work; older backups cannot resurrect authorization, lower the sequence, invent an owner universe or reverse Consumed |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Race stale/future law generations, dependency changes, forged pseudo catalogs, revocation emission/delay/reorder/duplicate/local CAS, expiry, activation, failover and response loss; exactly one local terminal wins and every selected owner is visible or none |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit no-invariant-catalog trust closure, law-dependency derivation, revocation authenticator/sequence/inbox/CAS/tombstone/result, fixed lock order, all-owner atomicity and idempotent terminal results |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze admitted-law tuple/manifest/dependency, contributor algorithm, revocation-intent/inbox/sequence/tombstone/result and shared authorization-row formats. Unknown, omitted, reordered, pseudo-catalog or remote-effect fields fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack stale/future law tuple, dependency omission/injection, algorithm/digest substitution, forged pseudo catalog, cross-candidate intent, stale key/sequence/continuity, remote-only revocation, delay/reorder/duplicate, activation/expiry CAS and lost responses; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for admitted-law owner derivation and every revocation intent/inbox/sequence/tombstone/result plus authorization and activation result |
...
- Production import cutover is governed by VIT-INV-062 and VIT-LAW-009. Trusted
  ...
  Owner-held capabilities authenticate
  ...
  or revoked-unused. Revocation is effective only after authenticated intent
  delivery commits the destination inbox, issuer-sequence high-watermark and
  same-row CAS; emission alone has no effect and late intent cannot reverse
  Consumed. Job/barrier and every selected owner guard are co-located;
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  matching owner set and canonical barrier result or no authority at all.
