...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain every profile/fence/Recovery control; freeze exact action-bound `CapacityDrainAuthorizationV1`, closed consumption lifecycle, canonical action result/typed conflict, trusted-time/key envelope and ratchets, long-drain/fresh-activation semantics, and activation checkpoint coverage of replay/time/key state |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain all prior capacity attacks; add exact/conflicting response-loss retry, consumed reuse, expired/revoked unused, expiry during drain, policy revocation, fresh activation/rejection, time loss/uncertainty/rollback/suspend, old restore, key rotation, replay-checkpoint deletion and resurrection attacks |
| `0.23.0` | SQLite single-node adapter | Persist prior profile/fence/activation state plus drain authorization lifecycle, atomic consumption/tombstone/result bundle, time/key/continuity ratchets and checkpoint-complete replay state; prove exact retry and typed conflict or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain concurrent profile/fence protections; prove serialization of authorization consumption versus exact/conflicting retry, expiry/revocation/key rotation, fresh activation after long drain, atomic result bundle and checkpointed failover |
| `0.25.0` | Experimental MySQL adapter | Prove all prior profile/fence semantics plus closed drain-authorization lifecycle, trusted-time validation, retry/conflict atomicity and replay-complete checkpoint parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior profile/fence semantics plus lifecycle/time/key/atomic-result/checkpoint parity under transactions and failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific prior semantics plus lifecycle/time/key/atomic-result/checkpoint parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve all prior profile/activation state plus authorization lifecycle, canonical request/result/validation digests, tombstones, time/profile/continuity/key/consumption/result high-watermarks and checkpoint linkage; reject lifecycle widening, authority resurrection, field loss, rollback or bundle split |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger prior capacity semantics plus exact drain lifecycle/time/key/retry/conflict/long-drain/checkpoint meaning; otherwise refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain prior profile/activation atomicity; freeze closed authorization lifecycle rows and one consumption/tombstone/result/mutation/event/audit/outbox transaction, canonical exact-retry lookup, typed conflict and all time/key/result high-watermarks |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Minimize prior evidence only after checkpointing authorization consumption/result high-watermarks, validation digests, trusted-time interval/profile/epoch, signer/key epochs/profile and permanent replay tombstones with the activation/pending/fence tuple |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Retain activation-selected recovery; freeze lifecycle/time/key/consumption/result/tombstone recovery and external ratchets, rejecting exact-result loss, conflicting replay acceptance, expired/revoked resurrection, missing checkpoint or rollback |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior profile/fence control plus closed drain-authorization consumption, atomic stable results/conflicts, trusted-time/key ratchets, long-drain semantics and replay-complete activation checkpoint/recovery |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve all prior envelopes plus authorization lifecycle/request/result/validation/time/key fields across RPC; response loss returns one result and changed retry never reuses authority or duplicates effects |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over all prior capacity state plus authorization lifecycle/time/key/consumption/result ratchets and replay-complete checkpoints without duplicate result/effect, conflict acceptance, resurrection, rollback or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore every prior capacity field and replay-complete authorization checkpoint; reject old lifecycle/time/key/result/tombstone state, authority resurrection, result loss, conflict acceptance and every prior chain/head rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; exercise exact/conflicting retry, consumed reuse, long-drain expiry, revocation/fresh authority, time loss/rollback/suspend, old restore, key rotation and authorization-checkpoint deletion across traffic/crash/failover |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit every prior profile/activation control plus closed lifecycle, atomic consumption/result/tombstone bundle, trusted-time/signer/key validation and ratchets, long-drain rule, exact/conflict behavior and checkpoint completeness |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze all prior formats plus lifecycle states, action/request/result/validation digests, conflict result, trusted-time interval/profile/epoch, issuer continuity, signer/key epoch/profile, tombstones and high-watermarks without semantic weakening |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add response-loss exact/conflicting retry, consumed/expired/revoked reuse, long-drain authority changes, time rollback/suspend/old restore, key rotation and checkpoint-deletion resurrection; retest fixes |
