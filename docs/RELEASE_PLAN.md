...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze canonical authorization receipt V1, complete issuer/consumer time schemas, atomic topology bundle, mandatory deadline-CAS, and bounded replay lifecycle with quotas/horizon/checkpoints/compaction/alerts |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Add deadline pauses plus quota/horizon/checkpoint/delete/archive/key-rotation/concurrent-replay crashes; require pre-expiry/no-late commit and no compacted receipt resurrection or unavailable-history reissue |
| `0.23.0` | SQLite single-node adapter | Prove deadline-CAS, complete fields, bounded quota/replay/checkpoint/compaction storage and archive-loss denial or refuse VIT-CAP-060/061; locking, rollback, injection, permissions, tenant partition |
| `0.24.0` | PostgreSQL reference production adapter | Prove no-late-commit, atomic topology bundle, field parity, bounded replay lifecycle and pause/failover/compaction matrix plus existing evaluator/rotation/recovery/floor controls |
| `0.25.0` | Experimental MySQL adapter | Prove deadline-CAS, field parity and bounded replay lifecycle or refuse VIT-CAP-060/061; isolation/encoding, rollback, tenant enforcement, no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove deadline-CAS under write concern/failover, field parity and bounded replay lifecycle or refuse VIT-CAP-060/061; transaction/query/collection isolation, no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific deadline-CAS, field parity and bounded replay lifecycle or refuse VIT-CAP-060/061; namespace/graph/query isolation, capability truthfulness, no default v1 claim |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve receipt/time/deadline-CAS plus issuance sequence/budgets/hot results/checkpoint chain/covered-through/archive/key/compaction state; reject downgrade, fork, reset, horizon shortening and resurrection |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Preserve every authorization/time/mechanism/replay-lifecycle field and admit only same-or-stronger deadline and historical-proof behavior; missing archive remains fail-closed, never unused |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze SQLite/PostgreSQL deadline-CAS plus exact replay quotas/horizon/algorithm/cadence/backlog/alerts; separately place canonical receipt/time/topology bundles and owner-local checkpoints; block profiles without no-late-commit and bounded anti-replay proof |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Separate erasable exact topology outcomes from minimal non-sensitive anti-replay checkpoint; missing copies, retention precedence, omitted surfaces, sensitive tombstone/evidence inflation |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Race authorization issue/change/revoke, trusted-time loss/discontinuity, expiry, and unknown responses through failover/restore; prove owner separation, bounded issued grants, no reissue/reconsume, receipt/time-ratchet monotonicity, suspend/clock-skew denial, and RPO/RTO |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Activate generation-2 laws; issue quota-admitted sequenced grants; consume through deadline-CAS; implement exact-horizon hot replay, checkpoint-before-delete compaction, unavailable-history denial and growth alerts |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Race issue/epoch/revoke/expiry/CAS and replay checkpoint/install/delete/archive/key rotation; fail over every owner and preserve no-late-commit plus no resurrection/reissue from compacted or unavailable history |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover receipt/deadline evidence plus budgets/sequences/hot results/checkpoints/covered-through/archive/key/compaction state; reject fork/reset/loss-as-absence, while erased exact payload remains fail-closed |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Prove sustained authorization abuse, quota/backlog pressure, bounded hot/checkpoint storage, coalescing, archive outage and concurrent replay plus existing contention |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit checkpoint/accumulator crypto, key rotation, archive integrity, compaction ordering, data minimization and bounded growth plus existing TCB/supply-chain assurance |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze issuance sequence/budgets/horizon/checkpoint/archive/compaction/key/historical-denial formats plus evaluator and remediation protocols |
