...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add predecessor-linked RevalidationCounterCapacityCheckpoint and CapacityArchiveReplayHead; dedicated immutable chunk manifest, proof budget, durable verification cursor and publication receipt/state; closed Stage/Verify/Commit/MarkOrphan/FinalizeGc commands; and independently witnessed CapacityArchiveHighWatermark. Bind exact charge/material/mutation/head/result membership, captured rows, chunks, epochs and current/proposed capacity state. Use Staged→Verified→ConsumedByCommit or OrphanGcEligible→Collected only. High-watermark Prepare first installs a charged local PreparePending writer fence, then obtains the exact external witness; reconciliation requires witness or authenticated definitely-not-witnessed evidence and never infers from timeout. Extend the capacity proof/state with canonical checkpoint and replay-head sequences, equal genesis/current values, one paired advance per ArchiveFinalize charge and separate sentinels | Same-material archived retry returns its result, changed material conflicts, exact nonmembership plus hot absence alone is unseen, missing history is typed unavailable, publication races are closed, local rollback below the witness cannot refund capacity, and neither canonical sequence can wrap or gap |
| `0.30.0` | Export/import full checkpoint/replay/high-watermark ancestry, witness receipts, manifest/publication lifecycle, proof budgets/cursors/results, both canonical sequences/sentinels, exact-set proofs and uncovered hot suffix | Import reads the independent greatest watermark first, exact-completes a witnessed prepared successor or remains unready, and rejects missing, downgraded, forked, partially deleted, epoch-mixed or sequence-inconsistent state |
| `0.140.1` | Freeze checkpoint/replay/publication/high-watermark domains and field widths, closed receipt tags/commands/conflicts, canonical chunk/charge ordering, proof budgets, cursor semantics, membership/result proofs, epochs and paired sequence equations | Review proves adapter-defined verification, dense/probabilistic/range inference, lifecycle/tag confusion, identity/result rebinding, unbounded verification and sequence alias/wrap impossible |
| `0.140.2` | Freeze Stage/Verify/high-watermark Prepare/reconcile/abort/finalize/orphan/GC transactions and routing→residual-state→capacity-state→attempt-set→high-watermark→publication→replay-head order. PreparePending is installed before external traffic and fences ordinary writers; only a witness finalizes or definitely-not-witnessed evidence aborts. Final CAS authenticates the witness and atomically charges history lifecycle, advances state/head/both archive sequences, CASes Verified→ConsumedByCommit, installs the replay head and deletes exact captured rows. Checkpoint coverage ends before Prepare, so Prepare/disposition and Commit charges remain hot | Every backend exposes each legal transition or no write; Verify/Commit/MarkOrphan/GC races have one winner, pending/witnessed candidates cannot be orphaned, unknown witness response never reopens, and partial head/delete state is unobservable |
| `0.140.3` | Separate charge writer, checkpoint builder, publisher/storage adapter, bounded verifier, external high-watermark authority/witness, publication owner, replay-head owner, orphan collector, result reader, Replan planner and restore/import validator | No publisher self-verifies, verifier installs authority, replay owner creates its witness, GC collects referenced/prepared data, reader chooses an older head, or planner/compactor mints capacity |
| `0.140.5` | Retain checkpoint/replay/high-watermark ancestry and witnesses, manifests/chunks, every lifecycle result/conflict/cursor, exact-set/result proofs, epochs, sentinel fence/result and hot suffix; GC only authenticated unreferenced OrphanGcEligible publications with Recovery capacity | Retention cannot erase retry/conflict evidence, collect a winning/prepared archive, manufacture unseen state, hide sentinel/sequence consumption or detach a result from its charge |
| `0.140.6` | Freeze failover at every cursor/CAS, local PreparePending fence, external witness query/reconciliation, deterministic witnessed-successor completion, definitely-not-witnessed abort, unknown deletion/final response, concurrent publication/writer/Replan and greatest-watermark recovery | Takeover joins stored results/cursors; unknown witness state stays fenced, and a witnessed successor commits exactly or remains unready, with no duplicate charge, lost row, older-head fallback, orphan race, counter reset or sequence fork |
| `0.142.0` | RPC exposes read-only publication state/progress/budget, checkpoint/replay/high-watermark identity, paired sequence/sentinel status and typed replay/conflict/historical-unavailable outcomes | RPC cannot upload as verified, choose/anchor a head, mark a viable candidate orphan, assert unseen, widen budget/headroom, clear a sentinel or authorize from stale local state |
| `0.143.0` | Model Stage/Verify/Commit/MarkOrphan/GC races, exact retry before/after compaction, changed material, proof-budget exhaustion, concurrent hot writers, archive loss, coordinated local rollback, prepared-successor crash, Replan across archives and both sequence boundaries | Exactly one lifecycle/result and charge meaning is reachable; external watermark prevents rollback refund, unavailable/inconsistent state never authorizes progress, and paired sequences advance only with ArchiveFinalize |
| `0.145.0` | Restore/migrate by reading the independent greatest high-watermark before local selection, verifying witness/chunks/epochs/results/cursors, exact-completing a prepared successor, replaying hot suffix and reproving class/total/sentinel/head/paired-sequence equations and Replan obligations | Missing witness/key/chunk/result, rollback, fork, unfinished deterministic completion, deletion mismatch or equation failure is unavailable/corrupt—not inferred absence or older-head fallback |
| `0.146.0` | Fuzz lifecycle tags/transitions, manifests/chunk ordering, proof-budget arithmetic and cursors, exact-set proofs, coverage boundaries, checkpoint/replay/high-watermark predecessors, epochs/results/deletion sets, archived/hot overlap and last/rollover archive sequences | Forged verification, omission, duplication, substitution, truncation, ABA, downgrade, false membership/nonmembership, invalid GC, local rollback and archive sequence overflow/gap fail closed |
| `0.147.0` | Audit publication completeness, bounded verification/Recovery reserve, exact replay/conflict/unseen semantics, external anti-rollback witness, deterministic precommit recovery, atomic finalization, sequence proof/sentinels, Replan, HA and restore | Zero high/critical archival authorization, lifecycle, unbounded-work, capacity-refund, rollback, split-transaction, sequence or result-integrity finding |
| `0.148.0` | Reject legacy broad-only coverage or imports lacking closed publication state/results, independent high-watermark continuity, exact charge/result proof, captured deletion, paired archive sequences or sentinels | No shim trusts adapter “verified,” infers unseen, picks an older local archive, fabricates witness/cursor/sequence, reconstructs a result or renumbers/coalesces charges |
| `0.149.0` | External pentest targets forged Verified, Verify/Commit/orphan/GC races, budget/cursor bypass, Recovery borrowing, false nonmembership, changed archived retry, archive/witness/key/result loss, coordinated rollback, partial deletion, prepared-successor crash, Replan reset and archive sequence boundary/rollover | Zero unresolved critical/high findings and no waived publication authority, bounded-work, historical authorization, external anti-rollback, atomic compaction, sequence or capacity-conservation issue |
| `0.150.0` and `1.0.0` | Demonstrate closed bounded capacity-archive publication, independently anchored rollback safety, exact charge replay and paired sequence conservation on every backend/RPC/HA/Replan/import/restore path | Production remains blocked until the unchanged candidate passes lifecycle, rollback, boundary, fault/model campaigns and independent pentest |
