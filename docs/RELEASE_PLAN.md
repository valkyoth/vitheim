...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Implement pinned recovery-genesis root/provenance/lineage and deterministic offline request/single-use receipt import; independent pointer-observation Register/Reconcile/Terminalize, sealed zero-unresolved completeness frontier and operational fencing epoch/token; disjoint Healthy `C/H` and Fenced `E/J` archive protocols; exact signed `B_EM`/`R_first` class equations and hard maxima with first-route escrow before query permit | Crash after external intent, query, local Fenced and each pre-`E/J/F` boundary, then restore an older backup; no path becomes Operational. Reject TOFU, omitted/unresolved intent, stale token, writer substitution, underfunding, overflow and replenishment |
| `0.30.0` | Export/import every root provenance/lineage/request/import claim/result, observation intent/receipt/disposition/frontier/epoch/token result, `E/J` archive record/result and evidence-maintenance class/maxima/consumption record | Round trip every boundary. Missing, forked, rolled-back, defaulted or cross-deployment material remains Unready; import never synthesizes trust, registry completeness, capacity or archive meaning |
| `0.140.1` | Freeze canonical encodings, domains and result/conflict surfaces for genesis trust lifecycle, registry/frontier/fencing token, dedicated `E/J` protocol and `B_EM`/`R_first` | Independent schema review rejects current-status no-fence, dense counts, TOFU, cross-protocol decode, unknown class/limit and cyclic or future-result commitments |
| `0.140.2` | Freeze root provisioning/import CAS, registry-before-query, terminalization/fence, `E→J→F`, first-route escrow and restore token+epoch+Operational atomic transactions and lock ranks | Fault every boundary all-or-none; a concurrent intent or fence advance defeats restore CAS, and FenceEvidence Commit installs only `J_m` while retaining hot evidence |
| `0.140.3` | Separate deployment-root governor, offline signers, importer, observation registrar/reconciler, query executor, fence/evidence custodian, archive publisher, restore activator and capacity auditor | No actor can choose and trust its own root, register and suppress its own observation, self-certify completeness, replenish funding or both publish and erase evidence |
| `0.140.5` | Retain root lineage/provenance and historical verification, every observation disposition/frontier/epoch, fence pending/anchor link, `E/J/F` ancestry, class consumption and exhaustion/hold evidence | Compaction cannot omit a registered intent, turn unresolved into absent, revive a distrusted request, alias archives, reset hard maxima or weaken custody |
| `0.140.6` | Freeze failover at registry permit delivery, query/terminalization, local fence, each `E/J/F` step, token acquisition/CAS, root rotation/distrust and budget exhaustion | Takeover performs status/Reconcile only, recreates no permit/token, preserves counters and blocks Operational whenever registry, epoch, trust or evidence continuity is unavailable |
| `0.140.11` | Decide the pinned-root source/custody/quorum and lifecycle; observation-registry provider/finality/availability and fencing-epoch port; FenceEvidence archive backend; exact hard maxima and custody/export policy. Keep replenishment and manual root recovery unsupported before `1.0.0` | Architecture, security, legal/forensic and operations approve one exact profile; any unresolved option keeps recovery activation disabled |
| `0.142.0` | Expose read-only root provenance/state, observation intent/disposition/frontier/epoch, token status, `E/J/F` evidence and `B_EM` consumption/exhaustion | RPC cannot trust a root, mint/reuse a receipt, register/terminalize without owner CAS, sign no-fence, acquire bearer authority, change epochs, translate archives or replenish capacity |
| `0.143.0` | Model registry/query/fence/restore races, root lifecycle/request imports, `C/H` versus `E/J` command products and every class/maximum conservation transition | No reachable state revives authority after an observation, consumes a stale token, trusts first-seen material, writes the wrong replay head or exceeds/reclaims custody funding |
| `0.145.0` | Restore root provenance and distrust lineage, greatest sealed registry frontier/epoch, pointer/fence anchors, exact `E/J/F` ancestry, budget state and hot suffix before acquiring a new activation token | Older backups, incomplete/permanently unresolved registry state, stale epochs, missing evidence or exhausted custody remain Unready/FencedEvidenceVerified, never Operational |
| `0.146.0` | Fuzz canonical root/preimage/receipt fields, registry exact-set membership/frontiers, epoch/token CAS, archive discriminants/results, equations and every numeric/encoded limit | Substitution, omission, reordering, replay, ABA/wrap, overflow, one-short escrow, class aliasing and replenishment fail closed |
| `0.147.0` | Audit trust provisioning/SoD, registry independence/completeness, universal epoch enforcement, fenced archive ownership, custody equations/retention and DR continuity | Zero high/critical trust, rollback, completeness, stale-authority, archive-alias, accounting, evidence-loss or role-confusion finding |
| `0.148.0` | Reject legacy imports lacking pinned provenance, explicit observation history/frontier/epoch, dedicated `E/J` state or exact immutable maintenance maxima/consumption | No shim uses TOFU, current absence, dense counts, local-only fencing, `H` for `J`, inferred escrow or reset capacity |
| `0.149.0` | External pentest attacks root substitution/import replay, observation suppression, local-fence/pre-anchor rollback, forged no-fence frontier, stale epoch/token authority, command/result confusion, first-route starvation and limit replenishment | Zero unresolved critical/high findings and no waived trust-root, anti-rollback, exact-completeness, fencing, archive-authority, custody or restore issue |
| `0.150.0` and `1.0.0` | Demonstrate the unchanged candidate across all selected storage/registry/archive/signing/HA/restore profiles, including the local-fence-before-anchor crash and hard-limit exhaustion runbooks | Production remains blocked until every profile passes the full Phase C corpus, independent audit and pentest with no unsupported option silently enabled |
