...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze non-borrowable per-lane durable charge capacity; immutable VIT-INV-061 profile identity/generation/digest/predecessor lineage; expected-version activation; drain-before-shrink; governed emergency/aggregate changes; physical provisioning evidence; exact-generation recovery; downgrade refusal; and `TopologyAuthorizationPresentationChargeLedgerCapacityDrainFenceV1` atomic PendingDrain install, one successor/fence, dual-profile admission, typed pre-debit denial and exact clear/activation |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Stall/fill lane resources; race shrink with stage-one reservation, stage-two terminalization and compaction; interrupt activation; fail over generations; restore older larger limits; reject false provisioning and Recovery reassignment; sustain traffic; race fence install/admit/activate/reject; crash/fail over/restore installed fences; reject stale workers/competing successors; preserve obligations and Recovery |
| `0.23.0` | SQLite single-node adapter | Persist capacity-profile lineage/activation/drain/evidence, lane obligations, exact drain fence and dual-profile result; prove exact-generation restore, safe shrink, physical bounds, atomic install/clear/activate, recovery and continuous-traffic convergence or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage charge/consume, profile CAS, shrink/drain races, non-borrowable maintenance, provisioning and aggregate bounds plus drain-fence locking, dual-profile admission, final-admission/activation serialization, exact rejection and failover recovery |
| `0.25.0` | Experimental MySQL adapter | Prove capacity-profile lineage, safe shrink, exact restore, lane-local lifecycle, drain-fence and dual-profile-admission parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove capacity-profile lineage, safe shrink, exact restore, lane-local lifecycle and drain-fence bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific capacity-profile lineage, safe shrink, exact restore, lane-local lifecycle and drain-fence bounds or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve profile lineage/generation/digest/state/evidence, lane reservations, pending drain and exact drain-fence predecessor/successor/digests/lanes/sequence/continuity; reject numeric merge, old-writer mutation, obligation movement, fence loss/duplication/stale clear/bypass or rollback |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger profile activation/safe-shrink/provisioning/exact recovery plus drain-fence/dual-admission/typed-denial/exact-clear/Recovery-isolation semantics; otherwise refuse import/profile |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze VIT-INV-061 as sole presentation-mapping owner, stable mapping identity/generation/fence/profile bindings, emergency-promotion requestor/approver/activator SoD, and existing identity/time/terminal role rules |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze capacity-profile lineage/activation/drain/provisioning rows, governed emergency/aggregate change transaction, per-lane obligation-fit predicate, physical-evidence contract and capability refusal plus durable fence row, one-successor uniqueness, dual-profile admission, typed denial and atomic install/clear/activate |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Minimize charge checkpoints while retaining debit, original terminal-kind and result-commitment proof; compaction never creates reusable evidence, refunds work, loses disposition meaning or makes history fresh |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze greatest-authenticated profile generation/digest and obligation recovery plus authenticated fence failover/restore, stale-worker fencing, missing/competing-fence refusal and Recovery availability |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement immutable profile lineage/activation/drain/provisioning evidence, per-lane obligation accounting, safe shrink, exact-generation recovery, durable fence install, dual-profile admission, typed denial and exact clear-or-activate alongside the two-stage lifecycle |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve profile generation/digest/state, activation CAS, provisioning proof, lane obligations and authenticated fence/dual-profile admission across RPC; timeout or stale worker cannot clear, bypass, activate, transfer capacity, cancel reservations or invent denial state |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over capacity-profile activation/drain, obligations and installed fences by exact generation/digest/sequence without numeric merge, duplicate/bypass, loss or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Select the greatest authenticated profile generation/exact digest; reconstruct obligations; retain pending drain and authenticated fence; reject older-larger/numeric merge, missing/stale/competing/unauthenticated fence and downgrade writers |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Race shrink/activation with both stages and compaction; fail over/restore generation conflicts; attack provisioning evidence and Recovery reassignment; sustain traffic and race fence install/admit/activate/reject across crash/failover/restore; prove convergence, typed denial and Recovery |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit profile owner/approval/SoD, immutable lineage, activation CAS, safe-shrink predicate, provisioning evidence, exact recovery, downgrade fencing plus fence authenticity, uniqueness, dual-profile predicate, exact clear and stale-worker resistance |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze profile identity/generation/digest/predecessor/state/evidence, obligation and drain-fence identity/generation/digests/state/lanes/sequence/continuity/typed-denial formats; mixed versions cannot mutate old profiles, merge ceilings, reinterpret pending drain, omit/reinterpret a fence or weaken either lifecycle |
| `0.149.0` | Release candidate and external pentest remediation | External testers attack profile governance, shrink races, interrupted activation, false physical evidence, generation-skew failover, older-larger restore, cross-lane transfer, infinite drain, final-old-admission activation, rejection races, stale clears, installed-fence restore and competing successors; retest every fix |
...
- Production topology authorization proves independently provisioned,
  ...
  successor with governed expected-version CAS, drain-before-shrink checks and
  authenticated physical provisioning evidence. Restore selects the greatest
  authenticated profile generation/digest and reconstructs obligations; it
  never revives an older larger numeric ceiling.
  Pending drain installs a lane-scoped durable fence. New stage-one work must
  satisfy active and successor profiles or fail before debit with a typed
  ...
  and failover/restore cannot lose, duplicate or bypass it.
