...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Activation binds an independently governed anchor-source manifest and atomically creates registry generation zero under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→clearance-anchor-source-manifest→history-obligation/corruption-fence/clearance-anchor-registry/lineage-disposition→retention/legal-hold→audit/result/outbox. Fencing creates one fence-wide clearance scope with sole live authorization, monotonic authorization generation, immutable lifetime budget/counters and terminal tombstones. Replacement grants receive only remaining capacity. Restoration uses field-specific typed state algebra, not blanket maxima. Rebuild uses one parent, bounded proposal attempts, independent permanent-rejection authority and one activated successor. Every prior disposition, fence, anchor, attempt, custody, charge, Fenced-outcome, conflict and CAS rule remains |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve source-manifest lineage, activation-created registry genesis, registry ratchet/receipt, clearance scope/authorization generations/lifetime charges/tombstones, attempt state, typed joins and parent/proposal/rejection/successor mapping isomorphically across backends. Missing/defaulted/lazy genesis, second live grant, reset scope counters, field-wise invalid joins, consumed parent, inferred rejection, duplicate successor or ambiguous archive selection deny beside every prior control |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze source-manifest and weakening-transition authentication/time/continuity beside collection/registry and clearance profiles. No clearance role defines expected sources or weakens class/quorum/nonresponse/time policy |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist activation-created registry genesis, fence-wide scope/lifetime budget, sole active generation, typed state algebra, parent rebuild/bounded proposals and permanent rejection. Missing rows, budget reset, invalid join or parent consumption refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate source-manifest/weakening, scope admission, attempt worker, rebuild proposal and permanent-rejection roles from clearance/registry/collection authority. No candidate failure can become permanent rejection |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Preserve source manifests/transitions, registry genesis, scope lifetime counters/tombstones, typed-join evidence, rebuild parent/proposals/rejection and both namespaces. Erasure cannot reopen scope or parent authority |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve sole scope authorization/attempt, lifetime work, terminal disposition, typed join and one parent/successor. Failover cannot admit competitors, reset work, lazily create genesis, field-wise merge state or infer rejection |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries source-manifest/genesis, scope/generation/lifetime, typed-join and rebuild-parent/proposal/rejection bytes, but cannot initialize, occupy/release scope, weaken policy, permanently reject or select a successor remotely |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check competing valid authorizations, replacement generations, scope precharges/terminal tombstones, genesis/manifest races, typed partial joins and bounded proposal/permanent-rejection/activation races. Require one live grant/attempt and one successor |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore manifest/genesis, scope active generation/lifetime counters/tombstone, typed-join inputs and rebuild parent/proposals/rejection before work. Older backups cannot reopen admission or proposal capacity |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz multiple valid grants, replacement budget reset, missing genesis, unauthorized source weakening, algebra laws/undefined joins, invalid proposal no-write and permanent-rejection authority beside prior cases |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit source-set authority, scope lifetime conservation, typed-join provenance and parent/proposal/rejection/successor lineage beside prior evidence |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze source manifest/genesis, scope, typed join and rebuild parent/attempt/rejection encodings. Old per-authorization-only budgets, lazy registry or Proposed/Rejected singleton rows fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack competing grants, lifetime reset, terminal-scope bypass, lazy genesis, source/quorum weakening, mixed-snapshot joins, parent consumption and unauthorized permanent rejection beside all prior cases; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for source/genesis, scope/lifetime, typed algebra, rebuild parent/proposals/rejection and every prior recovery artifact |
