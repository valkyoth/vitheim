...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze per-presentation and first-seen-request rate layers, per-request sequence and bounded denied-history checkpoint/archive permanence, separate admission/outstanding accounting, atomic issuance, original-claim settlement, live-receipt retention, and structurally separate terminal and reconciliation consumer evidence alongside existing range/sparse/reserve controls |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Crash denial sequence/checkpoint/delete and issuance/settlement writes; flood replays, race identical presentations, lose responses, retry under saturation and reuse changed digests; prove one request charge, per-presentation charging, immutable outcome reuse, terminal-only settlement typing, bounded denial resources and existing reservation/range tests |
| `0.23.0` | SQLite single-node adapter | Prove request-sequence/checkpoint/archive bounds, presentation/request/admission separation, terminal/reconciliation codec and settlement-role separation, atomic issuance, original-claim settlement, bounded verifier, deadline-CAS, sparse/late and reserve semantics or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove denied-history, two-rate concurrency and terminal/reconciliation parity plus corrected issuance/reservation lifecycle under pause/failover, no-late-commit, sparse/range eligibility and reserve isolation |
| `0.25.0` | Experimental MySQL adapter | Prove denied-history, two-rate and terminal/reconciliation parity plus retention/settlement/range behavior or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove denied-history, two-rate and terminal/reconciliation bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific denied-history, two-rate and terminal/reconciliation authorization bounds or refuse VIT-CAP-060/061; namespace/graph/query isolation and capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve presentation-rate windows, once-per-request charges, request/issuance sequences, denial/issuance checkpoints, terminal and reconciliation schemas/sequences/auth roles, original claims, reservations and terminal settlements; reject retry request recharge, denial reevaluation, proof-as-new, reconciliation settlement, forgery, rollback or weakening |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger denied-history permanence/resource bounds, both rate layers and structurally separate terminal/reconciliation semantics plus issuance atomicity, live-receipt retention and original-claim settlement; missing proof remains fail-closed |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze closed terminal outcomes, a separate reconciliation disposition/evidence/receipt family, terminal-only settlement arguments, sender-only consumer/verify-only issuer authentication, plus existing lineage/live-receipt/time/revocation rules |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze presentation-rate windows, once-per-request charges/sequences, denial checkpoint/archive/horizon/resource bounds, atomic success/bounded denial writes, original claims, separate terminal/reconciliation result/outbox sequences and all-or-none terminal settlement plus existing range/sparse/reserve profiles |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Minimize denial/issuance checkpoints and range evidence to domain-separated commitments; retain aggregate presentation-rate windows and request-charge proof without permanent per-presentation history; keep plaintext request/principal/approval/receipt data outside long-lived proof while never making erased history fresh |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze failover/restore of presentation/request rate state, request/issuance sequences, denial/issuance checkpoints, separate terminal/reconciliation sequences/auth roles, whole issuance, claims, reservations and terminal settlements |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement per-presentation plus once-per-request charging, bounded denial history, separate terminal/reconciliation evidence types and terminal-only settlement alongside reservation/range controls |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve both rate semantics, request replay identity/proof and separate terminal/reconciliation codecs across RPC; consumer sender credentials never enter issuer service and only terminal receipts settle |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over presentation windows, once-per-request charge/sequence/checkpoint/delete, separate terminal/reconciliation sequences/auth roles, issuance/reservations/settlement and bounded verification without duplicate request charge, denial reevaluation, false release or rollback |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover both rate layers, denied-history identity/checkpoints/proof bounds and separate terminal/reconciliation sequences/auth roles plus existing authorization state without reevaluation, false release, forgery or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Soak maximum presentation rate, unique-request rate, replay/concurrency/response-loss/saturation/conflict cases, checkpoint/archive loss, proof-work bounds, terminal/reconciliation races and existing reservation/range controls |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit two-rate enforcement, denied-history permanence/resource limits, terminal/reconciliation canonicalization, terminal-only settlement and credential separation alongside existing replay and supply-chain hardening |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze presentation/request/admission accounting, request/issuance sequence, denial/issuance checkpoint, separate terminal/reconciliation result/outbox/auth-role and reservation/range formats |
