...
| `0.140.2` | Freeze child+aggregate+state-head atomic transition, Begin/Dispatch/Complete attempt-set advance, MarkEligible effect fence, final-Commit root recheck, empty Commit representation and provider adapter conformance gate | Each backend/provider commits one continuous head/effect set/empty representation atomically or refuses |
...
| `0.143.0` | Model coordinated child+aggregate rollback, sibling/head races, Commit immediately before/after every effect transition, zero-survivor representation and provider retry/query/timeout/rotation/late-duplicate behavior | No rollback-equivalent state, effect escape, absent aggregate or duplicate/unknowable provider effect reaches Released |
