...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Persist one destination-local operation/job/budget/reservation/lifecycle/fence/candidate/tombstone/authorization/revocation-inbox/barrier/result/cleanup lineage plus the non-wrapping active coordinator generation/fence checked first by every mutation. Derive `MigrationImportOwnerManifestV1` only from source/destination schemas, migration plan, the admitted VIT-LAW-009 tuple/manifest and dependency closure; type VIT-INV-062 as the live non-importable coordinator and all other applicable entries as domain contributors. Explicit authorization admission shares one row with activation/expiry/revocation. Revocation before admission creates a permanent target-bound RevokedBeforeAdmission tombstone; its sequence is per authorization lineage and its lifetime covers the target authority. Freeze active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→audit/result/outbox; the co-located transaction activates every domain contributor plus live-coordinator barrier/result/audit/outbox, or none. Coordinator succession is its own stable-ID, independently authorized, budgeted closed lifecycle with exact result/conflict and atomic generation/fence handoff. Register the authenticated, bounded, post-activation-only terminal-history append semantic before cross-backend use |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Require the complete canonical two-head tuple, unified namespace, atomic checkpoint bundle and mixed-trigger proof; missing/defaulted or singular state is quarantined before import mutation. Export enumeration, transfer, decode, verification and staging cannot create fresh counters, jobs, cursors or reservations. The live destination VIT-INV-062 state stays outside the candidate; nonterminal source jobs, cycles and identity collisions deny. `MigrationImportRegistryHistoryV1` binds authenticated provenance/scope, original terminal result, export/import manifests, namespace, retention/classification, sequence/predecessor and idempotency. `AppendMigrationImportRegistryHistory` begins only after activation under bounded archive work and protected cleanup, records a canonical disposition/result/conflict, and can never roll back or ambiguously report activation. Partial owner preparation is never authority; destinations lacking the one local domain-owner activation boundary refuse before staging |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze activation and bootstrap authorization admission/revocation signer, key and authentication profiles; issuer continuity; exact-target sequences and lifetimes; trusted time; authenticated checkpoint/outbox/inbox receipts; exact retry/conflict and permanent tombstones. Only destination-local commits change authority |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist coordinator/domain-contributor roles, active coordinator generation/fence, immutable live state outside staging, shared activation-authorization row, scoped revocation sequence/inbox/result, bounded history archive head/disposition/budget/result, typed collisions and bootstrap identity/lifecycle/authorization/budget/checkpoint/result. Freeze active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→audit/result/outbox and coordinator-first prefixes for all other VIT-INV-062 mutations; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact requestor/approver/activator/issuer/admitter separation, per-target revocation sequencing and distinct bootstrap requestor/approver/activator/issuer/cancellation roles. Neither coordinator nor migration/domain actors self-bootstrap; existing authority remains exact-target-bound until consumed, expired or destination-locally revoked |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve coordinator/contributor derivation, active generation/fence, live coordinator exclusion, authorization admission, RevokedBeforeAdmission, exact-target sequence, history archive head/disposition/budget/result, bootstrap lifecycle/authorization/budget/checkpoint/result and job/candidate/receipt/barrier state across HA/DR. Failover cannot commit predecessor work after handoff, admit over a tombstone, import coordinator state, reset a sequence, revive source jobs, alter activation because archival failed or reverse Consumed |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries authenticated VIT-LAW-009 tuple/manifest/dependencies, coordinator/contributor roles, active coordinator generation/fence, explicit activation/bootstrap authorization and target-scoped revocation provenance, but never claims remote effect. A peer cannot import coordinator state, suppress revocation through another sequence, bypass a local tombstone, advance bootstrap, or turn archive transport into activation |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over admitted-law role derivation, active coordinator generation/fence, live coordinator state, explicit admission, pre/post-admission tombstones, scoped sequence, archive head/disposition/budget/result, full bootstrap lifecycle/authorization/budget/checkpoint/result and one job/candidate/authorization/barrier lineage. Race handoff with predecessor transactions, competing successors and archive publication; recover exactly one safe terminal with no partial owner visibility, coordinator replacement or activation ambiguity |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore role derivation, active coordinator generation/fence, live coordinator, activation/bootstrap authorization, tombstones/sequences, history archive state and bootstrap lifecycle/budget/result before work. Older backups cannot admit delayed authority, commit predecessor work after handoff, resurrect source jobs, replace the coordinator, lose archive disposition, lower a sequence or reverse Consumed |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Race revocation-before-admission, lost/late authorization, expiry, unrelated sequences, nonterminal/terminal source jobs, identity collisions, cycles, stale predecessor transactions, old binaries, competing successors, bootstrap cancel/handoff, archive collision/failure, activation, failover and response loss. Exactly one safe local terminal wins; every domain contributor is visible or none, and archive status never changes activation |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit admitted-law trust, coordinator classification/generation, complete activation/bootstrap authority and budgets, history append semantics, canonical active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→audit/result/outbox order, domain-owner atomicity and exact idempotent results |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze coordinator/contributor roles and generation/fence, activation/bootstrap authorization and lifecycles, history archive identity/head/disposition/budget/result, live-coordinator exclusion and shared-row formats. Unknown, omitted or guard-unaware fields/binaries fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack revocation-before-admission, delayed/lost authority, cross-target sequence suppression, source live-job replacement, history promotion/collision/failure, cyclic self-import, stale transactions and old binaries crossing coordinator handoff, competing successors, bootstrap authorization replay/revocation/cancel/rollback/restore/failover, activation races and lost responses; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for coordinator/contributor derivation, active coordinator generation/fence, activation and bootstrap authority/lifecycles/budgets/results, pre/post-admission revocation, history archive state, barrier and irreversible activation result |
