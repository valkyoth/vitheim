...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Make archive publication Staged/Verified/OrphanGcEligible and non-authoritative. Commit alone CAS-installs the proposed archive replay head with exact hot-row deletion, authorization consumption, settlement and results. Add a closed AuthenticatedDeleted/TransferredToCustodyLedger/Unknown disposition for every remaining workspace leg; transfer atomically charges the exact archive/legal-hold custody member before parent credit | Crash after upload leaves readers on the prior head and all hot rows authoritative. Prove no head-without-delete, deletion-without-head, credit-without-deletion-or-charged-transfer, partial leg, Unknown release or adapter head mutation |
...
| `0.140.2` | Freeze the one commit CAS over authoritative head install, exact hot-row deletion, receipt consumption, custody-ledger debit/member install, workspace tombstones/credit, lineage settlement, result/audit/outbox | Every selected backend proves the indivisible bundle within its declared transaction maximum or refuses; response loss returns the stored winner |
