...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Declare VIT-INV-060-G and VIT-INV-061-F/G enforcement/test children and VIT-INV-061 mapping/budget/charge/disposition/checkpoint storage; freeze isolated ingress, two-stage charging, a closed disposition table, and `TopologyAuthorizationPresentationChargeLedgerCapacityV1` with non-borrowable per-lane rows/bytes/awaiting/backlog/I/O/workers below aggregate ceilings |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Reject undeclared/duplicate enforcement/test children; destroy every stage boundary and disposition weakening; stall/fill every Normal ledger and maintenance resource while Recovery/BreakGlass complete both stages, then saturate BreakGlass while Recovery remains available and prove aggregate disk/work bounds |
| `0.23.0` | SQLite single-node adapter | Persist isolated ingress, mapping owner/state, atomic debit/evidence/sequence/awaiting, closed terminal/result links/checkpoints and atomic stage-two work; prove lane-local ledger/maintenance reserves, aggregate bounds, saturation, crash-orphan and mapping-TOCTOU behavior or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage charge/consume, locked mapping recheck, non-borrowable ledger rows/bytes/awaiting/backlog/I/O/workers, fail-atomic saturation, checkpointed original-kind compaction and aggregate bounds |
| `0.25.0` | Experimental MySQL adapter | Prove isolated ingress, lane-local charge-ledger/maintenance capacity, closed disposition/saturation and two-stage TOCTOU parity plus existing behavior or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove isolated ingress, lane-local charge-ledger/maintenance capacity, closed disposition/saturation and two-stage TOCTOU bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific isolated ingress, lane-local charge-ledger/maintenance capacity, closed disposition/saturation and two-stage TOCTOU bounds or refuse VIT-CAP-060/061; retain isolation and capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve declared child coverage, ingress/mapping state, charge identity/dispositions/checkpoints, original terminal kind, per-lane capacity ownership/reservations/high-watermarks, aggregate ceilings and unfinished maintenance obligations; reject borrowing, reset, refund/reuse or skipped recheck |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger child coverage, isolated ingress, mapping ownership, atomic awaiting creation, closed disposition/checkpoint, per-lane durable-ledger and maintenance isolation, aggregate saturation and stage-two recheck semantics |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze concrete VIT-INV-061 rows and `TopologyAuthorizationPresentationChargeLedgerCapacityV1`: non-borrowable per-lane rows/bytes/awaiting/backlog/I/O/workers, lifecycle reservations and aggregate disk/work ceilings; retain atomic awaiting, closed terminal/checkpoint, stage-two recheck and capability refusal |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze failover/restore of ingress/mapping state, charge evidence/dispositions/checkpoints, per-lane ledger and maintenance reservations/high-watermarks, aggregate ceilings and all existing authority state without cross-lane borrowing |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement declared 060-G/061-F/G enforcement, owned rows, atomic two-stage lifecycle, and non-borrowable per-lane ledger/awaiting/backlog/I/O/worker capacity under aggregate disk/work ceilings |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve internal charge evidence, dispositions, lane-capacity reservations and maintenance scheduling across authenticated RPC; distinct internal stages, mapping recheck, and RPC timeout invents no transition or borrowing |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over charge lifecycle plus per-lane reservations/maintenance high-watermarks and aggregate ceilings without merging capacity, starving Recovery, refund, transition rewrite, skipped recheck or rollback |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover charge evidence/dispositions/checkpoints, per-lane lifecycle obligations/high-watermarks and aggregate ceilings without merging reservations, starving Recovery, refund, reuse, skipped recheck or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Stall/fill Normal rows/bytes/awaiting/backlog/I/O/workers while emergency lanes finish; saturate BreakGlass while Recovery finishes; prove aggregate ceilings across crash/failover/restore |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit declaration closure, evidence confinement, disposition irreversibility, per-lane ledger/maintenance isolation, aggregate saturation, checkpointed compaction and mapping recheck |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze child IDs, charge lifecycle and per-lane capacity/reservation/high-watermark/aggregate-ceiling formats; mixed versions cannot merge lanes, lose obligations, refund, or weaken two-stage semantics |
| `0.149.0` | Release candidate and external pentest remediation | External testers stall/fill each lane's ledger and maintenance path, attack cross-lane reservation/restore/failover accounting and aggregate ceilings, and retest every fix alongside the complete prior platform scope |
