...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Add non-recursive publication-profile trust/bootstrap/rotation, compatibility/distrust succession and one external activation fence, protected attempt-completion reserves, separate operational/candidate heads and proposal/active high-watermarks. Split lifetime-work from physical-capacity accounting, freeze typed transfers and one universal fence-before-lineage lock rank, and release physical encumbrance only after archive/deletion settlement. Preserve canonical active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→control-settlement-archive-head→control-settlement-journal-head→corruption-control-reserve→history-obligation→corruption-fence→corruption-control-lineage→corruption-control-lineage-checkpoint→lineage-disposition→recovery-authorization→clearance-anchor-source-manifest-head→clearance-anchor-source-manifest-authorization→corruption-clearance-anchor-registry→corruption-clearance-scope→corruption-clearance-authorization→corruption-clearance-attempt→corruption-rebuild→corruption-rebuild-rejection-authorization→archive-head→history/idempotency→recovery-lineage-budget→attempt/successor-budget→retention/legal-hold→audit/result/outbox activation, registry rebinding, destructive authority, rebuild and every prior control |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve profile trust/lifecycle/compatibility/ratchet/activation-fence, completion reserve, witness/journal evidence, both local and external head pairs, both typed ledgers/transfers/equations, lock-rank version/acquisition evidence and settlement/reclaim membership exactly. Proposal-as-active, split activation/distrust winner, old-profile activation after distrust, terminalization-capacity loss, head conflation, work/physical aliasing, premature release, lock inversion or rollback denies |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze publication-profile root/bootstrap/rotation/high-watermark, compatibility decisions, revocation ratchet, governance-root activation fence and final current-profile check beside existing proposal/activation/abort authentication. Old profiles may terminalize but activate only under MayActivateExisting, and distrust racing activation has one external CAS winner |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist the activation-fence sequence, attempt-completion reserve, operational/candidate heads, split lifetime-work/physical-capacity ledgers, typed transfers, shared lock rank/traces, ReleasePending and settlement/archive/deletion evidence. Missing capacity, fence continuity, ledger alias, rank inversion, participant omission or premature release refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate trust bootstrap/rotation, profile transition/compatibility/distrust, publication/status/local/external terminalization, completion-reserve, work/physical transfer, custody release and archive roles. No self-selection, old-profile activation, accounting reset or inferred release |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain profile trust/ratchet/activation-fence sequence, both manifest heads, attempt completion reserve/checkpoint, both ledgers/transfers and settlement/archive/deletion membership until governed compaction. Erasure cannot orphan terminalization, refund spent work or release occupied capacity early |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze HA reconciliation for profile succession, activation-fence and both manifest heads; preserve completion reserve, work/physical ledgers, shared rank version/traces and settlement reclaim state. Failover cannot fork the winner, activate distrusted evidence, conflate heads, lose terminal capacity, invert locks or resurrect capacity |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries profile trust/compatibility/ratchet/activation-fence, completion-reserve status, operational/candidate heads, typed work/physical transfers and settlement proofs without remote selection, activation, debit, release or lock authority |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check publication versus completion-reserve admission/saturation, profile succession/distrust at LocalActivationCommitted through the external activation fence, both local/external heads, both ledger equations, lock traces and two-stage physical reclaim. Require one activation-or-distrust winner, one operational head, no stranded terminalization and no double spend/release |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore operational-active exactly from externally Activated evidence while preserving a reachable non-authoritative candidate; restore profile root/ratchet/activation-fence sequence, completion reserve, split ledgers, rank version and exact reclaim membership. Older snapshots cannot activate, refund or release |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz saturation before/after publication, profile rotation/emergency distrust at every boundary, head divergence, work/physical transfer overflow, bytes aliasing, lock inversion and archive/delete/settlement crashes |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit non-recursive profile trust and terminalization capacity, two-head restore, typed ledger conservation, acquisition traces and no-release-before-delete settlement membership |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze profile trust/compatibility/ratchet/activation-fence, completion-reserve, operational/candidate heads, work/physical ledger and transfer, lock-rank and ReleasePending/settlement encodings. Legacy aggregate/head schemas fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack completion-capacity starvation, profile distrust racing or following local CAS, activation-fence rollback/split winner, self-rooted rotation, head conflation, work/physical aliasing, premature reclaim, settlement participant omission and lock inversion alongside every prior lifecycle attack |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for protected publication terminalization, governed profile/activation-fence and two-head manifest state, split work/physical ledgers, typed lock order, rollback-safe reclaim settlement and all prior controls |
