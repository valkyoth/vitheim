...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add the closed attempt-set writer matrix for execution/finalization authorization, Begin/Dispatch/Complete/reconciliation, capability lifecycle, provider evidence and checkpoint/compaction/archive. Every mutation follows one routing→state-head→attempt-set→attempt→auxiliary→result order and atomically advances the head. A covered mutation against CommitEligible writes an exact invalidation, demotes to Preparing and retains a sticky revalidation fence blocking new effect/artifact admission; MarkEligible repeats its full validation. Dispatch persists one unique application-level transmission claim; after return/crash/lease loss/uncertainty only query reconciliation is permitted | No unrooted writer, lock inversion, blocked restrictive processing, reopened admission, stale-eligibility Commit trap or application resend is reachable |
| `0.30.0` | Export/import every attempt-set mutation predecessor/successor, eligibility invalidation/sticky fence, one transmission claim and terminal claim evidence with authorization/evidence/checkpoint/archive history | Restore never accepts an unrooted mutation, stale eligible result, reopened effect lane or second provider invocation |
