...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze activation plus bootstrap begin/handoff/cancellation authorization admission/revocation signer, key and authentication profiles; pre-admission tombstones; issuer continuity; exact-target sequences/lifetimes; trusted time; authenticated checkpoint/successor/outbox/inbox receipts; exact per-action results/conflicts. Only destination-local commits change authority |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist roles/generation/fence/live exclusion, activation rows, history obligation/archive head/disposition/budget/result/checkpoint, typed collisions and bootstrap identity/lifecycle/begin-handoff-cancel authorization/budget/checkpoint/successor receipt/results. Freeze active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation→audit/result/outbox and coordinator-first action prefixes; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact requestor/approver/activator/issuer/admitter separation, per-target revocation sequencing and distinct bootstrap begin/handoff/cancellation issuers, admitters and actors. Neither coordinator nor migration/domain actors self-bootstrap or self-cancel; existing authority remains exact-target-bound until consumed, expired or destination-locally revoked |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve coordinator derivation/generation/fence, activation state, history obligation/head/disposition/budget/result/checkpoint, bootstrap begin/handoff/cancel tombstones/lifecycles/budget/checkpoint/successor receipt/results and barrier state across HA/DR. Failover cannot strand a drain, lose Pending, infer NoHistory/NotRequested, clean before checkpoint, commit predecessor work, admit over a tombstone, alter activation because archival failed or reverse Consumed |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries exact law/roles/generation plus activation and bootstrap action authorization/revocation provenance, checkpoint/successor receipt and history-obligation delivery identity, but never claims remote effect. A peer cannot bypass a local tombstone, advance/cancel bootstrap, erase Pending, infer explicit absence or turn archive transport into activation |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over role/generation state, activation tombstones, history obligation/head/disposition/budget/result/checkpoint, full bootstrap begin/handoff/cancel authorization and tombstones/budget/checkpoint/receipt/results. Race action admission/revocation/expiry, cancellation/handoff, activation/obligation creation, missing worker, cleanup and archive publication; recover one safe terminal without stranded drain, missing obligation, partial visibility or ambiguity |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore active generation, activation state, bootstrap three-action tombstones/lifecycles/checkpoint/receipt/results and history obligation/head/disposition/checkpoint before work. Older backups cannot strand drains, admit delayed authority, erase Pending, invent NoHistory/NotRequested, clean early, commit predecessor work or reverse Consumed |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Race begin/handoff/cancel authorization before admission and at every expiry state, cancellation versus handoff, checkpoint/receipt substitution, activation versus obligation creation, missing worker, cleanup versus Pending, archive collision/failure, failover and response loss. Exactly one safe local terminal wins; no drain or obligation disappears and archive status never changes activation |
...
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze bootstrap begin/handoff/cancel and history obligation/head/checkpoint wire/storage formats plus coordinator/activation state. Unknown, omitted, generic-bootstrap-authority, absent-obligation or guard-unaware fields/binaries fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack action-grant confusion, pre-admission revocation bypass, expiry at every bootstrap state, lost cancellation, cancel/handoff races, digest-only checkpoint/receipt forgery, activation crash before/after obligation, missing worker, cleanup race, Pending restore, absence inference, stale handoff and archive failure; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for active generation, activation, bootstrap begin/handoff/cancel authority and exact results, typed checkpoint/receipt, history obligation/head/disposition/checkpoint, barrier and irreversible activation result |
