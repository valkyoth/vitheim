...
| `0.18.3` | Declared ownership roots including owned active law-catalog lineage, milestone-scoped catalogs/envelopes, independently admitted manifests, closed semantic realization, lifecycle, and authority registry | `VIT-INV-057`/`VIT-LAW-007` CAS/ratchet/revocation recovery; planning-superset isolation; no future active tuple; exact compiled/signed enum; complete signed-envelope digest; stage-aware Rust transitions/outcomes/recovery/P-N-M-F tests; ancestry and lifecycle completeness |
...
| `0.19.0` | Integrity chains and signed-checkpoint interface | Event tamper/domain separation plus active law-catalog owner epoch, payload/envelope digests, exact profile, version/scope/validity/signer/root/revocation/successor binding |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, active-catalog-lineage/envelope, and semantic-realization storage negotiation | `VIT-CAP-057` owner/CAS/ratchet state; planning/active separation; exact profile and full envelope support; manifest/catalog persistence; closed realization; provider/campaign placement |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Every effective tuple/realization including `VIT-LAW-007`; future planned tuple, combined profile, envelope omission, signer/root substitution, activation/revocation race, ratchet rollback, self-consistent-untrusted, and stage-realization rejection |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve `VIT-INV-057` lineage/ratchets and complete active payload/envelope/predecessor/revocation state beside manifests, realizations, ancestry, owner fences, floors, and campaign anti-resurrection |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Planning/active confusion, catalog/envelope/ancestry omission, owner/ratchet loss, untrusted/future tuple, silent generation/profile upgrade, substitution, truncation, tenant mix-up, and integrity loss |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, exact law-catalog profile/envelope, and credential-operation/rotation/recovery-root decision | Select exactly `CompiledCatalog` or `SignedCatalog`; freeze payload/envelope digest/signature, dedicated root/signer quorum/rotation/revocation/validity/scope/successor fields and existing credential/remediation profiles |
| `0.140.2` | Storage topology for invariant/law manifests, owned active catalogs/ratchets, realizations, and provider state | Zero-missing ancestry/root/generation/capability/realization matrix; `VIT-INV-057` authority placement; payload/envelope/predecessor/revocation/high-watermark persistence without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, admitted law-generation/catalog-lineage, and recovery decision | Owned catalog distribution/refresh/CAS/revoke/emergency-distrust fencing and ratchets; full envelope/ancestry/realization placement; failover rejects future/stale/unknown catalogs; typed recovery and RPO/RTO |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover `VIT-INV-057` lineage/distrust/high-watermarks and exact payload/envelope independently of mutable storage; reject future/stale/partial/combined/self-consistent-untrusted catalogs plus existing anti-resurrection cases |
