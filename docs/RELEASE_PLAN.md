...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Freeze the canonical recovery tuple and exact `MigrationImportWorkBudgetV1` production limits/profile: both heads/non-wrapping chains/authenticated coverage, root/key/publication/cursor, exact covered/current rows, settlement/trigger/bundle/result/linkage, settled/remaining legs and conservative balances; retain both CAS/lock boundaries and prior ledger schemas. Atomically persist immutable operation keys, profiles, counters, precharges, reservations, closed job lifecycle, candidate/tombstone, owner manifest/dormant generations/preparation receipts, barrier sequence/result/authorization and cleanup linkage. Select only a fixed-lock-order co-located all-owner activation transaction through 1.0; remote selectors and distributed cutover refuse |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, and activation decision | Freeze fail-closed complete-tuple admission, singular-schema pre-mutation quarantine/no migration, and no reopen/pre-settle behavior across checkpoint/deletion stages. Preserve the exact migration/import operation key, budget profile/counters/reservations/cursor/result/exhaustion/cleanup, candidate/tombstone, dormant owner receipt set, barrier/authorization and non-borrowable Recovery reserve across selected HA/DR topology; failover cannot turn prepared or terminal state active |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over the complete recovery tuple, atomic checkpoint bundles and one durable migration/import job/candidate/barrier lineage; preparation remains dormant, stale receipts/fences deny, and unknown activation responses recover the one local result. No partial owner visibility, counter/profile reset, duplicate reservation, exhausted/terminal promotion, cleanup starvation or remote-selector substitution |
...
| `0.145.0` | Backup, restore, and disaster recovery | Round-trip/omit every tuple member; quarantine singular backups regardless of claimed meaning/evidence and refusal retries; restore every checkpoint-to-deletion boundary without reopened/pre-settled legs; retain unverifiable charges. Restore migration/import jobs before work; prepared candidates stay dormant, terminal candidates stay fenced, and active state requires the exact barrier, receipts and matching owner generations |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Retain prior stress; add checkpoint response/duplicate/publication races, mixed archives, trigger substitution and unavailable checkpoint history; prove one decrement or conservative charge. Sustain bounded-work attacks while racing exhaustion/cancellation/takeover/cleanup/owner changes at every activation lock; malformed or partial receipts, response loss, failover and restore never expose less than the complete atomic owner set |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit prior controls plus job ownership, operation-key uniqueness, profile/counter/precharge/reservation conservation, closed lifecycle, terminal fences, dormant preparation, candidate/manifest/receipt/barrier bindings, fixed lock order, all-owner atomicity, idempotent activation result, digest-only quarantine and protected cleanup |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze prior recovery formats plus job phase/disposition, candidate/tombstone, ordered owner manifest, dormant generations, preparation receipts, barrier sequence/predecessor/result/authorization and cleanup linkage. Unknown, omitted, reordered, weaker or selector-based fields fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack and pentest budget reset/undercharge/amplification/staging/retry/failover/quarantine/Recovery paths. Attack cancellation/exhaustion/cleanup/takeover and owner-version races, partial/reordered/substituted receipts, one-owner rejection, response loss, concurrent activation, prepared restore, falsely active state and remote-selector fallback; retest fixes |
| `0.150.0` | Final production-readiness candidate | Existing lifecycle/TCB/recovery evidence plus clean install/upgrade/restore/failover continuity for every migration/import job, budget, reservation, cursor, terminal disposition, candidate/tombstone, owner manifest/dormant generation/receipt, activation barrier/result/authorization and cleanup link |
