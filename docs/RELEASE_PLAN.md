...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Add non-recursive publication-profile trust/bootstrap/rotation, fail-closed root loss, tenant/deployment-scoped one-attempt cardinality, compatibility/distrust succession and a concrete linearizable external activation-fence port. Protect completion reserves, separate operational/candidate heads and proposal/active high-watermarks. Split lifetime-work from physical-capacity accounting, define backend storage-cost profiles and atomic Recovery-parent/child capacity transfers, freeze one universal lock rank, and release physical encumbrance only with archive/deletion plus parent credit. Preserve canonical active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→control-settlement-archive-head→control-settlement-journal-head→recovery-capacity-parent-ledger→corruption-control-reserve→history-obligation→corruption-fence→corruption-control-lineage→corruption-control-lineage-checkpoint→lineage-disposition→recovery-authorization→clearance-anchor-source-manifest-head→clearance-anchor-source-manifest-authorization→corruption-clearance-anchor-registry→corruption-clearance-scope→corruption-clearance-authorization→corruption-clearance-attempt→corruption-rebuild→corruption-rebuild-rejection-authorization→archive-head→history/idempotency→recovery-lineage-budget→attempt/successor-budget→retention/legal-hold→audit/result/outbox activation, registry rebinding, destructive authority, rebuild and every prior control |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve activation-fence request/receipt/status/finality and attempt slot, completion reserve, both head pairs, both child ledgers/transfers/equations, destination storage-cost re-admission, Recovery parent ledger/double-entry transfers/equation, lock-rank evidence and settlement/reclaim membership exactly. Independent signatures as CAS, source charge copying, split/duplicate parent credit, proposal-as-active, root recovery inference, old-profile activation, terminalization-capacity loss, head/ledger aliasing, premature release, lock inversion or rollback denies |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze publication-profile root/bootstrap/rotation/high-watermark, immutable old-identity retirement plus non-inheriting new-identity genesis after root loss/compromise, exact tenant/deployment one-attempt slot, compatibility/ratchet and linearizable activation-fence CAS/status reads. Request IDs bind one digest, final results are immutable and sequence/compaction preserve readable predecessor and request-result membership. Independent signatures are not CAS |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist fence/slot/retirement state, completion reserve, two heads, split work/physical ledgers, signed backend storage-cost profile with canonical rational ceiling arithmetic/golden vectors, Recovery parent ledger and deterministic one-operation/one-transfer double entry, universal lock rank/traces, ReleasePending and settlement/archive/deletion/parent-credit evidence. Missing transactionality, conservative bound, cost/fence continuity, participant or capacity refuses |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate trust bootstrap/rotation, profile transition/compatibility/distrust, activation-fence service, publication/status/local/external terminalization, completion-reserve, cost-profile signer, parent-ledger allocator, child transfer, custody release and archive roles. No recovery-root inference, self-selection, old-profile activation, accounting reset or inferred credit/release |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain profile trust/ratchet/fence receipts/status/attempt slot, both heads, completion reserve/checkpoint, storage-cost profile, parent/child ledgers/transfers and settlement/archive/deletion/credit membership until governed compaction. Erasure cannot orphan terminalization or create/refund/release capacity |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze HA reconciliation for profile succession/fence/attempt slot, root-loss fencing, both heads, completion reserve, cost profile, parent/child ledgers, universal rank and reclaim state. Failover cannot fork a fence winner, synthesize root recovery, copy storage cost, split/double-credit capacity, invert locks or resurrect state |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries activation-fence request/receipt/status/finality, attempt-slot, completion-reserve, two-head, destination cost-profile and parent/child transfer/settlement proofs without remote selection, activation, debit, credit, release or lock authority |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check concurrent Activate/Rotate/Distrust CAS and status reads, request-ID reuse, sequence finality/compaction, root retirement/reprovision non-inheritance, stale slot clear, publication reserve admission, rational-cost boundaries, deterministic transfer retry, both ledger equations, lock traces and reclaim. Require one fence result, one attempt, one logical transfer identity, atomic parent/child state and no stranded terminalization |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore fence request-result membership/finality, exact attempt slot, old-identity retirement, operational Active/candidate, completion reserve, destination cost profile, parent/child transfers/equations, universal rank and exact reclaim/credit membership. Reject stale Empty slots, inherited replacement authority, unavailable compacted fence membership, one-sided ledgers and snapshots that activate, debit, credit or release |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz CAS/status concurrency, request-ID mutation, independent-signature substitution, root loss/replacement inheritance, stale slot clearing, publication saturation, rational rounding and every cost overflow boundary, destination re-cost, duplicate logical transfer IDs, parent/child response loss, lock inversion and archive/delete/credit crashes |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit activation-fence consensus assumptions/finality and fail-closed root policy, attempt bound, terminal capacity, cost-profile conservatism, parent/child double entry, two-head restore, acquisition traces and release/deletion/credit membership |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze fence port/receipt/status, root-loss and attempt-slot policy, completion reserve, two heads, backend cost profile, child/parent ledgers/transfers/equations, universal rank and settlement encodings. Legacy independent-signature, raw-byte, child-only release or shared-profile schemas fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack non-linearizable fence/status reads, request-ID rebinding, final-result mutation, unsafe compaction, quorum nonintersection, fork/rollback, root-retirement omission or replacement inheritance, stale/cross-tenant slot clearing, rational-rounding/overflow undercharge, source-counter copy, duplicate transfer minting, parent leak/double credit/split repair, terminal-capacity starvation, head aliasing, premature reclaim and lock inversion |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for fail-closed profile governance, linearizable fence/one attempt, protected terminalization, destination cost profiles, atomic parent/child capacity, two manifest heads, universal lock order and rollback-safe reclaim |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  independent signatures, forked status, shared-profile enumeration and
  inferred in-place root recovery fail closed. Root loss/full compromise
  requires a permanently fenced old identity and controlled new-identity
  reprovisioning. Backend cost-profile tests re-cost canonical artifacts at
  destination and vary backend/schema/index/overhead/expansion/rounding.
