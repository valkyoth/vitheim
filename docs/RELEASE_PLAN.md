...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Persist one destination-local operation/job/budget/reservation/lifecycle/fence/candidate/tombstone/authorization/revocation-inbox/barrier/result/cleanup lineage plus the non-wrapping active coordinator generation/fence checked first by every mutation. Derive the exact owner manifest from admitted VIT-LAW-009 and retain domain ownership. Freeze active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation→audit/result/outbox; activation creates bounded Pending or explicit NoHistory/NotRequested atomically. Archive exhaustion becomes ManualRecoveryPending. Recovery request/results are closed RetryAppend/Waive/Abandon tagged unions. The shared authorization row has a total six-state table and canonical admission/expiry/revocation/recovery results; expiry remains expiry. Revocation binds complete signer/key/profile/time material and is destination-local. RetryAppend reaches only Appended/ManualRecoveryPending; Waive/Abandon alone create matching custody records. A platform-capped cumulative obligation budget has no amendment/increase path through `1.0.0`; current policy/legal hold/independent compliance approval governs terminal evidence before checkpoint/cleanup |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Require the complete canonical two-head tuple, unified namespace, atomic checkpoint bundle and mixed-trigger proof; missing/defaulted or singular state is quarantined before import mutation. Export enumeration, transfer, decode, verification and staging cannot create fresh counters, jobs, cursors or reservations. The live destination VIT-INV-062 state stays outside the candidate; nonterminal source jobs, cycles and identity collisions deny. Activation-atomic history survives missing delivery/restore; append advances the canonical archive head. Export/import preserves ManualRecoveryPending descriptors, closed action-tagged recovery authorization/result bytes, total six-state winner results, revocation authentication/inbox/sequence/tombstone, platform hard maximum and immutable cumulative/successor budgets, retry-exhaustion result, policy/hold evidence and custody records. Remote revocation remains inert; no cross-backend step resets/increases lineage, changes action, converts expiry, weakens custody, infers absence or alters activation |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze activation/bootstrap plus every recovery revocation wire field: signer identity, key identity/epoch, authentication profile, issued-at, not-before, exact target expiry, maximum uncertainty and trusted-time profile/epoch. Freeze exact obligation/action/authorization sequence scope, issuer continuity and canonical destination winner results. Only destination-local commits change authority |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist history obligation/archive head/disposition, closed action-tagged recovery request/result, the total six-state row and canonical winner results, revocation state, platform hard maximum, immutable cumulative/successor budgets, exhaustion/custody records/checkpoint, typed collisions and complete activation/bootstrap state. Freeze canonical absence, no amendment/increase path, counters and lock/CAS prefixes; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact migration/bootstrap roles and independent history-recovery requestor/approver/operator/issuer/admitter plus records/compliance/legal approval separation. Neither coordinator, worker nor affected domain actor may self-authorize, self-waive or self-abandon; retry authority cannot become abandonment authority |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Freeze ManualRecoveryPending plus canonical waiver/abandonment records, retention/classification policy generation/digest, legal-hold state/epoch, compliance/legal approval, evidence floor and deletion rules. Active hold cannot be weakened by waiver/abandonment, emergency flags or erasure |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve closed action-tagged authorization/result, every six-state winner result, complete authenticated revocation, platform hard maximum, immutable cumulative budget, action matrix, retry-exhaustion and custody evidence across HA/DR. Failover cannot change action, convert expiry, reset/increase lineage, infer remote revocation, lose ManualRecoveryPending, weaken legal hold or clean before terminal checkpoint |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries exact action-tagged recovery bytes, authenticated revocation, six-state result, immutable cumulative-budget and custody evidence but never claims remote effect. A peer cannot add mixed fields, reset/increase counters, convert expiry, change RetryAppend into Abandon, bypass a local tombstone/hold or erase ManualRecoveryPending |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over action-tagged recovery authorization/result, six-state winner state, authenticated revocation inbox/sequence/tombstone, immutable budgets and custody evidence. Race admission/revocation/expiry/consumption and require one canonical winner without state conversion, action-field mixing or counter increase |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore action-tagged bytes, all six-state winner results, authenticated revocation, platform maximum, exact ceilings/counters and custody evidence before work. Older backups cannot reset/increase a limit, convert expiry/action, erase ManualRecoveryPending or weaken a hold |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Exhaust successors; race every six-state operation; fuzz unknown/mixed/action-inapplicable fields, changed-action identity reuse and noncanonical options. Successor exhaustion remains pending, expiry remains expiry, cumulative limits hold and Waive/Abandon require current matching authority |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit action non-interchangeability/canonical absence, six-state winner provenance, complete revocation authentication, destination-only effect, platform-capped no-increase budget conservation, custody rechecks and checkpoint gating |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze action-tagged authorization/result, admission/expiry/revocation/recovery winner, revocation authentication, platform maximum, immutable lineage and custody wire/storage formats. Unknown/defaulted/mixed fields fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack action-tag confusion, changed-action identity reuse, noncanonical options, expiry conversion, revocation-field omission/substitution, counter reset/increase, remote-effect inference, custody weakening, response loss, restore and cleanup; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for action-tagged recovery, every six-state winner, authenticated revocation, platform-capped immutable counters, exhaustion/custody outcomes and checkpoint beside all prior evidence |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  decoding, every transition in the total six-state table, complete revocation
  authentication/time binding, expiry-winner preservation, the platform hard
  maximum and no lineage amendment/increase path. Unknown/mixed/inapplicable
  fields, changed-action identity reuse, noncanonical options, policy-based
  capacity creation, old-backup counter reset and remote-effect inference all
