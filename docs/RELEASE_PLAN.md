...
| `0.51.47` | Closed one-shot activation-, intent-, route- and local-version-bound remote effect authorization consumed with the physical effect | Inactive/disposed root, cross-intent use, stale route/object ABA, expiry/key rotation, replay, receipt authorization substitution |
...
| `0.51.51` | Independent current-generation incident and funded residual ownership for truly unauthorized post-closure physical effects | Terminal history reopened, old capacity reused, successor restriction omission, unfunded owner, incident overflow |
| `0.51.52` | No-effect intent disposition, dispatch fencing, abandonment or predecessor-linked cumulatively funded replanning | Dead destination, expired/never-dispatched authority, uncertain delivery mislabeled absent, identity/budget reuse, dual successor intent |
