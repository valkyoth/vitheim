...
| --- | --- | --- |
| `0.29.0` | Add the single-use six-state retirement authorization, genesis-created Operational/RetirementPending/Retired fence and transaction-atomic terminalization reserve. Make that fence the first universal lock rank. Replace multi-transaction child re-cost prose with an epoch-cut Preflighting→RecostPending→Applying→Complete→Activated campaign, bounded post-cut allocation/release log, dual-profile charges and explicit reversible abort. Use ValidNonWeakening/ValidWeakening/InvalidOrUnverifiable classification and reserve complete parent-checkpoint verification capacity before a checkpoint becomes restorable | Race authorization admission/revocation/expiry/consumption; prove no partial reserve/fence/Pending commit; recheck the retirement fence at every boundary; race live allocation/release against campaign apply/finalize/abort; reverse all pending charges; reject invalid profiles without destructive override; show an admitted maximum checkpoint can finish |
...
| `0.140.1` | Freeze retirement grant authentication/time/quorum/nonce preimages and the symbolic-classifier proof format, arithmetic family and three-outcome validity rule | Independent review proves the retirement issuer cannot self-admit or operate the deployment, and no authority can override invalid, unknown, over-budget or unverifiable accounting |
| `0.140.2` | Freeze all rows, indexes, universal lock ranks, campaign epoch/log/charge types, checked maxima and atomic parent-bucket/child-forward/inverse/checkpoint transactions for retirement, re-cost and verification reservation | Every selected storage adapter proves first-rank retirement, live dual-profile mutation, whole-delta reservation, full pending-charge abort reversal and complete-verification reservation or declares the profile unsupported |
| `0.140.3` | Freeze issuer/admitter, quorum and separation-of-duty roles; separate retirement, re-cost worker, cost classifier/authorizer, parent allocator and parent verifier capabilities | No role can both authorize and consume destructive retirement, classify and approve weakening, or repair/credit capacity |
| `0.140.5` | Freeze retention and custody rules for grant tombstones, fence history, campaign transfers/abort evidence, classifier proofs and verification reservation/spend evidence | Compaction and erasure cannot reopen authority, hide a partial campaign, reset work or make old custody usable |
| `0.140.6` | Freeze HA takeover, lease fencing, response-loss reconciliation and restore behavior for all four refinements | Failover resumes the same authorization winner, campaign epoch/log/cursor/bucket/pending charges and verification reservation without duplicate transfer, retained abort charge, free rescan or inferred terminal state |
| `0.142.0–0.148.0` | Carry typed status through RPC; model-check races; drill restore; fuzz maxima and crash boundaries; audit roles/capacity; freeze incompatible legacy encodings | Each milestone retains the exact `0.29.0` security state and produces its named evidence before advancing |
| `0.149.0` | External pentest targets authority theft/replay, lock inversion or partial reserve commit, live campaign churn/log exhaustion, active/pending charge confusion, abort inverse-transfer fraud, invalid-profile authorization, classifier resource exhaustion and under-reserved maximum snapshots | Zero unresolved critical/high findings and no waived destructive-authority, semantic-validity, bounded-work, conservation, locking or recovery issue |
...
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist retirement, terminal CAS, completion reserve/two heads; governed cost-profile trust/lineage/head/transition/activation/checkpoint/high-watermark/classifier and complete re-cost checkpoint; Recovery parent aggregate/exact membership/commitment/predecessor checkpoints and VerificationPending/Ready/Fenced cursor-budget state beside prior ledgers/transfers/rank/settlement. Missing bounded snapshot, transactionality, analytical comparison, conservative bound, protected capacity or continuity refuses |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate retirement authority, publication root/profile owner, terminal-fence service, cost-profile trust/ordinary owner/destructive weakening classifier-authorizer, backend selector/re-cost worker, parent verifier/allocator, child transfer and custody roles. Retirement cannot activate/import; adapters cannot classify/authorize cost weakening; verifiers cannot repair ledger state |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain retirement/incident/custody result, transition terminal receipts/status, cost lineage/classifier/re-cost proof, parent membership/checkpoints/verification charges plus prior evidence until governed compaction. Erasure cannot turn EvidenceUnavailable into usable custody, admit a second terminal meaning, hide weakening or reset verification |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze failover reconciliation for retirement, six-way governance CAS/terminal map, cost lineage and activation, parent aggregate/membership/checkpoint/cursor/budget/state plus prior controls. Failover cannot deadlock empty bootstrap, split a terminal winner, activate partial re-cost, reset/rescan verification or mutate a non-Ready partition |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries retirement state/authority result, transition terminal CAS receipt/status, cost-profile lineage/classifier/re-cost checkpoint and bounded parent-verification checkpoint/cursor/budget/state beside prior proofs without remote retirement, terminal, cost, repair, debit, credit, release or lock authority |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check retirement collection versus empty bootstrap/import; Activate/Abort/Seal/Rotate/Distrust/Retire with one terminal meaning; cost successor weakening and complete re-cost activation; parent verification crash/cursor/chunk/churn with bounded work. Require quarantined unavailable custody, one terminal result, no partial cost activation and no unbounded/non-Ready capacity mutation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore retirement terminal, unique transition terminal map, cost lineage/high-watermark/re-cost checkpoint and parent VerificationPending with fixed checkpoint/cursor/cumulative budget. Empty replacement remains possible; old custody, cost activation and affected capacity stay unavailable until their distinct proofs complete |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz retirement EvidenceUnavailable, all six CAS operations, terminal response loss, bounded symbolic cost classification/weakening/rollback/partial re-cost, parent aggregate overflow, checkpoint gaps, cursor recreation, free rescans, child churn and protected-capacity exhaustion beside prior crashes |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit retirement independence and non-authority, terminal mutual exclusion, complete-domain cost comparison/destructive weakening, bounded parent verification/protected capacity and every prior governance/accounting/recovery control |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze retirement states/receipt, six-operation fence/terminal map, cost lineage/classifier/checkpoint encodings and parent aggregate/membership/checkpoint/verification state. Legacy missing-retirement, independently writable journal, signed-only cost replacement or scan-on-start schemas fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack retirement deadlock/self-signing/custody escape; Activated+Aborted/Unresolved dual meaning; abort-before-local-CAS races; cost signer/lineage rollback, sampled-comparison bypass and partial re-cost; parent aggregate forgery, unbounded startup, cursor reset/free replay, churn and non-Ready mutation beside all prior attacks |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for independent retirement and empty bootstrap, mutually exclusive external terminal state, governed cost succession and bounded parent verification beside protected terminalization, two heads, atomic transfers, universal locking and rollback-safe reclaim |
