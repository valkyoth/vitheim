...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Activation commits Pending plus immutable zero-counter platform-capped lineage, or NoHistory/NotRequested plus no-executable-lineage proof, under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation/lineage-disposition→audit/result/outbox. Initial append locks lineage before attempt budget and atomically co-charges both; missing nonterminal lineage is corruption. Recovery authorization/result enum tags are the sole action authority and revocation derives its target from exact authorization bytes. The total six-state row returns one closed Admitted/Expired/Revoked/Consumed/NotAdmitted outcome; explicit expiry is Issued-only and absent expiry/consumption writes nothing. Retain destination-only authenticated revocation, no-increase budget, closed RetryAppend/Waive/Abandon transitions and current custody authority |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve activation-created lineage or no-executable proof, exact initial co-charges, sole-tag canonical bytes/derived indexes, Issued-only expiry and closed outcomes across export/import. Missing/defaulted lineage is quarantined, never reconstructed; denormalized action mismatch is corruption. Retain the complete canonical two-head tuple, live destination coordinator exclusion, inert history, exact revocation state, immutable counters, custody evidence and activation atomicity. No cross-backend step creates capacity, guessed expiry tombstones, adapter-specific result unions, duplicate action authority, remote revocation effect, weakened custody or inferred absence |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze complete recovery revocation signer/key/profile/time fields, exact authorization-scoped sequence, issuer continuity, Issued-only trusted-time expiry and the one closed operation outcome. Revocation derives action solely from authenticated authorization bytes; only destination-local commits change authority |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist activation-created zero-counter lineage or no-executable proof, atomic initial attempt/cumulative charges, sole-tag authorization/result bytes, derived/read-verified indexes, the total row and closed outcomes. Freeze no-lazy-creation, Issued-only expiry, NotAdmitted no-write, platform cap/no amendment, counters and lock/CAS prefixes; unsupported adapters refuse |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve activation-created lineage/no-executable proof, both initial-charge legs, sole-tag bytes/derived indexes, Issued-only expiry and closed outcomes beside every prior recovery artifact. Failover/restore cannot invent lineage, split charges, create an absent expiry tombstone, change action/outcome, reset counters, infer revocation or clean early |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries exact sole-tag bytes, closed outcome, activation-created lineage/no-executable proof and both initial charges, but never a second action or remote effect. Peers cannot invent lineage, tombstone absent expiry, substitute an outcome/index, reset counters, change action, bypass a hold or erase pending evidence |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Race activation lineage creation, initial dual-budget charge, append, failover and cleanup; then race all authorization operations. Require complete lineage/no-executable proof, indivisible charges, one sole-tag action and one canonical outcome without absent expiry writes |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore exact lineage for every nonterminal or no-executable proof for terminals, both initial charges, sole-tag bytes/index checks and closed outcomes before work. Older backups cannot invent lineage/capacity, tombstone absent expiry, alter action/outcome or weaken custody |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault activation/dual-charge atomicity; fuzz missing/lazy lineage, derived-action mismatches, absent expiry/consumption, adapter-union variants and prior malformed action encodings. Lineage/outcomes stay canonical and all prior bounds/actions hold |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit lineage creation/proof provenance, dual-charge conservation, sole action-tag derivation, NotAdmitted no-write and closed-outcome provenance beside prior revocation/budget/custody controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze lineage disposition/no-executable proof, atomic charge representation, sole-tag authorization/result/revocation targeting, derived indexes and closed outcomes. Old/defaulted/duplicate action or adapter-specific outcome encodings fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack lazy/late lineage, split charges, guessed expiry DoS, outcome confusion, outer/payload action contradiction, derived-index corruption, restore reconstruction, counter increase, remote effect and custody weakening; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove clean install/upgrade/restore/failover continuity for activation-created lineage/no-executable proof, dual charges, sole-tag bytes/indexes, Issued-only expiry, closed outcomes and every prior recovery artifact |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  decoding, activation-created zero-counter lineage or canonical
  no-executable-lineage proof, atomic attempt/cumulative initial charges, every
  transition in the total six-state table, Issued-only expiry, typed no-write
  NotAdmitted, one closed operation outcome, sole-tag action authority and
  derived-index verification. Complete revocation authentication/time binding,
  expiry-winner preservation, the platform hard maximum and no amendment path
  remain mandatory. Missing/lazy lineage, split charges, guessed expiry
  tombstones, adapter-specific outcomes, outer/payload contradictions,
