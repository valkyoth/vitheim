...
| `0.140.2` | Freeze local retirement co-admission/finalization; replacement stage; inbox import; source reconciliation; transfer-witness Prepare/status reconciliation; activation/current-pointer CAS; candidate rejection; exhaustion and audit/outbox transactions under the recovery lock rank | Backend faults expose each local bundle all-or-none. Same-backend optimization must prove one transaction over the same logical records; portable profiles use outbox/inbox and never assume distributed atomicity |
| `0.140.3` | Separate retirement quorum/admitter/finalizer, Recovery budget owner, replacement selector, source freezer/outbox owner, destination importer, reservation-mapping verifier, independent transfer witness, conformance verifier, activator/current-pointer owner and source reconciler | No actor can retire and replace alone, select and activate its own candidate, map its own unsafe reservation, witness its own transfer, import and self-acknowledge, or consume/refund another writer class |
