...
| `0.140.2` | Freeze rows/indexes and checked atomic transactions for all four parent aggregates, workspace parent transfer/inverse, logical/physical checkpoint, old-fence cleanup and `MigrationWorkspaceSettlementCheckpointV1`, plus revocation inbox/tombstone/result | Each adapter proves no double reservation/credit, same-cut verification, safe cleanup after None/new slot, exact-once whole workspace settlement and destination-local revocation or refuses |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze prior authority plus permanent-quarantine revocation intent/target-sequence/key-continuity/time preimage and the exact shared logical/physical checkpoint digest. Remote intent, cross-target sequence, independently valid different cuts, lost-root self-retirement, dual terminal disposition or inferred custody readiness deny |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist prior controls plus four parent aggregates/memberships; workspace parent transfer/inverse; exact-cut checkpoint; stable old-fence cleanup; deletion/settlement checkpoint; and quarantine revocation inbox/tombstone/result. Missing bounded work, transactionality, exact logical/physical identity, conservative capacity or lifecycle continuity refuses |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate prior roles plus quarantine issuer/admitter/revocation-applier/operator, workspace worker/settler and parent verifier/allocator. No role may issue+apply revocation, build+falsely settle, mutate a new campaign during old cleanup, or repair/credit ledger state |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain prior evidence plus workspace parent transfers, exact-cut proof, old Closed fence, settlement checkpoint and quarantine grant/revocation tombstones. Erasure cannot drop the fourth member, invent a cut/credit, partially refund quarantine, reopen Closed or revive authority |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze failover reconciliation for the fourth parent member, exact cut, old cleanup fence, settlement and revocation inbox/sequence beside prior state. Failover cannot infer a cut/sequence, mutate a new campaign, duplicate credit/release or mutate a non-Ready partition |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries all four parent members/transfers, exact campaign/workspace cut, old cleanup fence, settlement and quarantine revocation status without remote retirement, terminal, recovery, revocation-apply, workspace settlement, debit, credit, release or lock authority |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check parent/workspace transfer conservation, logical+physical cut closure, slot-clear/new-campaign/old-cleanup interleavings, revocation/apply/consume CAS winners and all prior campaign races. Require no double credit, mismatched cut, stale mutation, cross-target revocation, partial settlement or refund |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore fourth aggregate/membership/transfers, exact cut, old terminal fence, settlement checkpoint and revocation sequence/inbox beside all prior state. Unavailable proof leaves custody/cost/capacity unavailable without guessed credit/cut/revocation |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz transfer/checkpoint/revocation codecs, bounds, crash points and changed retries; malformed/stale material stays within hard work limits and cannot split parent/workspace state |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit issuer/applier, worker/settler, allocator/verifier and old/new campaign separation; every retained/refunded unit, cut and revocation is attributable |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Reject legacy encodings lacking workspace parent membership/transfer, exact-cut fields, stable cleanup fence, settlement checkpoint or revocation sequence/inbox; no defaults or downgrade |
| `0.149.0` | Release candidate and external pentest remediation | Attack double workspace spending, forged inverse/deletion, mismatched-cut activation, stale-slot cleanup mutation/starvation, quarantine revocation theft/replay/cross-target suppression, partial quarantine refund and all prior attacks |
