...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze pre-auth ingress-work bounds, authenticated endpoint/audience/credential-profile presentation lanes, non-borrowable emergency lane capacity and exact lane/class match; retain per-presentation/first-seen-request rates, denied-history permanence, admission/outstanding accounting, atomic issuance/settlement and terminal/reconciliation separation |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Exhaust ingress bytes/concurrency/crypto/decode work; forge/substitute/rotate/revoke/fail over/restore lanes and flood normal/break-glass/recovery capacity; retain replay/concurrency/response-loss/saturation/conflict, one-request-charge, terminal-only settlement, bounded denial and reservation/range tests |
| `0.23.0` | SQLite single-node adapter | Prove ingress-work and authenticated-lane mapping persistence, lane non-borrowing/class match, request/checkpoint bounds, rate separation, terminal/reconciliation type separation, atomic issuance, bounded verifier, deadline-CAS and reserve semantics or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove ingress/lane mapping and two-rate concurrency parity plus terminal/reconciliation, issuance/reservation, pause/failover, no-late-commit, sparse/range and reserve isolation |
| `0.25.0` | Experimental MySQL adapter | Prove ingress/lane, denied-history, two-rate and terminal/reconciliation parity plus settlement/range behavior or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove ingress/lane, denied-history, two-rate and terminal/reconciliation bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific ingress/lane, denied-history, two-rate and terminal/reconciliation bounds or refuse VIT-CAP-060/061; namespace/graph/query isolation and capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve ingress profiles, lane mapping generation/fence/revocation high-watermarks, presentation windows, request charges/sequences/checkpoints, terminal/reconciliation state, original claims/reservations/settlements; reject stale/caller-selected/borrowed/mismatched lanes and existing replay/release weakening |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger ingress limits, authenticated lane mapping/non-borrowing/class-match semantics, denied-history permanence, both rates, terminal/reconciliation types, issuance atomicity, live-receipt retention and original-claim settlement |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze authenticated endpoint/audience/credential-profile lane derivation and separate emergency identities alongside terminal/reconciliation types, terminal-only settlement, sender-only consumer/verify-only issuer and existing lineage/time rules |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze ingress-work limits, lane mapping generation/fence/revocation state, non-borrowable lane capacities and lane/class match alongside presentation/request rates, denial history, atomic writes, claims, terminal/reconciliation sequences and settlement |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze failover/restore of ingress profiles, greatest lane mapping generation/fence/revocation, presentation/request state, sequences/checkpoints, terminal/reconciliation roles, issuance, claims, reservations and settlement |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement ingress-work limits and authenticated non-borrowable presentation lanes with exact class match alongside both rates, denial history, terminal/reconciliation types, settlement and range controls |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve ingress and authenticated lane identity/mapping plus both rates, request proof and terminal/reconciliation codecs across RPC; only terminal receipts settle |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over ingress profiles, lane mapping high-watermarks/non-borrowing, presentation windows, request charge/sequence/checkpoint, terminal/reconciliation roles, issuance/settlement and bounded verification without stale-lane admission or rollback |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover ingress profiles, lane mapping high-watermarks/non-borrowing/class match, both rates, denied history and terminal/reconciliation state without stale-lane admission, reevaluation, false release, forgery or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Soak ingress exhaustion; lane forgery, rotation, revocation, failover, restore and cross-lane floods; both rates, replay/concurrency/conflict, denial proof, terminal/reconciliation and reservation/range controls |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit ingress and authenticated lane enforcement/non-borrowing/class match, both rates, denied-history limits, terminal/reconciliation canonicalization, terminal-only settlement and supply-chain hardening |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze ingress budget, lane mapping generation/fence/revocation, lane capacity/class match, presentation/request/admission accounting, sequences/checkpoints, terminal/reconciliation and reservation/range formats |
