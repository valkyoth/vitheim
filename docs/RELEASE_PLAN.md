...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Independently authorized manifest initialization creates one generation zero and a CAS current head over authenticated committed activation records before activation. Activation binds that head, creates registry generation zero and reserves non-borrowable corruption-control capacity under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→clearance-anchor-source-manifest-head→corruption-control-reserve→history-obligation/corruption-fence/clearance-anchor-registry/lineage-disposition→retention/legal-hold→audit/result/outbox. Fencing creates one lifetime-budgeted scope even under ordinary saturation. Registry advancement atomically rebinds Open scope state and stale-terminalizes a live grant without losing charges. Manifest weakening and permanent rebuild rejection use complete destination-local admission/revocation/expiry/consumption protocols. Typed restoration and parent/proposal/one-successor rebuild retain every prior control |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve manifest lineage identity/state/genesis/current head/activation records/destructive-authority tombstones, registry genesis/ratchet/scope-rebinding result, protected control reservation/profile/platform bound, scope lifetime state, typed joins and rebuild parent/proposal/rejection authorization/successor isomorphically. Raw-generation selection, fork/gap/reorder, missing reserve, detector-supplied maximum, stranded Open scope, remote-only revocation, reset charge, invalid join or consumed parent denies beside every prior control |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze manifest bootstrap/activation-record authentication and complete weakening/rebuild-rejection grant timing, key continuity, revocation and tombstone profiles beside collection/registry/clearance. No remote message or clearance role changes source policy or reconstruction availability |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist manifest lineage/current head/activation records, registry-to-scope rebinding, control reserve/profile/platform bound, scope lifetime state, typed algebra and rebuild parent/proposals/rejection lifecycle. Missing rows, raw-generation selection, stranded scope, saturation bypass, invalid join or parent consumption refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate manifest bootstrap/strengthening/weakening, scope admission, attempt worker, rebuild proposal and permanent-rejection admission/revocation roles from clearance/registry/collection authority. No actor self-admits a destructive grant or infers rejection |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Preserve manifest activation/destructive-authority history, registry rebinding, control reservations, scope lifetime tombstones, typed-join evidence and rebuild parent/proposals/rejection. Erasure cannot reopen head, scope, reserve or parent authority |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve manifest head/records/tombstones, protected reserve, atomic registry/scope rebind, sole scope grant/lifetime work, typed join and one parent/successor. Failover cannot fork a head, lose control capacity, strand scope, apply remote revocation, reset work or infer rejection |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries manifest head/activation records, admitted destructive-grant state, registry/scope rebinding and reserve evidence, but cannot initialize/advance a head, apply revocation, change scope occupancy, weaken policy, consume reserve, permanently reject or select a successor remotely |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check manifest genesis/head/fork races, every destructive-grant CAS, registry/scope/active-grant rebinding, reserve/fence saturation, competing clearance grants, typed joins and proposal/rejection/activation. Require one manifest head, live grant, rebinding result and successor |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore greatest committed manifest activation/head, destructive-grant tombstones, control reserve/profile bound, registry/scope rebinding, lifetime state, typed inputs and rebuild parent before work. Raw high generation or an older reservation snapshot cannot reopen authority |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz genesis fork/gap/reorder, destructive-grant races, remote-only revocation, scope rebinding, all ordinary-capacity saturation/profile reduction, budget reset, algebra laws and invalid proposal no-write beside prior cases |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit manifest current-head activation provenance, destructive-authority transitions, reserve derivation/conservation, scope rebinding/lifetime conservation, typed-join provenance and parent/successor lineage |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze manifest lineage/head/activation and destructive-grant codecs, control-reserve and scope-rebinding encodings beside typed join/rebuild. Lazy/raw generation, absent reserve and abbreviated signed grants fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack manifest bootstrap/head selection, destructive revocation/admission races, saturated fencing, capacity-profile reduction, stranded scope, competing grants, mixed joins, parent consumption and unauthorized rejection beside all prior cases; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for manifest head/authority, protected reserve, registry/scope rebinding, lifetime algebra, rebuild authority and every prior recovery artifact |
