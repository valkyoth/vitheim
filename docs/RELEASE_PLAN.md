...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add the closed attempt-set writer matrix for execution/finalization authorization, Begin/Dispatch/Complete/reconciliation, capability lifecycle, provider evidence and checkpoint/compaction/archive. Every mutation follows one routing→state-head→attempt-set→attempt→auxiliary→result order and atomically advances the head. A covered mutation against CommitEligible writes an exact invalidation and selects PreparingRevalidationRequired with mandatory old/new root bindings; PreparingOpen alone admits new effect/artifact work and the retained fence is integrity evidence. MarkEligible repeats its full validation. Dispatch persists one unique application-level transmission claim; after return/crash/lease loss/uncertainty only query reconciliation is permitted | No unrooted writer, lock inversion, blocked restrictive processing, preparing-mode confusion, reopened admission, stale-eligibility Commit trap or application resend is reachable |
...
| `0.140.5` | Retain mutation ancestry, invalidation/revalidation results, sticky fences, claim/redemption/terminal evidence and authenticated archival replacements | Compaction cannot erase a writer, revive eligibility, reopen dispatch or recreate a send right |
