...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Candidate, barrier and authorization bind exact Pending/NoHistory/NotRequested evidence and policy epochs. Activation commits the chosen evidence plus Healthy fence generation zero under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation/corruption-fence/lineage-disposition→retention/legal-hold→audit/result/outbox. Every history path follows one universal order. Clearance uses independent single-use authority plus a destination-ratcheted, mandatory-class/quorum anchor registry and authenticated collection receipt; authorization admission and final restore recheck its generation. One durable leased attempt precharges all proof work and follows the normative six-state operation table. Canonical restoration commits the component-wise authenticated counter join. Permanent fallback records exactly one predecessor-bound successor and disjoint archive selection. Fenced remains an outcome and only Authorization/Revocation/Recovery mismatches are conflicts; every prior lineage, custody, charge, no-amendment and deterministic CAS/terminal rule remains |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve disposition commitments; Healthy/Fenced/ClearedAfterRestore lineage; universal order; clearance lifecycle; anchor-registry ratchet/receipt; attempt lease/fence/cursor/precharges/results; canonical counter join; unique rebuild mapping; Fenced outcome; three-variant conflicts; and every prior custody/charge/CAS rule across backends. Missing/defaulted registry or attempt state, downgraded anchors, reset charges, duplicate successors or ambiguous old/new archive selection deny. No adapter may infer health, reorder locks, create capacity, substitute disposition, remotely clear, partially restore, guess expiry or weaken custody |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze clearance authentication/time and independently authenticated anchor collection/registry profiles with mandatory classes, minimum quorums, issuer continuity and monotonic high-watermarks. No database-local or clearance-role assertion alone proves completeness |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist fence lineage/order, clearance lifecycle, ratcheted anchor registry/receipt, normative operation table, unique leased attempt/cursor/precharge/result, canonical counter join and predecessor-unique rebuild mapping. Missing rows, stale generation, reset work, incomparable counters or ambiguous archive selection refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate clearance decision roles from anchor registry/collection quorum, attempt worker/takeover and rebuild requestor/approver/operator. Detector, importer, archive/domain actor or ordinary recovery issuer cannot self-clear, nominate anchors or choose competing successors |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Preserve fence/authority, anchor-registry lineage/receipt, attempt charges/results and unique rebuild/archive mapping beside custody evidence. Erasure cannot downgrade a registry, reset proof work, clear a fence, delete the old obligation or make both histories current |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve disposition/fence/order plus registry ratchet, attempt lease/cursor/counters, operation-table winner, canonical counter join and one-successor mapping. Failover cannot nominate anchors, reset charges, resurrect workers, duplicate successors, reuse authority or select ambiguous history |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries exact registry/receipt, attempt/cursor/charge/result, counter-join and rebuild-mapping bytes beside prior clearance state, but never remote registry advancement, clearance, work-charge reset or successor selection |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check anchor advancement versus admission/final restore, every six-state operation, attempt lease takeover/precharge/cursor crash, terminal CAS and competing rebuilds. Require one winner, monotonic anchors/charges, one successor and deterministic adapter-isomorphic outcomes beside prior races |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore the registry ratchet/receipt, exact attempt/charges/result, computed counter join and predecessor-successor mapping before work. Older backups cannot downgrade anchors, rerun charged work, revive a fenced worker, duplicate a successor or select both namespaces |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz omitted anchor classes, stale registries, quorum/receipt forgery, every attempt transition, crash-before/after precharge, evidence outage, budget exhaustion, authority loss, counter overflow/incomparability and rebuild races beside prior cases |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit anchor provenance/ratchet, attempt work conservation, terminal table result, counter join and unique rebuild/archive selection beside prior authority, custody and fence provenance |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze registry/receipt, attempt lifecycle/cursor/counters/results, operation table, counter join and rebuild record. Old unconstrained-and/or anchors, stateless proof, conservative-but-undefined counters or unmapped rebuilds fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack stale signed anchor omission, local registry nomination, quorum collapse, proof-charge reset, worker takeover races, temporary-evidence loops, authorization loss, counter underjoin/overflow, duplicate rebuild and dual-current history beside all prior cases; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove clean install/upgrade/restore/failover continuity for the anchor ratchet, operation table, durable attempts/charges, canonical counter join, one-successor mapping and every prior recovery artifact |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  registry and authenticated collection receipt. One leased durable attempt
  precharges proof work, persists cursors/counters/results across takeover and
  uses the normative authorization table; canonical restored counters equal
  the component-wise authenticated high-watermark join. Stale/incomplete/
  unanchored lineage remains fenced, and one predecessor-bound rebuild record
  selects exactly one successor namespace while retaining the old evidence.
  ...
  proof escape/reset, stale registry or omitted anchor class, collection-receipt
  forgery, temporary-evidence retry loop, worker takeover race, noncanonical
  counter join, duplicate successor or dual-current archive, split charges,
