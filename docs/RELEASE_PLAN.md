...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain every prior sparse-replay control; freeze cumulative publication plus `DrainReplayAdmissionGuardV1`: verify proof for writer-authoritative `H` outside the transaction, then lock/re-read head and exact key, check hot state, uniquely claim execution and atomically commit; typed head change restarts without writes |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain all prior attacks; pause after proof, after head lock and before unique insert while racing compaction/first execution; inject mismatched replica head/hot state and require restart/refusal with no duplicate execution, then retain all publication/fork/GC tests |
| `0.23.0` | SQLite single-node adapter | Persist all prior state; prove writer-authoritative head-first/key-second locking, exact head re-read, current-hot check and unique replay insert share one local write transaction with the result bundle, plus publication/GC fencing, or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove writer routing and transaction isolation prevent replica/weak-snapshot authorization, head/key/hot/unique-claim linearization, concurrent compaction parity and publication cleanup without cross-system transactions |
| `0.25.0` | Experimental MySQL adapter | Prove all prior admission-guard isolation/locking/uniqueness and cumulative-publication parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior admission-guard isolation/locking/uniqueness and cumulative-publication parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific admission-guard isolation/locking/uniqueness and cumulative-publication parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve all prior state plus admission isolation profile, authoritative routing, head/key lock-order contract, replay uniqueness/claims, cumulative head/publication/current-hot/covered-row evidence; reject reset, synthesis, fork, rollback, staged promotion, lost rows or unsafe GC |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger writer-authoritative admission guard, head-change restart, current-hot check and unique replay claim plus cumulative publication/unknown-outcome/GC meaning; otherwise keep action execution fenced and refuse the import/profile |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Retain prior authentication bindings; freeze not-before/issued/expiry, maximum uncertainty, trusted-time profile/epoch, issuer continuity, signer/key identity/epoch/authentication profile, conservative validation, monotonic ratchets, historical consumed validity and fresh activation authority |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain prior sparse schema; freeze cumulative-head row/CAS/index and publication lifecycle plus proof-to-execution admission guard, writer-authoritative read routing, head-first/key-second locks, unique replay-row claim, adapter isolation requirements, coalescing/orphan-GC metadata and unknown-outcome reconciliation |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain sparse selection; freeze cumulative-head retention/coalescing, staged/orphan exclusion, covered-row deletion evidence and GC eligibility without deleting committed history |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Retain prior recovery; freeze writer-authoritative admission routing and failover plus cumulative head/publication/current-hot/unique-key/object-visibility/GC recovery and high-watermarks, rejecting stale-head proof, split replica visibility, fork, partial head/delete, staged authority or rollback |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus cumulative replay publication and `DrainReplayAdmissionGuardV1`: verify proof for authoritative `H` outside the transaction; lock/re-read head then key; check hot state; claim the unique row and atomically commit, or return typed no-write `DrainReplayHeadChanged` |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve all prior fields plus admission-head identity and authoritative-read provenance across RPC; follower/cache/weak-snapshot proof cannot authorize, and peers cannot promote staged data, use stale heads or invent deletion/visibility/GC |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over all prior state plus writer-authoritative routing, admission guard, unique replay key, cumulative head/publication/current-hot/object visibility/GC without stale proof, split replica visibility, fork, partial head/delete, staged authority, rollback or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore every prior field plus admission isolation profile, unique replay claims, cumulative head/publication/current-hot/object/GC state; reject stale proof, split visibility, fork, staged promotion, partial delete, premature GC and head rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; pause proof readers before transaction, after head lock, and before unique insert while compaction/first execution race; inject mismatched replica head/hot views plus publication, migration, and failover races |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit every prior control plus admission-guard isolation, authoritative routing, shared lock order, replay-key uniqueness, cumulative-head CAS, publication authority, indivisible local delete, reconciliation and GC fences |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze all prior formats plus admission guard/head-changed error, head sequence/predecessor/root/scope/version, authoritative-read provenance, unique replay claim, publication identity/state, covered-row evidence, reader rules and GC eligibility |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add proof-for-H/post-H compaction TOCTOU, ignored head change, weak/replica authorization, unique-claim bypass, first-execution/compaction schedules, stale heads, fork/substitution, visibility/unknown outcomes, staged/orphan authority, premature GC and rollback; retest fixes |
