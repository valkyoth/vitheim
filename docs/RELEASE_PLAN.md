...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Add an independently rooted Pending/Retired/EvidenceUnavailable deployment-retirement protocol and permit only empty new-identity bootstrap when old evidence is unavailable. Extend the linearizable governance fence so Activate/Abort/PermanentlyUnresolved are one per-transition terminal CAS beside Rotate/Distrust/Retire. Govern storage-cost profiles through lineage/head/classifier/destructive weakening and one parent/selector active re-cost slot. Re-cost conserves active + campaign-RecostPending + pending-successor encumbrances; uses a pre-reservation release tombstone matrix and folded reservation cut; closes campaign/mutation-fence product states and bounded prior-state recovery; terminal-checkpoints slot release; requires one-shot authorization for permanent quarantine; and runs physical workspace build/catch-up/verify/cleanup/quarantine/exact settlement under campaign ownership before authority-consuming activation. Add constant-time parent aggregates plus checkpointed budgeted streaming verification. Preserve protected completion reserves, two manifest heads, split ledgers, atomic parent/child transfers and canonical deployment-retirement-fence→active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→control-settlement-archive-head→control-settlement-journal-head→recovery-capacity-parent-ledger→backend-storage-cost-active-recost-campaign-slot→backend-storage-cost-recost-campaign-fence→corruption-control-reserve→history-obligation→corruption-fence→corruption-control-lineage→corruption-control-lineage-checkpoint→lineage-disposition→recovery-authorization→clearance-anchor-source-manifest-head→clearance-anchor-source-manifest-authorization→corruption-clearance-anchor-registry→corruption-clearance-scope→corruption-clearance-authorization→corruption-clearance-attempt→corruption-rebuild→corruption-rebuild-rejection-authorization→archive-head→history/idempotency→recovery-lineage-budget→attempt/successor-budget→retention/legal-hold→audit/result/outbox order |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve retirement states/authority evidence, governance-fence terminal receipts/status/mutual exclusion, cost-profile lineage/high-watermark/classifier, active campaign slot/state/prior-state/intent/fence generation/product, release tombstones/reservation cut, three accounting aggregates, post-cut fold, command results/conflicts, terminal checkpoint, quarantine authorization/tombstone, workspace state/cursor/physical high-watermark/capacity/settlement and authority-consuming activation, plus parent verification and prior heads/reserves/ledgers/transfers/equations/rank exactly. Missing/defaulted lifecycle state, pre-reservation credit, invented refund, unbounded recovery/finalization, premature workspace release or source inference denies |
...
| --- | --- | --- |
| `0.29.0` | Add the single-use retirement grant/fence/reserve and one active-slot re-cost campaign. Freeze three-case release semantics around whole-delta reservation, the folded-log reservation cut, complete campaign+mutation-fence product, prior-state-specific bounded recovery, terminal checkpoint/slot release, six-state permanent-quarantine authority and campaign-owned workspace lifecycle/ledger/cursors/high-watermark/exact deletion settlement. Retain three-bucket conservation, bounded final fold, atomic activation authority and complete parent-verification reservation | Race preflight release before/with/after reservation; prove no successor credit before the bucket exists. Model every product-state/CAS terminal winner and recovery origin without unbounded work. Race quarantine grant revocation/expiry/consumption. Crash copy/catch-up/verify/cleanup/settlement and prove no premature/double capacity release. Retain all prior classifier, retirement, conservation and maximum-checkpoint proofs |
| `0.30.0` | Export/import preserves retirement state plus campaign prior/product state, release tombstones/reservation high-watermark, terminal checkpoint, quarantine authority/tombstone, and workspace state/cursor/physical high-watermark/capacity/settlement beside every prior campaign/classifier/verification artifact | Missing/defaulted state, copied source charges, pre-reservation successor credit, partial campaign/workspace import, quarantine authority loss, premature settlement, invalid-as-weakening mapping, a restorable unreserved checkpoint or changed retry material leaves the destination fenced and unready |
| `0.140.1` | Freeze retirement and permanent-quarantine grant authentication/time/quorum/SoD/nonce preimages, symbolic-classifier proof format/arithmetic/validity and final re-cost activation authority preimage | Independent review proves neither destructive issuer can self-admit/consume, no authority overrides invalid accounting, quarantine cannot be selected by generic recovery, and selector/profile activation cannot outlive or split from current authority |
| `0.140.2` | Freeze rows/indexes for release tombstone/fold cut, campaign+fence product/prior state, terminal checkpoint, quarantine lifecycle, workspace state/cursor/high-watermark/capacity/settlement and every prior atomic transaction/rank | Each adapter proves no pre-bucket credit, bounded recovery by origin, one terminal CAS/checkpoint, authorized quarantine, exact-once workspace deletion settlement and all prior constraints or refuses |
| `0.140.3` | Freeze issuer/admitter/quorum/SoD roles for retirement, campaign/recovery, permanent quarantine, workspace work/settlement, classifier, parent allocation and verification | No role can self-authorize quarantine, approve and execute Fenced recovery, build and falsely settle workspace, classify/approve weakening, or repair/credit capacity |
| `0.140.5` | Freeze custody for release tombstones/cuts, product/fence/terminal state, quarantine grant tombstones, workspace mutation/settlement evidence and all prior artifacts | Compaction/erasure cannot create preflight credit, reopen Closed, reuse quarantine authority, discard cleanup encumbrance, invent settlement/refund or reset work |
| `0.140.6` | Freeze HA takeover/restore for prior-state recovery, release folds, terminal checkpoint, quarantine authorization and workspace lifecycle | Failover resumes identical cursor/fold/reservation/workspace/settlement and authority winner without unbounded transaction, reopened fence, duplicate credit or release |
| `0.142.0–0.148.0` | Carry the refined states through RPC; model-check release/reservation and product CAS races; drill restore; fuzz work/capacity/crashes; audit roles; freeze legacy encodings | Each milestone retains exact `0.29.0` state and produces named evidence before advancing |
| `0.149.0` | External pentest targets pre-bucket credit, reservation-cut race, unbounded recovery, invalid product transition/Closed reopen, quarantine authorization theft/replay, terminal-checkpoint/slot-release forgery, workspace high-watermark/cleanup/settlement fraud and all prior attacks | Zero unresolved critical/high findings and no waived authority, bounded-work, conservation, locking, physical-capacity, settlement or recovery issue |
...
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist retirement and prior controls plus pre-reservation release tombstones/fold cut; unique slot; complete campaign+mutation-fence product/prior state; terminal checkpoint; quarantine authorization/result; workspace state/cursor/physical high-watermark/capacity aggregate/operation result/settlement/checkpoint; three parent aggregates; bounded folds; typed results; atomic authority-consuming activation and parent verification. Missing bounded work, transactionality, conservative logical/physical capacity or lifecycle continuity refuses |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate retirement authority, publication root/profile owner, terminal-fence service, cost-profile trust/ordinary owner/destructive classifier-authorizer, campaign owner, fenced-recovery authorizer, quarantine issuer/admitter/operator, workspace worker/settler, backend selector, parent verifier/allocator, child transfer and custody roles. Campaign/recovery/workspace actors cannot self-authorize quarantine; workspace builders cannot falsely settle deletion; verifiers cannot repair ledger state |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain retirement/custody, terminal receipts, cost lineage/classifier, release tombstone/cut, campaign product/prior state/terminal checkpoint, quarantine authority tombstones, workspace high-watermark/capacity/settlement and parent verification evidence. Erasure cannot create credit, reopen Closed, reuse authority, discard cleanup reserve or invent deletion settlement |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze failover reconciliation for all prior state plus prior-state bounded recovery, release fold cut, one terminal checkpoint, quarantine winner and workspace cursor/high-watermark/capacity/settlement. Failover cannot force unbounded preflight, reopen a fence, split terminal meaning, duplicate credit/release or mutate a non-Ready partition |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries retirement and transition state plus release tombstone/cut, campaign prior/product state/fence/fold/terminal checkpoint, quarantine authority/result, workspace state/cursor/physical high-watermark/capacity/settlement/checkpoint and bounded parent verification without remote retirement, terminal, recovery, quarantine, workspace settlement, debit, credit, release or lock authority |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check preflight release versus fold/reservation; every campaign+mutation-fence product transition; prior-state recovery; activation/abort/quarantine terminal CAS/checkpoint/slot clearing; workspace build/catch-up/verify/cleanup/settle crashes; and all prior races. Require no pre-bucket credit, unbounded recovery, Closed reopen, dual terminal, invented refund or premature capacity release |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore release tombstone/fold cut, campaign prior/product/fence/terminal state, quarantine authorization tombstone and workspace cursor/high-watermark/capacity/settlement beside all prior state. Recovery resumes the same bounded state; unavailable proof leaves custody/cost/capacity unavailable |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz releases in all three matrix cases, reservation-cut boundaries, every valid/invalid product pair, prior-state takeover, quarantine grant races, terminal slot release, workspace copy/catch-up/cleanup/deletion/settlement and all prior maxima/crashes |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit release matrix/cut conservation, bounded recovery, terminal checkpoint uniqueness, quarantine SoD/tombstones, workspace source ledger/aggregate/exact settlement, physical peak capacity, atomic authority and all prior controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze release tombstone/cut, campaign product/prior/terminal, quarantine lifecycle and workspace lifecycle/capacity/settlement encodings. Legacy pre-bucket credit, Fenced-to-Applying-only, unspecified fence combinations, reservation-only workspace or two-bucket schemas fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack release-before-bucket credit, reservation-cut gap, unbounded preflight recovery, invalid product transition/Closed reopen, quarantine grant theft/replay, terminal checkpoint/slot forgery, workspace high-watermark rollback, cleanup deletion forgery and double settlement beside all prior attacks |
