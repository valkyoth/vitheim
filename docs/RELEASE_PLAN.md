...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Persist one destination-local operation/job/budget/reservation/lifecycle/fence/candidate/tombstone/authorization/revocation-inbox/barrier/result/cleanup lineage plus the non-wrapping active coordinator generation/fence checked first by every mutation. Derive the exact owner manifest from admitted VIT-LAW-009 and retain domain ownership. Freeze active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation→audit/result/outbox; activation creates bounded Pending or explicit NoHistory/NotRequested atomically. Coordinator succession uses a stable closed lifecycle, distinct pre-admission-revocable begin, fresh checkpoint/receipt-bound handoff and cancellation grants, one action-discriminated exact-target revocation intent/apply protocol, bounded work, canonical checkpoint/successor receipt, safe drain release/fence advance and exact per-action results/conflicts. Archive exhaustion becomes nonterminal ManualRecoveryPending; independently authorized resolution uses one bounded successor budget or records evidenced waiver/abandonment before checkpoint and cleanup. Register all archive obligation/head/recovery/checkpoint semantics before cross-backend use |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Require the complete canonical two-head tuple, unified namespace, atomic checkpoint bundle and mixed-trigger proof; missing/defaulted or singular state is quarantined before import mutation. Export enumeration, transfer, decode, verification and staging cannot create fresh counters, jobs, cursors or reservations. The live destination VIT-INV-062 state stays outside the candidate; nonterminal source jobs, cycles and identity collisions deny. `MigrationImportRegistryHistoryV1` binds authenticated provenance/scope, original terminal result, export/import manifests, namespace, retention/classification, sequence/predecessor and idempotency. Activation-atomic obligation survives missing delivery/restore; append consumes only Pending under bounded work/protected cleanup and advances the canonical archive head. Export/import preserves ManualRecoveryPending descriptors, recovery authorization and successor-budget lineage; remote bootstrap revocation messages remain inert until destination apply. Cleanup waits for true terminal obligation checkpoint; no absence inference or archival rollback/activation ambiguity is allowed |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze activation plus bootstrap begin/handoff/cancellation authorization admission and the shared action-discriminated revocation intent/apply signer, key and authentication profiles; pre-admission tombstones; issuer continuity; exact-target/action sequences and target-covering lifetimes; trusted time; authenticated checkpoint/successor/outbox/inbox receipts; exact per-action/revocation results/conflicts. Only destination-local commits change authority |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist roles/generation/fence/live exclusion, activation rows, history obligation/archive head/disposition/budget/recovery-authorization/successor-budget/result/checkpoint, typed collisions and bootstrap identity/lifecycle/begin-handoff-cancel authorization/shared-revocation-inbox-and-sequence/budget/checkpoint/successor receipt/results. Freeze active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation→audit/result/outbox and coordinator-first action prefixes; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact requestor/approver/activator/issuer/admitter separation, per-target revocation sequencing, distinct bootstrap begin/handoff/cancellation issuers, admitters and actors, and independent history-recovery requestor/approver/operator roles. Neither coordinator nor migration/domain actors self-bootstrap, self-cancel, self-waive or self-abandon; existing authority remains exact-target-bound until consumed, expired or destination-locally revoked |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain prior retention rules; freeze mixed checkpoint/deletion archive evidence, trigger/leg minimization without aliasing, conservative checkpoint-history retention, and distinct ManualRecoveryPending/WaivedFenced/AbandonedWithEvidence descriptor, authorization, budget, result and minimum-evidence lifetimes |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve coordinator derivation/generation/fence, activation state, history obligation/head/disposition/budget/recovery authority and attempt/result/checkpoint, bootstrap shared revocation inbox/sequence/tombstones/lifecycles/budget/checkpoint/successor receipt/results and barrier state across HA/DR. Failover cannot strand a drain, lose Pending/ManualRecoveryPending, infer NoHistory/NotRequested, clean before checkpoint, commit predecessor work, infer remote revocation effect, admit over a tombstone, alter activation because archival failed or reverse Consumed |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries exact law/roles/generation plus activation and shared action-discriminated bootstrap revocation provenance, checkpoint/successor receipt, history-obligation and recovery delivery identities, but never claims remote effect. A peer cannot bypass a local tombstone, advance/cancel bootstrap, erase Pending/ManualRecoveryPending, infer explicit absence or turn archive transport into activation |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over role/generation state, activation tombstones, history obligation/head/disposition/budget/recovery authority/attempt/result/checkpoint, full bootstrap begin/handoff/cancel authorization and shared revocation inbox/sequence/tombstones/budget/checkpoint/receipt/results. Race action admission/revocation/expiry, cancellation/handoff, activation/obligation creation, missing worker, recovery resolution, cleanup and archive publication; recover one safe terminal without stranded drain, missing obligation, lost recovery evidence, partial visibility or ambiguity |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore active generation, activation state, bootstrap shared action-revocation inbox/sequence plus three-action tombstones/lifecycles/checkpoint/receipt/results and history obligation/head/disposition/recovery authority/budgets/attempt/result/checkpoint before work. Older backups cannot strand drains, admit delayed authority, erase Pending/ManualRecoveryPending, invent NoHistory/NotRequested, clean early, commit predecessor work, infer remote effect or reverse Consumed |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Race begin/handoff/cancel authorization before admission and at every expiry state, shared revocation destination apply and per-action sequence, cancellation versus handoff, checkpoint/receipt substitution, activation versus obligation creation, missing worker, cleanup versus Pending/ManualRecoveryPending, successor-budget recovery, waiver/abandonment, archive collision/failure, failover and response loss. Exactly one safe local terminal wins; no drain, obligation or recovery evidence disappears and archive status never changes activation |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit admitted-law trust, action-specific bootstrap authority/tombstones/checkpoint/receipt/budgets, history obligation/head/checkpoint semantics, canonical active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation→audit/result/outbox order, domain atomicity and exact results |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze bootstrap begin/handoff/cancel/shared-revocation and history obligation/head/recovery-authorization/budget/result/checkpoint wire/storage formats plus coordinator/activation state. Unknown, omitted, generic-bootstrap-authority, terminalized-manual-recovery, absent-obligation or guard-unaware fields/binaries fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack action-grant confusion, pre-admission revocation bypass, remote-effect inference, cross-action sequence suppression, expiry at every bootstrap state, lost cancellation, cancel/handoff races, digest-only checkpoint/receipt forgery, activation crash before/after obligation, missing worker, cleanup race, Pending/ManualRecoveryPending restore, successor-budget substitution, forged waiver/abandonment, absence inference, stale handoff and archive failure; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for active generation, activation, bootstrap begin/handoff/cancel authority/shared revocation state and exact results, typed checkpoint/receipt, history obligation/head/disposition/recovery authority/budget/attempt/result/checkpoint, barrier and irreversible activation result |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  inert-history promotion, and interrupt coordinator bootstrap at every boundary.
