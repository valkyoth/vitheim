...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Persist control receipts/tombstones, exact claim/freshness/owner-protocol scope, tombstone-first historical outcomes, issuer/local uncertainty, topology authorization/epoch/approval evidence, future three-state handoff, exact envelope/verifier, and closed realization |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Destructively test claim crash/expiry/revocation/restore replay precedence, freshness misuse, topology authorization/CAS atomicity, control-receipt authentication, topology initialization order, clone, time, ancestry, scope, signature, and untrusted rejection |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/receipt authentication, topology authorization, and credential-operation decision | Freeze the action-claim/freshness/owner-protocol scope matrix, replay precedence, lifetimes, issuer/consumption uncertainty, `TopologyMutationAuthorizationReceipt`, both control receipts, closed authenticators, MAC role separation, and durable anchors |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology state, active catalogs/ratchets, and provider state | Separately place control receipts/tombstones, action-claim issuer/local evidence, topology authorization/tombstones, and identity/policy epoch bindings; persist authorization pin and three-state topology handoff without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology, and recovery decision | Race control receipts, action-claim uncertainty/replay precedence, topology authorization, and normal-path topology rechecks through failover/restore; prove no reissue/reconsume, topology order, fully fenced activation policy, four-owner recovery, time loss, and RPO/RTO |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus policy-governed topology handoff and normal-path dynamic admission | Activate/converge epoch 12 under generation 1; admit `VIT-LAW-007@g02`/`VIT-LAW-008@g02`; require atomic topology authorization/claim/CAS; after commit reject stale placements at admission/readiness/dispatch/start even when rollout is complete and fence delivery is suppressed |
