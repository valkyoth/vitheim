...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze per-request sequence and bounded denied-history checkpoint/archive permanence, separate attempt/admission/outstanding accounting, atomic issuance, original-claim settlement, live-receipt retention, and the complete closed-outcome sender-only consumer terminal envelope alongside existing range/sparse/reserve controls |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Crash denial sequence/checkpoint/delete and issuance/settlement writes; prove late compacted denial never reevaluates, missing proof fails closed, denial resources stay bounded, every terminal field/outcome/auth role is enforced, `Reconciling` retains capacity, and existing reservation/range tests hold |
| `0.23.0` | SQLite single-node adapter | Prove request-sequence/checkpoint/archive bounds, canonical terminal-envelope roles/outcomes, atomic issuance, original-claim settlement, live-receipt retention, distinct rates, bounded verifier, deadline-CAS, sparse/late and reserve semantics or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove denied-history and terminal-envelope parity plus corrected issuance/rate/reservation lifecycle under pause/failover, no-late-commit, sparse/range eligibility and reserve isolation |
| `0.25.0` | Experimental MySQL adapter | Prove denied-history/terminal-envelope and corrected issuance/rate/retention/settlement/range parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove denied-history/terminal-envelope and corrected issuance/rate/retention/settlement/range bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific denied-history/terminal-envelope and corrected authorization bounds or refuse VIT-CAP-060/061; namespace/graph/query isolation and capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve request/issuance sequences, denial/issuance checkpoints and proof bounds, terminal envelope/outcome/result/outbox/auth roles, rate ledgers, original claims, reservations and settlements; reject retry recharge/renumber, denial reevaluation, proof-as-new, `Reconciling` release, forgery, rollback or weakening |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger denied-history permanence/resource bounds and terminal-envelope semantics plus rate separation, issuance atomicity, live-receipt retention and original-claim settlement; missing proof remains fail-closed |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze complete canonical consumer terminal envelope, closed outcomes, sender-only consumer/verify-only issuer authentication and non-terminal `Reconciling`, plus existing lineage/live-receipt/time/revocation rules |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze request sequence, denial checkpoint/archive/horizon/resource bounds, distinct rate ledgers, atomic success/bounded denial writes, original claims, terminal result/outbox sequences and all-or-none settlement plus existing range/sparse/reserve profiles |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Minimize denial/issuance checkpoints and range evidence to domain-separated commitments; keep plaintext request/principal/approval/receipt data outside long-lived proof while never making erased history fresh |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze failover/restore of request/issuance sequences, denial/issuance checkpoints, terminal envelope/outcome/sequence/auth roles, rate ledgers, whole issuance, claims, reservations and settlements |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement bounded denied-request sequence/checkpoint/archive lifecycle and canonical terminal envelope/state machine alongside corrected rate/reservation/range controls |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve request replay identity/proof and every terminal field/outcome/sequence across RPC; consumer sender credentials never enter issuer service and `Reconciling` never settles |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over denial sequence/checkpoint/delete, terminal envelope sequences/auth roles, rate ledgers, issuance/reservations/settlement and bounded verification without retry recharge, denial reevaluation, false release or rollback |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover denied-history identity/checkpoints/proof bounds and terminal envelope/outcome/sequences/auth roles plus existing authorization state without reevaluation, false release, forgery or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Soak denial history at maximum attempt rate, checkpoint/archive loss, late retries, proof-work bounds, all terminal outcome/envelope races and existing rate/reservation/range controls |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit denied-history permanence/resource limits and terminal-envelope canonicalization/credential separation alongside existing replay, settlement and supply-chain hardening |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze request/issuance sequence, denial/issuance checkpoint, terminal envelope/outcome/result/outbox/auth-role and existing accounting/reservation/range formats |
