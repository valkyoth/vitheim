...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add predecessor-linked RevalidationCounterCapacityCheckpoint and CapacityArchiveReplayHead. Bind capacity state/proof/predecessor, all per-class and total/advance/rollover/sentinel equations, current counters, the cumulative exact set of canonical charge/material/mutation/head/result identities, exact captured hot rows, bounded chunks, result lookup and publication/key/encoding epochs. Authoritative lookup is greatest verified head plus hot suffix. Final compaction atomically charges history lifecycle, advances capacity/head, installs the verified archive head and deletes only the captured earlier charges; its own charge remains hot | Same-material archived retry returns the historical result, changed material conflicts, exact nonmembership plus hot absence alone is unseen, and missing history returns typed historical-state-unavailable without a charge/head; split head/deletion is unobservable |
| `0.30.0` | Export/import the full checkpoint/replay-head ancestry, exact-set/result proofs, chunks/publication receipts and uncovered hot suffix with the current capacity state | Import selects the greatest authenticated head and rejects missing, downgraded, forked, partially deleted or epoch-mixed coverage; no older readable archive is substituted |
| `0.140.1` | Freeze checkpoint/replay-head domains, field widths, canonical charge ordering, exact membership/nonmembership/conflict/result proof formats, chunk bounds and publication/key/encoding epoch bindings | Review proves dense watermarks, probabilistic membership, range-only coverage, identity aliasing and result rebinding cannot authorize a charge |
| `0.140.2` | Freeze stage/verify/finalize transactions and routing→residual-state→capacity-state→attempt-set→capacity-archive-replay-head order. The final CAS couples one history charge, capacity/head transitions, verified head installation, exact captured-row deletion and outputs | Every backend exposes the entire bundle or no change; the compaction charge cannot self-cover/self-delete and partial deletion cannot become readable |
| `0.140.3` | Separate charge writer, checkpoint builder, archive publisher, verifier, replay-head owner, result reader, Replan planner and restore/import validator | No publisher self-verifies, reader chooses an older head, compactor mints capacity or Replan omits archived consumption |
| `0.140.5` | Retain checkpoint/replay-head ancestry, exact-set/result proofs, chunks, publication/key/encoding evidence, sentinel fence/result and the uncovered hot suffix for the governed horizon | Compaction cannot erase retry/conflict evidence, create a false unseen identity, hide sentinel consumption or detach a result from its charge |
| `0.140.6` | Freeze failover at upload/verification and final CAS, unknown response reconciliation, concurrent compaction/writer/Replan behavior and greatest-head recovery | Takeover joins the stored bundle; no duplicate history charge, lost hot row, older-head fallback, counter reset or fork is reachable |
| `0.142.0` | RPC exposes read-only replay-head/checkpoint identity, coverage status and typed exact replay/conflict/historical-unavailable outcomes without exposing proof internals as authority | RPC cannot select a checkpoint, assert unseen, bypass unavailable history, install a head or charge capacity |
| `0.143.0` | Model exact retry before/after compaction, changed-material reuse, concurrent new hot charges, archive loss, partial deletion, head rollback/fork, Replan across archives and sentinel recovery | Only one linearized charge/result meaning exists; unavailable or inconsistent history remains unready and never authorizes progress |
| `0.145.0` | Restore/migrate the greatest authenticated replay head, verify every chunk/epoch/result proof, replay the hot suffix and reprove class/total/sentinel/head equations and retained Replan obligations | Missing key/chunk/result, rollback, fork, incomplete deletion evidence or equation mismatch returns historical-state-unavailable/corruption, never inferred absence |
| `0.146.0` | Fuzz canonical ordering, exact-set proofs, coverage boundaries, checkpoint/head predecessors, chunks, epochs, result bindings, captured deletion set and archived/hot overlap | Omission, duplication, substitution, truncation, ABA, downgrade, false membership/nonmembership and partial compaction fail closed |
| `0.147.0` | Audit exact replay/conflict/unseen semantics, publication verification, atomic compaction, self-charge exclusion, Replan carryover, HA and restore | Zero high/critical archival authorization, capacity refund, replay, rollback, split-transaction or result-integrity finding |
| `0.148.0` | Reject legacy broad-only capacity coverage or imports that cannot prove exact charge/result membership, greatest-head continuity and captured deletion | No compatibility shim infers unseen, picks an older readable archive, reconstructs a result or renumbers/coalesces charges |
| `0.149.0` | External pentest targets false nonmembership, changed-material archived retry, archive/key/result loss, partial deletion, checkpoint/head rollback or fork, compaction response loss, Replan reset and sentinel recovery | Zero unresolved critical/high findings and no waived exact historical authorization, atomic compaction, anti-rollback or capacity-conservation issue |
| `0.150.0` and `1.0.0` | Demonstrate exact capacity-charge replay and conservation through compaction on every backend/RPC/HA/Replan/import/restore path | Production remains blocked until the unchanged candidate passes archival fault/model campaigns and independent pentest |
