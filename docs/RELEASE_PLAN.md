...
The canonical roadmap contains 403 exact stops: 150 base `0.x.0` stops, 252
...
`0.51.3–0.51.37`, `0.60.4`, `0.70.5`, `0.100.2`, `0.118.5`,
...
| --- | --- | --- |
| `0.1.0` | Workspace, architecture laws, threat-model format, CI, private crates, evidence-status manifest, canonical 403-stop roadmap manifest, and release baseline | Repository trust, CI permissions, action pins, source/publication policy, count/claim drift, fail-closed release gate |
...
| `0.51.23` | Source-partition reinstall activation admission with one-shot permit redemption, closed effective-restriction admission authority, and successor-restriction bridges | Signature-as-admission, permit race/replay, top/lowered tag substitution, rejected/unknown admission, abort-fenced late result, bridge omission |
| `0.51.24` | Durable successor-restriction bridge ownership, funded capacity, effective-restriction admission root, canonical empty root, and typed root-bound activation preparation CAS | Missing/double bridge owner, mixed authority branch, preparation without complete roots, abort/expiry race, absent/empty confusion, restore divergence |
...
| `0.51.33` | Per-partition pre-operational lowered-bridge adoption through the shared admission and funded ownership pipeline | Partial/duplicate adoption, response-loss acceptance, top/lowered owner mixing, stale adoption root, bridge/capacity duplication |
| `0.51.34` | Tagged initial/replacement final activation consuming the same effective-restriction admission branch with commit-time freshness and deferred predecessor cleanup | Genesis/admission substitution, post-prepare generation race, double eligibility consumption, missing lowering manifest/capacity, abort race, partial activation |
...
| `0.51.37` | Predecessor-top release or accepted funded-retention reconciliation with explicit pending uncertainty and zero-pending completion | Omitted enforcement point, unknown relabeled retained, unsafe release, unauthenticated/unfunded retention, mixed-result completion with pending, conservation mismatch, premature cleanup |
