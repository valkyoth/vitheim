...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze canonical authorization receipt V1, complete issuer/consumer time schemas, atomic topology bundle, and mandatory deadline-conditional CAS; persist existing ratchets/claims/handoff/verifier/realizations |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Add lock/time/pre-CAS/commit/timeout/response-loss/failover pauses; require pre-expiry commit or provably absent commit; test canonical fields, ratchets, tombstones, authorization ordering, replay, clone, ancestry, and trust rejection |
| `0.23.0` | SQLite single-node adapter | Prove admitted deadline-CAS mechanism and complete VIT-CAP-060/061 field parity or refuse dynamic topology; locking, rollback, injection, permissions, tenant partition |
| `0.24.0` | PostgreSQL reference production adapter | Prove authoritative commit-time/no-late-commit mechanism, atomic topology bundle, receipt/time schema parity, and pause/failover matrix plus existing evaluator/rotation/recovery/floor controls |
| `0.25.0` | Experimental MySQL adapter | Prove deadline-CAS and field parity or refuse VIT-CAP-060; isolation/encoding, rollback, tenant enforcement, no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove deadline-CAS under write concern/failover and field parity or refuse VIT-CAP-060; transaction/query/collection isolation, no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific deadline-CAS and field parity or refuse VIT-CAP-060; namespace/graph/query isolation, capability truthfulness, no default v1 claim |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve canonical receipt/time/deadline-CAS schemas and mechanism, reject downgrade writers and partial/reset state, plus separate owners, control/claim receipts, exact identities/fences, envelope/revocation, and anti-resurrection |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Preserve every authorization/time/tombstone/mechanism field and admit only same-or-stronger destination deadline-CAS; reject owner/receipt loss, topology rewrite, planning/ancestry/scope/predecessor confusion, untrusted/future tuple, truncation, and integrity loss |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze SQLite/PostgreSQL deadline-CAS mechanisms; separately place canonical receipt V1, issuer/consumer time ratchets/high-watermarks/tombstones, atomic topology bundle, claims/control receipts, and three-state handoff; block profiles without no-late-commit proof |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Activate generation-2 laws; issue canonical class-bounded grants; consume through backend deadline-CAS atomic bundle yielding only pre-expiry commit or no-late-commit absence; ratchet topology receipts and reject stale placement |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Race issue/epoch/revoke/expiry/CAS; pause locks/time/commit/timeout, lose response, fail over issuer/topology/database owners, attempt late commit, and preserve no-late-commit proof, bounded grants, time ratchets, rollout winner, MAC roles, and separation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover canonical receipt/deadline-CAS mechanism/result evidence, three-state handoff, topology/issuer/consumer ratchets, independent authorization/break-glass state, and tombstones; reject mechanism downgrade, inferred/later commit, field loss, validity extension, rollback, reissue, merge, clone, or resurrection |
