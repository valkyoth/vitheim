...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Governed publication-profile lineage binds exact witnesses/quorum into every manifest artifact. Transitions use Prepared→ProposalPublishedVerified→LocalActivationCommitted→Active plus Aborted/PermanentlyUnresolved, durable per-witness reconciliation and separate proposal/active high-watermarks, so a proposal never poisons active recovery; PermanentlyUnresolved requires an authenticated external terminal seal. Activation creates the protected reserve and obligation-wide capacity transfer ledger under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→clearance-anchor-source-manifest-head→corruption-control-reserve→history-obligation/corruption-control-lineage/corruption-fence/clearance-anchor-registry/lineage-disposition→retention/legal-hold→audit/result/outbox. Every dimension conserves Available/EpisodeReserved/TerminalizationReserved/Consumed/Settled; episode reservation and proof consumption are exact-once atomic transfers, never double charges. Release enters separate authenticated settlement journal/archive heads. Registry rebinding, destructive authority and typed rebuild retain every prior control |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve publication-profile lineage/head, durable attempts/witness request-receipts, external lifecycle/dispositions, proposal and active high-watermarks, activation/abort evidence and local head; preserve every control capacity bucket/transfer/conservation proof plus settlement journal/archive heads and exact membership; retain registry/scope/rebuild state isomorphically. Proposal-as-active, descendant use of unresolved publication, profile substitution, double charge, cross-dimension transfer, consumed decrease, missing/forked settlement history or capacity resurrection denies; the last proved Active predecessor remains usable |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze governed publication-profile lineage, exact witness/source set/quorum, proposal/activation/abort journal authentication, per-witness reconciliation, separate high-watermarks and profile weakening authority. No worker-selected mode, proposal-as-active, local-only latest claim or abbreviated destructive effect |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist the complete external lifecycle/profile/attempt/request/receipt/head state; capacity buckets, stable transfers and conservation; settlement rows plus separate journal/archive heads and membership. Missing/reset/recreated/duplicate state, double charge, dimension movement, consumed decrease or local/settlement rollback refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Separate profile succession/weakening, prepare, per-witness publication/status query, local activation, external activation/abort, control transfers and settlement publication from release/scope/rebuild roles. No actor self-selects witnesses, self-witnesses activation, resets accounting, deletes uncertain proposals or infers settlement |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain external proposal/activation/abort lifecycle and bound profile, capacity transfer/conservation history and settlement journal/archive membership until governed compaction. Erasure cannot orphan a proposal, turn it active/aborted, erase a consumed charge, or reopen/re-settle capacity |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Freeze HA reconciliation of proposal/local-CAS/external-activation gaps and status-query identities; preserve profile heads, both manifest high-watermarks, capacity transfers and both settlement heads. Failover cannot poison readiness with a proposal, select another witness profile, double-charge, resurrect capacity or settle twice |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries bound publication-profile and external lifecycle/status evidence, both high-watermarks, capacity transfer and settlement-head proofs, but cannot select witnesses, infer proposal activation/abort, perform unjournaled debit/settlement, reset capacity, weaken policy, release or reject remotely |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check proposal publication versus authority/profile/CAS/crash/lost response, external activation/abort reconciliation, both high-watermarks, every capacity-bucket transfer and settlement journal transaction. Require one externally Activated head, no orphan cleanup, conservation after every step and no duplicate debit/release |
...
| `0.145.0` | Backup, restore, and disaster recovery | Derive latest from externally Activated journal membership, not greatest proposal; reconcile unresolved publication and restore bound profiles, capacity buckets/transfers/conservation, separate settlement heads and exact membership. Older snapshots cannot resurrect or re-settle capacity |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz per-witness timeouts/status queries, activation/abort loss, profile change/invalidation, proposal/active head divergence, every transfer overflow/underflow/double-charge and settlement journal/archive crash/fork/rollback |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit publication-profile governance and lifecycle terminalization, active-head derivation, per-dimension conservation/transfer identities, nondecreasing consumption and rollback-safe settlement membership |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze profile/attempt/witness/journal/disposition/proposal-active-head/activation-abort encodings, capacity buckets/transfers/equation and settlement heads/membership. Old proposal-as-active or standalone settlement schemas fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack proposal poisoning, lost witness responses, authority/profile change between publication and CAS, activation/abort ambiguity, caller-selected quorum, double charging, cross-dimension transfer, consumed rollback, unjournaled/duplicated settlement and capacity resurrection; retest every prior case |
| `0.150.0` | Final production-readiness candidate | Prove install/upgrade/restore/failover continuity for governed externally Activated manifest lifecycle, conservation-preserving control ledger, rollback-safe settlement journal/archive, complete authority, registry/scope, rebuild and prior recovery artifacts |
