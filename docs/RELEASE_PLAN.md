...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain the transactional attempt-capacity ledger, original reservation set, reservation-free joins, lifecycle transfers and per-leg settlement; split the prior ambiguous settlement head into DB-local journal and verified archive replay heads with separate CAS boundaries, exact authoritative lookup and all prior bounded replay/conflict/conservative-unavailable guarantees |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain every prior attack; add crashes after journal commit/before upload, upload/before verification, verification/before archive CAS and at archive-CAS/hot-delete, plus duplicate-delete/compaction, captured-version, stale-H proof, absent-envelope non-membership and two-head restore/migration races |
| `0.23.0` | SQLite single-node adapter | Retain all prior adapter proofs; prove local settlement journal CAS, separate verified archive replay-head CAS/exact-range delete, captured-version recheck and head-H authoritative revalidation across busy/crash/reopen, or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove replay-head→journal-head deletion order, archive-head→journal-head→covered-row publication order, both CAS boundaries, exact-range compaction and conservative lookup under concurrency/pool failover |
| `0.25.0` | Experimental MySQL adapter | Prove both settlement heads/CAS boundaries, captured-row version/range, authoritative lookup and all prior bounds/exact-ID/conservative semantics plus attempt-ledger parity, or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove both settlement heads/CAS boundaries, captured-row version/range and all prior semantics under transactions/failover, or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific journal/archive heads, both CAS boundaries, captured-row version/range and all prior semantics, or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve both settlement heads, both predecessor chains and their proved relationship, exact hot-row IDs/versions/ranges, archive root/key/publication, tombstones, cursor/budgets and balances plus all prior state; reject fork, rollback or lost proof |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require both CAS boundaries, same-or-stronger two-head authoritative lookup, settlement archive/proof/bounds and exact duplicate/conflict semantics with conserved balances; otherwise retain charges, fence execution and refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze journal-head and archive-replay-head schemas/semantics, both CAS and lock boundaries, captured hot-row version/range, two-head relationship and all prior settlement/ledger fields |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain bounded settlement retention/key/tombstone/envelope/GC rules; freeze exact captured hot-row IDs/version/range and prohibit journal-head exposure as proof of archive availability |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze both settlement heads/predecessor chains/relationship, hot versions, publication/tombstone/cursor and conservative missing-proof recovery plus prior balances; no fork, rollback or inferred release |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus distinct local journal and verified archive replay heads, both CAS boundaries, captured-range compaction and archive-head-H revalidation |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve both settlement heads, relationship, proof/publication/hot-row-version/tombstone provenance across RPC; peers cannot conflate local commit with archive availability or invent settlement/conflict/absence/release |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over both settlement heads/predecessor chains/relationship, exact hot versions, archive/publication/tombstones/cursor and balances with prior state; no duplicate decrement, head conflation/fork/rollback or false absence |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore both settlement heads, predecessor relationship, exact hot-row versions/ranges, archive/key/publication/cursor/tombstones and balances; reject missing/forked/rolled-back proof and retain charge |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; crash every boundary between journal commit, upload, verification and archive CAS/delete; race duplicate deletion, captured versions and stale-H proofs; restore/migrate and prove one decrement or conservative charge |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit two-head domain separation, no archive implication from journal state, both CAS/lock orders, captured-range deletion, H revalidation and conservative failure plus prior controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze settlement journal/archive-head/checkpoint/proof/hot-version/tombstone/error formats plus prior records; old nodes cannot conflate heads, infer dense settlement or discard proof |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add two-head conflation, unavailable archive referenced by journal head, boundary crashes, captured-range over-delete, stale-H proof, absent-envelope non-membership, head relationship fork/rollback and duplicate-delete/compaction races; retest fixes |
