...
| `0.18.1` | Durable quota claims with all-or-none canonical set reservation and fair recovery | Overlap deadlock/livelock, partial reserve/restore, token/member substitution, cross-kind settlement, refund/write-off fraud |
| `0.18.2` | Atomic work family with one-owner grant lineages, exact-set quota tokens, and effect resolution | Approval/grant crash/reorder, pre-issuance revocation, successor fork, two-stream mutation, member reacquisition, atomic split |
...
| `0.22.0` | Mandatory storage, grant-lineage, and quota-linearization conformance testkit | Two-stream grants, partial claim sets, deadlock, token/digest mismatch, test bypass, backend divergence |
| `0.23.0` | SQLite single-node adapter | Locking, rollback, injection, file permissions, tenant partition |
| `0.24.0` | PostgreSQL reference production adapter | Grant issuance/revocation ordering, claim-set all-or-none/deadlock/restore, transactions, authentication, injection, tenant isolation |
...
| `0.30.1` | Durable work queue preserving grant lineage and exact quota-set tokens | Approval/grant reorder, pre-issuance revocation, successor fork, token/member substitution, partial set, failover, unfair recovery |
...
| `0.46.0` | Change management with explicitly owned durable grant lineages | Approval/grant crash/reorder/duplicate, pre-issuance revocation, successor fork, two-stream mutation, unsafe backout |
...
| `0.58.0` | Field redaction, obligations, and execution-authority enforcement registry | Hidden-field leaks, missing grant issuance/redemption/revocation gate, unsafe freshness/authority classification |
...
| `0.60.0` | Human, workload, grant, and delayed-effect authorization conformance suite | Principal/authority confusion, offline-human impersonation, grant replay/drift/revocation, target substitution, interface equivalence |
...
| `0.63.0` | Human approvals with inline or dedicated grant-lineage issuance | Task theft, self-approval, owner ambiguity, approval-receipt substitution, issuance reorder, pre-revocation, successor fork |
| `0.64.0` | Timers, deadlines, grant redemption, and retries | Timer-created authority, grant window/attempt/revocation races, clock manipulation, duplicated effects |
...
| `0.70.0` | HA workers with grant-lineage process management and exact-set quota settlement | Grant issuance/revocation/successor races, overlapping-set deadlock, partial set/failover, token/member substitution, network/crash windows |
...
| `0.140.6` | Deployment, HA, grant-ownership, quota-linearization, regional, and recovery decision | Grant owner/process-manager consistency, all-or-none ordering/token profile, whole-set recovery, fairness, partition/RPO/RTO |
...
| `0.143.0` | HA leases, grant-lineage authority, exact quota-set settlement, failover, and partitions | Split brain, pre-issuance revocation loss, successor fork, partial/mutated set, deadlock, unfair recovery, duplicate remote effect |
...
| `0.145.0` | Backup, restore, and disaster recovery | Grant revocation/supersession resurrection, partial quota-set reconstruction, atomic/rollup-proof integrity, disposition/authority loss |
| `0.146.0` | Performance, exact-set quota linearization, fair recovery, soak, and chaos certification | Overlap deadlock/livelock, partial reserve/failover/restore, token/member substitution, cross-kind drift, starvation, cascading failure |
...
- Documented single-node, HA, regional, backup/restore, rebuild, recovery,
  ...
  ownership, exact-set quota settlement/fair recovery capacity, and backpressure
  operations.
...
- Tenant and authorization matrices cover every API, UI, search, workflow,
  ...
  pre-issuance revocation, target substitution, and worker confused-deputy
  attempts.
...
- Atomic audit-intent crash tests, protected-read release receipts, exact
  ...
  quarantine, per-kind settlement, exactly-once refunds, write-off separation,
