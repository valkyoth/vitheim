...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain the transactional attempt-capacity ledger, original reservation set, reservation-free joins, lifecycle transfers and per-leg settlement; add domain-separated authenticated sparse settlement checkpoint/archive/head, exact duplicate/conflict/conservative-unavailable semantics, settlement-head lock order, bounded retention and exact-ID tombstones |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain every prior ledger-edge, join, stale-owner, transfer, wrong-leg and profile-change attack; add lost deletion response, retry after envelope removal and settlement compaction, binding substitution, checkpoint/head fork/rollback, archive/key/chunk loss, bounds, restore and migration |
| `0.23.0` | SQLite single-node adapter | Retain atomic reservation/create, fixed ordering, join, recheck, transfer and per-leg settlement proofs; add atomic envelope-delete/decrement/settlement/head advance, authenticated settlement compaction and archived duplicate replay across busy/crash/reopen or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove replay-head→settlement-head ordering, concurrent exact settlement replay/conflict, publication compaction and conservative unavailable-history behavior |
| `0.25.0` | Experimental MySQL adapter | Prove all prior settlement checkpoint/archive/head, bounds, exact-ID and conservative-release semantics plus attempt-ledger parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior settlement checkpoint/archive/head, bounds and exact-ID semantics under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific settlement checkpoint/archive/head, bounds and exact-ID semantics or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve greatest settlement head/predecessor/root/key/publication, hot rows, exact tombstones, cursor/budgets and original balances plus all prior ledger state; reject fork, rollback or lost proof |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger settlement archive/proof/bounds and exact duplicate/conflict semantics with conserved balances; otherwise retain charges, fence execution and refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze settlement checkpoint/archive/head, domain separator, replay-head→settlement-head lock order, publication/hot-row schema and all prior ledger fields |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Freeze bounded settlement hot/archive retention, key/tombstone lifecycle and archive-before-delete rule plus prior checkpoint/envelope/GC policy |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze settlement head/hot/publication/tombstone/cursor recovery and conservative missing-proof behavior plus prior balances; no fork, rollback or inferred release |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus authenticated sparse settlement replay, physical-delete/head atomicity, archived duplicate results, conservative failure and bounded compaction |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve settlement head/proof/publication/hot-row/tombstone provenance across RPC; peers cannot invent settlement, conflict, absence or release |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over settlement head/hot/archive/publication/tombstones/cursor and balances with prior ledger state; no duplicate decrement, head fork, rollback or false absence |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore greatest settlement head, hot rows, archive/key/publication/cursor/tombstones and balances; reject missing/forked/rolled-back proof and retain conservative charge |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; lose deletion responses, retry after envelope/settlement compaction, corrupt proof, saturate bounds, restore/migrate and prove one decrement or conservative charge |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit settlement archive domain separation, exact sparse semantics, archive-before-delete, bounded proofs/compaction and conservative failure plus prior ledger controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze settlement checkpoint/archive/head/proof/tombstone/error formats plus prior settlement records; old nodes cannot infer dense settlement or discard proof |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add post-delete duplicate decrement, settlement evidence compaction, substitution, head fork/rollback, archive/key/chunk loss, dense inference and unavailable-proof release; retest fixes |
