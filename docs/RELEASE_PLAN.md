...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain transactional original reservations, reservation-free joins, lifecycle transfers, per-leg exact-once settlement, distinct journal/archive heads, both CAS boundaries, captured-range/H-revalidation and bounded conservative replay; add one namespace for every checkpoint/deletion leg, atomic checkpoint decrement/rows/journal/attempt-checkpoint/audit/result and mixed-trigger archives |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain every prior attack; add checkpoint response loss, duplicate before/after compaction, checkpoint-versus-publication race, mixed-leg archives, trigger/leg substitution, migration between settlement stages and unavailable checkpoint-history charge |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve all prior two-head state plus checkpoint/deletion trigger kinds, attempt-checkpoint linkage, exact settled/remaining legs and balances; migration between checkpoint and deletion cannot reopen, pre-settle or reclassify legs |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require one unified checkpoint/deletion namespace, atomic checkpoint bundle, mixed-trigger archive proof and all prior two-head semantics; otherwise retain charges, fence execution and refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain both head/CAS/lock, captured-range, two-head-relationship and prior ledger schemas; freeze unified checkpoint/deletion row namespace, ordered multi-leg journal bundle, atomic attempt-checkpoint linkage, trigger kind and remaining-leg fields |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Retain prior two-head recovery; freeze attempt-checkpoint linkage, exact checkpoint/deletion settled bitmap, remaining legs and balances across the two settlement stages |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over atomic checkpoint bundles, mixed trigger rows, settled/remaining legs and every prior two-head field; no duplicate checkpoint/deletion decrement or stage drift |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore between checkpoint and deletion with exact bundle/linkage/settled/remaining legs and every prior two-head field; reject reopened or pre-settled legs and retain unverifiable charges |
...
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze checkpoint/deletion trigger, ordered bundle, attempt-checkpoint linkage, settled/remaining-leg and mixed-archive formats plus every prior two-head format |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add split checkpoint replay path, partial checkpoint bundle, duplicate checkpoint before/after compaction, mixed-leg alias, trigger substitution, between-stage restore drift and unavailable checkpoint-proof release; retest fixes |
