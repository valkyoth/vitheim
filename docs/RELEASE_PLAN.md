...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain prior controls; freeze closed single-active admission-attempt lifecycle, duplicate join/conflict, fenced takeover, irreversible terminals, atomic success, checkpoint-gated cleanup and bounded/reserved attempt capacity in addition to canonical key, cumulative budget and fairness |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain prior attacks; test duplicate join/conflict at every state, owner crash at every edge, stale-owner/takeover races, cancellation, terminal-to-success denial, premature cleanup, all attempt saturation dimensions and Recovery during Normal floods |
| `0.23.0` | SQLite single-node adapter | Persist lifecycle/CAS/lease/fence, single-active constraint, joins, counters and capacity reservations; prove atomic success and checkpoint cleanup survive busy/crash/reopen, or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove concurrent join, owner takeover fencing, terminal atomicity, bounded capacity and cleanup under pool retry/failover with no duplicate budget or stale-owner progress |
| `0.25.0` | Experimental MySQL adapter | Prove all prior attempt lifecycle/ownership/capacity/cleanup plus key, cumulative-budget, fairness and publication parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior attempt lifecycle/ownership/capacity/cleanup plus key, cumulative-budget and publication parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific attempt lifecycle/ownership/capacity/cleanup plus key, cumulative-budget and publication parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve attempt state/owner/boot/lease/fence/CAS, join key, counters/deadline, capacity reservations/backlogs/checkpoints/links plus all prior replay state; reject reset, duplicate active owner, terminal reversal or premature cleanup |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger attempt lifecycle, takeover fencing, capacity/cleanup and join semantics plus canonical-key/cumulative-budget/fairness parity; otherwise fence execution and refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain prior schema; freeze attempt lifecycle/owner/lease/fence/CAS, one-nonterminal-key index, atomic success, capacity/reservation/checkpoint/link schema plus canonical key, restart budget, guard and publication |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Freeze terminal-attempt retention/minimization, checkpoint/result/audit cleanup prerequisites and envelope-only deletion plus prior scheduling, Recovery, archive and GC policy |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze attempt owner takeover, lease/fence/CAS, lifecycle/capacity/backlog/checkpoint recovery plus prior key/budget/scheduler/publication state; no failover reset, dual owner or terminal reversal |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus closed admission-attempt owner, duplicate joining, fenced takeover, atomic success, irreversible terminals, bounded reserved capacity and checkpoint-gated cleanup |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve attempt state/owner/lease/fence/CAS, join identity, counters/deadline, reservations and checkpoint/link provenance across RPC; peers cannot duplicate budgets or revive terminals |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over attempt lifecycle/owner/lease/fence/counters/reservations/backlogs and prior replay state; no stale-owner progress, duplicate attempt/budget, terminal reversal, reset or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore attempt state/ownership/fence/CAS, counters/deadline, reservations/backlogs/checkpoints/links and all prior replay state; reject dual owner, reset, terminal reversal or cleanup loss |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; flood joins/conflicts, crash owners at every state, race takeover/stale owners, saturate every attempt bound, interrupt cleanup and prove Recovery under Normal floods |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit lifecycle closure, single-active joining, ownership fencing, atomic success, no-write finality, bounded reservations and checkpoint cleanup plus all prior replay controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze attempt state/owner/lease/fence/CAS, join, capacity, reservation, checkpoint/link and cleanup formats plus all prior replay schemas; old nodes cannot duplicate or revive attempts |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add attempt-population amplification, duplicate-budget allocation, stale-owner execution, takeover races, cancellation reset, terminal reversal, premature cleanup and Recovery starvation; retest fixes |
