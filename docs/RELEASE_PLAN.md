...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Declare VIT-INV-060-G and VIT-INV-061-F/G enforcement/test children and VIT-INV-061 mapping/budget/charge/disposition/checkpoint storage; freeze isolated ingress, two-stage charging and a closed disposition table with atomic awaiting creation, four irreversible terminal kinds, checkpointed original-kind preservation and fail-atomic ledger saturation |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Reject undeclared/duplicate enforcement/test children; exhaust every ingress and charge-ledger bound; destroy every stage boundary; exercise every disposition edge and reject unknown, cross-terminal, rollback, timeout-derived or pre-checkpoint compaction transitions; retain all mapping/replay/settlement cases |
| `0.23.0` | SQLite single-node adapter | Persist isolated ingress, mapping owner/state, atomic debit/evidence/sequence/awaiting, closed terminal/result links/checkpoints and atomic stage-two work; prove saturation, transition, crash-orphan and mapping-TOCTOU behavior or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage charge/consume, locked mapping recheck, closed disposition transitions, fail-atomic ledger saturation, checkpointed original-kind compaction and all existing controls |
| `0.25.0` | Experimental MySQL adapter | Prove isolated ingress, closed charge-disposition/saturation and two-stage TOCTOU parity plus existing behavior or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove isolated ingress, closed charge-disposition/saturation and two-stage TOCTOU bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific isolated ingress, closed charge-disposition/saturation and two-stage TOCTOU bounds or refuse VIT-CAP-060/061; retain isolation and capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve declared enforcement/test child coverage, ingress/mapping state, charge IDs/sequences/evidence/closed dispositions/result links/continuity/checkpoints/ledger bounds and original terminal kind; reject unknown/reversible states, refund/reuse, skipped recheck and existing weakening |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger enforcement/test coverage, isolated ingress, mapping ownership, atomic debit/evidence/awaiting creation, closed disposition/result-link/checkpoint, ledger-saturation and stage-two recheck semantics alongside existing controls |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze concrete VIT-INV-061 mapping/budget/charge/disposition/checkpoint rows, isolated ingress, atomic debit/evidence/awaiting transaction, closed irreversible terminal/result links, ledger-saturation behavior, stage-two recheck and capability refusal |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze failover/restore of ingress/mapping state, charge evidence/sequences/closed dispositions/result links/continuity/checkpoints/ledger bounds and all existing authority state |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement declared 060-G/061-F/G enforcement, VIT-INV-061 owned rows, isolated ingress, atomic stage-one awaiting creation, closed dispositions/result links/checkpoint compaction/saturation and stage-two recheck alongside existing controls |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve internal charge evidence and closed disposition/result-link/checkpoint semantics across authenticated RPC; expose distinct stage commands only internally, recheck mapping, and never let RPC timeout invent a transition |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover declared child coverage, ingress/mapping state and charge evidence/closed dispositions/result links/continuity/checkpoints/ledger bounds without unknown state, terminal rewrite, refund, reuse, skipped recheck or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Soak every ingress and charge-ledger bound; stage crashes; every valid/invalid disposition edge; evidence attacks; mapping races; failover/restore and cross-lane floods |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit enforcement/test declaration closure, ingress/mapping isolation, evidence confinement, closed disposition irreversibility, fail-atomic saturation, checkpointed compaction and commit-time recheck |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze child IDs, ingress/mapping profiles, charge evidence/sequence/closed disposition/result link/continuity/checkpoint/ledger-saturation and two-stage semantics alongside existing formats |
