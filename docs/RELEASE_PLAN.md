...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add generational high-watermark guard with immutable Committed/Aborted tombstones and atomic next-Unprepared rollover; Prepare-only single submission claim/permit; closed query attempt states/outcomes with exact-once concurrency settlement; trusted-time backoff; writer-sequence-derived query IDs; acyclic witness proposal; and paired archive counters. Commit atomically installs replay head, deletes captured rows, terminalizes `g` and opens `g+1`; negative Reconcile similarly rolls without identity reuse. Query admission and terminalization are separately charged, with authenticated evidence co-committing the disposition | Repeated archives resume ordinary writers, no retry resubmits, every query settles once, no extra counter is unproved, and no callback/cycle/rollback/split sentinel exists |
| `0.30.0` | Export/import guard generation/current binding/terminal chain, submission claim, proposal DAG, three core charges, query admission+terminalization charges/states/outcomes/concurrency settlements, trusted-time bindings/backoff/exhaustion, replay/watermark ancestry, publication lifecycle, rank evidence and pair sentinel | Import reads external watermark first, proves generation continuity and `3 + 2q - e`, preserves non-bearer claims/time continuity/query settlement, and rejects an older open guard, missing tombstone/settlement, identity reuse, cyclic state or asymmetric pair |
| `0.140.1` | Freeze guard-generation/state/disposition and submission-claim schemas; query state/outcome tags; proposal dependency directions; trusted-time/backoff preimages; query-ID derivation; checkpoint/replay/publication/disposition/pair widths and ArchiveFinalize definition | Review proves Witnessed cannot remain current after Commit, abort cannot reuse identity, durable claim is non-bearer, query counter is absent, unknown cannot advance disposition, and dependency cycles/ambiguous encodings are impossible |
| `0.140.2` | Freeze atomic Commit replay-install+captured-delete+Committed(`g`)+Unprepared(`g+1`) CAS and equivalent negative rollover; common-prefix/archive-suffix order; query admission→permit and Reconcile terminalization+concurrency settlement CAS; three core plus query charges | Backend faults expose all rollover/query-settlement writes or none. Traces preserve rank; `3 + 2q - e`, one submission/query permit, one outcome and one concurrency settlement hold with no stuck successful guard |
| `0.140.3` | Separate guard-generation owner, proposal builder/signature verifier, submission executor, disposition importer, query budget/admission executor, query outcome importer/concurrency settler, charge/head owner, publication/replay/GC owners, Replan and restore validators | No executor recreates a permit, importer resubmits, query worker settles itself twice or chooses trusted time, Commit omits rollover, planner resets generation/budget, or adapter mutates authority state |
| `0.140.5` | Retain every generation binding/tombstone/result, submission claim, proposal/receipt/result DAG, three core and query admission/terminalization charges, attempt states/outcomes/settlements, trusted-time/backoff evidence, lock traces, ancestry, lifecycle, pair fence and hot suffix | Retention cannot reopen an old guard, reuse identity, recreate bearer authority, leak concurrency, erase a charge/settlement/fence, turn timeout/exhaustion into negative evidence or detach replay state from its generation |
| `0.140.6` | Freeze failover at Prepare claim/permit, query permit/invocation/response/settlement, trusted-time continuity, Witnessed Commit rollover, negative rollover, response loss, proposal/signature/receipt, deletion, Replan and counter boundaries | Takeover returns status only, terminalizes each query once, exact-retries old generation results, keeps time rollback fenced, opens exactly one next guard after success/abort and never exposes replay installation with Witnessed still current |
| `0.142.0` | RPC exposes read-only current/terminal guard generations, replay/watermark binding, submission-claim status, query state/outcome/settlement/time/backoff/exhaustion, charge equation, proposal DAG and pair sentinel | RPC cannot issue/reconstruct a permit, roll a guard, reuse identity, import/settle outside Reconcile, choose time/counter/order, reset concurrency/budget or authorize from a terminal/stale generation |
| `0.143.0` | Model repeated Commit and negative rollovers; stale-generation writers; Prepare/submission uncertainty; all query states/outcomes; settlement/response races; trusted-time rollback/profile drift; no-query and `q`-query charge paths; dependency graph, compaction, Replan and exhaustion | Only continuous generational histories and exact `3 + 2q - e` accounting are reachable; successful archive never permanently blocks ordinary writers, every query terminates/settles once and no resubmission or extra counter appears |
| `0.145.0` | Restore/migrate by reading watermark first, proving guard-generation/tombstone/replay continuity, submission non-bearer status, proposal DAG, query states/outcomes/concurrency settlements/time evidence, exact charges/hot suffix and pair equations | Older open guard, missing terminal/next generation, split replay rollover, live concurrency without attempt, duplicated settlement, time rollback, identity reuse or missing/cyclic evidence is unavailable/corrupt—not repaired or reopened |
| `0.146.0` | Fuzz guard generations/tags/tombstones/bindings; submission/query claim material; query states/outcomes/settlements and sequence-derived IDs; trusted-time/backoff fields; charge equations; dependency graph/encodings; lock subsets and pair sentinel | Stuck Witnessed, split rollover, stale-generation write, permit reconstruction/resubmission, outcome substitution, concurrency leak/double release, invented counter, time rollback, charge mismatch, cycle and pair asymmetry fail closed |
| `0.147.0` | Audit generational liveness/anti-rollback, single submission, closed query lifecycle/concurrency conservation, writer-derived IDs, trusted-time enforcement, exact charge conservation, commitment DAG, lock order, publication, pair sentinel, Replan, HA and restore | Zero high/critical stuck-guard, replay/rollover atomicity, resend, query lifecycle, concurrency, time, counter, dependency, charge, lock, rollback, sequence or result-integrity finding |
| `0.148.0` | Reject legacy imports lacking guard generation/tombstones/next binding, submission claim, closed query state/outcome/settlement, trusted-time backoff, writer-derived query identity, exact charge evidence, proposal DAG, expanded rank, independent watermark or pair sentinel | No shim opens a Witnessed guard, invents generation/settlement/time/counter, reconstructs a permit/result, trusts callback state, guesses query outcome, treats GC as ArchiveFinalize or selects older history |
| `0.149.0` | External pentest targets stuck Witnessed guard, split Commit/abort rollover, stale generation, replay rollback, submission/query permit replay, witness resubmission, outcome substitution, concurrency leak/double settlement, time rollback/backoff bypass, identity/counter confusion, charge mismatch, dependency cycles, equivocation and asymmetric restore | Zero unresolved critical/high findings and no waived generational-lifecycle, single-send, query-settlement/concurrency, trusted-time, exact-charge, commitment-DAG, locking, anti-rollback or pair-conservation issue |
| `0.150.0` and `1.0.0` | Demonstrate continuous guard generations, repeatable archives, Prepare-only submission, deterministic closed bounded query recovery, exact `3 + 2q - e` conservation, trusted-time enforcement, acyclic commitments, closed publication, rollback safety and atomic pair conservation across every backend/RPC/HA/Replan/import/restore path | Production remains blocked until the unchanged candidate passes guard-liveness, submission/query permit, settlement/concurrency/time, dependency, acquisition, charge, witness, rollback, boundary, fault/model and independent-pentest campaigns |
