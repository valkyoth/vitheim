...
| `0.18.3` | Declared ownership roots including global lineage, exact local placement owners, serialized durable rollout root/law, activation-floor catalogs, shared verifier, and closed semantic registry | `VIT-INV-057/058/059` and `VIT-LAW-007/008@g01`; compiled immutable singleton topology; typed attested/fenced workload identity and authenticated-receipt contract; one active rollout generation; terminal `Superseded`; `AllRequired`; no distributed transaction; cryptographic artifact/time/ancestry gates; P/N/M/F |
| `0.18.4` | Source-delivery-complete evaluator re-evaluation plus first real catalog successor rollout | Actual predecessor verification and crash at every candidate/prepare/global-activate/converge/complete/block/revoke/abandon/supersede boundary; two prepared candidates with lost CAS responses/restart and one winner; static-topology mutation rejection; cloned/stale identity; forged receipt digest; unreachable placement; revocation reconciliation |
...
| `0.19.0` | Integrity chains and signed-checkpoint interface | Event tamper/domain separation plus verified catalog envelope, immutable rollout/placement manifest, prepare/global/convergence/revocation receipts, exact local owner identity/fence, global activation receipt, and local catalog/distrust/time ratchets |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Distinct `VIT-CAP-057/058/059`; exact local owner key/fence; rollout state/manifest/message/receipt/deadline support; planning/active separation; exact envelope/verifier; closed realization |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Every tuple including `VIT-LAW-007/008`; cloned identity, mutable topology, illegal state, missing/contradictory receipt, unfenced quorum, topology race, revocation lag, time rollback, artifact/ancestry/predecessor/scope/signature mutation, and untrusted rejection |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve separate global/rollout/local owners, immutable rollout/placement generations, messages/receipts/deadlines, exact local identity/fences, payload/envelope/revocation state, and anti-resurrection |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/receipt authentication, and credential-operation decision | Freeze verifier/trust roots plus `HardwareAttestedKey` or `OrchestratorAttestedFencedLease`, issuer/key/attestation/lease lifecycle, authenticated canonical receipts, scope/validity/time uncertainty, rollback/suspend, and credential profiles |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology state, active catalogs/ratchets, and provider state | Separate future `VIT-INV-060` topology plus `VIT-INV-057/058/059` placement; expected-version manifest/fence/tombstone rows; rollout active generation/state/messages/authenticated receipts; local identity and catalog/time ratchets without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology, and recovery decision | Select `AllRequired` or prove fully fenced quorum; independent topology owner; serialized competing rollout recovery; authenticated prepare/activate/converge/revoke races; attested identity clone detection; four-owner failover; revocation convergence; time loss; typed RPO/RTO |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus governed topology-authority handoff | Introduce `VIT-INV-060`, hand off the compiled singleton by expected-version CAS, persist generation/fence/tombstone state, activate `VIT-LAW-008@g02`, and retain existing authority/floor startup ratchets |
| `0.142.0` | Split deployments with governed credential-operation TCB and exact topology-issued catalog placement owners | Typed topology successor allocation; attested/fenced workload identity lifecycle; authenticated prepare/convergence routing; replacement/clone fencing; existing evaluator/remediation routing and incompatible executor rejection |
...
| `0.143.0` | HA atomic work, catalog rollout/topology, governed execution, and cancellation recovery | Two-candidate winner/superseded recovery; independent topology-command races; authenticated manifest/prepare/global-CAS/convergence/revocation; cloned keys/leases; rollout/global/topology/local failover; existing evaluator/remediation cases |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover topology generations/fences/tombstones, rollout active generation/messages/authenticated receipts, global lineage, and exact local identity/catalog/distrust/time ratchets separately; reject cloned authority, late losers, invented completion, topology resurrection, expiry extension, and untrusted catalogs |
