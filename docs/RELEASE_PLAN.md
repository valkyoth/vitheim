...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Activation commits Pending plus immutable zero-counter platform-capped lineage, cryptographic NoHistory zero-eligibility proof, or a custody-governed `MigrationImportRegistryHistoryNotRequestedRecordV1`, under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation/lineage-disposition→retention/legal-hold→audit/result/outbox. Active hold rejects custody-weakening NotRequested. Initial append locks lineage before attempt budget and atomically co-charges both. Missing nonterminal lineage durably installs an exact-obligation corruption fence that blocks append/recovery/cleanup and clears only through independently authorized complete atomic restoration; unprovable lineage stays fenced. Recovery authorization/result enum tags remain sole action authority. Every authorization operation returns one closed conflict envelope; CAS losers reread/reapply the total table, valid different operations are not changed material, and consumption/expiry/revocation have one first terminal winner |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve distinct NoHistory proof and NotRequested custody record, activation-created lineage, exact initial co-charges, corruption fence/evidence/clearance state, closed conflict envelope and deterministic CAS race semantics across export/import. Missing/defaulted lineage is fenced, never reconstructed; incomplete restoration remains permanently fenced. Retain the complete canonical two-head tuple, live destination coordinator exclusion, inert history, exact revocation state, immutable counters, custody evidence and activation atomicity. No cross-backend step creates capacity, widens a fence beyond its obligation, clears it from inferred counters, guesses expiry tombstones, invents result unions, duplicates action authority, infers remote revocation, weakens custody or equates NotRequested with NoHistory |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze complete recovery revocation signer/key/profile/time fields, exact authorization-scoped sequence, issuer continuity, Issued-only trusted-time expiry, one closed operation outcome and one closed Authorization/Revocation/Recovery/LineageCorrupt conflict envelope. Revocation derives action solely from authenticated authorization bytes; only destination-local commits change authority. Freeze CAS-loser reread/reapply, first-terminal-winner and exact changed-material rules |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist distinct NoHistory zero-eligibility proof and full NotRequested custody record; activation-created lineage and atomic initial charges; exact-obligation corruption fence, evidence/result and atomic restoration/clearance; sole-tag authorization/result bytes, total row, closed outcomes/conflicts and deterministic CAS semantics. Freeze retention/legal-hold activation locks, no lazy lineage, no partial/inferred repair, Issued-only expiry, NotAdmitted no-write, platform cap/no amendment and fixed lock/CAS prefixes; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact migration/bootstrap roles; independent history-recovery roles; compliance/legal SoD for NotRequested; and separately governed corruption restoration. Neither detector, coordinator, activation/import actor, archive worker nor affected domain actor may unilaterally choose custody weakening, authorize its own recovery, or clear its own fence |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Freeze NotRequested under the same custody precedence as waiver/abandonment, with eligible-history commitment, current policy/classification/legal-hold inputs, evidence floor, provenance, approval SoD and activation/audit linkage. Preserve corruption-fence evidence until exact clearance. Active hold cannot be weakened by NotRequested, waiver/abandonment, emergency flags, erasure or cleanup |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve distinct NoHistory/NotRequested evidence, activation-created lineage, both charge legs, exact corruption fence and restoration clearance, sole-tag bytes/indexes, closed outcomes/conflicts and CAS race semantics beside every recovery artifact. Failover/restore cannot invent lineage, infer custody, widen/bypass/partially clear a fence, split charges, guess an expiry tombstone, misclassify operations, reset counters, infer revocation or clean early |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries exact sole-tag bytes, closed outcome/conflict envelope, distinct NoHistory/NotRequested evidence, activation-created lineage, both initial charges and obligation-scoped corruption state, but never a second action, remote effect or remote fence clearance. Peers cannot invent lineage/custody, widen or bypass a fence, guess an expiry tombstone, substitute an outcome/index, reset counters, change action, bypass a hold or erase evidence |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Race activation custody decisions, lineage creation, initial dual-budget charge, corruption fencing/restoration, append, failover and cleanup; then race admission/consumption/expiry/revocation/recovery. Require distinct complete evidence, obligation-only fences, indivisible charges, CAS-loser reread/reapply, one first terminal winner, one sole-tag action and canonical outcome/conflict without absent writes |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore exact lineage or distinct NoHistory/NotRequested evidence, both charges, corruption fence/evidence/clearance, sole-tag bytes/indexes and closed outcomes/conflicts before work. Older backups cannot invent lineage/custody/capacity, widen or partially clear a fence, guess expiry state, alter races/action/outcome or weaken custody |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault activation custody/dual-charge/fence atomicity; fuzz missing/lazy lineage, false zero eligibility, incomplete restoration, fence-scope widening, CAS losses, operation-kind races, action mismatches, absent expiry/consumption and adapter-union variants. Evidence, fences, lineage and outcomes stay canonical |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit NotRequested custody and NoHistory proof provenance, corruption fence/clearance lineage, dual-charge conservation, sole action-tag derivation, CAS loser/terminal winner, NotAdmitted no-write and closed outcome/conflict provenance beside prior controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze distinct NoHistory/NotRequested encodings, corruption fence/restoration state, atomic charge representation, sole-tag authorization/result/revocation targeting, derived indexes, closed outcomes/conflicts and race semantics. Old/defaulted/duplicate/partial/inferred encodings fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack custody-free NotRequested, false NoHistory, lazy/late lineage, fence widening/bypass/partial clearance, split charges, guessed expiry DoS, operation-race confusion, outer/payload contradiction, index corruption, restore reconstruction, counter increase and remote effect; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove clean install/upgrade/restore/failover continuity for distinct custody evidence, activation-created lineage, exact corruption fence/clearance, dual charges, sole-tag bytes/indexes, deterministic authorization races, closed outcomes/conflicts and every prior recovery artifact |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  attempt/cumulative initial charges. They prove exact-obligation corruption
  fencing on missing lineage, denial of append/recovery/cleanup while fenced,
  and clearance only by independently authorized complete atomic restoration;
  incomplete or inferred lineage remains permanently fenced. Every transition
  in the total six-state table, Issued-only expiry, typed no-write NotAdmitted,
  one closed operation outcome and conflict envelope, sole-tag action authority
  ...
  remain mandatory. Custody-free NotRequested, false NoHistory, missing/lazy
  lineage, widened or bypassed fences, partial/inferred clearance, split
  charges, guessed expiry tombstones, adapter-specific outcomes/conflicts,
