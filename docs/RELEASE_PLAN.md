...
| `0.18.3` | Declared ownership roots including global lineage, exact local placement owners, serialized rollout root/law, activation-floor catalogs, shared verifier, and closed semantic registry | Define external action-claim issuer/local atomic consumption split with typed uncertainty; authenticate authorization and global-result receipts; enforce sender-generate/receiver-verify MAC roles; retain irreversible authorization, static topology, `Superseded`, `AllRequired`, ancestry, and P/N/M/F |
| `0.18.4` | Source-delivery-complete evaluator re-evaluation plus first real catalog successor rollout | Crash/race both authenticated control receipts and action-claim issue/consume boundaries; reject receiver MAC forgery, replay/substitution, digest-mismatched claim reuse, uncertain reissue, and existing authorization/topology/identity failures |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Persist both control receipts/replay tombstones, action-claim issuer/local consumption split and uncertainty, authorization bundle/pin, future uninitialized/dormant/committed handoff, exact envelope/verifier, and closed realization |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Destructively test control-receipt authentication/MAC roles, action-claim atomicity/replay/uncertainty/restore, topology initialization order, authorization races, clone, topology, time, ancestry, scope, signature, and untrusted rejection |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve separate owners, both authenticated control receipts/tombstones, action-claim issuer sequence/local consumption/outcome/uncertainty, authorization/outbox/pin, exact identities/fences, envelope/revocation, and anti-resurrection |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/receipt authentication, and credential-operation decision | Freeze action-claim issuer/idempotency/sequence/atomic consumption/uncertainty/restore and maximum lifetime; freeze both control receipt schemas, closed authenticators, sender-only MAC generation, receiver-only verification, and durable anchors |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology state, active catalogs/ratchets, and provider state | Place control receipts/replay tombstones and action-claim issuer/local consumption evidence separately; persist authorization bundle/pin and future uninitialized/dormant/committed topology handoff without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology, and recovery decision | Race both control receipts and action-claim issuance/consumption uncertainty through failover/restore; prove MAC-role separation, no reissue, topology order, fully fenced activation policy, four-owner recovery, revocation, time loss, and RPO/RTO |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus governed topology-authority handoff | Keep topology uninitialized; activate/converge epoch 12 under generation 1; after local generation-2 admission initialize/verify exact dormant singleton and commit; prove no pre-activation generation-2 command and exclusive authority |
| `0.142.0` | Split deployments with governed credential-operation TCB and exact topology-issued catalog placement owners | Typed topology successors; external action-claim issuer plus atomic local consumption/uncertainty; authenticated authorization/result routing; MAC-role separation; exact receipt anchors; replacement/clone fencing |
...
| `0.143.0` | HA atomic work, catalog rollout/topology, governed execution, and cancellation recovery | Lose/replay/substitute both control receipts; lose claim issuance/consumption responses and reject uncertain reissue; retain pinned winner/pre-authorization loser, topology ordering, MAC roles, and four-owner failover |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover uninitialized/dormant/committed order, both control receipts/tombstones, action-claim issuer/local high-watermarks/outcomes/uncertainty, authorization bundle/pin, exact owners/ratchets; reject inferred transition, reissue, forged MAC, clone, or resurrection |
