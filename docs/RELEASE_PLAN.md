...
| `0.18.3` | Declared ownership roots including global lineage, exact local placement owners, durable rollout root/law, activation-floor catalogs, shared verifier, and closed semantic registry | `VIT-INV-057/058/059` and `VIT-LAW-007/008`; canonical non-clonable local owner key; immutable placement manifest; closed rollout states; transactional messages/receipts; `AllRequired`; no distributed transaction; planning isolation; cryptographic artifact/time/ancestry gates; P/N/M/F |
| `0.18.4` | Source-delivery-complete evaluator re-evaluation plus first real catalog successor rollout | Actual predecessor verification and crash at every candidate/prepare/global-activate/converge/complete/block/revoke/abandon boundary; join/leave/replace/move/topology races; cloned/stale identity; unreachable placement; revocation reconciliation; existing evaluator campaign barriers |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, exact catalog/receipt profile, and credential-operation decision | Freeze verifier/trust roots plus cryptographic receipt digests, non-clonable workload/continuity identity evidence, scope/validity/time uncertainty, rollback/suspend, and credential profiles |
| `0.140.2` | Storage topology for invariant/law manifests, rollout state, active catalogs/ratchets, and provider state | Separate `VIT-INV-057/058/059` placement; immutable topology manifest, process state, outbox/inbox, receipts/deadlines, local identity/fence and catalog/time ratchets without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout, and recovery decision | Select `AllRequired` or prove fully fenced quorum; catalog prepare/activate/converge/revoke races; topology/placement changes; cloned/stale owner fencing; three-owner failover; revocation convergence; time loss; typed RPO/RTO |
...
| `0.142.0` | Split deployments with governed credential-operation TCB and exact catalog placement owners | Catalog owner-key allocation, service-role/partition generation, workload/boot identity, prepare/convergence routing, replacement fencing; existing evaluator/remediation routing and incompatible executor rejection |
...
| `0.143.0` | HA atomic work, catalog rollout, governed execution, and cancellation recovery | Catalog manifest/prepare/global-CAS/convergence/revocation/topology races, contradictory receipts, cloned/stale identities, rollout/global/local failover; existing evaluator and remediation cases |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover rollout root/messages/receipts plus global lineage separately from exact local identity/catalog/distrust/time ratchets; replacement needs fresh placement generation and receipt; reject cloned authority, invented completion, expiry extension, rollback, and untrusted catalogs |
