...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze independent non-borrowable ingress resources/global ceiling, server-controlled ingress routing without authority, VIT-INV-061 mapping ownership/emergency-promotion SoD, and the two-stage presentation charge/consume protocol with bound evidence, commit-time mapping recheck, orphan-spent semantics and checkpointed disposition; retain request/admission/outstanding, denial, issuance/settlement and terminal/reconciliation controls |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Exhaust every ingress resource across lanes; destroy every stage-one/stage-two commit boundary; forge/export/reuse evidence; crash after debit; race mapping promotion/rotation/revocation; fail over/restore continuity; retain replay/concurrency/response-loss/saturation/conflict, one-request-charge, terminal-only settlement, bounded denial and reservation/range tests |
| `0.23.0` | SQLite single-node adapter | Persist isolated ingress profiles, mapping owner/generation/fence, stage-one charge evidence/disposition/continuity/checkpoint and atomic stage-two request/outcome/issuance; prove crash-orphan and mapping-TOCTOU behavior or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage charge/consume, locked mapping recheck, isolated ingress resources, evidence compaction and all existing terminal/reconciliation, deadline-CAS, sparse/range and reserve semantics |
| `0.25.0` | Experimental MySQL adapter | Prove isolated ingress and two-stage evidence/TOCTOU parity plus denied-history, settlement/range behavior or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove isolated ingress and two-stage evidence/TOCTOU bounds under transactions/failover plus existing controls or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific isolated ingress and two-stage evidence/TOCTOU bounds plus existing controls or refuse VIT-CAP-060/061; namespace/graph/query isolation and capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve ingress resource profiles/global ceiling, sole mapping owner/SoD and generation/fence/revocation high-watermarks, presentation-charge identities/sequences/bindings/dispositions/continuity/checkpoints, request charges/sequences/checkpoints and all existing terminal/claim/settlement state; reject stale evidence, refund/reuse, skipped recheck and existing weakening |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger isolated ingress resources, mapping ownership/SoD, two-stage charge/consume atomicity, evidence/continuity/checkpoint and commit-time recheck semantics alongside all existing denial, terminal, issuance and settlement controls |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Freeze concrete isolated ingress resource partitions/global ceiling and the two-stage charge/evidence/consume transaction layout, locked mapping recheck, orphan-spent disposition and capability refusal alongside existing request/issuance/settlement state |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Minimize denial/issuance and presentation-charge checkpoints to domain-separated commitments; retain debit/disposition proof without permanent reusable evidence or plaintext request/principal data, while erased evidence never refunds work or makes history fresh |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze failover/restore of isolated ingress partitions/global ceiling, greatest mapping state/owner continuity, charge evidence/sequences/dispositions/checkpoints, request state, terminal/reconciliation roles, issuance, claims, reservations and settlement |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement isolated ingress resources/global ceiling, VIT-INV-061 mapping ownership/SoD, stage-one committed charge evidence, stage-two consume/recheck/request transaction, orphan-spent and bounded checkpoint behavior alongside existing controls |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve ingress partition and mapping identity plus internal non-exportable charge evidence across the service boundary; expose separate stage commands only on authenticated internal RPC, recheck mapping at stage two, and retain existing terminal-only settlement rules |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over isolated ingress resources, mapping owner/high-watermarks, charge evidence/dispositions/continuity/checkpoints and stage boundary without refund, reuse, skipped recheck, stale admission or rollback |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover ingress partitions/global ceiling, greatest mapping owner/generation/fence state and charge evidence/disposition/continuity checkpoints without stale admission, refund, reuse, forgery, skipped recheck or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Soak every isolated ingress resource; stage-boundary crashes; evidence forgery/replay; mapping promotion/rotation/revocation races; failover/restore and cross-lane floods alongside existing controls |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit ingress resource isolation/global ceiling, mapping ownership/SoD, internal evidence confinement, non-refundable stage boundary, commit-time recheck and all existing topology replay/settlement controls |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze ingress resource profiles, mapping owner/generation/fence/profile bindings, charge evidence/sequence/disposition/continuity/checkpoint, two-stage transaction semantics and all existing request/terminal/reservation formats |
...
- Production topology authorization proves independently provisioned,
  ...
  non-refundable presentation debit and bounded internal evidence before any
  protected lookup. Stage two consumes that evidence, rechecks the mapping
