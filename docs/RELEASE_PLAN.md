...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Implement pinned genesis trust; named observation-registry authority port/profile; total OperationalClaimsOpen→ObservationDrainPending→QueryPermitMayIssue→ObservationBlocked/FenceAnchorPending→absorbing AuthorityFenced plus RestoreActivationPending/RestoreUnready machine; `OperationalAuthorityClaimCapacityV1` with per-kind/scope `live + outcome_unknown <= c_max`; PreparedNonAuthoritative mutation, registry Confirm/Reject, LocalFinalizeCommitted/VersionConflict/Unavailable/OutcomeUnknown and signed receipt/status settlement; exact-set claim checkpoint/archive replay; provider redemption; complete observation response-loss lanes; monotone evidence lattice; sealed frontier; disjoint `C/H` and `E/J`; expanded `B_EM`/`R_first` | Crash/race every state edge, claim boundary, prepare/confirm/finalize/receipt, history Stage/Verify/Commit, effect and observation lane. Test `c_max−1/c_max/c_max+1`, hostile tenants and stale local restore. Reject authority-state reopening, registry-only settlement, unsafe compaction, distributed CAS, check/use, transient-as-permanent, weak adapters, TOFU, substitution and underfunding |
| `0.30.0` | Export/import root state plus registry conformance identity; complete authority state/predicates; claim capacity/sublimits/reservations; prepared mutation, registry disposition, local-finalization disposition/receipt/settlement; claim checkpoint/archive replay/historical results; observation attempts and restore protocol; `E/J` and maintenance ledger | Round trip every cut. Missing/forked/rolled-back/defaulted blocked/fenced state, capacity proof, claim, finalization receipt, checkpoint/replay member, provider receipt or conformance identity remains RestoreUnready; import invents no settlement, authority, permanence or capacity |
| `0.140.1` | Freeze encodings, domains and result/conflict surfaces for all authority states/edges, claim-capacity sublimits, aggregate sequencing, local-finalization receipt/status, claim checkpoint/archive/history-unavailable, response-loss lanes, evidence lattice, restore, genesis, `E/J` and equations | Review rejects an unlisted/reopening state edge, external/local atomicity, current-status proof, generic alias, admission above drain capacity, confirmation-as-settlement, missing history/late-evidence edge, stale redemption, unknown field and cyclic commitment |
| `0.140.2` | Freeze external claim acquisition, capacity/reservation CAS, overlapping aggregate/version exclusion, local prepare, external disposition, exact local finalize, finalization-receipt publish/reconcile/settle; zero-lock-across-I/O; authority transitions; effect redemption; observation/status/seal; claim-history Stage/Verify/Commit; `E→J→F`; restore | Fault every boundary. No Prepared/Rejected/confirmed-unsettled state is authoritative or released, no blocked/fenced state admits, no drain outruns live claims, stale effect never sends, compaction preserves referenced members, and no backend requires a distributed transaction |
| `0.140.3` | Separate registry operator/state-transition authority, claim/capacity issuer, local preparer/finalizer/receipt signer, receipt settler, provider executor, claim checkpointer/archiver, observation/seal/fence/restore/genesis roles and capacity auditor | No actor reopens AuthorityFenced, self-confirms and settles its mutation, exceeds `c_max`, manufactures no-commit evidence, bypasses provider fence, suppresses late evidence, self-certifies conformance or erases claim/evidence history |
| `0.140.5` | Retain conformance identity, authority transition history, capacity/reservations, claim/disposition/finalization receipt frontier, exact-set claim checkpoints/replay/historical results, observations, root history, `E/J/F` ancestry and consumption | Compaction cannot lose live/redeemed/unknown/confirmed-unsettled/frontier-referenced claims, infer settlement, reopen a blocked/fenced state, turn prepared into committed, forge completeness, alias archives, reset limits or weaken custody |
| `0.140.6` | Freeze failover at every authority edge, capacity admission/drain/expiry, prepare/confirm/reject/finalize/final-receipt settlement, provider redemption, claim checkpoint/archive replay, observation, local fence/`E/J/F`, restore and budget boundary | Takeover uses status/Reconcile only, recreates no permit/claim/token, preserves exact state/counters/results/history and recovery-priority fairness, and refuses authority when registry/provider/archive/conformance continuity is unavailable |
| `0.140.11` | Decide pinned root; concrete registry provider/finality/failure domain/keys; authority-state realization; `c_max` and command/effect/queue/projection plus tenant/deployment/global ceilings; local-finalization receipt authority; active/terminal row-byte limits, checkpoint cadence, archive/shard/region and latency/throughput SLOs; local DB/provider fence/FenceEvidence/custody/export profiles. Keep replenishment/manual root recovery unsupported | Architecture/security/legal/forensic/operations approve one exact end-to-end profile and capacity proof. Any local backend, registry, archive or provider unable to prove state absorption, settlement, retention, performance and failure-domain guarantees refuses recovery activation |
| `0.142.0` | Expose read-only conformance identity, external authority state/reason, capacity/sublimits/utilization, claim/preparation/disposition/finalization settlement, claim checkpoint/archive/replay/history availability, observation lattice, restore, genesis, `E/J/F` and budget | RPC cannot transition/reopen authority, acquire/redeem/release claims, confirm/settle preparations, forge no-commit evidence, compact referenced history, seal permanence, mint permits or replenish capacity |
| `0.143.0` | Model the total authority machine, `c_max` capacity/fairness, aggregate sequencing, local prepare/registry confirm/local-finalization receipt settlement, provider redemption, claim history/compaction, all response-loss lanes, late contradiction, restore, root lifecycle, `C/H` versus `E/J` and conservation | No reachable state reopens AuthorityFenced, admits while blocked, exceeds drain capacity, settles an unknown local outcome, loses referenced history, commits from stale state, sends after fence, revives authority or violates conservation |
| `0.145.0` | Restore conformance identity, greatest external authority state, capacity/claim frontier, preparation/disposition/finalization pairs, claim checkpoint/archive replay plus hot suffix, complete observation state, anchors, `E/J/F` and budgets; then run confirmed local finalize and receipt settlement | Older backups, ObservationBlocked/FenceAnchorPending/AuthorityFenced/RestoreUnready, live/unknown/confirmed-unsettled claims, missing history/provider evidence or rejected/conflicted preparation remain Unready/FencedEvidenceVerified and issue no authority |
| `0.146.0` | Load/soak the registry on every authority path across selected sharding and regional-failure profiles; measure added registry round trips and three local mutation stages; fuzz state/capacity/aggregate/finalization/checkpoint/replay IDs, response loss, evidence transitions, archives and equations | Selected throughput/tail-latency/backpressure/fairness SLOs pass at row/byte and `c_max` limits. Tenant/profile/receipt substitution, ABA/wrap, stale redemption, status alias, unsafe compaction, overflow and one-short escrow fail closed |
| `0.147.0` | Audit TCB state absorption, claim-capacity/drain/provider enforcement, aggregate sequencing and finalization evidence, claim-history lifecycle, preparation boundary, observation semantics, trust/SoD, custody and DR | Zero high/critical state-reopening, drain-starvation, registry-only-settlement, unsafe-compaction, distributed-atomicity, check/use, stale-send, evidence-loss, adapter, accounting, trust or role finding |
| `0.148.0` | Reject imports lacking conformance identity, explicit blocked/fenced/unready state, capacity/reservations, claim/disposition/prepared/finalization history, checkpoint/archive replay and historical lookup, observation response/lattice, dedicated `E/J` or immutable budget | No shim uses local Healthy or epoch reads as authority, maps confirmed to settled, defaults capacity/history/transient/permanent state, drops late evidence, aliases status/archive or infers escrow |
| `0.149.0` | Pentest blocked/fenced-state reopening, `c_max` starvation and hostile tenants, overlapping expected-version claims, lost/forged finalization receipts, unsafe claim compaction/history rollback, partitions/stale leaders/failover/epoch races, stale send, seal/late-evidence/root/archive/funding attacks | Zero unresolved critical/high and no waived authority-absorption, complete-drain, local-finalization settlement, history-lifecycle, port-conformance, provider-fence, response-loss, evidence, anti-rollback, trust, archive or custody issue |
| `0.150.0` and `1.0.0` | Demonstrate an unchanged candidate across every selected database, registry, claim-history archive, executor/provider fence, signer, sharding/region, HA and restore profile, including state-transition, `c_max`, finalization-response-loss, compaction/restore, local-fence-before-anchor and hard-exhaustion cases | Production remains blocked until Phase C, cross-adapter conformance, performance/soak, DR, independent audit and pentest pass with no high/critical issue; unsupported or over-capacity combinations remain explicitly refused |
