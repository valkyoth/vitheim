...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Freeze non-borrowable per-lane durable charge capacity and an immutable VIT-INV-061 capacity-profile lineage with stable identity/generation/digest/predecessor, expected-version activation, drain-before-shrink, governed emergency/aggregate changes, physical provisioning evidence and downgrade refusal |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Stall/fill lane resources and race shrink with stage-one reservation, stage-two terminalization and compaction; interrupt activation, fail over generations, restore older larger limits, reject insufficient provisioning evidence and attempts to take Recovery capacity |
| `0.23.0` | SQLite single-node adapter | Persist capacity-profile lineage/activation/drain/evidence and lane obligations; prove exact-generation restore, safe shrink and physical-capacity bounds or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Prove concurrent two-stage charge/consume plus capacity-profile CAS, shrink/drain races, non-borrowable ledger maintenance, physical provisioning evidence and aggregate bounds |
| `0.25.0` | Experimental MySQL adapter | Prove capacity-profile lineage, safe shrink, exact restore and lane-local lifecycle parity plus existing controls or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove capacity-profile lineage, safe shrink, exact restore and lane-local lifecycle bounds under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific capacity-profile lineage, safe shrink, exact restore and lane-local lifecycle bounds or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve capacity-profile lineage/generation/digest/state/provisioning evidence, lane reservations and pending drain; reject numeric-ceiling merge, older-writer mutation, obligation movement or rollback |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger profile-lineage activation, physical-evidence, safe-shrink, exact-generation restore and lane-isolation semantics; otherwise refuse import/profile |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Freeze greatest-authenticated capacity-profile generation/digest recovery, usage/reservation reconstruction, pending-drain continuity and explicit prohibition on maximum-numeric-ceiling merge |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement immutable capacity-profile lineage/activation/drain/provisioning evidence, per-lane obligation accounting, safe shrink and exact-generation recovery alongside the two-stage lifecycle |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve profile generation/digest/state, activation CAS, provisioning proof and lane obligations across authenticated RPC; timeout cannot activate, transfer capacity or cancel reservations |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over capacity-profile activation/drain and charge lifecycle by greatest authenticated generation/digest without merging numeric ceilings, moving obligations or starving Recovery |
...
| `0.145.0` | Backup, restore, and disaster recovery | Select greatest authenticated capacity-profile generation/digest, reconstruct obligations, retain pending drain, and reject older-larger ceiling revival or downgrade writers |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Race shrink/activation with both stages and compaction; fail over/restore generation conflicts; attack provisioning evidence and Recovery-capacity reassignment under load |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit profile owner/approval/SoD, immutable lineage, activation CAS, safe-shrink predicate, provisioning evidence, exact recovery and downgrade fencing |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze capacity-profile identity/generation/digest/predecessor/state/evidence and obligation formats; mixed versions cannot mutate old profiles, merge ceilings or reinterpret drain |
| `0.149.0` | Release candidate and external pentest remediation | External testers attack profile governance, shrink races, interrupted activation, false physical evidence, generation-skew failover, older-larger restore and cross-lane transfer; retest every fix |
