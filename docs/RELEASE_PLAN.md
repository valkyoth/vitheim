...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Preserve pinned genesis, AuthorityFenced, `R_first`, `B_EM`, `c_max`, PreparedNonAuthoritative, DomainAggregateTarget/ControlOwnerTarget/CanonicalNoAggregateTarget, ProposedEventDescriptor, ClaimAcquisitionPending through typed LocalFinalize and post-commit attestation/signing, safe orphan/archive and every prior guarantee. Add closed ProposedEventPayload lifecycle with no classified plaintext outside Phase A storage; target predecessor plus authoritative recorded-time/EventCommitChain mapping; separately unique command-ID and idempotency-ID claims with HistoricalIdentityConflict; CommitReceiptSignerRecoveryRequired plus pre-funded threshold/redundant or root-authorized successor recovery; extend `R_claim` with payload/chain and non-borrowable `S_claim` | Canary every storage/request/status/log/archive surface; race payload erasure/hold/export, stream head/time/chain, both identity indexes across hot/archive and signer loss/distrust/recovery. No prior test is removed; no plaintext duplication, guessed chain, one-sided identity reuse, changed commit meaning or unfunded recovery is accepted |
| `0.30.0` | Export/import canonical target/predecessor, descriptor and payload-lifecycle/tombstone roots, recorded-time policy/actual interval and complete commit chain, independent command/idempotency identity membership/conflicts, signer-recovery profile/state/authorization/capacity, attestation and every prior acquisition/proposal/signature/archive state | Round trip every cut without moving classified data across residency/hold boundaries. Missing/forked/defaulted payload custody, chain, identity claim, recovery evidence or prior state is RestoreUnready; import never recreates plaintext, frees an ID, invents a successor or rewrites an event/result |
| `0.140.1` | Freeze target predecessor, ProposedEventDescriptor/ProposedEventPayload variants and lifecycle bindings, recorded-time policy/chain algorithm and commit mapping, separate identity-claim/conflict schemas, CommitReceiptSignerRecovery profile/state/evidence plus all prior encodings | Review rejects inline classified data, incomplete key/residency/hold fields, caller-selected commit time/chain, composite-only identity, ambiguous recovery state, mutable commit meaning and every prior unsafe alias |
| `0.140.2` | Freeze exact-reference reuse, co-located custody-transfer or authenticated precommitted external-transfer-receipt consumption and tombstone transactions; stream-head/predecessor/time/chain finalization CAS; two independent unique indexes plus identity-root/archive membership lookup; threshold/successor recovery and `S_claim` reservations; all prior transactions | Fault every boundary. Unknown external transfer remains non-authoritative; no plaintext copy, distributed transaction, split custody transfer, stale predecessor, invalid uncertainty, partial identity insert, cross-pair join, unfunded recovery, second event append or result rewrite can commit |
| `0.140.3` | Separate payload lifecycle/erasure/key/hold owners, descriptor builder, trusted-time source, chain finalizer/verifier, command-ID and idempotency-ID index owners, signer-recovery root/quorum/authorization/status owners and all prior command/target/attester/signer/archive roles | No actor can inline classified data, erase through a hold, choose its commit time/predecessor, alias identities, authorize itself as successor, change the committed record or bypass prior SoD |
| `0.140.4` | Freeze redacted proposal/command/signer-recovery APIs, lifecycle/tombstone and chain-verification status, HistoricalIdentityConflict mapping, last-cancellable rule and prior target/attestation observability | API returns no payload bytes, opaque store handles, key material, signer authorization or bearer permit; it never maps erased/active/recovery-required to absent/success, accepts one-sided reuse, cancels late or offers unsafe recovery |
| `0.140.5` | Retain non-sensitive descriptor/tombstone/chain/identity/recovery commitments, key/distrust history and prior evidence while enforcing payload residency, retention, legal hold, external-copy and erasure policy | Compaction cannot preserve forbidden plaintext, move references, free either identity, lose chain/time provenance, erase signer failure/successor lineage, invalidate historical receipts or weaken prior custody/cleanup controls |
| `0.140.6` | Freeze failover during payload reference transfer/erasure, authoritative time/head-chain commit, dual-index admission/archive lookup, threshold quorum/successor signing and every prior command/attestation/archive/restore boundary | Takeover joins exact identities/status, preserves residency/hold/chain/signer lineage and `S_claim`, recreates no permit or plaintext, and refuses when required payload, identity, time, signer or archive evidence is unavailable |
| `0.140.11` | Decide per backend: payload store/reference variants, key/erasure/hold/residency/external-copy policy; trusted-time source/profile/max uncertainty and EventCommitChain encoding; unique-index and archived membership proof; exactly threshold/redundant signer set or root-authorized successor governance/quorum/SoD; plus every prior target/attester/archive/capacity/SLO choice | Architecture/security/privacy/legal/forensic/operations approve one exact profile. A backend without erasable storage, atomic head/chain, independent durable identities or funded signer recovery refuses irreversible recovery claims; hosted/runtime and successor trust are stated honestly |
| `0.142.0` | Expose read-only lifecycle/tombstone and chain-verification metadata, independent identity/conflict status and redacted CommitReceiptSignerRecoveryRequired provenance beside prior target/descriptor/attestation/recovery state | RPC cannot reveal payload/reference secrets, weaken residency/hold, mutate chain/identity/recovery, mint successor authority, free capacity or imply success while recovery is required |
| `0.143.0` | Model payload lifecycle/hold/erasure/export, recorded-time uncertainty and zero/one/multi-event predecessor chains, every dual-identity hot/archive combination, threshold/successor/lost-primary paths and all prior state machines | No reachable state leaks classified data, breaks chain/head/time policy, admits one-sided reuse, loses an identity, changes a committed event/result, releases a stranded claim or misstates signer recovery |
| `0.145.0` | Restore payload lifecycle/tombstones without prohibited bytes, target predecessor/complete chain, both identity claims, signer-recovery profile/state/authorization/capacity, signatures/settlement and all prior claim replay state | Missing/forked payload/chain/identity/recovery evidence remains RestoreUnready; restore never recreates erased plaintext, guesses chain/time, frees IDs, invents quorum/successor, re-signs changed material or resets capacity |
| `0.146.0` | Run plaintext canaries across database/registry/signer/status/log/trace/error/archive/export, load/soak payload/dual-index/status/signer-recovery lanes under failover, and fuzz lifecycle/time/chain/identity/quorum/root/key fields beside prior conformance | SLOs pass with recovery progress under ordinary saturation. Plaintext leakage, reference/hold/residency substitution, stale head/time, chain reorder, cross-pair insert, signer equivocation, capacity theft, overflow and one-short reserves fail closed |
| `0.147.0` | Audit Phase A payload lifecycle inheritance, privacy/residency/hold/erasure, event time/chain authority, independent command identities, signer-loss governance and prior one-stream/descriptor/attestation/archive/DR controls | Zero high/critical plaintext, custody, chain, replay-identity, signer-recovery, immutable-event, command, authority, evidence, accounting or role finding |
| `0.148.0` | Reject imports lacking payload variant/lifecycle/tombstone, time policy/actual interval/predecessor chain, both identity memberships/conflict history, signer-recovery profile/evidence/capacity or any prior recovery field | No shim maps classified bytes to inline, missing payload to erased, absent chain to trusted, composite ID to independent claims, missing signer to successor or RecoveryRequired to terminal success |
| `0.149.0` | Pentest plaintext extraction across every proposal/evidence surface, erasure/hold/residency bypass, recorded-time/predecessor substitution, concurrent one-sided/cross-pair reuse, signer loss/distrust/quorum/root authorization and all prior proposal/attestation/archive/fence attacks | Zero unresolved critical/high and no waived payload-lifecycle, chain, identity, signer-recovery, one-stream, descriptor, command, attestation, settlement, orphan-GC, provider-fence, evidence or trust issue |
| `0.150.0` and `1.0.0` | Demonstrate one unchanged candidate across selected payload store, time/chain, dual-identity, signer-recovery, target/process-manager, journal, registry, proposal, command-status, attester, signer, archive/GC, HA and restore profiles with every response-loss/substitution/failover/exhaustion case | Production remains blocked until Phase A/B/C conformance, canary/privacy tests, signer recovery under saturation, performance/soak, DR, independent audit and pentest pass with no high/critical issue; unsupported payload, identity, time, target, attestation or signer-recovery profiles remain refused |
