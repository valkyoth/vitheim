...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add a predecessor-linked exact-set ReplacementRecoveryCheckpoint; staged/verified/committed-or-orphan recovery publication with manifest/state/receipt/proof budget/cursor; ReplacementRecoveryArchiveReplayHead; bounded restore cursor/state; typed RecoveryHistoricalStateUnavailable; and independently witnessed non-wrapping current-pointer generation/version/digest. Cover every Recovery charge/reservation, candidate rejection, retirement result, replacement head, transfer side/receipt/mapping/withheld/baseline, activation result and current pointer. Collapse retirement to one AuthorityRetirement class/charge and one recovery-head advance. Implement the complete replacement adversarial/conformance suite in Phase C now | Exact retry/conflict survives compaction; verified Commit installs the replay head and deletes only captured rows while its charge stays hot. Restore independently obtains greatest recovery/pointer heads, replays the suffix and exact-completes witnessed activation or remains unready. Every requested competing-ID, pointer rollback, crash, duplicate, budget exhaustion, mapping/quarantine, missing-history, second-equivocation and cross-backend case passes before `0.30.0` |
| `0.30.0` | Export/import checkpoint/replay/publication/proof/restore state, exact lookup commitments, pointer predecessor chain/high-watermark and the single retirement charge alongside every prior replacement artifact | Round trip every hot/archived cut and sequential-equivocation pointer generation. Missing/forked result or pointer history returns unavailable/unready; import cannot use the old archive, reconstruct a retired result or restore an earlier authority |
| `0.140.1` | Freeze recovery checkpoint/exact-set, publication, replay-head, restore-state/cursor, historical-unavailable and pointer-generation/high-watermark schemas plus the one-class AuthorityRetirement encoding | Schema/reference review rejects range/dense coverage, pointer predecessor omission, old/recovery archive alias, ambiguous retirement cardinality, unknown publication state and historical-result inference |
| `0.140.2` | Freeze recovery Stage/Verify/Commit/MarkOrphan/FinalizeGc, replay-head install/captured-delete, restore-cursor and exact-complete-witnessed-pointer transactions under the extended recovery rank | Fault every boundary; only closed publication paths win, archive Commit remains hot, pointer exact-completion is deterministic and AuthorityRetirement consumes/advances exactly once |
| `0.140.3` | Separate recovery checkpoint builder, publisher, verifier, replay-head committer, pointer high-watermark witness, restore driver and historical-result reader beside prior recovery roles | No actor can publish/verify/commit its own history, witness and restore its own pointer, hide a result, select an older readable head or treat archive unavailability as authority |
| `0.140.5` | Retain exact-set recovery checkpoints/publications/replay ancestry, hot suffix, result lookup, restore cursors and every current-pointer generation/high-watermark across all sequential replacements | Compaction cannot revive a loser/retired authority, forget a Recovery charge/reservation, detach transfer evidence, roll back the pointer or make unavailable history look unseen |
| `0.140.6` | Freeze failover across checkpoint capture, publication, verified Commit, delete, pointer witness/activation response loss, restore replay and second equivocation | Takeover obtains external greatest heads first, resumes bounded cursors, returns archived exact results and either exact-completes the witnessed pointer or stays RecoveryUnready |
| `0.140.11` | Repeat and freeze the already implemented `0.29.0` replacement/history conformance suite for the selected production profile, adding any profile-specific vectors | The decision cannot waive an earlier Phase C failure or introduce first-time semantics/tests. Selected backends pass the unchanged core corpus plus independent architecture/security review |
| `0.142.0` | RPC exposes read-only recovery checkpoint/publication/replay/restore/pointer-high-watermark and historical-unavailable status | RPC cannot stage/verify/commit/delete, advance a cursor/pointer, choose an older head, infer unseen state, issue restore authority or retrieve deleted history without exact proof |
| `0.143.0` | Model hot-to-archive races, concurrent pointer generations, activation before/after external witness, recovery archive rollback/fork/loss and second/third equivocation | No model state revives an earlier authority, returns a different historical result, loses a charge/reservation or permits two current pointers; liveness is exact-complete or bounded unready |
| `0.145.0` | Restore the greatest externally witnessed current pointer and recovery replay head, verify checkpoint ancestry, replay hot suffix and resume the durable cursor before readiness | Locally consistent older heads, missing exact-set evidence, unavailable results and incomplete witnessed activation all fail closed without old-archive fallback |
| `0.146.0` | Fuzz recovery checkpoint/replay/pointer sequences, predecessor digests, exact-set members, publication/receipt states, captured sets, cursor bounds, result identities and retirement charge cardinality | Wrap, ABA, omission, substitution, duplicate retirement, double delete, false unseen, historical-result rewrite and pointer rollback fail closed |
| `0.147.0` | Audit recovery-history ownership, compaction authority, external greatest-pointer anchoring, sequential-equivocation restore, exact result lookup, early test coverage and single retirement accounting | Zero high/critical history-loss, stale-authority revival, double-charge/head, rollback, bounded-work, role-confusion or test-deferral finding |
| `0.148.0` | Reject legacy imports lacking recovery checkpoint/replay/publication/restore state, exact-set lookup, pointer predecessor/high-watermark or unambiguous one-charge retirement | No shim uses the old archive, guesses historical results, defaults an earlier pointer, rebuilds a cursor or interprets two retirement legs |
| `0.149.0` | External pentest targets recovery archive deletion/fork/rollback, exact-result substitution, pointer downgrade across sequential equivocations, verification/GC races, cursor exhaustion, old-archive fallback and retirement double accounting | Zero unresolved critical/high findings and no waived recovery-history, stale-authority, exact-replay, pointer-monotonicity, single-writer or early-conformance issue |
| `0.150.0` and `1.0.0` | Demonstrate exact recovery history and greatest-pointer restore across every selected backend/RPC/HA/backup/import/compaction profile and multiple sequential equivocations on the unchanged candidate | Production remains blocked until the full early/core plus freeze/model/fault/DR/audit and independent-pentest corpus passes with no fallback to old-lineage history |
