...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain every prior capacity/authorization control; select bounded authenticated sparse drain replay checkpoint/archive, exact result or authenticated reference, exact-set membership/non-membership, proof budgets/cursor, atomic archive-before-delete, Recovery reservation/backpressure and typed historical-state-unavailable; reject permanent unbounded retention and arbitrary-ID dense inference |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain all prior attacks; add sparse/nonsequential IDs, first late exact/changed retry, archive outage, result-reference/chunk/key loss, checkpoint fork, proof bytes/entries/chunks/depth/decode/work/jobs exhaustion, cursor/compaction crashes and dense-watermark inference attempts |
| `0.23.0` | SQLite single-node adapter | Persist every prior field plus sparse replay checkpoint/archive entries, exact result/reference, predecessor/key commitment, budgets/cursor and archive-before-delete transaction; prove bounded late replay/conflict/unavailable-history or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain concurrent protections; prove checkpoint/archive/delete serialization, exact sparse membership/non-membership, bounded restartable verification, backpressure and failover with result/archive/key loss |
| `0.25.0` | Experimental MySQL adapter | Prove all prior semantics plus sparse replay archive/proof-budget/cursor/atomic-delete parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior semantics plus sparse replay archive/proof-budget/cursor/atomic-delete parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific prior semantics plus sparse replay archive/proof-budget/cursor/atomic-delete parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve all prior state plus sparse archive entries/results/references, predecessor roots, key epochs, budgets, cursors, exact membership semantics and atomic deletion evidence; reject dense conversion, result loss, proof widening, rollback or unavailable history becoming unseen |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger prior semantics plus sparse exact-result/conflict/archive/unavailable-history/proof-budget/cursor meaning; otherwise refuse import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain prior atomicity; freeze sparse replay checkpoint/archive schema, complete result/reference, exact-set proofs, bounded roots/chunks/indexes/cursor, archive availability, Recovery reservation/backpressure and one checkpoint/archive/cursor/hot-delete transaction |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Select authenticated sparse replay archives and reject permanent retention; freeze minimized complete result/reference, sparse membership/non-membership, retention/key/archive availability, deletion and typed unavailable-history policy without weakening permanence |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Retain prior recovery; freeze sparse archive/root/key/budget/cursor/backlog recovery and external ratchets, rejecting exact-result loss, dense unseen inference, missing proof becoming executable, checkpoint fork or rollback |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus bounded sparse drain replay checkpoint/archive, exact result/conflict/unavailable-history, proof budgets/cursor, archive-before-delete and Recovery backpressure |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve all prior envelopes plus sparse archive entry/proof/result-reference/key/budget/cursor fields across RPC; peers cannot invent membership, non-membership, availability or unseen state |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over all prior state plus sparse replay archive/root/key/budget/cursor/backlog without result loss, dense inference, proof reset, missing-history execution, rollback or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore every prior field plus sparse archive/root/key/budget/cursor/availability/backlog; reject result loss, dense inference, missing-history execution, proof/key rollback and checkpoint fork |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; exercise sparse IDs, late exact/changed retry, outage/loss/fork, every proof budget, cursor/compaction crash and cross-backend migration under traffic/failover |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit every prior control plus sparse set construction, archive/reference authentication, proof bounds/cursor, backpressure, atomic deletion and unavailable-history non-execution |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze all prior formats plus sparse checkpoint/archive/root/chunk/result-reference/proof-budget/cursor/availability and historical-state-unavailable semantics; forbid dense or unbounded downgrade |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add sparse-ID compaction, late exact/changed retry, archive/reference/chunk/key loss, fork, proof exhaustion, cursor/compaction crash, migration and dense inference; retest fixes |
