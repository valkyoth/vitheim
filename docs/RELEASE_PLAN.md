...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Persist challenge/sequence topology ratchets, exact claim/freshness scope, historical outcomes, independent authorization lineage/issuance/unknown-response/break-glass state, local consumption tombstones, future three-state handoff, exact verifier, and closed realization |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Destructively test authorization issue-versus-epoch-change/revoke ordering, lost response, profile discrimination, issuer/topology separation, signed-old topology replay, receipt ratchet/clock/restore rollback, claim replay, initialization order, clone, ancestry, and trust rejection |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze currentness challenge/sequence/window, local observation ratchets, independent authorization issuer, issuance-time bounded-grant semantics, profile discrimination, claim scope/replay, control receipts, MAC separation, and durable anchors |
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Separately place topology receipt/observation sequences, authorization lineage/issuance/unknown/break-glass high-watermarks, local consumption tombstones, claims, and control receipts; persist three-state handoff without storage-derived trust |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Race authorization issue/change/revoke and unknown responses, topology challenge/sequence/currentness, control receipts, and claims through failover/restore; prove owner separation, bounded issued grants, no reissue/reconsume, receipt-ratchet monotonicity, time loss, and RPO/RTO |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Activate both generation-2 laws; declare VIT-INV-060/061 separately; issue bounded grants at current-authority linearization; consume locally; ratchet challenge/sequence/expiry topology receipts; reject stale placement despite rollout/fence loss |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Typed topology successors; issuer/topology credential separation; idempotent authorization issuance/unknown recovery; profile discrimination; fresh topology challenges/sequences; claims/control receipts; replacement/clone fencing |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Race authorization issue versus epoch/revoke, lose issuance response, fail over issuer/topology owners separately, replay signed-old topology receipts, roll back clocks/ratchets, and preserve bounded grants, pinned rollout winner, MAC roles, and owner separation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Recover three-state handoff, topology receipt/observation ratchets, independent authorization lineage/issuance/unknown/break-glass state, control/claim tombstones, and exact owners; reject inferred issuance, validity extension, ratchet rollback, reissue, owner merge, clone, or resurrection |
