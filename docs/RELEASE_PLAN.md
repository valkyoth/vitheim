...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Persist one destination-local operation/job/budget/reservation/lifecycle/fence/candidate/tombstone/authorization/revocation-inbox/barrier/result/cleanup lineage. Derive `MigrationImportOwnerManifestV1` only from source/destination schemas, migration plan, the admitted VIT-LAW-009 tuple/manifest and dependency closure; type VIT-INV-062 as the live non-importable coordinator and all other applicable entries as domain contributors. Explicit authorization admission shares one row with activation/expiry/revocation. Revocation before admission creates a permanent target-bound RevokedBeforeAdmission tombstone; its sequence is per authorization lineage and its lifetime covers the target authority. The co-located transaction activates every domain contributor plus live-coordinator barrier/result/audit/outbox, or none |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, and exact candidate-to-owner activation protocol | Require the complete canonical two-head tuple, unified namespace, atomic checkpoint bundle and mixed-trigger proof; missing/defaulted or singular state is quarantined before import mutation. Export enumeration, transfer, decode, verification and staging cannot create fresh counters, jobs, cursors or reservations. The live destination VIT-INV-062 state stays outside the candidate; nonterminal source jobs, cycles and identity collisions deny; terminal source history is inert and archived only after activation. VIT-INV-062 schema succession uses a separate predecessor-owned drain/checkpoint/dormant-successor/CAS handoff. Partial owner preparation is never authority; destinations lacking the one local domain-owner activation boundary refuse before staging |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze authorization admission and revocation-intent signer/key/authentication profiles, issuer continuity, exact-target sequence key, target-authorization lifetime binding, trusted time, authenticated outbox/inbox receipts, exact retry/conflict and pre/post-admission tombstones; only destination-local commit is revocation |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist coordinator/domain-contributor roles, immutable live coordinator state outside staging, shared authorization row with RevokedBeforeAdmission, scoped sequence/inbox/result, inert history archive, typed collisions and coordinator-bootstrap checkpoint/handoff. Admission, activation, expiry and revocation share one lock prefix; unsupported adapters refuse |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze exact requestor/approver/activator/issuer separation, authority admission, per-target revocation sequencing and coordinator-bootstrap roles; existing authority remains candidate-bound until consumed, expired or destination-locally revoked |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve coordinator/contributor derivation, live coordinator exclusion, authorization admission, RevokedBeforeAdmission, exact-target sequence, inert archive/bootstrap and job/candidate/receipt/barrier state across HA/DR; failover cannot admit over a tombstone, import coordinator state, reset a sequence, revive source jobs or reverse Consumed |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries authenticated VIT-LAW-009 tuple/manifest/dependencies, coordinator/contributor roles, explicit authorization admission and target-scoped revocation provenance; a peer cannot import coordinator state, suppress revocation through another sequence or make authority usable over a local tombstone |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over admitted-law role derivation, live coordinator state, explicit admission, pre/post-admission tombstones, scoped sequence, archive/bootstrap and one job/candidate/authorization/barrier lineage. Reorder authorization/revocation and lose responses; recover exactly one safe terminal with no partial owner visibility or coordinator replacement |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore role derivation, live coordinator, authorization admission, RevokedBeforeAdmission, scoped sequence, inert archive/bootstrap and barrier before work; older backups cannot admit delayed authority, resurrect source jobs, replace the coordinator, lower a target sequence or reverse Consumed |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Race revocation-before-admission, lost/late authorization, target expiry in transport, unrelated sequences, nonterminal/terminal source jobs, identity collisions, cycles, coordinator bootstrap, activation, failover and response loss; exactly one safe local terminal wins and every domain contributor is visible or none |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit admitted-law trust and coordinator classification, shared-row authorization admission/revocation, target sequence/lifetime, live-state exclusion, inert archive/bootstrap, fixed lock order, domain-owner atomicity and idempotent results |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze coordinator/contributor roles, authorization admission/RevokedBeforeAdmission, intent target lifetime/scoped sequence, live-coordinator exclusion, inert history/collision/bootstrap and shared-row formats. Unknown or omitted fields fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack revocation-before-admission, delayed/lost authorization, expiry in transport, cross-target sequence suppression, source live-job replacement/merge, terminal-history promotion, identity collision, cyclic self-import, coordinator bootstrap rollback, activation races and lost responses; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for coordinator/contributor derivation, live coordinator, authorization admission and pre/post-admission revocation state, inert archive/bootstrap, barrier and activation result |
