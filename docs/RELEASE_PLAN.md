...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze activation/bootstrap plus recovery-authorization signer, key, trusted-time and shared-row admission/revocation profiles; exact obligation/action/authorization sequence scope, target-covering lifetime, issuer continuity and canonical destination inbox/tombstone/result. Only destination-local commits change authority |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Select no separate invariant catalog. Persist roles/generation/fence/live exclusion, activation rows, history obligation/archive head/disposition/recovery authorization-revocation/cumulative-lineage and successor budgets/retry-exhaustion result/waiver-abandonment records/checkpoint, typed collisions and complete bootstrap state. Freeze the closed action matrix, cumulative counters and canonical activation/action lock prefixes; unsupported adapters refuse |
...
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve complete recovery authorization/revocation, cumulative budget, action matrix, retry-exhaustion result, policy/hold receipts and custody records across HA/DR. Failover cannot let RetryAppend abandon, reset lineage counters, infer remote revocation, lose ManualRecoveryPending, weaken legal hold or clean before terminal checkpoint |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Migration RPC carries exact recovery authorization/revocation, cumulative-budget, retry-exhaustion and custody-policy evidence but never claims remote effect. A peer cannot reset counters, change RetryAppend into Abandon, bypass a local tombstone/hold or erase ManualRecoveryPending |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Fail over recovery authorization/revocation inbox/sequence/tombstone, cumulative and successor budgets, retry-exhaustion result, policy/hold receipts, custody records and checkpoint. Race retry/waive/abandon admission, revocation, expiry, consumption, worker loss and cleanup; only the authorized action wins and no counter/evidence disappears |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore recovery revocation/tombstones, cumulative counters, successor lineage, retry-exhaustion results, policy/hold epochs and custody records before work. Older backups cannot reset a limit, convert RetryAppend to Abandon, erase ManualRecoveryPending or weaken an active hold |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Exhaust repeated RetryAppend successors; race all action admissions/revocations/expiries/consumptions, hold activation, policy rotation, failover and cleanup. Successor exhaustion remains ManualRecoveryPending; cumulative limits hold and Waive/Abandon require their own current authority |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit recovery action non-interchangeability, destination-only revocation, cumulative budget conservation, policy/hold commit rechecks, custody records, checkpoint gating and the existing activation order/domain atomicity |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze recovery revocation, lineage-budget, retry-exhaustion, waiver/abandonment record and policy/hold evidence wire/storage formats. Unknown/defaulted action, counter, hold or custody fields fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack RetryAppend-to-Abandon escalation, authorization/counter reset, revocation reorder/remote-effect inference, successor-budget churn, stale hold/policy receipt, forged compliance/legal approval, custody-floor weakening, response loss, restore and cleanup; retest fixes |
| `0.150.0` | Final production-readiness candidate | Clean install/upgrade/restore/failover continuity for recovery action/revocation/cumulative counters, retry-exhaustion outcome, policy/hold authority, waiver/abandonment records and terminal checkpoint beside all prior activation/bootstrap evidence |
