...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add signed tenant/deployment-bound `P_0` at initial authority bootstrap with initial authority/lineage/profile/key/distrust epochs, recovery/replay genesis, generation zero, explicit CanonicalNoPredecessor, encoding/integrity epochs, independent receipt and Healthy recovery guard. Require `C_1→P_0`, then exact-set checkpoint/publication/replay/restore and stagger `C_n→P_(n-1), H_n→C_n, P_n→H_n`. Add bounded typed pointer/query/restore protocols. Pointer contradiction atomically stores both receipts/requests/epochs/discovery evidence and CASes Healthy→Fenced from a pre-reserved terminal leg. Signed `P_n` alone operationalizes; all writers are fence-first | Reject missing/duplicate/forged/rolled-back/cross-tenant `P_0`, absence predecessor, DAG cycles and pending authority. Crash-test all-or-none fence installation/result/audit/outbox/head/settlement; exact retry joins and changed evidence conflicts. Fenced is absorbing/hot with no clear or recursive automatic replacement. Complete core suite before `0.30.0` |
| `0.30.0` | Export/import `P_0`/receipt/guard, every `C_n/H_n/P_n` cut, pending/Operational state, query reservations, contradiction bundle/fence/result and all recovery artifacts | Round trip Healthy, pending, Operational and Fenced cuts. Import rejects genesis absence/substitution/rollback, back-edges, missing results, stale proofs, permit reconstruction and fence loss; unavailable history remains unready |
| `0.140.1` | Freeze genesis/no-predecessor/guard, staggered DAG, pending/Operational, fence/evidence/result, non-reference and all typed command schemas | Schema review rejects absence genesis, unsigned or cross-tenant `P_0`, `C_n→P_n`, future fields, unknown tags, non-absorbing fence and ambiguous operational authority |
| `0.140.2` | Freeze bootstrap; checkpoint/publication/GC; pointer publish/Reconcile/query; Healthy→Fenced terminalization; pending→Operational; and restore transactions under fence-first recovery rank | Fault every boundary. Fence artifacts and settlement are all-or-none; signed witness alone operationalizes; each operation owns one charge/head advance and external reference blocks orphan |
| `0.140.3` | Separate genesis trust-root signer, checkpoint/publisher/verifier/committer, pointer requestor/signer/reconciler, contradiction custodian, guard/fence owner, restore roles and retention/legal owners | No actor can self-create the recovery root, witness and operationalize its pointer, suppress contradictory evidence, clear a fence, self-assert non-reference or choose an older head |
| `0.140.5` | Retain `P_0`, guard/fence/bundle/result, all staggered edges, hot results, queries, restore state, references and hold decisions across replacements | Compaction cannot synthesize genesis, omit/clear fence, introduce back-edge, orphan referenced data, revive authority, forget reservation or make pending Operational |
| `0.140.6` | Freeze failover across genesis, checkpoint/archive, pointer claim/Reconcile/query, contradiction fence, pending operationalization, restore and later equivocation | Takeover recreates no permit, obtains greatest external generation, requires Healthy, preserves absorbing fence and either exact-operationalizes witnessed pointer or stays Unready |
| `0.140.11` | Decide before `1.0.0` the genesis trust-root/signer profile, pointer witness profile, fence custody/retention and supported operational recovery runbook; manual root-governance recovery remains explicitly unsupported | Independent architecture/security/legal/operations approval is mandatory. No option may synthesize `P_0`, clear/replace a recovery fence recursively, weaken witness independence or authorize pending state |
| `0.142.0` | RPC exposes read-only genesis/receipt/guard/fence evidence, staggered generations, pointer requests/receipts/query, pending/Operational state, restore and typed outcomes | RPC cannot bootstrap, publish/Reconcile, operationalize, fence/clear, orphan/delete, obtain permits, choose an older head or translate negative state into authority |
| `0.143.0` | Model genesis corruption, hot/archive/GC races, DAG edges, concurrent pointer/query/contradiction, pending dispatch, response loss and later equivocations | No cycle, absent genesis, partial fence, pending authority, collected reference, recreated permit, revived authority or dual current pointer is reachable |
| `0.145.0` | Restore authenticated `P_0`, greatest signed `P_n/H_n`, exact ancestry, guard/fence and hot suffix; resume cursor/query before readiness | Older heads, missing/forged genesis, non-Healthy guard, incomplete fence/witness, stale proof or absent exact result fails closed without old-archive fallback |
| `0.146.0` | Fuzz genesis/guard/fence codecs, DAG/predecessors, command/result tags, query/fence reservations, greatest-generation authority guard, reference proofs and cursor bounds | Absence synthesis, cycle, ABA, omission, partial fence, permit replay, result substitution, pending dispatch, double settlement/delete and rollback fail closed |
| `0.147.0` | Audit genesis trust root, DAG/typed ownership, fence-first rank, bounded query/restore, evidence retention, operationalization guard, compaction and sequential restore | Zero high/critical genesis, cycle, partial-fence, stale-authority, pending-dispatch, history-loss, evidence-suppression, permit, rollback or role-confusion finding |
| `0.148.0` | Reject imports lacking `P_0`, signed receipt, Healthy/Fenced guard, explicit state, fence evidence/results, staggered generations, exact results or non-reference evidence | No shim invents genesis, defaults Healthy/Operational, clears a fence, guesses a result/pointer, reconstructs permits or treats absence as non-reference |
| `0.149.0` | External pentest targets genesis forgery/rollback/tenant swap, DAG cycles, pending-authority window, pointer response loss/contradiction, fence starvation/clearing, archive rollback, referenced GC, restore exhaustion and pointer downgrade | Zero unresolved critical/high and no waived genesis, DAG, structural-fence, operational-authority, reference, retention, monotonicity, bounded-work or early-conformance issue |
| `0.150.0` and `1.0.0` | Demonstrate signed-genesis-rooted acyclic recovery, absorbing fence, witness-gated operationalization and reference-safe retention across every selected profile and sequential equivocation | Production remains blocked until early/core, schema/fault/DR/audit and independent-pentest evidence passes with no genesis ambiguity, partial fence, pending authority, cycle, referenced deletion, permit reconstruction or fallback |
