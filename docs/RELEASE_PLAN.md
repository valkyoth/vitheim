...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.21.0` | Stable-invariant, law-manifest, catalog lineage/rollout/local-ratchet, and semantic-realization storage negotiation | Retain prior replay controls; freeze canonical replay key with independently unique action/idempotency IDs, durable cumulative logical-attempt restart budget, typed contention distinct from unavailable history, and finite authenticated-admission/compaction fairness with protected Recovery capacity |
| `0.22.0` | Declaration-derived destructive invariant and ancestry-complete generation-pinned law/catalog conformance | Retain all prior attacks; continuously advance heads through every cumulative counter, churn publication, crash/fail over/recreate cursors/retry adapters, race one-sided ID reuse and both unique indexes, and prove Normal/unauthenticated traffic cannot starve Recovery or pin compaction |
| `0.23.0` | SQLite single-node adapter | Persist both scoped unique indexes and durable restart/deadline counters; prove writer-authoritative head/key/hot/claim transaction and fair scheduler semantics survive busy/crash/reopen, or refuse VIT-CAP-060/061 |
| `0.24.0` | PostgreSQL reference production adapter | Retain all prior protections; prove both uniqueness constraints, monotonic logical-attempt accounting and scheduler quanta across pool retry/failover, with no replica/weak-snapshot authorization or budget reset |
| `0.25.0` | Experimental MySQL adapter | Prove all prior key, cumulative-budget, fairness, isolation/locking/uniqueness and publication parity or refuse VIT-CAP-060/061; no default v1 claim |
| `0.26.0` | Experimental MongoDB adapter | Prove all prior key, cumulative-budget, fairness, isolation/locking/uniqueness and publication parity under transactions/failover or refuse VIT-CAP-060/061; no default v1 claim |
| `0.27.0` | Experimental SurrealDB adapter | Prove version-specific key, cumulative-budget, fairness, isolation/locking/uniqueness and publication parity or refuse VIT-CAP-060/061; retain capability truthfulness |
...
| `0.29.0` | Resumable invariant-owner and trusted law/catalog migrations preserving monotonic authority | Preserve canonical key encoding, both uniqueness constraints/claims, logical-attempt counters/deadline, scheduler state and protected Recovery classification plus all prior admission/publication state; reject reset, collision, rollback or lost accounting |
| `0.30.0` | Cross-backend export and import with explicit law trust closure | Require same-or-stronger canonical-key uniqueness, cumulative restart accounting and fairness semantics plus all prior admission/publication meaning; otherwise keep action execution fenced and refuse the import/profile |
...
| `0.140.2` | Storage topology for invariant/law manifests, rollout/topology/authorization state, active catalogs/ratchets, and provider state | Retain prior schema; freeze canonical replay-key encoding, separate action/idempotency unique indexes, durable logical-attempt counters/deadline, admission guard, writer routing, lock order, claim, adapter isolation, publication and reconciliation |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Retain sparse selection; freeze admission/compaction quanta, bounded caller-independent yield, protected Recovery capacity, retention/coalescing, staged/orphan exclusion and GC eligibility |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, and recovery decision | Retain prior recovery; freeze writer routing/failover plus replay key/unique claims, restart counters/deadline, scheduler state, cumulative head/publication/hot/object/GC high-watermarks; no retry/failover reset or split visibility |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.141.0` | Single-node packaging plus independent topology authorization, governed handoff, and replay-safe dynamic admission | Implement every prior control plus canonical dual-unique replay key, durable cumulative restart budget, typed contention distinction and authenticated-admission/compaction fairness with protected Recovery progress |
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | Preserve attempt ID, replay key, counters/deadline, lane/class and head provenance across RPC; reconnect or peer retry cannot reset work, and follower/cache proof cannot authorize |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, and cancellation recovery | Fail over replay key/claims, cumulative restart budget/deadline, scheduler state and writer routing with all prior publication state; no budget reset, split visibility, duplicate execution or Recovery starvation |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore canonical keys/claims, monotonic attempt counters/deadline, fairness state and all prior admission/publication state; reject counter reset, collision, stale proof, split visibility, staged promotion or rollback |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, and cancellation contention certification | Retain prior stress; continuously churn heads through every cumulative bound across crash/failover/retry, race both unique indexes, saturate Normal/unauthenticated traffic and prove Recovery plus compaction progress |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, and supply-chain hardening | Audit every prior control plus canonical-key uniqueness, cumulative-budget durability, contention classification, fair scheduling, protected Recovery progress, admission isolation and publication/GC fences |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, and topology replay lifecycle | Freeze replay-key encoding/unique scopes, attempt/counter/deadline schema, contention error, scheduler semantics and every prior admission/publication format; old nodes cannot reset or reinterpret them |
| `0.149.0` | Release candidate and external pentest remediation | Retain every prior attack; add restart amplification, publication churn, crash/failover/reconnect budget reset, one-sided ID collision, unfair scheduling, compaction pinning and Normal/unauthenticated starvation of Recovery; retest fixes |
