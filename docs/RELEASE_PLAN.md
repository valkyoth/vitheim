...
| `0.51.34` | Tagged initial/replacement final activation with a nested closed top/lowered final authority, distinct guard-slot/candidate/successor commit-time generations, and deferred predecessor cleanup | Optional/mismatched final-authority member, genesis/admission substitution, nonexistent operational-guard check, post-prepare generation race, double eligibility consumption, missing cleanup capacity, partial activation |
...
| `0.51.37` | Per-member expected-version predecessor release or accepted funded-retention reconciliation with absorbing terminals, explicit pending uncertainty, and zero-pending completion | Delayed pending overwrites terminal, release/retention double winner, member-version rollback, mixed-version parent fold, unknown relabeled retained, unauthenticated retention, completion with pending |
