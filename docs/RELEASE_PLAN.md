...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Preserve pinned genesis, absorbing AuthorityFenced, `R_first`, `B_EM`, `c_max`, loss-safe acquisition, PreparedNonAuthoritative non-journal proposals, typed LocalFinalize settlement, staged signing and safe archive cleanup. Add exactly-one DomainAggregateTarget/ControlOwnerTarget/CanonicalNoAggregateTarget; reject multi-target work to Phase B process manager/outbox; sign full ordered ProposedEventDescriptor roots; map descriptors to allocator fields in the commit record; add unique command execution from ClaimAcquisitionPending through terminal state with active/history retry join and ProposalPrepared cancellation boundary; add commit-attestation port/profile/evidence for trusted adapter read, database attestation or explicit hosted runtime; extend `R_claim` and history | Conformance rejects multi-aggregate/mixed targets and every schema/target/tenant/identity/time/custody substitution. Crash/retry every command state and attestation mode; exact retry never creates a second claim, post-disposition cancellation fails, caller bytes never prove commit, and all prior proposal/signer/archive/fence tests remain mandatory |
| `0.30.0` | Export/import canonical target, ordered descriptors/root, descriptor-envelope map, full command execution/status/cancellation history, commit-attestation profile/evidence/backend continuity and every prior acquisition/proposal/signature/archive state | Round trip every cut. Missing/forked/defaulted target, descriptor semantic, active command state, terminal receipt, attestation or continuity is RestoreUnready; import never creates multi-target authority, semantic wrapper changes, duplicate claims, post-commit cancellation or caller-attested signatures |
| `0.140.1` | Freeze target union, ProposedEventDescriptor fields/root/encoding, allocator-field allowlist, descriptor-envelope mapping, command-execution states/results and commit-attestation port/profile/evidence alongside prior schemas | Review rejects target lists, unknown/mixed targets, payload-only roots, mutable semantic fields, ambiguous retry terminality, missing cancellation boundary, generic/caller attestation and every prior unsafe alias |
| `0.140.2` | Freeze one-target uniqueness/CAS and Phase B journal conformance; active-or-terminal command/idempotency lookup before acquisition; atomic command-state edges; one-stream final append; descriptor-map commit record; cancellation/registry terminalization order; authoritative-read or adapter-attestation verification before signing; all prior transactions | Fault every edge. No multi-stream transaction, second active execution, changed retry join, post-disposition cancel, semantic rewrap, replica/cache proof or unsupported attester can commit; no distributed transaction is assumed |
| `0.140.3` | Separate command admission/status/cancel owner, target validator, descriptor builder/verifier, process manager, journal allocator, commit attester, signer adapter/HSM, hosted-runtime attester and prior issuer/proposal/finalizer/settler/archive roles | No actor bypasses one-stream law, rewrites descriptors, self-attests caller bytes, combines runtime attestation with independent-signer claims, creates a second command execution or cancels committed work |
| `0.140.4` | Freeze read-only command-status API and typed nonterminal/terminal mapping, cancellation endpoint and last-cancellable rule, target/descriptor/attestation observability redaction and process-manager decomposition contract | API never returns bearer permits or signing/attestation secrets, maps nonterminal to absent/success, accepts changed retry material, hides PermanentlyUnresolved provenance, cancels after disposition publication or offers multi-target mutation |
| `0.140.5` | Retain target/descriptor roots and maps, active/terminal command execution/status/cancel evidence, attestation profiles/evidence/backend continuity plus all acquisition/proposal/journal/signature/settlement/archive history | Compaction cannot forget an active retry, change target/descriptor semantics, erase cancellation timing, downgrade attestation trust, invalidate terminal receipts or weaken prior cleanup/custody controls |
| `0.140.6` | Freeze failover in every command state, target/descriptor validation, process-manager handoff, authoritative-read/database/runtime attestation, signing and prior acquisition/proposal/archive/restore boundaries | Takeover joins exact status, recreates no claim/cancel/sign permit, preserves descriptor and backend continuity, and refuses when command/attestation/signer/archive history is unavailable |
| `0.140.11` | Decide supported target kinds; descriptor schema/time/custody fields; command-status budgets/retention/cancel policy; per-backend attestation mode and read-only authoritative port or attestation keys; whether hosted-runtime trust is production-acceptable; and all prior proposal/registry/signer/archive/capacity/SLO choices | Architecture/security/legal/forensic/operations approve one exact profile per backend. A backend without one-stream enforcement or trustworthy post-commit proof refuses independent-signing recovery; hosted-runtime claims are never presented as independent attestation |
| `0.142.0` | Expose read-only canonical target, descriptor-root/map verification, command execution/status/terminal provenance, cancellation disposition and attestation profile/continuity alongside prior recovery state | RPC cannot mutate target/state, expose payload/attestation secrets, create a claim, map active to absent/success, cancel late or imply independent trust for hosted runtime |
| `0.143.0` | Model one-target law, descriptor semantics, every command-state/retry/cancel edge, three attestation profiles and prior acquisition/proposal/signing/archive/authority state machines | No reachable state writes multiple aggregates, rewraps payload, creates duplicate active execution, loses active retry, cancels committed events, signs caller bytes or misstates attestation trust |
| `0.145.0` | Restore targets/descriptors/maps, active and terminal command execution/history, cancellation boundary, attestation profile/evidence/continuity, signatures/settlement and all prior claim replay state | Missing target/descriptor/status/attestation evidence remains RestoreUnready; restore never creates a second claim, converts active to terminal, changes semantics, cancels a commit or re-attests blindly |
| `0.146.0` | Load/soak command-status joins, process-manager decomposition, descriptor verification, each selected backend-attestation/HSM path and prior registry/proposal/archive cleanup under failover; fuzz target/state/map/attestation fields | SLOs pass without duplicate claims or status starvation. Multi-target, semantic substitution, retry races, late cancel, continuity/nonce/key substitution, caller/cache proof, overflow and one-short reserves fail closed |
| `0.147.0` | Audit Phase B one-stream preservation, descriptor completeness, command lifecycle/idempotency/cancellation and attestation trust claims with prior immutable-event, signer, registry, archive, custody and DR controls | Zero high/critical multi-aggregate, semantic-rewrap, duplicate-command, late-cancel, false-attestation, provisional-event, permit/signing, cleanup, authority, evidence, accounting or role finding |
| `0.148.0` | Reject imports lacking explicit target, descriptor semantic/root/map, command active/terminal state, cancellation disposition, attestation profile/evidence/backend continuity or any prior recovery state | No shim maps multi-target to one, payload bytes to full semantics, active to absent, committed to cancellable, caller bytes to attested or hosted runtime to independent signer |
| `0.149.0` | Pentest multi-aggregate bypass, descriptor/schema/tenant/identity/time/custody substitution, active-retry duplicate claims, status/history deletion, cancellation races and all attestation modes/continuity/nonce plus prior proposal/signer/archive/fence attacks | Zero unresolved critical/high and no waived one-stream, descriptor, command-lifecycle, attestation, immutable-event, signer, settlement, orphan-GC, authority, provider-fence, evidence or trust issue |
| `0.150.0` and `1.0.0` | Demonstrate one unchanged candidate across selected target/process-manager, journal, registry, proposal, command-status, backend-attester, signer, archive/GC, provider, HA and restore profiles with every response-loss, substitution, retry/cancel, failover and exhaustion case | Production remains blocked until Phase B/C conformance, backend-attestation/signer tests, performance/soak, DR, independent audit and pentest pass with no high/critical issue; unsupported targets or attestation profiles remain refused |
