...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add a fence-keyed WitnessAuthorityReplacementHead with NoReplacement→Staged→Activated expected-version CAS, immutable predecessor, unique old-fence→genesis constraint, permanently rejected competing candidates and one authoritative current-authority/current-lineage pointer. Make retirement one local atomic Active→RetiredForEquivocation transition. Add a pre-reserved parent Recovery budget and replacement attempt-set head. Stage one frozen source outbox/non-operational destination; import through a unique destination inbox; independently witness the transfer; activate the pointer only after exact import and passing conformance. Partition source reservations into fresh one-to-one mappings or permanent quarantine-as-consumed and preserve inherited consumed as the destination genesis baseline | Concurrent different bootstrap IDs produce one Staged winner and durable conflicts. Prove per class `Q + M = S.reserved`, destination genesis consumed=`S.consumed + Q`, destination reserved=`M`, source remaining=destination remaining+withheld and source admitted=destination admitted+withheld. No quantity or permit is live twice; no post-genesis head charge is fabricated from inherited consumed |
| `0.30.0` | Export/import replacement head/version/predecessor, rejected candidates, current pointer, Recovery budget/charges, recovery attempt head, frozen source version/root, transfer/outbox/source disposition, inbox/applied/activation receipts, reservation mappings, withheld member, inherited baseline, independent transfer high-watermark and every typed result/conflict | Round trips at NoReplacement, Staged, destination-applied, ready-to-activate, Activated and source-terminal cuts. Missing/forked state remains non-operational; import never infers completion from timeout/source absence or creates a second successor |
| `0.140.1` | Freeze replacement-head/state/pointer, local retirement, recovery-budget writer-class, transfer/outbox/inbox/disposition/mapping/withheld/baseline, high-watermark, receipt and result/conflict schemas plus all identity domains | Review rejects absent-as-NoReplacement, mutable predecessor, duplicate fence mapping, ambiguous inherited charges, unknown reservation disposition, cross-authority transfer witness and old permit/namespace material |
| `0.140.2` | Freeze the single local AuthorityRetirement charge/head advance; replacement stage; inbox import; source reconciliation; transfer-witness Prepare/status reconciliation; activation/current-pointer CAS; candidate rejection; exhaustion and audit/outbox transactions under the recovery lock rank | Backend faults expose each local bundle all-or-none. Retirement can neither double-charge nor advance twice. Same-backend optimization must prove one transaction over the same logical records; portable profiles use outbox/inbox and never assume distributed atomicity |
| `0.140.3` | Separate retirement quorum and retirement executor, Recovery budget owner, replacement selector, source freezer/outbox owner, destination importer, reservation-mapping verifier, independent transfer witness, conformance verifier, activator/current-pointer owner and source reconciler | No actor can approve and execute retirement alone, retire and replace alone, select and activate its own candidate, map its own unsafe reservation, witness its own transfer, import and self-acknowledge, or consume/refund another writer class |
| `0.140.5` | Permanently retain the replacement-head ancestry, all candidate dispositions, frozen source proof, both transfer sides/receipts, mapping root, equations/baseline/withheld member, independent high-watermarks, activation/current-pointer history and Recovery charges | Compaction cannot erase the winning constraint, resurrect a loser, treat a quarantined reservation as remaining, convert inherited baseline into new charge credit, roll back one transfer side or detach the pointer |
| `0.140.6` | Freeze failover at every retirement, stage, outbox, inbox, status-query, witness, activation and source-terminal boundary. Restore both sides and the external transfer commitment before readiness | Takeover performs status/Reconcile only, never restages with new material, preserves one winner and funding, keeps destination non-operational until activation and detects rollback of either side |
| `0.140.11` | Extend the equivocation recovery freeze decision with the exact single-successor head, local-only retirement semantics, reservation disposition policy, portable outbox/inbox profile, independent transfer witness and current-pointer activation runbook | Architecture/security/operations approve one supported profile or keep replacement disabled. The decision cannot select distributed-transaction assumptions, external-retirement uncertainty, implicit reservation carryover or multi-successor recovery |
| `0.142.0` | RPC exposes read-only replacement head/candidates/pointer, recovery budget, transfer sides/status, mappings/equations/baseline, high-watermark and typed outcomes | RPC cannot stage/import/reconcile/activate, choose a candidate/mapping, forge an applied/activation receipt, move budget, infer completion or reconstruct old/new permit material |
| `0.143.0` | Model concurrent administrators, different idempotency keys, all outbox/inbox crashes, status response loss, conformance drift, activation races, source-terminal delay, mapping partitions and last-unit Recovery funding | Exactly one replacement can activate; source or destination alone is never interpreted as complete; global and per-lineage conservation hold; every losing candidate is terminally rejected and bounded |
| `0.145.0` | Restore/migrate the absorbing source, replacement coordinator, non-operational/active destination, both transfer sides, pointer and independent predecessor-linked commitment from authenticated snapshots plus hot suffix | Any missing/forked/rolled-back head, budget, mapping, receipt, conformance or pointer leaves both recovery and destination unready; restore cannot replay stage/import/activation authority |
| `0.146.0` | Fuzz fence/head/version/identity uniqueness, source/destination quantities, mapping partitions, baseline/head deltas, outbox/inbox/receipt tags, high-watermark ancestry, Recovery class charges and lock traces | Overflow, duplicate successor, different-material retry, double-live reservation, fabricated inherited charge, source-absence inference, stale activation and rank inversion fail closed |
| `0.147.0` | Audit single-successor ownership, local retirement semantics, post-fence funding, closed recovery writer matrix, portable transfer protocol, reservation safety, global conservation, external rollback detection and source/destination recovery | Zero high/critical split-brain, unfunded-writer, distributed-atomicity, double-capacity, inherited-charge, permit-reuse, rollback or recovery-liveness finding |
| `0.148.0` | Reject legacy imports lacking replacement head/pointer uniqueness, explicit retirement terminal state, Recovery budget/attempt head, both transfer sides, mappings/equations/baseline, external commitment or activation receipt | No shim invents the winner, assumes Pending retirement succeeded, maps old reservations, treats an absent side as complete, rebuilds funding or derives a current pointer |
| `0.149.0` | External pentest targets concurrent replacement creation, candidate/result suppression, stale expected versions, forged quorum/conformance/transfer receipts, outbox/inbox rollback, source disappearance, double-live capacity, unsafe mapping, Recovery exhaustion/borrowing and pointer hijack | Zero unresolved critical/high findings and no waived single-successor, cross-lineage conservation, writer-funding, transfer atomicity, rollback-detection, SoD or activation-authority issue |
| `0.150.0` and `1.0.0` | Demonstrate one replacement winner and exact source→destination transfer across every selected backend/RPC/HA/backup/import/restore profile on the unchanged candidate, including independent transfer-witness outage and source-terminal delay | Production remains blocked until the full model/fault/DR/audit and independent-pentest campaign passes and no supported profile depends on a distributed transaction or implicit recovery authority |
...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Add a predecessor-linked exact-set ReplacementRecoveryCheckpoint; staged/verified/committed-or-orphaned recovery publication with exact non-reference proof; ReplacementRecoveryArchiveReplayHead; bounded restore cursor/state; typed RecoveryHistoricalStateUnavailable; and independently witnessed non-wrapping current-pointer generation/version/digest. Stagger generations as `C_n→P_(n-1)`, `H_n→C_n`, `P_n→H_n`; checkpoint creation stays hot. Add typed checkpoint-create, pointer publish/Reconcile, pointer-query admission/terminalization, restore-cursor advance and restore-completion surfaces with bounded reservations/outcomes. Collapse retirement to one AuthorityRetirement class/charge and one recovery-head advance. Implement the complete replacement adversarial/conformance suite in Phase C now | Generated schema/hash DAG rejects `C_n→P_n` and every cycle. Exact retry/conflict survives compaction; verified Commit installs the replay head/deletes only captured rows. Externally referenced publication cannot orphan. Restore obtains greatest external pointer through its dedicated bounded lane and exact-completes witnessed activation or remains unready. All competing-ID, pointer, crash, typed-outcome, budget, orphan/GC, missing-history, sequential-equivocation and cross-backend cases pass before `0.30.0` |
| `0.30.0` | Export/import staggered checkpoint/replay/pointer generations, checkpoint-create results, publication non-reference proofs, pointer request/signed receipt/Reconcile/query state and restore command results alongside every prior recovery artifact | Round trip every `C_n/H_n/P_n` hot/archived cut, query reservation/settlement and sequential equivocation. Import rejects same-generation back-edges, missing command results, stale non-reference proofs and reconstructed permits; unavailable history remains unready |
| `0.140.1` | Freeze staggered checkpoint/replay/pointer schemas and generated hash-DAG, publication non-reference proof, all typed command/result/conflict/outcome schemas, restore state/cursor and one-class AuthorityRetirement encoding | Schema/reference review rejects `C_n→P_n`, cycles, future signature/result fields, range/dense coverage, pointer predecessor omission, unknown outcome tags, stale proof and ambiguous retirement cardinality |
| `0.140.2` | Freeze checkpoint-create; recovery Stage/Verify/Commit/MarkOrphan/FinalizeGc; pointer publish/Reconcile/query admission/query terminalization; restore-cursor and restore-completion transactions under the extended recovery rank | Fault every boundary. One charge/head advance owns each operation; permit follows durable claim/reservation; external reference blocks orphan; GC proves non-reference/retention/chain preservation; restore completes or remains typed Unready |
| `0.140.3` | Separate recovery checkpoint builder, publisher, verifier, replay-head committer, orphan-proof issuer, evidence/retention/legal-hold owners, pointer requestor, external signer, query reconciler, restore driver/completer and historical-result reader | No actor can create and approve its archive, witness and restore its pointer, self-assert non-reference, erase held evidence, reconstruct a permit, hide a result or select an older readable head |
| `0.140.5` | Retain every staggered checkpoint/publication/replay/pointer edge, hot creation/Commit result, request/receipt/query result, restore cursor/result, external reference and non-reference/hold decision across sequential replacements | Compaction cannot introduce a back-edge, orphan an externally referenced publication, break predecessor closure, revive retired authority, forget reserved work or make unavailable history look unseen |
| `0.140.6` | Freeze failover across checkpoint cut/create, publication/Commit/delete, pointer claim/invocation/Reconcile/query states, orphan proof/GC, restore cursor/completion and later equivocation | Takeover resumes only persisted bounded state, never recreates permits, obtains greatest external heads, rejects stale non-reference proof and either exact-completes the witnessed pointer or stays RecoveryUnready |
| `0.140.11` | Repeat and freeze the already implemented `0.29.0` staggered-DAG, typed-protocol and exact orphan/GC conformance suite for the selected production profile | The decision cannot waive an earlier Phase C failure or introduce first-time semantics/tests. Selected backends pass the unchanged core corpus, generated DAG and independent architecture/security/legal review |
| `0.142.0` | RPC exposes read-only staggered generations, checkpoint creation, publication/reference, pointer request/receipt/query, restore progress and typed unavailable/contradiction/exhaustion status | RPC cannot create/publish/Reconcile/orphan/delete, obtain permits, terminalize queries, advance/complete restore, choose an older head or translate a negative outcome into authority |
| `0.143.0` | Model hot-to-archive and orphan/GC races, `C_n/H_n/P_n` dependency edges, concurrent pointer requests/queries, activation before/after witness, response loss and later equivocations | No reachable cycle, collected reference, recreated permit, lost reservation/result, revived authority or two current pointers; liveness is exact-complete or bounded typed Unready |
| `0.145.0` | Restore the greatest externally witnessed `P_n` and `H_n`, verify `C_n→P_(n-1)` ancestry, replay hot suffix and resume the durable cursor/query protocol before readiness | Locally consistent older heads, same-generation back-edges, missing exact-set/results, stale non-reference proof and incomplete witness all fail closed without old-archive fallback |
| `0.146.0` | Fuzz recovery schemas/hash DAG, predecessor digests, command/result/conflict tags, query state/outcomes/reservations, orphan reference proofs, retention/hold state, captured sets and cursor bounds | Cycle, wrap, ABA, omission, signature inclusion, stale proof, referenced GC, permit replay, result substitution, double settlement/delete and pointer rollback fail closed |
| `0.147.0` | Audit DAG generation, typed command ownership, bounded query/restore work, orphan/GC reference authorities, compaction, external pointer anchoring, sequential restore and single retirement accounting | Zero high/critical cycle, history loss, stale-authority revival, collected reference, untyped operation, permit reconstruction, double-charge/head, rollback or role-confusion finding |
| `0.148.0` | Reject legacy imports lacking staggered generations, checkpoint-create and pointer/query/restore results, non-reference/hold evidence, exact-set lookup or unambiguous retirement | No shim creates a back-edge, uses old archive, guesses a result, defaults a pointer, reconstructs permit/cursor, treats absence as non-reference or interprets two retirement legs |
| `0.149.0` | External pentest targets hash cycles, future signature/result inclusion, pointer publication/query response loss, contradictory evidence, recovery archive rollback, referenced orphan/GC, legal-hold bypass, restore exhaustion and sequential pointer downgrade | Zero unresolved critical/high findings and no waived DAG, exact-protocol, reference-integrity, evidence-retention, pointer-monotonicity, bounded-work, single-writer or early-conformance issue |
| `0.150.0` and `1.0.0` | Demonstrate acyclic exact recovery history, typed bounded pointer/restore protocols and reference-safe retention across every selected backend/RPC/HA/backup/import/compaction profile and multiple sequential equivocations | Production remains blocked until early/core plus schema-DAG/fault/DR/audit and independent-pentest evidence passes with no cycle, referenced deletion, permit reconstruction or old-lineage fallback |
