...
| `0.29.0` | Declare `VIT-INV-062 MigrationImportJobAuthorityState` and `VIT-LAW-009 AtomicMigrationImportActivation` for resumable migration/import | Candidate, barrier and authorization bind exact Pending/NoHistory/NotRequested evidence and policy epochs. Activation commits the chosen evidence plus Healthy fence generation zero under active-coordinator-generation→job→candidate/barrier→authorization→ordered-domain-owner→history-obligation/corruption-fence/lineage-disposition→retention/legal-hold→audit/result/outbox. Every history path follows one fence-before-lineage/budget order and absent fence denies. Missing lineage moves the exact obligation to Fenced. Clearance requires its own independently issued/admitted/revocable/expiring single-use authorization, bounded proof work and greatest-known external anchors; stale/incomplete/unanchored proof leaves permanent fencing and only a new coordinator generation may rebuild. Fenced is an outcome, while the conflict wrapper contains only changed-material Authorization/Revocation/Recovery variants; deterministic CAS/terminal rules remain |
| `0.30.0` | Cross-backend export and import with explicit law trust closure, the same durable budget, exact candidate-to-owner activation and inert history archival | Preserve activation disposition commitments; complete non-wrapping Healthy/Fenced/ClearedAfterRestore lineage; universal lock order; clearance authority/revocation/tombstones/budget/anchor set/results; distinct NoHistory/NotRequested evidence; exact charges; Fenced outcome and three-variant conflict wrapper across backends. Missing/defaulted fence or lineage denies, and bundles below any greatest-known anchor cannot clear. No adapter may invert fence/budget locks, infer Healthy, create capacity, substitute disposition, remotely clear, partially restore, guess expiry or weaken custody |
...
| Version | Goal and deliverable | Release-specific verification / pentest target |
| --- | --- | --- |
| `0.140.1` | Cryptography/time, executable workload identity, exact catalog/topology-receipt authentication, topology authorization, and credential-operation decision | Freeze clearance authorization/revocation authentication and trusted time plus accepted external-anchor authentication profiles. Freeze recovery outcome with Fenced and conflict wrapper with only Authorization/Revocation/Recovery changed-material variants, beside prior CAS/terminal semantics. No database-local assertion alone proves completeness |
| `0.140.2` | Storage topology for law manifests/dependency closure, rollout/topology/authorization state, active catalogs/ratchets, provider state, destination-local migration/import jobs, and activation handoff | Persist activation disposition/evidence commitments, activation-created Healthy fence generation zero, non-wrapping transitions, universal fence-before-budget order, and complete clearance authorization lifecycle/proof budget/anchors/results beside prior lineage/custody/charge/outcome state. Missing fence, stale anchor, partial restore or unsupported absent-row locking refuses the profile |
| `0.140.3` | Human/workload/session, migration-authorization issuer/SoD, and worker-instance identity decision | Freeze activation-disposition approval and separate clearance requestor/approver/operator/issuer/admitter roles. Detector, coordinator, importer, activation/archive/domain actor and ordinary recovery issuer cannot self-clear; anchor quorum cannot be the clearance decision quorum |
...
| `0.140.5` | Privacy, tenant-surface lifecycle, evidence, and residency decision | Preserve every fence generation, clearance authority/tombstone/result, proof-budget charge and external anchor beside NotRequested/waiver/abandonment custody evidence. Erasure cannot make absent mean Healthy, drop a greatest-known anchor, clear a fence or erase the old obligation after successor rebuild |
| `0.140.6` | Deployment/HA invariant-owner, catalog rollout/topology/authorization, recovery, migration/import budget, revocation and activation decision | Preserve disposition commitments, Healthy/Fenced/ClearedAfterRestore generations, universal order, clearance lifecycle/budget/anchors/results, Fenced outcome and three-variant conflict semantics beside all prior artifacts. Failover/restore cannot infer health, roll back an anchor/head, reuse authority, substitute disposition, invert locks or clear partially |
...
| `0.142.0` | Split deployments with separated topology/authorization services and exact topology-issued catalog placement owners | RPC carries exact disposition commitments, fence generation/state, clearance authority/budget/anchor/result, Fenced outcome and three-variant conflict bytes, but never remote clearance or a local-completeness assertion. Peers cannot infer Healthy, reorder locks, roll back anchors/heads, substitute disposition or reinterpret Fenced as conflict |
...
| `0.143.0` | HA atomic work, catalog rollout/topology/authorization, governed execution, cancellation recovery, and fenced migration/import jobs | Model-check universal history locks and race activation disposition/fence genesis, detection, clearance authorization/revocation/expiry/consumption, anchored restoration, successor rebuild, append/recovery/cleanup and failover. Require no deadlock, absent-as-healthy, stale clearance, disposition substitution or conflict/outcome confusion beside prior CAS/charge guarantees |
...
| `0.145.0` | Backup, restore, and disaster recovery | Restore disposition commitments, exact fence generations, clearance lifecycle/budget/anchors/results and old permanent fences before work. Older backups cannot infer Healthy, clear below a greatest-known anchor, roll back heads, reuse clearance authority or substitute the chosen disposition |
| `0.146.0` | Provider-governance, topology anti-replay, rotation/drift, credential-TCB, cancellation and migration/import contention certification | Fault/fuzz lock permutations, missing fence genesis, generation wrap/rollback, clearance lifecycle/revocation, proof-budget exhaustion, stale/forked/unanchored bundles, disposition substitution, Fenced/conflict confusion and all prior lineage/charge/race cases |
| `0.147.0` | Final profile-governance, topology replay-lifecycle, bearer-memory, executor, migration/import-budget/activation, and supply-chain hardening | Audit disposition approval, fence genesis/transitions, clearance authority/proof-work/anchor coverage, successor rebuild retention, Fenced outcome and changed-material conflicts beside prior provenance and conservation |
| `0.148.0` | Compatibility freeze for provider authority, rollout recovery, topology replay lifecycle, and migration/import budget/activation | Freeze disposition commitments, Healthy/Fenced/ClearedAfterRestore generations, universal order, clearance authority/budget/anchors/results, Fenced outcome and three-variant conflicts. Old absent-fence, LineageCorrupt-conflict, unanchored-clearance or unbound-disposition encodings fail closed |
| `0.149.0` | Release candidate and external pentest remediation | Attack disposition substitution, absent-as-healthy, fence/budget deadlock, generation rollback, forged/replayed/revoked clearance, proof exhaustion, stale/unanchored head rollback, remote clearing, successor rebuild erasure and Fenced/conflict confusion beside all prior cases; retest fixes |
| `0.150.0` | Final production-readiness candidate | Prove clean install/upgrade/restore/failover continuity for disposition commitments, exact fence lineage, clearance lifecycle/budget/anchors, permanent-old-fence rebuild, Fenced outcome, three-variant conflicts and every prior recovery artifact |
...
- Backup restoration, event integrity, projection/search rebuild, workflow
  ...
  all greatest-known external anchors; stale/incomplete/unanchored lineage
  remains permanently fenced and successor rebuild retains it. Every transition
  ...
  proof escape, stale/unanchored/partial clearance, anchor/head rollback, split
  charges, guessed expiry tombstones, adapter-specific outcomes/conflicts or
  Fenced-as-conflict,
