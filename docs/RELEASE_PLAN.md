...
| Version | Required context and deliverable | Verification and exit condition |
| --- | --- | --- |
| `0.29.0` | Preserve pinned genesis, AuthorityFenced, `R_first`, `B_EM`, `c_max`, PreparedNonAuthoritative, typed LocalFinalize, post-commit signing, DomainAggregateTarget/ControlOwnerTarget/CanonicalNoAggregateTarget, ProposedEventDescriptor, ClaimAcquisitionPending, no-classified-plaintext payload lifecycle, predecessor chain, independently unique identities with one-sided conflict, CommitReceiptSignerRecoveryRequired and every prior guarantee. Add PayloadReferenceUseClaim proposal/event/shared membership and release proof; journal RecordedTimeAuthorityRatchet with commit-cut semantics; CanonicalNoDomainEventChain for non-domain targets and explicit empty-domain rejection. Freeze `e_claim_max/p_claim_max/n_signer_max/k_signer`; replace the single payload unit with cardinality-safe `P_claim` and expand non-borrowable `S_claim` per signer member/partial/status/terminal route plus quorum import | Race shared reference/transfer/collection/hold/erasure, clock rollback/suspend/failover/restore, domain/no-domain chain substitution and maximum/plus-one payload/signer cardinality. No prior test is removed; every admitted maximum route completes and oversize rejects before claim/permit |
| `0.30.0` | Export/import payload-use membership/transfer/release roots, descriptor/reference cardinality, time-ratchet source/continuity/lower-bound/commit interval, domain-or-no-domain chain kind, signer member/partial/quorum state and `P_claim/S_claim` accounting beside every prior target/payload/identity/attestation/archive state | Round trip every cut without moving classified data. Missing/forked/defaulted use member, unknown transfer, release proof, ratchet continuity, chain kind, signer partial/quorum or reservation is RestoreUnready; import never releases a live reference, regresses time, fabricates a domain chain or underfunds recovery |
| `0.140.1` | Freeze PayloadReferenceUseClaim states/membership/proofs, `e_claim_max/p_claim_max/n_signer_max/k_signer` and `P_claim/S_claim` equations, RecordedTimeAuthorityRatchet and commit-cut interval, CanonicalNoDomainEventChain plus prior target/payload/chain/identity/signer schemas | Review rejects ambiguous shared ownership, local-count erasure, unbounded cardinality, transaction-start-as-commit time, resettable ratchet, empty domain chain, mutable commit meaning and every prior unsafe alias |
| `0.140.2` | Freeze exact reuse/co-located transfer/external transfer response-loss and global release transactions; ratchet/head/commit-cut/chain CAS; per-reference and per-signer-member reservation/settlement/quorum import; dual identity and every prior transaction | Fault every boundary. No early/double release, partial member set, split custody, stale/rolled-back time, false commit interval, partial quorum, one-short budget, distributed transaction, duplicate identity, second append or result rewrite can commit |
| `0.140.3` | Separate payload-use membership/transfer/release, proposal collection/event retention/hold, trusted-time continuity/ratchet, domain-chain/no-domain validator, signer-member/partial/quorum and prior lifecycle/finalizer/attester/signer/archive owners | No actor can erase its own references, reset time, fabricate commit containment/no-domain meaning, count a signer twice, spend another cardinality lane, authorize itself as successor or bypass prior SoD |
| `0.140.4` | Freeze redacted payload-use/transfer/release, ratchet/commit-cut/chain-kind, cardinality/reservation and signer member/quorum status beside prior proposal/command/recovery APIs | API reveals no reference/store/key/signer secrets and never maps transfer unknown, shared use, missing continuity, no-domain, incomplete quorum or RecoveryRequired to absent/released/success |
| `0.140.5` | Retain use membership/transfer receipts/release proofs, ratchet continuity/interval history, chain kind, signer partial/quorum/reservations and prior non-sensitive commitments under residency/retention/hold/erasure policy | Compaction cannot free a shared reference, turn unknown transfer into absence, reset lower bound, change chain kind, discard a needed partial/reservation, free either identity or weaken prior custody/cleanup controls |
| `0.140.6` | Freeze failover during transfer/release, ratchet/time-source/profile changes, domain/no-domain finalization, maximum signer-member recovery and every prior command/attestation/archive/restore boundary | Takeover joins exact membership/status, preserves `P_claim/S_claim`, time lower bound, chain kind and signer set, recreates no permit/partial/plaintext, and unreadies on missing evidence |
| `0.140.11` | Decide per backend: `e_claim_max/p_claim_max`, payload-use store/reference/membership/proof and transfer/release authorities; trusted-time source, atomic commit-cut interval and continuity/rotation profile; EventCommitChain/no-domain encoding; `n_signer_max/k_signer`, member/quorum transport and all prior identity/attester/archive/SLO choices | Architecture/security/privacy/legal/forensic/operations approve one exact profile. A backend without global release proof, atomic time-ratchet/head/commit semantics, cardinality-funded signer recovery, erasable storage or durable identities refuses irreversible claims |
| `0.142.0` | Expose read-only redacted payload-use/transfer/release proof, time-ratchet continuity/commit interval, domain/no-domain chain kind, cardinality consumption and signer member/quorum provenance beside prior lifecycle/identity/RecoveryRequired status | RPC cannot reveal payload/reference/signer secrets, authorize release, weaken hold, reset time, mutate chain kind/set/quorum, free capacity or imply success while any route is pending |
| `0.143.0` | Model shared payload use/transfer/release, `P_claim/S_claim` cardinality, time-ratchet rollback/commit-cut continuity, zero-domain rejection/no-domain value, every signer member/quorum and all prior payload/identity/recovery state machines | No reachable state erases a live reference, strands an admitted maximum batch/quorum, regresses time, mislabels commit cut, accepts empty domain history, fabricates a domain chain, counts duplicate partials or weakens prior guarantees |
| `0.145.0` | Restore payload-use membership/transfer/release, cardinality reservations, ratchet source/lower-bound/commit intervals, exact chain kind, signer member/partial/quorum state and all prior claim replay evidence | Missing/forked evidence remains RestoreUnready; restore never releases references, completes unknown transfer, resets time, maps no-domain to domain, invents partial/quorum/successor, frees IDs or resets capacity |
| `0.146.0` | Canary every surface; load/soak maximum shared-reference, transfer-status, ratchet/failover, dual-index and signer-member/quorum lanes; fuzz cardinality, membership, receipt, time continuity/interval, chain kind, quorum/root/key and prior fields | SLOs pass under ordinary saturation. Plus-one payload/signer profiles, early/double release, rollback, false commit interval, chain substitution, duplicate partial, capacity theft, overflow and one-short reserves fail closed |
| `0.147.0` | Audit Phase A lifecycle inheritance, payload-use ownership/global release, cardinality accounting, journal-time/commit-cut authority, domain/no-domain semantics, independent identities, signer recovery and prior one-stream/attestation/archive/DR controls | Zero high/critical plaintext, shared-release, accounting, time, chain-kind, replay-identity, signer-recovery, immutable-event, authority, evidence or role finding |
| `0.148.0` | Reject imports lacking use membership/transfer/release proof, cardinality maxima/reservations, time-ratchet continuity/commit interval, chain kind, signer member/partial/quorum evidence or any prior recovery field | No shim maps terminal proposal to release, unknown transfer to absent, unbounded batch to funded, start time to commit, missing ratchet to trusted, no-domain to EventCommitChain or partials to quorum |
| `0.149.0` | Pentest shared-reference early/double erasure, transfer response loss, collection/retention/hold races, cardinality exhaustion, clock rollback/suspend/failover/restore, commit-cut forgery, domain/no-domain substitution, signer-member/partial/quorum attacks and every prior payload/identity/attestation/archive/fence attack | Zero unresolved critical/high and no waived payload-use, cardinality, recorded-time, chain-kind, signer-recovery, one-stream, identity, command, attestation, settlement, orphan-GC, evidence or trust issue |
| `0.150.0` and `1.0.0` | Demonstrate one unchanged candidate across selected payload-use, time-ratchet/chain-kind, dual-identity, cardinality-funded signer-recovery, target/process-manager, journal, registry, proposal, attester, signer, archive/GC, HA and restore profiles with every response-loss/substitution/failover/exhaustion case | Production remains blocked until Phase A/B/C conformance, maximum-cardinality progress, canary/privacy, clock/commit-cut, performance/soak, DR, independent audit and pentest pass with no high/critical issue; unsupported payload, time, chain, identity, attestation or signer profiles remain refused |
