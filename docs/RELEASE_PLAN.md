...
| --- | --- | --- |
| `0.29.0` | Add the single-use six-state retirement authorization, genesis-created Operational/RetirementPending/Retired fence and complete retirement terminalization reserve. Replace multi-transaction child re-cost prose with a fixed-snapshot Preflighting→RecostPending→Applying→Complete→Activated campaign plus authenticated abort. Bound the rounded-affine symbolic classifier and reserve complete parent-checkpoint verification capacity before a checkpoint becomes restorable | Race authorization admission/revocation/expiry/consumption; recheck the retirement fence at admission, readiness, dispatch, send start and commit; crash/retry/abort the re-cost campaign at every transfer boundary; prove symbolic/exhaustive equivalence on bounded domains; show an admitted maximum checkpoint can finish from its reservation |
| `0.30.0` | Export/import preserves retirement authorization tombstones, retirement fence and completion reserve, re-cost campaign snapshot/commitment/bucket/transfers/cursor/lease/budget/terminal state, classifier proof/budget and verification reservation exactly | Missing/defaulted state, copied source charges, partial campaign import, a restorable unreserved checkpoint or changed retry material leaves the destination fenced and unready |
| `0.140.1` | Freeze retirement grant authentication/time/quorum/nonce preimages and the symbolic-classifier proof format, arithmetic family and UnknownOrOverBudget→Weakening rule | Independent review proves the retirement issuer cannot self-admit or operate the deployment, and no profile input causes unbounded classification |
| `0.140.2` | Freeze all rows, indexes, locks, checked maxima and atomic parent-bucket/child-transfer/checkpoint transactions for retirement, re-cost and verification reservation | Every selected storage adapter proves the fixed snapshot, whole-delta reservation, resumable abort and complete-verification reservation or declares the profile unsupported |
...
| `0.140.6` | Freeze HA takeover, lease fencing, response-loss reconciliation and restore behavior for all four refinements | Failover resumes the same authorization winner, campaign cursor/bucket and verification reservation without duplicate transfer, free rescan or inferred terminal state |
...
| `0.149.0` | External pentest targets authority theft/replay, missing fence checks, completion-reserve exhaustion, partial re-cost/abort refund fraud, classifier CPU exhaustion and under-reserved maximum snapshots | Zero unresolved critical/high findings and no waived destructive-authority, bounded-work, conservation or recovery issue |
