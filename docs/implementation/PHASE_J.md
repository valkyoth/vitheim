...
`v0.288.0 implementation stop reached. Run pentest for this exact commit.`
...
other engines remain unsupported until separately admitted. `v0.289.0
implementation stop reached. Run pentest for this exact commit.`

## `0.290.0` — Facets, Queues, And Saved Views
Status: planned. Setup: count visibility, view owner/sharing, parameters, pagination, quotas. Goal: reusable operational discovery. Deliverables: facets/view aggregates and APIs. Verification: unauthorized counts, shared-view leakage, parameter injection, stale policy, expensive facets pass. Exit criteria: aggregates disclose no more than records. `v0.290.0 implementation stop reached. Run pentest for this exact commit.`

## `0.291.0` — Permission-Aware Indexing
Status: planned. Setup: document/field labels, query-time checks, revocation propagation, cache keys, snippets. Goal: policy-equivalent search. Deliverables: visibility filters and reindex/revocation protocol. Verification: field/snippet/cache/count leaks, lag windows, role change, tenant pairs pass. Exit criteria: denied data never appears in result metadata. `v0.291.0 implementation stop reached. Run pentest for this exact commit.`

## `0.292.0` — Temporal And History Search
Status: planned. Setup: valid/recorded time filters, event access, retention, historical redaction, bounds. Goal: safe history discovery. Deliverables: temporal query operators/index projections. Verification: historical access bypass, deleted/expired inference, interval edges, huge ranges pass. Exit criteria: historical search honors policy at query time. `v0.292.0 implementation stop reached. Run pentest for this exact commit.`

## `0.293.0` — Relationship And Impact Search
Status: planned. Setup: path grammar, allowed edges, authorization, depth/work, ranking, explanation. Goal: searchable service/evidence graph. Deliverables: graph search port/planner. Verification: path disclosure, cycles, unauthorized intermediate nodes, explosion, ranking manipulation pass. Exit criteria: results include policy-safe path proof. `v0.293.0 implementation stop reached. Run pentest for this exact commit.`

## `0.294.0` — Semantic-Index Interface
...
embeddings. `v0.294.0 implementation stop reached. Run pentest for this exact commit.`

## `0.295.0` — Hosted Semantic-Index Adapter And Isolation

Status: planned only if semantic similarity is intended for a production
profile; otherwise it is explicitly unavailable at `0.378.0`. Blocked until an
...
similarity remains disabled. `v0.295.0 implementation stop reached. Run pentest for this exact commit.`

## `0.296.0` — Embedding Generation And Provenance

Status: conditional planned milestone. Production semantic similarity requires
both this milestone and `0.295.0`; otherwise it remains disabled. Any local
...
provider implementation and activation remain deferred to the `0.340.0`
...
mixed-model rejection. Specify, but do not bypass, the later `0.340.0` gateway
...
and outage cases are owned and retested at `0.340.0`.
...
and absence of an admitted generator keeps semantic search disabled. `v0.296.0
implementation stop reached. Run pentest for this exact commit.`

## `0.297.0` — Knowledge Articles And Runbooks
Status: planned. Setup: draft/review/publish, audience, safe content, versions, expiry, feedback, provenance. Goal: governed reusable knowledge. Deliverables: knowledge aggregate, renderer, contextual suggestions. Verification: stored injection, publication bypass, poisoning, hidden linkage, stale article, search parity pass. Exit criteria: trusted content has approval/version evidence. `v0.297.0 implementation stop reached. Run pentest for this exact commit.`

## `0.298.0` — Unified Search Conformance Suite
Status: planned. Setup: register every search interface in the `0.223.0`
...
verify `0.153.0` lifecycle registration for every lexical, vector, embedding,
...
Exit criteria: no search surface lacks a negative authorization case. `v0.298.0
implementation stop reached. Run pentest for this exact commit.`
