...
Status: planned. Setup: pin Component Model/WIT sources, ABI versions, owned types, compatibility, limits. Goal: stable extension contracts. Deliverables: WIT packages/bindings/test fixtures. Verification: malformed components, ABI/version/type confusion, oversized values pass. Exit criteria: unknown ABI fails closed. `v0.111.0 implementation stop reached. Run pentest for this exact commit.`
...
Status: planned. Setup: unforgeable handle identity, scope, tenant, expiry, delegation, revocation, non-export. Goal: least-authority plugins. Deliverables: manifest/evaluator and opaque secret broker. Verification: forging, scope escalation, extraction, reuse, stale/revoked handles pass. Exit criteria: plaintext secrets never enter guest memory unless explicitly unavoidable/reviewed. `v0.114.0 implementation stop reached. Run pentest for this exact commit.`

## `0.115.0` — Resource Metering
Status: planned. Setup: per-plugin/tenant memory, CPU/fuel, wall time, network, output, concurrency quotas. Goal: resist plugin denial of service. Deliverables: meters, cancellation, quota evidence. Verification: limit bypass, amplification, slow output, connection storms, accounting overflow pass. Exit criteria: exhaustion isolates the plugin. `v0.115.0 implementation stop reached. Run pentest for this exact commit.`
...
Status: planned. Setup: sender verification, signed webhooks, URLs, redirects, headers, action links, replay, quotas. Goal: common secure integrations. Deliverables: connectors and recipes. Verification: SSRF, header/HTML injection, spoofing, replay, redirect/DNS rebinding, notification abuse pass. Exit criteria: remote input has no implicit command authority. `v0.118.0 implementation stop reached. Run pentest for this exact commit.`
