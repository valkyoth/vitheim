...
non-clean tenant-local residual handoff/lineage/delivery cuts, target-slot
takeover authority, reserved permanent-guard completion, archive-safe cleanup,
partition-fenced evaluation, and current-state guard supersession wait for
`0.51.5–0.51.18` after
...
Dedicated
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationChunkManifestV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationStateV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationReceiptV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationProofBudgetV1`
and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationVerificationCursorV1`
close publication ownership rather than reusing the reserve-settlement archive
implicitly. The immutable manifest binds publication/candidate/checkpoint IDs,
proposed canonical checkpoint and replay-head sequences/digests, exact captured
rows, canonically indexed chunk hashes, entry/byte counts, exact-set/result
roots, archive location digest, integrity-key/encoding epochs and predecessor
replay head. The proof budget sets checked maxima for bytes, entries, chunks,
proof depth, allocations, decoding, hashing, verification work, continuations
and time; a durable cursor advances within those bounds. Verification
reconciliation and orphan cleanup use a non-borrowable Recovery slice reserved
by Begin/Replan.

The closed receipt states are Staged, Verified, ConsumedByCommit,
OrphanGcEligible and Collected. Only
`StageMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublication`,
`VerifyMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublication`,
`CommitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublication`,
`MarkOrphanMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublication`
and
`FinalizeGcMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublication`
with their typed results, conflicts and AttemptClosed outcome may mutate them.
The closed results are
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationStageResultV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationStageConflict`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationVerifyResultV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationVerifyConflict`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationCommitResultV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationCommitConflict`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationMarkOrphanResultV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationMarkOrphanConflict`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationGcResultV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationGcConflict`
and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchivePublicationAttemptClosed`.
Stage persists the immutable manifest and receipt before upload; Verify resumes
the bounded cursor and reaches Verified only after independent full manifest/
chunk/root/epoch verification. CAS makes Verified→ConsumedByCommit race
exclusively with Verified→OrphanGcEligible. MarkOrphan requires authenticated
proof that the candidate cannot become the successor because its bound plan/
attempt is terminal or another replay head won; it cannot touch a high-
watermarked candidate. FinalizeGc rechecks replay-head and high-watermark non-
reference and records Collected only after authenticated exact deletion.
Unknown deletion remains OrphanGcEligible. Publishers and storage adapters are
evidence-only: they cannot mutate lifecycle rows, install a replay head, delete
...
Only after that exact Prepare transaction commits may the returned permit
invoke the high-watermark authority for generation `g`; no other guard state or
generation can produce or use it.
...
AbortedDefinitelyUnwitnessed(`g`) or
PreparePending(`g`)→AbortDrainPending(`g`)→Witnessed(`g`)→Committed(`g`).
Committed and
...
Unprepared(`g + 1`) as the sole current guard. The adapter never
mutates this row:
...
The first terminal Reconcile imports Witnessed or completes a permanently
sealed abort and consumes exactly one witness-disposition writer charge with
capacity state, attempt-set mutation/head and stored result;
...
An ordinary current-status “not witnessed” response is never terminal negative
evidence. Only
`BeginMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortDrain`
may CAS PreparePending(`g`)→AbortDrainPending(`g`), deny new query admission
and persist
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortSealClaimV1`
plus
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortDrainCompletionReservationV1`
before returning one process-local seal permit. The completion reservation
binds the final sealed-abort disposition charge/head advance; the existing
future ArchiveFinalize charge needed if WitnessWon; the complete configured
seal-status admission/terminalization envelope; result/audit/outbox bytes and
work; existing-query drain and concurrency-settlement work; one
seal-reconciliation-exhaustion terminalization; and authority-equivocation
fencing capacity. If any mandatory dimension cannot be reserved,
BeginAbortDrain is no-write and returns no seal permit. Timeout, cancellation,
disconnect, worker loss, takeover, Replan and restore cannot release, reassign
or borrow any leg. Only a terminal route consumes its applicable legs and
closes unused mutually exclusive route legs through
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortDrainCompletionReservationSettlementV1`;
its immutable reservation and settlement evidence remain hot until
authenticated archive coverage.

The winning admission stores and returns canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortDrainAdmissionResultV1`
while its process response envelope additionally carries non-durable
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortSealPermitDeliveryStatusV1::SealPermitDelivered`;
the persisted result excludes delivery status and permit material. Same-
material retry returns the stored result with response status `StatusOnly` and no permit,
while changed material returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortDrainAdmissionConflict`.
The initial external invocation has typed
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortSealInvocationResultV1`
outcomes SealWon, WitnessWon or Unknown. After that permit is returned or may
have invoked, only
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortSealInvocation`
may import a returned outcome. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortSealInvocationReconciliationResultV1`
has only SealWonDeferredNoWrite, SealWonDispositionCommitted,
WitnessWonDispositionCommitted, ContradictionFenceCommitted and UnknownNoWrite;
committed variants co-commit the already counted route disposition/fence
charge and changed material returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkAbortSealInvocationReconciliationConflict`.
SealWon without every final-drain predicate and Unknown are no-write; recovery
later rediscovers status through the bounded seal-query lane, so no extra
unaccounted writer exists. After the original permit may have invoked,
every recovery path is query-only and no actor may reseal.
That permit invokes the
linearizable external
`SealCapacityArchiveWitnessProposalDefinitelyUnwitnessed` operation with exact
tenant/lineage, generation, expected predecessor and proposal digest. The
authority CASes submission versus sealing: a winning seal writes a permanent
durable authority-side proposal-rejection tombstone and rejects every late
submission for that proposal; a prior positive witness makes sealing lose and
returns its positive identity. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessProposalDefinitelyUnwitnessedSealReceiptV1`
authenticates the winning tombstone and is independently discoverable by exact
proposal identity during reconciliation, failover and restore.

Lost or uncertain seal responses use a distinct bounded protocol rather than
the ordinary PreparePending witness-query lane. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessSealReconciliationBudgetV1`
is funded by the abort completion reservation and fixes hard maxima for calls,
request/response bytes, verification work, trusted elapsed time, concurrent
workers and durable backoff. Only
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQuery`
may admit one
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryAttemptV1`
while the exact generation is AbortDrainPending. It atomically consumes one
seal-query admission charge/budget leg, creates a stable ID from the bounded
writer-charge sequence, persists Admitted→PermitIssued, and reserves one
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryTerminalizationReservationV1`
covering its terminal charge/head, result/audit/outbox bytes/work and
concurrency settlement before returning one process-local, non-serializable,
unreconstructable query permit. Insufficient budget/reservation is no-write and
returns no permit. It returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryAdmissionResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryAdmissionConflict`.

Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryAttemptStateV1`
has only Admitted, PermitIssued, InvocationReturned and ResponseImported. Its closed
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryAttemptOutcomeV1`
outcomes are SealWon, WitnessWon, Unknown, TransportFailure, DeadlineExceeded
and ContradictoryEvidence. Only
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQuery`
authenticates and imports one outcome, consumes that attempt's terminalization
reservation, advances its state, stores
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryReconciliationResultV1`
or its material-specific
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryReconciliationConflict`,
and writes one
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealStatusQueryAttemptConcurrencySettlementV1`
exactly once. SealWon may co-commit the sealed-abort disposition; WitnessWon may
co-commit
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealLostToWitnessResultV1`;
changed WitnessWon route material returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealLostToWitnessConflict`.
ContradictoryEvidence may co-commit the authority-equivocation fence and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityEquivocationResultV1`;
changed contradictory material returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityEquivocationConflict`.
SealWon co-commits AbortedDefinitelyUnwitnessed only if every final-drain
predicate below holds; otherwise it stores authenticated seal evidence and
remains AbortDrainPending. WitnessWon atomically consumes the witness-
disposition route leg and CASes AbortDrainPending→Witnessed with the
SealLostToWitness result; ArchiveFinalize later requires the ordinary complete
query drain and commits the already reserved successor. ContradictoryEvidence
fences without either terminal generation transition.
Unknown/transport/deadline terminalize only the attempt. Trusted-time profile,
maximum uncertainty, continuity and epoch bind elapsed use and backoff.
Exhaustion atomically consumes its pre-reserved terminal leg, writes
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealReconciliationBudgetExhausted`,
with changed-material
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealReconciliationBudgetExhaustedConflict`,
leaves the lineage fenced/unready in AbortDrainPending and performs no external
traffic.

AbortDrainPending(`g`) may reach AbortedDefinitelyUnwitnessed(`g`) only when
Reconcile authenticates that permanent seal receipt, proves every query attempt
for `g` and every seal-status attempt is ResponseImported, all terminalization
reservations and concurrency settlements are consumed, and no positive witness
receipt exists locally or at the authority. The same transaction consumes the
sealed-abort route legs, writes the immutable abort tombstone and canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealedAbortReconciliationResultV1`,
whose changed-material retry is
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkSealedAbortReconciliationConflict`,
advances the checked guard generation and installs Unprepared(`g + 1`) bound
to the unchanged replay head and external watermark. No proposal, request,
submission/seal claim, operation or query identity from `g` is reusable in
...
exhaustion and every result/audit/outbox. Its non-releasable
EvidenceMaintenance partition funds post-fence checkpoint/archive
Stage/Verify/Commit, fence-high-watermark publication/query/reconciliation,
restore verification/evidence export and retention/hold strengthening even
after AuthorityChanging is denied. The reservation for every pointer query
terminalization includes the maximum fence artifact/evidence/result/audit/
outbox leg before its permit is issued. Ordinary
...
absence is never equivalent.

Deployment trust-root initialization creates a separate, non-borrowable
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityRecoveryBootstrapReserveV1`
before `P_0`. It funds exactly the bootstrap writer, genesis/receipt, initial
recovery/replay heads, guard, result, audit and outbox. It is owned by the
deployment-bootstrap capacity root and cannot depend on, borrow from or credit
the fence-keyed Recovery budget or replacement head that this bootstrap makes
possible. Insufficient or corrupt reserve is no-write. Bootstrap returns only
...
DAG test constructs every selected schema and rejects cycles before an adapter
may pass conformance.
Only
`CreateMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryCheckpoint`
creates `C_n`; it returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCheckpointCreationResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCheckpointCreationConflict`.
The creation charge, result, audit/outbox and successor recovery-head mutation
are after `r_n`, excluded from `C_n` and remain hot until a later checkpoint,
exactly as recovery archive Commit remains outside its captured cut.

Fenced EvidenceMaintenance uses a distinct staggered evidence DAG. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceCheckpointV1`
(`E_m`) binds the complete guard/fence/bundle/result/custody set and only the
greatest predecessor recovery anchor `A_(m-1)`; the first `E_1` binds the
greatest healthy `P_n`, and later `E_m` may bind only `F_(m-1)`. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceArchiveReplayHeadV1`
(`J_m`) commits `E_m`; only then may `F_m` commit `J_m`. The evidence DAG is
therefore `E_m→A_(m-1), J_m→E_m, F_m→J_m`. Only
`CreateMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryFenceEvidenceCheckpoint`
creates `E_m` and returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceCheckpointCreationResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceCheckpointCreationConflict`.
...
Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryArchivePublicationNonReferenceProofV1`
is required before MarkOrphan. Under the full recovery rank, MarkOrphan locks
and rechecks that the publication/manifest/checkpoint is referenced by neither
the recovery replay head nor any transfer high-watermark, current-pointer
high-watermark, activated replacement head, live restore cursor, signed
activation receipt, evidence-custody record or active retention/legal hold.
An externally signed `P_n` that binds `H_n` makes its publication permanently
ineligible for orphaning: recovery must exact-commit that publication or remain
unready. FinalizeGc accepts only a current authenticated non-reference proof,
expired retention, no legal/evidence hold and proof that deletion preserves
the predecessor chain and every still-referenced exact result. Absence,
timeout, stale snapshots and local non-reference alone never authorize
orphaning or collection.
...
version/digest, recovery replay head and the next shared recovery-anchor
sequence tagged HealthyPointer.
...
Pointer publication has its own authority and outcome vocabulary. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkPublicationRequestV1`
contains `P_n`'s semantic preimage but no future signature or result digest;
the independent authority returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkSignedReceiptV1`.
Only
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermark`
may atomically persist the request/single-use submission claim and return one
unreconstructable process-local publication permit, with
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkPublicationResultV1`
or its
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkPublicationConflict`.
Every external response, including response loss discovered later, enters only
through
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkPublication`,
returning
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkPublicationReconciliationResultV1`
or
...
`SignedNoFenceAtPointer` is never a current-status negative. The independent
rollback-resistant authority is exposed only through canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryPointerObservationRegistryAuthorityPortV1`
under
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryPointerObservationRegistryConformanceProfileV1`.
The profile requires tenant/deployment structural partitioning; linearizable
expected-frontier CAS; monotonic non-wrapping epochs; non-equivocation and
signed request/receipt/result binding; durable failover and read-after-write;
key rotation/distrust with historical verification; exact-set frontier and
compaction retention; an independent failure domain; bounded
admission/concurrency/timeout/query work; and adapter refusal when any
guarantee is unavailable. A database adapter, cache or local replica is never
this port.
...
owns every pointer-query intent and operational claim. Canonical non-wrapping
...
and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryPointerObservationRegistryAuthorityStateV1`
has the closed control states OperationalClaimsOpen(`e`),
ObservationDrainPending(`e`, intent), QueryPermitMayIssue(`e + 1`, intent),
ObservationBlocked(`e + 1`, intent, reason), FenceAnchorPending(`e + 1`,
intent), AuthorityFenced(`e + 2`, `F_m`), RestoreActivationPending(`e`,
token) and RestoreUnready(`e`, reason). Observation registration first CASes
OperationalClaimsOpen→ObservationDrainPending and closes new operational
claims. Existing claims may only settle or expire under the bound trusted-time
profile; only definitely unredeemed claims may expire, while a redeemed or
outcome-unknown effect remains live until authenticated terminal evidence.
After zero live claims, zero unsettled concurrency and an authenticated
complete claim frontier, the registry alone advances the epoch and enters
QueryPermitMayIssue. No local query permit precedes that state.

The registry transition table is total and fail-closed:

- QueryPermitMayIssue→OperationalClaimsOpen at the already advanced epoch
  requires an authenticated ConsistentReceiptObserved terminalization for the
  exact intent and sealed frontier.
- Query terminalization that remains RegisteredUnresolved, or becomes
  LocallyExhaustedUnresolved or ExternallySealedPermanentlyUnresolved, enters
  ObservationBlocked. That state denies every operational claim and restore
  token. For RegisteredUnresolved or LocallyExhaustedUnresolved only, a later
  authenticated consistent terminal receipt may reopen OperationalClaimsOpen
  when the exact successor frontier is sealed; contradictory evidence instead
  enters FenceAnchorPending. An ExternallySealedPermanentlyUnresolved reason
  never reopens as consistent and can only remain blocked or strengthen to
  FenceAnchorPending on contradiction.
- Contradictory or LateContradictoryEvidenceObserved enters
  FenceAnchorPending and denies operational claims, restore and no-fence
  assertions while `E_m→J_m→F_m` completes.
- Exact fence-anchor terminalization advances the epoch and enters
  AuthorityFenced. AuthorityFenced is absorbing: only evidence-preserving or
  evidence-strengthening self-transitions exist, and no receipt, restore,
  rotation, operator action or later consistent observation can reopen it.
- RestoreActivationPending enters OperationalClaimsOpen only after the exact
  registry confirmation and authenticated local-finalization receipt described
  below. Rejection, unavailability, outcome uncertainty, missing history or
  changed frontier enters RestoreUnready. RestoreUnready issues no authority;
  only a new fully validated token attempt from an unchanged complete frontier
  may re-enter RestoreActivationPending.

Every state predicate binds tenant/deployment, epoch, intent/token, predecessor
frontier, greatest pointer/fence anchors, claim-history replay head and all
authority/key/distrust/encoding epochs. Local Healthy, Fenced or Unready is a
mirror and can only be equally or more restrictive; local rollback cannot
reopen the external ObservationBlocked, FenceAnchorPending, AuthorityFenced or
RestoreUnready state.

Every authority-bearing command, queue/projection claim and external-effect
attempt first obtains canonical short-lived
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimV1`,
its signed receipt and dedicated terminalization reservation from the registry.
The claim binds tenant/deployment, exact epoch/frontier, command/effect kind,
stable operation and idempotency IDs, request/material digest, one canonical
target, claimant/boot continuity, issued/not-before/commit-before time, maximum
uncertainty and every authority/key/distrust/encoding epoch. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimTargetV1`
is a closed exactly-one union:

- DomainAggregateTarget { tenant, stream kind, aggregate ID, expected stream
  version, exact expected predecessor envelope digest };
- ControlOwnerTarget { invariant owner ID, expected owner version }; or
- CanonicalNoAggregateTarget for an operation such as an external-effect
  attempt that mutates no aggregate or control owner.

A command cannot supply a list or set of targets. DomainAggregateTarget may
propose multiple ordered events, but every descriptor and final envelope must
belong to that one tenant/stream-kind/aggregate-ID stream. ControlOwnerTarget
...
domain-separated CanonicalNoEventPredecessor digest; row absence or an
all-zero caller value is never predecessor evidence.
...
Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimedCommandExecutionV1`
owns the entire user-visible multi-stage command lifecycle. Its closed states
...
Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimedCommandExecutionIdentityRegistryV1`
owns two domain-separated exact maps in each tenant/deployment scope:
command ID→execution digest and idempotency ID→execution digest. Active hot
rows have separate database unique constraints for each ID; the authenticated
predecessor-linked identity-registry root plus hot suffix remains the
authoritative uniqueness source after execution compaction.

Before admitting any claim, command admission locks one identity-registry root
and performs two independent lookups—tenant/deployment plus command ID, and
tenant/deployment plus idempotency ID—across that exact root/hot suffix and
active command/claim/proposal state. Neither ID is merely part of a composite
key or reusable namespace. Both absent may atomically insert both map claims,
advance the root and create one execution; both present may join only when they
resolve to that same execution row. Root CAS prevents a concurrent hot/archive
cross-pair insert from using stale non-membership. One present while the other
is absent, or both resolving to different executions, returns typed
OperationalAuthorityClaimedCommandExecutionHistoricalIdentityConflict and
writes no claim. Identical request, target and descriptor material joins the
existing execution and returns its typed state/status/result digest without a
new claim or permit. Changed request, target, descriptor or command bytes
returns a conflict. Checkpoint/archive exact membership preserves both
independent identity claims and the continuous registry root after hot-row
deletion; the current root is never inferred from an archive maximum.
Unavailable proof/history is RestoreUnready, never “not found.”

The execution row advances under expected-version CAS with the claim,
proposal, registry disposition, local commit record, signing attempt and
registry settlement that justify each edge. The local commit may persist its
committed result bytes, but retry/status remains SignaturePending,
CommitReceiptSignerRecoveryRequired or RegistrySettlementPending and cannot
report terminal Succeeded until registry settlement is reconciled. Read-only
`GetMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimedCommandExecutionStatus`
returns this typed status and no bearer material.

ProposalPrepared is the last cancellable state, and only before any registry-
disposition publication permit is issued. A typed
CancelOperationalAuthorityClaimedCommandBeforeRegistryDisposition transaction
locks the command execution, claim and proposal, proves the claim is
definitely unredeemed, obtains the registry's authenticated terminal claim
settlement and then enters Rejected(CancelledBeforeRegistryDisposition).
ClaimAcquisitionPending cancellation can only request stop and must reconcile
to Rejected/ExpiredDefinitelyUnredeemed before terminalization; it cannot infer
non-issuance. RegistryDispositionPending and every later state reject
cancellation and must reconcile forward. No cancellation, timeout or operator
action can delete or compensate an already committed immutable event.
...
digests, authorization/fencing/key epochs, exactly one
OperationalAuthorityClaimTargetV1 and, for DomainAggregateTarget only, an
ordered non-empty sequence of canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimProposedEventDescriptorV1`.
ControlOwnerTarget and CanonicalNoAggregateTarget require an empty descriptor
sequence, bind their canonical control/effect material digest and produce
canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimCanonicalNoDomainEventChainV1`
instead of an EventCommitChainV1. A zero-descriptor DomainAggregateTarget is
invalid; the no-domain-chain value cannot be substituted for a domain chain.

Each ProposedEventDescriptorV1 contains every proposal-owned semantic
event-envelope field: tenant/deployment; event type; schema ID/version;
stream/aggregate kind and aggregate ID; command/idempotency, causation and
correlation identities; valid/occurrence/effective-time values; accepted
recorded-time policy, trusted-time profile/epoch and maximum uncertainty;
sensitivity/classification/custody policy; authorization/fencing epochs; and
the descriptor encoding/integrity-chain version. Its payload is canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimProposedEventPayloadV1`,
a closed union:

- InlineNonSensitive { canonical bytes, plaintext digest }, permitted only
  when schema and policy classify the complete value non-sensitive;
- EncryptedPayloadReference { opaque reference, Phase A hash-authority
  plaintext commitment, ciphertext digest, key owner/epoch, residency,
  retention, legal-hold and custody policy };
- ExternallyErasablePayloadReference with the same bindings plus external
  erasure authority/profile; or
- ErasedPayloadTombstone { original reference/digests, erasure disposition,
  evidence digest and retained non-sensitive lifecycle bindings }.

For classified or low-entropy values the plaintext commitment is keyed and
domain-separated under the Phase A hash-authority profile; an unkeyed
dictionary-testable hash is prohibited. Classified material is never inline.
Proposal rows, registry requests and
receipts, commit attestations, signer requests, audit/status/result rows,
...
live until its Phase A reconciliation proves one disposition. No route may
materialize an untracked plaintext copy. Rejected or
cancelled proposals retain immutable ErasedPayloadTombstone evidence after
policy/hold-authorized crypto-erasure. Checkpoint/archive/export enforces the
original residency, retention, legal-hold, key-custody and external-copy
constraints rather than treating the descriptor root as permission to move
data.

Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimPayloadReferenceUseClaimV1`
owns reference use independently of immutable proposal or event evidence. It
binds one opaque reference/digest/custody identity; the exact proposal and
descriptor-index members; every committed event-envelope member; current
payload-store authority/epoch; residency, retention, hold and erasure policy;
transfer attempt/receipt identity; and predecessor-linked exact membership
root/count. Its closed states are ProposalCustodyOnly, TransferPending,
TransferOutcomeUnknown, SharedProposalAndEventCustody, EventCustodyOnly,
ReleaseEvaluationPending, ReleaseEligible and ErasedTombstone. Several
descriptors or events may share one claim; cardinality is charged by distinct
references, while its membership root prevents a shared use from being
mistaken for another transferable copy.

Preparation atomically installs the proposal members. Exact-reference reuse
adds committed-event members and enters SharedProposalAndEventCustody without
duplicating bytes. A co-located transfer changes custody and installs all
event members in the local finalization transaction. An external transfer
uses stable Admission→one-shot Publish→Reconcile/status/terminalization state;
lost response remains TransferOutcomeUnknown, retains the proposal use claim,
treats both possible custodians as live and blocks finalization, collection,
release and erasure. An exact duplicate
receipt joins its stored disposition; changed reference, member set, custody
or receipt conflicts. After an atomic domain batch either every intended event
member exists or none does; there is no partial reference release on batch
failure.

Proposal collection may remove only its exact proposal members. Release
evaluation then requires authenticated non-reference across every live
proposal, committed event and retained hot/checkpoint/archive membership,
transfer attempt or unknown outcome, restore cursor, retention authority and
legal hold. Event retention or hold therefore dominates proposal terminality.
Only the lifecycle owner may advance ReleaseEligible to policy-authorized
...
binds the exact expected predecessor envelope digest at its expected stream
version. Under canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimEventCommitChainV1`,
finalization obtains one backend-authoritative trusted-time interval satisfying
the signed policy/profile/uncertainty; event zero names the target predecessor,
and each later event names the immediately preceding newly accepted envelope
digest in descriptor order. The descriptor therefore excludes only event ID,
stream version, journal offset, transaction position, actual recorded-time
interval and derived integrity predecessor/digest, all governed by this closed
algorithm. Registry confirmation signs the ordered descriptor root, exact
target and proposal digest, not a payload-only body root.

Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimRecordedTimeAuthorityRatchetV1`
is scoped to the journal authority and binds trusted-time source identity,
continuity/boot/fence evidence, profile/key epoch, prior authenticated lower
bound, last accepted conservative commit interval, journal head/position and
ratchet predecessor digest. The selected backend profile must allocate an
interval that conservatively encloses the authoritative transaction commit
linearization cut—not request receipt, transaction start or journal
allocation—and atomically advance the nondecreasing lower-bound ratchet with
the journal append. Journal position remains the ordering authority and
intervals may overlap, but a source result below the authenticated lower bound
or without commit-cut containment fails closed.

Clock step, suspend, process/database restart, leader change, restore and time-
profile rotation cannot reset this ratchet. A new source/boot/profile may
continue only with authenticated continuity proving a lower bound at least as
...
their dedicated checkpoint/retention/collection lifecycle. Readers, readiness
and downstream writers cannot interpret it as committed authority.
...
structurally couples ordinary admission to observation drain capacity. Signed
per-tenant, per-deployment and global maxima cover live plus outcome-unknown
claims, with separate command, external-effect, queue and projection
sublimits. At every scope:

`live_claims + outcome_unknown_claims <= admitted_claim_capacity <= c_max`.

Every admission atomically reserves its claim row, terminalization/result
route, drain-verification work, history bytes and exact concurrency member
before issuing a permit. If the complete future drain/settlement route is not
available, admission rejects without a claim or permit. The registry admits at
most one live claim for the same exact canonical target and expected stream/
owner version; CanonicalNoAggregateTarget is uniquely sequenced by operation/
material digest. An exact operation/material retry joins the stored claim and
changed target/material conflicts. Recovery drain and terminalization queues have priority over new
ordinary work, tenant scheduling is fair, and per-tenant pressure cannot spend
another tenant's or the global recovery reserve. Capacity is not released for
redeemed or outcome-unknown effects, RegistryConfirmed preparations awaiting
local finalization, or lost finalization receipts until authenticated terminal
evidence proves their exact disposition.

The prepared digest is published to the registry through one-shot
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimDisposition`
and enters local state only through
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimDisposition`.
...
confirmation; authorization/fencing/key/distrust epochs; the exact canonical
target, its single expected stream/owner version and domain predecessor
envelope digest; command/material/descriptor-root digest; payload-reference
use-claim membership/custody/transfer/erasure state; and accepted recorded-time
policy plus journal-authority time-ratchet predecessor.
DomainAggregateTarget then atomically appends one or more new immutable events
to that one exact stream plus the committed command-result record, audit intent
and outbox records. ControlOwnerTarget atomically mutates only its one named
invariant owner plus result/audit/outbox. CanonicalNoAggregateTarget appends no
domain/control mutation and records only its bounded attempt/result/audit/
outbox state. A transaction with descriptors for two aggregate IDs or any
target-kind mixture is rejected, never split or partially applied.

For DomainAggregateTarget, the journal allocates fresh event IDs, consecutive
stream versions, journal offsets and transaction positions only in this
transaction; obtains the policy-compliant conservative recorded-time interval
that encloses its commit cut; atomically advances RecordedTimeAuthorityRatchetV1;
and derives the complete predecessor/envelope-digest chain under
EventCommitChainV1 from the target's expected predecessor. Every proposal-
owned field must be byte-for-byte and semantically equal to its registry-
confirmed ProposedEventDescriptorV1.
Finalization cannot change event/schema ID or version, aggregate kind/ID,
tenant/deployment, command/causation/correlation identity, valid/occurrence/
effective time, recorded-time policy/profile/uncertainty,
sensitivity/classification/custody or payload representation while wrapping
the confirmed descriptor. InlineNonSensitive bytes are reused exactly;
encrypted/externally erasable references are reused or custody-transferred in
the supported local/receipt-backed transaction while atomically installing
every PayloadReferenceUseClaimV1 event member without plaintext persistence.
Actual recorded time and
predecessor values must be exactly those produced by the closed algorithm, not
caller, proposal-store or adapter substitutions. The committed result exists as nonterminal command-execution
evidence until registry settlement; only Succeeded is the terminal successful
command receipt. Finalization never mutates, toggles, promotes or relocates a
proposal row.
...
target and ordered descriptor root; committed transaction/journal identity;
for each descriptor index, the exact descriptor digest mapped to allocated
event ID, stream version, journal offset and transaction position, actual
recorded-time interval, integrity predecessor and complete accepted-envelope
digest; the initial expected predecessor and final batch head; payload
reference-use membership root plus custody-transfer or tombstone digest;
for domain work, RecordedTimeAuthorityRatchetV1 predecessor/successor,
source/profile/continuity and commit-cut containment evidence; for control/no-
aggregate work, CanonicalNoDomainEventChainV1 and an explicit no-ratchet-
advance marker; result/audit/outbox digests; authorization/fencing/key epochs;
and local backend continuity. It is
immutable evidence that the local commit occurred and that only the frozen
finalizer-owned fields were derived or allocated under the selected
domain-or-no-domain chain rule,
but is not yet a registry-acceptable signed receipt.
...
the final local CAS nevertheless checks the one target's expected stream/owner
version exactly.
...
material, availability/response-loss behavior and separation of duties.
Because a remote HSM/KMS cannot query arbitrary Vitheim databases, commitment
is proved through canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimLocalFinalizationCommitAttestationPortV1`
under
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimLocalFinalizationCommitAttestationConformanceProfileV1`.
The selected profile is exactly one of:

- TrustedSignerAdapterAuthoritativeRead: a trusted signer adapter uses a
  read-only authoritative-backend port to reread the commit record and verify
  the exact event/control mutation, command result, audit intent and outbox
  tuple before invoking the HSM/KMS;
- BackendAuthenticatedCommitAttestation: the database adapter emits a
  profile-specific authenticated, non-replayable commit attestation that the
  signer adapter can validate without querying the database; or
- HostedRuntimeCommitAttester: the explicitly selected in-process profile
  trusts the fenced hosted runtime as commit attester and records that trust
  boundary in deployment conformance evidence.

Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimLocalFinalizationCommitAttestationV1`
binds tenant/deployment, attestation profile/version, backend identity and
continuity/boot/lease/fence, transaction ID, commit-record digest, target,
descriptor-to-envelope root, journal/control version, result/audit/outbox root,
attester identity/key epoch, trusted-time interval and anti-replay nonce. The
signing request binds the exact attestation digest/profile and signer/key
epoch. Caller-supplied record bytes, a replica/cache read, transaction-return
success or an unsigned outbox is never commit proof. A profile without
trustworthy post-commit attestation refuses independent-signing claims. No
profile signs a pre-commit proposal.
...
preserves historical verification; distrust prevents new signatures from that
signer/key and makes unknown pending work fail closed without invalidating
already valid historical receipts. An in-process signer is supported only as an explicitly selected,
independently tested conformance profile and still signs after commit through
this same state machine and HostedRuntimeCommitAttester evidence; it is never
an implicit transaction capability.

Claim admission additionally requires a selected, passing canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimLocalFinalizationCommitReceiptSignerRecoveryProfileV1`.
Exactly one recovery option is frozen per backend/deployment:

- ThresholdRedundantSignerSet: a provisioned threshold or redundant signer set
  has `1 <= k_signer <= n_signer <= n_signer_max`, independent canonical
  members, a fixed acceptance threshold, shared record semantics, tested
  failover and enough currently healthy members before claim admission;
  or
- RootAuthorizedSuccessorSigner: an independently governed pinned root may
  authorize one successor signer/key lineage to sign the already committed
  record after authenticated disposition of the failed/lost/distrusted
  primary signer; its recovery-member cardinality is canonically
  `n_signer = k_signer = 1`.

Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimLocalFinalizationCommitReceiptSignerRecoveryV1`
binds the original claim, proposal/descriptor root, command execution,
immutable commit record, exact EventCommitChainV1 or
CanonicalNoDomainEventChainV1 root, commit attestation,
primary signer/key/attempt and authenticated failure or distrust disposition,
recovery profile/set/quorum, root authorization if applicable, successor
signer/key lineage, every authority/key/distrust epoch and recovery
attempt/status/result. Each member owns a stable attempt, response-loss lane,
partial-signature evidence, concurrency settlement and terminalization.
Canonical member ordering and one separately reserved quorum-combination/
import operation verify exactly `k_signer` distinct accepted members from the
frozen set; duplicate members or partials cannot count twice. Every accepted
primary, threshold-member or successor receipt signs the identical commit-
record digest and registry settlement meaning. A late primary receipt may join
that same meaning but cannot create a different event, result or settlement.

If the selected signer can no longer complete, signing reconciliation advances
SignaturePending→CommitReceiptSignerRecoveryRequired and preserves the
committed claim, result, attestation, payload reference and all capacity.
Only the selected threshold set or a verified root authorization may advance
RecoveryRequired→recovery signing→SignaturePending with a recovered receipt.
Recovery never appends events, changes payload custody/recorded time/
predecessor chain, rewrites command result, reacquires a claim, resets
idempotency or releases/replenishes capacity. If no valid threshold or
successor exists, CommitReceiptSignerRecoveryRequired remains nonterminal,
visible through redacted status, blocks observation drain and keeps authority
fail-closed. A deployment unable to pre-provision one recovery profile refuses
irreversible claim admission.
...
LocalFinalizationReceiptStatusQuery Admit/Reconcile state with stable IDs,
...
followed by
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimDispositionStatusQuery`;
it owns stable attempt state, a terminalization reservation and exact-once
concurrency settlement. For an external effect, the selected executor or
provider-fence port must atomically redeem the still-current claim immediately
before transmission and return an authenticated terminal receipt; a stale,
draining, expired, rejected or unavailable claim causes definitely-no-send.
Successful redemption keeps the claim live until the authenticated terminal
effect receipt is reconciled, so ObservationDrainPending waits for in-flight
effects rather than treating invocation return or timeout as completion.
Profiles that cannot enforce this provider/executor fence refuse recovery
authority rather than emulate it with a check-then-use read.

Operational-claim history has a bounded, authority-preserving lifecycle.
Canonical predecessor-linked
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimCheckpointV1`
commits the exact active, live, redeemed, outcome-unknown, prepared, confirmed,
locally finalized, permanently-no-commit and terminal claim sets plus their
receipts, command-execution states/results, canonical targets, proposal
descriptor roots, commit-attestation/signature state, reservations, target
sequence, capacity consumption and completeness-frontier references.
Dedicated ClaimHistory Stage, bounded
...
capacity is exceeded. Compaction retains every live, redeemed,
outcome-unknown, RegistryConfirmed-not-locally-settled and frontier-referenced
claim hot; it may remove a terminal hot row only after committed exact-set
membership, immutable historical result lookup and predecessor continuity are
verified. Restore authenticates the greatest replay head, exact-set checkpoint
and hot suffix before opening claims. Missing, forked, unavailable or
unverifiable history returns typed
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationalAuthorityClaimHistoricalStateUnavailable`
and enters RestoreUnready; an adapter may not infer settlement or invent a
compaction rule. Load certification measures admission, registry disposition,
local finalization-receipt settlement and all three local mutation stages
under shard movement, regional failure, hostile tenants and selected latency/
throughput/service-level ceilings before production enablement.
...
registry frontier. Its evidence state is closed to RegisteredUnresolved,
LocallyExhaustedUnresolved, ExternallySealedPermanentlyUnresolved,
LateContradictoryEvidenceObserved, CancelledBeforeQueryPermit,
ConsistentReceiptObserved, FenceAnchorPending or FenceAnchored. Transitions
never weaken or discard evidence. Unknown and Unavailable preserve
RegisteredUnresolved; local budget exhaustion strengthens only to
LocallyExhaustedUnresolved. Only an authenticated registry terminal-seal
receipt may produce ExternallySealedPermanentlyUnresolved. Late contradictory
evidence is appendable from every non-fenced state, becomes
LateContradictoryEvidenceObserved and must progress local Healthy→Fenced then
FenceAnchorPending→FenceAnchored; “permanently unresolved” permanently blocks
no-fence eligibility but never blocks later evidence strengthening.

Observation registration has a complete response-loss protocol. Only
...
persists the stable request/claim, reserves registration publication,
terminalization and query recovery, and returns one process-local publication
permit. Only
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentRegistration`
invokes the registry; only
...
imports its signed receipt. Lost registration response uses only
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentRegistrationStatusQuery`
and
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentRegistrationStatusQuery`.
The operation owns stable attempt ID, Admitted→PermitIssued→InvocationReturned→
ResponseImported state, one terminalization reservation, exact-once
concurrency settlement and distinct admission/publication/reconciliation/query
results/conflicts. Exact retry returns stored state and no permit.

Cancellation-before-permit is an external permanent seal serialized with
registration receipt issuance. Only
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentCancelledBeforeQueryPermitSeal`
may reserve the full seal/query route and return one process-local seal permit;
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentCancelledBeforeQueryPermitSeal`
invokes it and
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentCancelledBeforeQueryPermitSeal`
imports the result. Response loss uses only
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentCancelledBeforeQueryPermitSealStatusQuery`
and
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentCancelledBeforeQueryPermitSealStatusQuery`.
CancellationWon and
RegistrationWon are signed terminal outcomes; Unknown/Unavailable remain
unresolved. Exactly one seal or registration receipt wins the registry CAS, so
a lost local process cannot convert an issued registration into cancellation.

The ordinary pointer-query Admit command below requires exact
QueryPermitMayIssue state, the signed RegisteredUnresolved receipt, the same
stable attempt and `R_first` escrow. Failure, timeout, cancellation, restore or
claim-drain incompleteness returns no pointer-query permit.

Every observed query outcome is first admitted by
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentTerminalization`,
which reserves all terminal/query/fence legs and returns one process-local
permit. It is published through the dedicated one-shot
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentTerminalization`
and imported only by
...
Lost terminalization response uses its own bounded status Admit/Reconcile
query lane,
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentTerminalizationStatusQuery`
and
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationIntentTerminalizationStatusQuery`.
ConsistentReceiptObserved requires a verified noncontradictory
receipt. ExternallySealedPermanentlyUnresolved requires the distinct
authenticated PermanentlyUnresolvedSeal operation with separate Admit,
Publish and Reconcile commands/results/conflicts and its distinct
`PermanentlyUnresolvedSealStatusQuery`
Admit/Reconcile lane. Contradiction or late
contradiction installs local evidence/Fenced and publishes
FenceAnchorPending; after exact signed `F_m`, dedicated
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationFenceAnchorTerminalization`
reserves the recovery lane and returns one process-local permit;
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationFenceAnchorTerminalization`
and its
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryPointerObservationFenceAnchorTerminalization`
plus distinct `FenceAnchorTerminalizationStatusQuery` Admit/Reconcile lane
alone may install FenceAnchored.
Every lane has stable IDs, one-shot process-local permits, invocation state,
terminalization reservations, exact-once concurrency settlement, exact
retry/changed-material conflict and distinct `B_EM` charges.
...
intent IDs/digests through its sequence, every evidence state/receipt,
operational claim/disposition frontier, claim-history checkpoint/archive
replay head, greatest pointer/fence anchors, registry root, authority
state/fencing epoch and all authority/key/distrust/encoding epochs. It proves
zero RegisteredUnresolved,
LocallyExhaustedUnresolved, ExternallySealedPermanentlyUnresolved,
LateContradictoryEvidenceObserved or FenceAnchorPending members.
...
CancelledBeforeQueryPermit, no fence exists, and the pointer is the exact
greatest `P_n`. Registry absence, current-status “none,” a range count,
unknown membership or an unsealed frontier never proves no fence.

Restore never performs a distributed external/local CAS. Only
...
writes the local non-authoritative activation record. Only
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryRestoreActivation`
sends its digest to the registry; the registry expected-frontier CAS either
confirms it and remains RestoreActivationPending while awaiting the exact
authenticated local-finalization receipt, or rejects it and enters
RestoreUnready.
Only
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryRestoreActivation`
imports the signed confirmation/rejection. Response loss uses only dedicated
RestoreActivationStatusQuery Admit/Reconcile commands under a stable attempt,
terminalization reservation and exact-once settlement. RegistryConfirmed makes
the exact local preparation eligible for its final local CAS; only
reconciliation of the signed local-finalization receipt opens the successor
OperationalClaimsOpen epoch and makes that preparation Operational.
LocalFinalizeUnavailable/OutcomeUnknown keeps RestoreActivationPending and
LocalFinalizeVersionConflict requires permanent no-commit evidence before
RestoreUnready settlement. A later observation cannot race a prematurely open
epoch because no claims open before that receipt. No cached epoch, snapshot,
claim or token is bearer authority, and every unsupported backend/profile
refuses this recovery capability.
...
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryAttemptV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryAttemptStateV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryAttemptOutcomeV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryTerminalizationReservationV1`
and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryAttemptConcurrencySettlementV1`
own hard call/byte/work/time/concurrency maxima, stable attempt state, future
terminal work and exact-once settlement. Attempt state is closed to Admitted,
PermitIssued, InvocationReturned and ResponseImported. The outcome is closed to
SignedReceipt, Unknown, Unavailable and ContradictoryEvidence; exhaustion is
typed
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryBudgetExhausted`.
Only
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQuery`
may reserve every terminal leg and return one process-local non-serializable
query permit, plus
...
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryCurrentPointerHighWatermarkQueryReconciliationResultV1`
or
...
Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryOperationClassV1`
splits every post-bootstrap recovery writer into a closed class.
AuthorityChanging includes retirement, replacement creation/transfer,
activation, pointer publication/query/operationalization, capacity release and
...
bytes, or any earlier anchor. MarkOrphan/FinalizeGc over fence evidence,
operationalization, replacement creation, capacity release, retention/hold
weakening, clearing and recursive recovery remain forbidden.

Fenced is therefore absorbing for authority but maintainable for evidence. A
future manual root-governance authority-recovery path remains explicitly
...
`o_max`, `c_max`, `a_max`, `v_max`, `q_max`, `r_max`, `x_max` and `t_max` for
observation intents, drained operational claims per observation, fence-anchor
cycles, archive-verification quanta per cycle, response-loss queries per
external operation, restore cursor quanta, evidence exports and
custody-strengthening mutations. These maxima and every per-class
...
`c_max` is a structural admission ceiling, not an observation-time estimate.
The selected
`RecoveryOperationalAuthorityClaimCapacityV1` fixes
`c_command_max`, `c_effect_max`, `c_queue_max` and `c_projection_max`, each
within tenant/deployment/global ceilings, and proves
`c_command + c_effect + c_queue + c_projection <= c_max` for every reachable
...
The signed claim profile fixes nonzero `e_claim_max` descriptors,
`p_claim_max <= e_claim_max` distinct payload references, signer-set bound
`n_signer_max` and selected `k_signer`. Admission rejects an oversized or
unfunded batch/profile before claim issuance. Per-claim payload capacity is:

`P_claim = p_claim_max(1_PayloadReferenceUseClaim +
...
1_SignerRecoveryAuthorization + 1_SignerRecoveryRequiredStateAdvance +
1_SignerRecoveryResumeStateAdvance +
n_signer_max(1_SignerMemberAttempt + 1_SignerMemberPublish +
1_SignerMemberReconcile + Q_claim(SignerMember) +
1_SignerMemberPartialSignature + 1_SignerMemberConcurrencySettlement +
1_SignerMemberTerminalize) +
1_SignerQuorumCombinationImport + Q_claim(SignerQuorumCombination)`.
...
P_claim + 1_DescriptorRootVerify +
1_DispositionPublish + 1_DispositionReconcile + Q_claim(Disposition) +
1_LocalFinalizeCommitRecord + 1_DomainOrNoDomainChain +
1_RecordedTimeRatchetAdvanceOrNoAdvanceProof + 1_CommitAttestation +
1_SigningAdmission + 1_SigningPublish +
1_SigningReconcile + Q_claim(Signing) + S_claim +
...
identity-registry map node/proof/hot-suffix/archive/backlog/verification
limits, payload membership nodes/transfer attempts/receipts, signer member/
partial/quorum bytes, checkpoint/archive/orphan-GC quanta and every result/
audit/outbox/signer byte are fixed in the same signed profile. Identity-root maintenance
has protected compaction capacity and admission backpressures before an exact
lookup or atomic pair insertion cannot be funded. The archive-publication orphan/GC reserve
is separately protected from ordinary claim traffic. The pre-disposition
...
until registry settlement and never resets claim capacity. Reservations remain encumbered through redeemed,
...
The class ledger is closed to ObservationDrainBegin, ObservationDrainVerify,
QueryPermitIssue; RegistrationAdmission/Publish/Reconcile/StatusQueryAdmission/
StatusQueryTerminalize; CancellationSealAdmission/Publish/Reconcile/
StatusQueryAdmission/StatusQueryTerminalize; ObservationTerminalizationAdmission/
Publish/Reconcile/StatusQueryAdmission/StatusQueryTerminalize;
PermanentUnresolvedSealAdmission/Publish/Reconcile/StatusQueryAdmission/
StatusQueryTerminalize; FenceInstall; FenceAnchorTerminalizationAdmission/
Publish/Reconcile/StatusQueryAdmission/StatusQueryTerminalize;
...
For a route suffix `X`, define
`Q(X) = q_max(1_XStatusQueryAdmission + 1_XStatusQueryTerminalize)`.
With `o <= o_max`, `c <= c_max`, `a <= a_max`, `v <= v_max`,
`q <= q_max`,
`r <= r_max`, `x <= x_max` and `t <= t_max`, the required escrow is:

`R_observe = 1_DrainBegin + c_max_DrainVerify + 1_QueryPermitIssue +
1_RegAdmit + 1_RegPublish + 1_RegReconcile + Q(Reg) +
1_CancelAdmit + 1_CancelPublish + 1_CancelReconcile + Q(Cancel) +
1_TermAdmit + 1_TermPublish + 1_TermReconcile + Q(Term) +
1_PermSealAdmit + 1_PermSealPublish + 1_PermSealReconcile + Q(PermSeal) +
1_FenceInstall + 1_FenceTermAdmit + 1_FenceTermPublish +
1_FenceTermReconcile + Q(FenceTerm)`.
...
escrow the entire `R_observe` envelope plus one complete first anchor route:
`R_first = R_observe + 1_ECreate + 1_ArchiveStage + v_max_Verify + 1_CommitJ +
...
is one checked non-wrapping total order shared by healthy pointer and fenced-
evidence anchors. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkV1`
binds tenant/deployment, guard Fenced version/generation, fence and evidence-
bundle/result digests, exact fence checkpoint/replay head, the greatest healthy
`P_n` it dominates, immutable predecessor anchor, encoding/integrity and
authority/key/distrust epochs. Its sequence must be greater than every prior
healthy pointer anchor; once it exists, any later pointer anchor is invalid.

Fence-high-watermark publication has a distinct closed protocol. Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkPublicationRequestV1`
contains only the semantic preimage, and independent authority returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkSignedReceiptV1`.
Only
`PublishMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryFenceHighWatermark`
may persist its single-use claim and return one process-local permit, with
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkPublicationResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkPublicationConflict`.
Every response enters only through
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryFenceHighWatermarkPublication`,
which returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkPublicationReconciliationResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkPublicationReconciliationConflict`
and may only advance the evidence anchor.

Response loss uses a separate non-borrowable
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryBudgetV1`
with canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryAttemptV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryAttemptStateV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryAttemptOutcomeV1`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryTerminalizationReservationV1`
and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryAttemptConcurrencySettlementV1`.
State is closed Admitted→PermitIssued→InvocationReturned→ResponseImported and
outcome is closed SignedReceipt/SignedNoFenceAtPointer/Unknown/Unavailable/
ContradictoryEvidence. Only
`AdmitMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryFenceHighWatermarkQuery`
returns one process-local query permit and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryAdmissionResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryAdmissionConflict`;
only
`ReconcileMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryFenceHighWatermarkQuery`
returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryReconciliationResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryReconciliationConflict`,
consumes the reservation and settles concurrency exactly once. Exhaustion is
typed
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceHighWatermarkQueryBudgetExhausted`.
Unknown, Unavailable, contradiction or exhaustion preserves Fenced and grants
no authority; contradictory fence-anchor evidence is appended under the same
immutable fence custody and cannot select a receipt.

Fenced restore may only verify evidence. Only
`VerifyMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryFenceEvidenceRestore`
writes
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceRestoreVerificationResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceRestoreVerificationConflict`
and transitions Unready→FencedEvidenceVerified, never Ready/Operational.
Canonical
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryFenceEvidenceExportV1`
is a bounded authenticated read/export binding the verified fence anchor,
evidence set and retention/hold state; it confers no mutation or authority.

Every post-bootstrap recovery writer is guard-first: it locks the canonical
recovery guard before its other recovery rows, selects exactly one operation
class and applies that class's state predicate. The contradiction-
terminalization writer is the sole Healthy→Fenced transition. Missing guard
state is corruption/unready, never Healthy. Fence, bundle, result, checkpoint,
replay head and fence high-watermark are mandatory restore/evidence-export
members whenever present and may not be omitted, compacted, orphaned or
dropped.
...
equals the greatest authenticated witnessed generation. This local predicate
never substitutes for registry authority: each operation must own a current
short-lived registry claim and complete the portable prepare→registry-confirm→
local-finalize protocol. External effects additionally redeem the claim at the
executor/provider fence immediately before send. Absence, drain-pending,
stale/expired/rejected claim, stale cache/epoch/token, mismatch, unavailable
registry/provider fence, contradiction or exhausted proof denies authority.
...
and independently obtains the greatest transfer high-watermark, greatest
...
with zero unresolved, locally exhausted, externally sealed permanently
unresolved, late-contradictory or pending-fence intents. Restore then drives
TokenAcquired→LocalActivationPrepared→RegistryConfirmed/Rejected and imports
the signed registry result. Only RegistryConfirmed finalizes the exact local
preparation as Operational; rejection stays Unready. A concurrent observation
drain before registry confirmation wins expected-frontier CAS and rejects
activation, while a drain after confirmation closes all new successor-epoch
claims even if local reconciliation is delayed. If a fence anchor exists, its shared anchor sequence
...
history or local absence is corruption/unready. Restore then selects the
greatest recovery archive-replay head, replays its hot suffix with the durable
cursor and either exact-completes the witnessed deterministic Operational
state under Healthy or preserves Fenced with evidence export only. A locally
consistent pre-fence backup, older pointer, missing recovery archive, one-sided
transfer or unverified successor can never revive an earlier authority,
including after a second or later equivocation.
Each bounded cursor step is exclusively
`AdvanceMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryRestoreCursor`,
returning
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreCursorAdvanceResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreCursorAdvanceConflict`.
Final readiness is exclusively
`CompleteMigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveWitnessAuthorityReplacementRecoveryRestore`,
returning
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreCompletionResultV1`
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreCompletionConflict`.
Typed
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreUnavailable`,
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreContradictoryEvidence`
and
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkWitnessAuthorityReplacementRecoveryRestoreBudgetExhausted`
leave restore Unready and grant no pointer/activation authority.
...
special, unmetered authority I/O; it selects local guard state only after the
bounded protocol reaches authenticated evidence and applies the same
equivocation rule.
...
or
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkPrepareConflict`;
ordinary PreparePending witness reconciliation returns
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkReconciliationResultV1`
or
...
AbortDrain admission, seal-status import, SealLostToWitness, sealed abort,
exhaustion and equivocation use only their dedicated results/conflicts above;
the generic pair rejects those operation/route tags.
...
reserving one
`MigrationImportRegistryHistoryCorruptionControlLineageCustodyReleaseCommitEligibilityRevalidationCounterCapacityArchiveHighWatermarkQueryTerminalizationReservationV1`
and persisting its deadline and next-eligible-query value under the canonical
common lock prefix. Admission is legal only in PreparePending(`g`); it is
denied in AbortDrainPending and every other state. The reservation binds one
future query-terminalization writer-class charge, one capacity-state and
attempt-set-head advance, terminal result/audit/outbox bytes and work, and one
concurrency-settlement operation to the stable attempt/generation. Failure to
reserve every dimension makes admission no-write and returns no permit.
Deadline/backoff uses the governed trusted-time profile,
...
It consumes the attempt's terminalization reservation in the same transaction;
timeout, cancellation, disconnect, worker loss, takeover and Replan cannot
release, reassign or borrow it.
Only authenticated positive/permanently sealed negative evidence also advances the witness
disposition in that same transaction. DefinitelyNotWitnessedReceipt means only
the permanent proposal-seal receipt; a current-status negative remains
Unknown. Unknown, transport failure and deadline
...
budget state, admission/terminalization charges, terminalization reservations,
concurrency settlements and results remain in the hot suffix and Replan/
restore preserve them exactly.
...
It rejects AbortDrainPending and requires no seal tombstone or authority-
equivocation fence.
...
charges and terminalization reservations, remain in the hot suffix and cannot
delete themselves. Exact retry
...
positive/permanently sealed negative query evidence resolves the guard, that terminalization
co-commits as the one mandatory witness-disposition charge rather than adding
a second charge. Route accounting is explicit and never applies the normal
witnessed formula to a generation after BeginAbortDrain. Let `q` be admitted
ordinary witness queries, `s` admitted seal-status queries and `e ∈ {0, 1}` be
one only when exactly one query terminalization co-commits that route's
disposition or fence:

- direct Witnessed→Commit: `3 + 2q - e` for Prepare, Witnessed disposition and
  ArchiveFinalize Commit;
- permanent sealed abort: `3 + 2q + 2s - e` for Prepare, BeginAbortDrain and
  AbortedDefinitelyUnwitnessed disposition, with no ArchiveFinalize;
- AbortDrainPending where WitnessWon then Commit: `4 + 2q + 2s - e` for
  Prepare, BeginAbortDrain, Witnessed disposition and ArchiveFinalize Commit;
- seal/witness contradiction: `3 + 2q + 2s - e` for Prepare,
  BeginAbortDrain and authority-equivocation fence, with neither abort rollover
  nor ArchiveFinalize;
- seal-reconciliation budget exhaustion: `3 + 2q + 2s` for Prepare,
  BeginAbortDrain and the exhaustion terminal writer, leaving the generation
  fenced in AbortDrainPending.

At most one ordinary or seal-query terminalization supplies `e`; direct
invocation/import makes `e = 0`. Every `q` and `s` attempt still owns and
consumes its own admission and terminalization reservation even when one
terminalization co-commits a disposition/fence. Timeout/absence never
fabricates disposition authority, but its admitted attempt must terminalize
and settle concurrency. Unknown response reconciliation observes that
indivisible bundle. Every route-specific charge, reservation, settlement and
typed result remains hot; a terminal capacity archive may later cover it, but
the route cannot delete or reclassify its own accounting.
For every `q`, conservation additionally proves exactly `q` immutable
terminalization reservations were created and exactly `q` were consumed by
the corresponding terminalizations; a co-committed disposition consumes that
same reservation once rather than an extra one.
For every `s`, conservation likewise proves exactly `s` seal-status
terminalization reservations created and consumed. The abort completion
reservation proves mutually exclusive terminal-route coverage before the seal
permit and remains bound across timeout, Replan, failover and restore.
...
independently trusted greatest high-watermark, reject a local replay head below
it, and exact-complete a prepared witnessed successor when its predecessor rows
remain; missing required rows/chunks/witness leaves the owner unready. Replan
binds the current high-watermark and capacity archive replay head and carries
archived plus hot cumulative consumption and retained old-plan obligations.
...
| Witness query admission | admit one stable high-watermark status-query attempt | Only in PreparePending, derive its ID from guard generation plus query-admission charge sequence and commit bounded Recovery-budget/concurrency debit, trusted-time deadline/backoff, PermitIssued state, dedicated terminalization reservation and successor head before issuing one process-local query permit |
| Witness query terminalization | import one invocation outcome through Reconcile | Consume that attempt's reservation and commit InvocationReturned→ResponseImported, closed outcome/result, exact-once concurrency settlement and successor head; positive/permanently-sealed negative evidence co-commits the terminal witness disposition, while Unknown/TransportFailure/DeadlineExceeded do not |
| Witness abort drain | admit permanent seal and its complete terminal-route envelope | PreparePending→AbortDrainPending atomically commits BeginAbortDrain charge, seal claim, complete non-releasable completion reservation and typed result before one seal permit; insufficient disposition/query-drain/seal-query/output/equivocation/exhaustion capacity is no-write |
...
holds a database lock across registry/provider I/O or treats the external
frontier as a locally lockable row.
...
abort-drain/seal reconciliation/equivocation fencing, query admission/
terminalization/reservation creation and consumption, guard-generation
rollover, response-loss replay and terminalization writer admitted by the
...
seal permit. The capacity proof separately enumerates each route equation and
does not add mutually exclusive branch charges as if they could all commit.
...
reset. Exhaust every publication proof-budget dimension and fault every durable
...
the fully drained permanent authority seal may complete AbortDrainPending,
while ordinary negative/unknown stays fenced and a late witness exact-
completes once. Feed
...
and one unreconstructable permit per sequence-derived stable ID. Admit every
query only after its dedicated terminalization charge/head/result/work/
settlement reservation commits; concurrently exhaust remaining class/head/
byte/work capacity and prove admitted attempts still terminalize while an
unreservable admission is no-write. Crash, cancel, disconnect, lose workers,
Replan and restore without releasing/reassigning a reservation; co-commit one
as disposition and prove exactly `q` reservations created/consumed. Roll trusted
...
Unprepared(`g + 1`) and PreparePending(`g`)→AbortDrainPending(`g`)→
AbortedDefinitelyUnwitnessed(`g`)→Unprepared(`g + 1`), plus
AbortDrainPending(`g`)→Witnessed(`g`)→Committed(`g`)→Unprepared(`g + 1`),
repeatedly. Fault
...
Prove no submission permit can be produced or used from Unprepared, Witnessed,
AbortDrainPending, Committed, Aborted or a stale generation. Race a delayed
original submission and every PermitIssued query against the authority-side
proposal-seal CAS. A seal winner permanently rejects the late submission and
is independently discoverable after local loss/restore; a witness winner makes
the seal lose and exact-completes Witnessed. Do not leave AbortDrainPending
until every ordinary and seal-status query is terminal, every reservation/concurrency settlement is
consumed and absence of positive evidence is rechecked. Inject signed positive
and permanent-negative receipts for one proposal and require the authority-
equivocation fence rather than conflict, abort or next-generation opening.
Fault before/after BeginAbortDrain completion-reservation creation and seal-
permit delivery; exhaust each disposition/head/output/query-drain/seal-query/
equivocation/exhaustion dimension and prove failure is no-write/no-permit.
Lose the original seal response, then traverse every seal-status query state
and SealWon/WitnessWon/Unknown/TransportFailure/DeadlineExceeded/
ContradictoryEvidence outcome under trusted-time rollback and backoff. Prove
one process-local query permit, one terminalization-reservation consumption
and one concurrency settlement per stable attempt; retry, cancel, fail over,
Replan and restore without releasing/reassigning capacity or resealing.
Restore enters Unready and uses only this metered lane. Exhaust it and prove
the typed exhaustion writer leaves AbortDrainPending fenced. For arbitrary
bounded `q` and `s`, exercise direct Commit, sealed abort, seal-loses-to-
witness then Commit, contradiction fence and exhaustion; mechanically prove
their respective `3 + 2q - e`, `3 + 2q + 2s - e`,
...
`P_0`, head, guard, result, audit or outbox partial write; prove the operation
cannot borrow the absent fence-keyed Recovery budget. Trace the initial-
bootstrap rank separately from every post-bootstrap rank and reject any
post-bootstrap row acquired before the guard or any bootstrap acquisition of a
replacement/recovery row.
...
capacity.
Generate the schema/reference/hash-dependency DAG for every recovery generation
and prove exactly `C_n→P_(n-1), H_n→C_n, P_n→H_n`; inject `C_n→P_n`, future
signature/result references, alternate encodings and every other back-edge and
require schema admission to fail before storage execution. Crash and lose
...
replacement or clearing, and durable hot fence retention. Then exercise the
closed Fenced EvidenceMaintenance set: checkpoint the exact fence set,
Stage/Verify/Commit its archive without deleting hot guard/fence/anchor, publish
and reconcile the independent fence high-watermark, lose its response and
traverse SignedReceipt/SignedNoFenceAtPointer/Unknown/Unavailable/
ContradictoryEvidence/exhaustion, verify restore evidence, export it and
strengthen retention/legal hold. Attempt replacement, operationalization,
capacity release, MarkOrphan/GC, evidence mutation, retention weakening and
every undeclared writer and require denial. Restore a locally consistent pre-
fence backup: the greatest fence anchor must dominate its earlier pointer and
force FencedEvidenceVerified, never Ready. Inject a later pointer, incomparable
anchor, missing fence checkpoint or unavailable fence query and require
...
it. Unknown/Unavailable must remain RegisteredUnresolved and local exhaustion
must become LocallyExhaustedUnresolved. Only a valid external terminal-seal
receipt may install ExternallySealedPermanentlyUnresolved. Deliver a late
contradictory receipt from each unresolved/sealed state and require append-only
LateContradictoryEvidenceObserved→Healthy-to-Fenced→FenceAnchorPending→
FenceAnchored; no terminal label may discard stronger late evidence.
...
and LocalActivationPrepared/registry confirmation against a new observation
drain and fence publication. Expected-frontier CAS must choose one registry
winner; local Reconcile either finalizes that signed winner or stays Unready.
Prove there is no cross-system atomic transaction.
For every authority-bearing command, queue/projection claim and effect:
acquire a bounded registry claim, crash before/after local
PreparedNonAuthoritative, registry Confirm/Reject, response loss and local
finalize. No prepared or rejected state is authoritative. Race observation
drain against claim admission, claim expiry, registry confirmation and local
finalization. QueryPermitMayIssue requires the complete zero-live-claim
...
recheck claim, authorization/fencing epochs and the exact one target/version,
append a fresh immutable batch/result/audit intent/outbox atomically, and leave
the proposal unchanged. Generate DomainAggregateTarget, ControlOwnerTarget and
CanonicalNoAggregateTarget cases. Reject zero/multiple targets, two aggregate
IDs, aggregate-plus-control, descriptors outside the target stream and any
attempt to hide cross-aggregate work in claim finalization; prove the Phase B
process-manager/outbox decomposition instead. A domain batch may contain many
events only for its one stream.
For each ProposedEventDescriptorV1, compare every semantic field and ordered
descriptor root with the final envelope. Generate InlineNonSensitive,
EncryptedPayloadReference, ExternallyErasablePayloadReference and
ErasedPayloadTombstone. Reject inline classified bytes, classification
downgrade, reference/digest/key/residency/retention/hold/custody substitution
and an untracked plaintext copy during finalization. Exercise exact immutable-
reference reuse, co-located custody transfer and precommitted external-transfer
receipt consumption; external response loss/Unknown must remain
non-authoritative and reconcile without a distributed transaction. For rejected/cancelled
proposals, race legal hold, erasure authorization and checkpoint/export; prove
crypto-erasure retains the immutable tombstone and never relocates restricted
data. Exercise one reference shared by several descriptors/events and several
proposals; proposal-plus-event custody, duplicate transfer receipts, early and
double release, event-retention/proposal-collection/hold races, restore during
TransferPending/TransferOutcomeUnknown and every partial local-batch failure.
...
attestation request, result/status/audit record, log, trace, error, checkpoint,
archive and export. Only the governed payload store may contain recoverable
plaintext/ciphertext according to its profile.

Prove only event ID, stream version, journal offset, transaction position,
actual recorded-time interval and integrity predecessor/digest are allocated
or derived later. Bind the first predecessor to the exact target stream head,
race head advancement, and reject a correct expected version with a substituted
predecessor digest. Reject zero descriptors for DomainAggregateTarget; require
one and maximum-sized domain batches to derive every predecessor under
EventCommitChainV1. Require empty descriptors only for ControlOwnerTarget and
CanonicalNoAggregateTarget, both binding CanonicalNoDomainEventChainV1 with no
EventCommitChainV1 row. Validate recorded-time policy/profile/maximum
uncertainty and commit-cut containment against backend-authoritative time.
Step the clock backward/forward across allocation and commit, suspend, restart,
...
a different event/schema ID or version, aggregate kind/ID, tenant/deployment,
command/causation/correlation ID, valid/occurrence/effective-time or recorded-
...
history and require RestoreUnready. Cancel at ClaimAcquisitionPending and
ProposalPrepared, race cancellation against disposition permit issuance, and
deny cancellation from RegistryDispositionPending onward. No cancellation may
undo the committed journal batch.
Independently race command-ID and idempotency-ID unique inserts. Exercise both
absent, both resolving to the same execution, command-only existing,
idempotency-only existing and each ID resolving to a different execution,
across hot/hot, hot/archive, archive/hot and archive/archive combinations.
One-sided or cross-pair reuse must return
OperationalAuthorityClaimedCommandExecutionHistoricalIdentityConflict with no
claim, row or permit; compaction must not make either ID reusable.
...
Run ThresholdRedundantSignerSet member loss before and after each partial
signature/quorum boundary and RootAuthorizedSuccessorSigner after lost,
unavailable and distrusted-primary dispositions. Substitute claim, proposal,
descriptor, commit chain, attestation, result, failed-signer disposition,
root authorization, quorum/member set and successor lineage. The recovered
receipt must cover the identical commit; late primary and recovered receipts
may only join one settlement meaning. Prove no recovery path appends an event,
rewrites result/payload/time/chain, reacquires a claim, refunds capacity or
resets idempotency. Exhaust ordinary signing capacity while preserving
`S_claim`; for every `1 <= k_signer <= n_signer <= n_signer_max`, lose or
delay each member before/after attempt, response, partial import and
terminalization, reject duplicate-member quorum, and prove the separately
reserved canonical quorum-combination/import progresses. Test
`n_signer_max - 1`, `n_signer_max` and `n_signer_max + 1`; the last profile
rejects before claim issuance. Signer recovery/status must progress. With no valid threshold or
successor, require redacted CommitReceiptSignerRecoveryRequired, live capacity,
blocked observation and fail-closed authority.
Run TrustedSignerAdapterAuthoritativeRead against every selected backend's
read-only authoritative port; BackendAuthenticatedCommitAttestation against
each adapter-specific attestation verifier; and HostedRuntimeCommitAttester
only in its explicit profile. Substitute caller bytes, replica/cache reads,
backend/boot/lease/fence continuity, transaction/commit-record/target/
descriptor-envelope/result-audit-outbox roots, attester/key epoch, trusted time
and nonce. A remote HSM/KMS never needs database access and never accepts
caller bytes without the selected attestation. Unsupported attestation
profiles refuse independent-signing claims.
...
no claim/token; contradiction must enter FenceAnchorPending; exact `F_m`
terminalization alone enters absorbing AuthorityFenced. Restore a stale local
Healthy database beside each external state and prove it cannot reopen claims.
Only exact ConsistentReceiptObserved plus a sealed successor frontier may
reopen OperationalClaimsOpen, while no transition leaves AuthorityFenced.
Admit hostile command/effect/queue/projection mixtures at `c_max - 1`,
`c_max` and `c_max + 1` under tenant, deployment and global contention.
Prove the first two retain complete drain/terminalization reservations, the
last is no-write/no-permit, recovery work has scheduling priority, and one
tenant cannot consume another tenant's or the global recovery reserve. Hold
redeemed and outcome-unknown provider operations indefinitely and require them
to remain live and charged through registry failover.
Race two different operations over the same canonical target/expected version:
the registry admits at most one, exact material joins and changed target/
descriptor material conflicts. At every post-RegistryConfirmed crash cut drive
LocalFinalizeCommitted, LocalFinalizeVersionConflict,
LocalFinalizeUnavailable and LocalFinalizeOutcomeUnknown; lose the signed
local-finalization receipt and reconcile it through its dedicated status lane.
Only authenticated commit or permanent no-commit evidence settles the registry
claim. Unknown/unavailable/row absence remains live and blocks observation;
exact retry returns stored state and changed receipt material conflicts.
Fill active and terminal claim row/byte ceilings, checkpoint exact sets, Stage/
...
trips and all three local mutation stages under sharding, hostile tenants,
regional failure and the selected latency/throughput ceilings.
Lose every response in registration Publish, cancellation seal,
terminalization Publish, permanent-unresolved seal, fence-anchor
terminalization and restore confirmation. Traverse their dedicated status
Admit→PermitIssued→InvocationReturned→ResponseImported states, reservations
and exact-once concurrency settlements; retry returns no permit and changed
material conflicts.
Run the observation-registry conformance suite across every candidate adapter:
partition and stale leader, delayed receipt, rollback/forked frontier,
read-after-write/failover, epoch race/wrap sentinel, key rotation/distrust,
compaction exact-set retention, tenant substitution, admission/concurrency/
timeout/query-work exhaustion and independent-failure-domain loss. Any
unproved guarantee must refuse the recovery capability.
...
Mechanically evaluate `B_EM`, `R_observe`, every `Q(X)` and `R_first` at zero,
one and every `o/c/a/v/q/r/x/t` hard maximum;
...
and externally bound publication exact-commits or keeps recovery unready.
